...
You can use {update-service} with {acm} policies to upgrade multiple clusters in a disconnected environment.
...
{update-service} is a separate operator and operand that monitors the available versions of your managed clusters in a disconnected environment, and makes them available for upgrading your clusters in a disconnected environment. After {update-service} is configured, it can perform the following actions:
...
* Access credentials to a local repository where you can mirror the cluster images. See link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/disconnected_installation_mirroring[Disconnected installation mirroring] for more information about how to create this repository.
+
...
+
See link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/registry/configuring-registry-operator#images-configuration-cas_configuring-registry-operator[Configuring additional trust stores for image registry access] in the {ocp-short} documentation for more information.
...
. From the managed cluster, confirm that the _ClusterVersion_ upstream parameter is currently the default public {update-service} operand by entering the following command:
+
----
...
. From the hub cluster, identify the route URL to the {update-service} operand by entering the following command: `oc get routes`. Note this value for later steps.
...
----
+
<1> Replace the value of `objectDefinition.spec.upstream` with the path to your hub cluster {update-service} operand. 
+
...
. In the managed cluster CLI, confirm that the upstream parameter in the `ClusterVersion` is updated with the local hub cluster {update-service} URL by entering: 
+
----
oc get clusterversion -o yaml
----
...
You can use the {acm-short} console to select a channel for your cluster upgrades on {ocp-short}. Those versions must be available on the mirror registry. Complete the steps in link:../clusters/install_upgrade/upgrade_cluster.adoc#selecting-a-channel[Selecting a channel] to specify a channel for your upgrades. 
