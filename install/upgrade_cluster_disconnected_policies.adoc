...
----
+
<1> Replace the content inside the `objectDefinition` section of the template with content and add the settings for your `ImageContentSourcePolicy`.
+
<2> Replace `your-registry` with the path to your local mirror repository. You can find your path to your local mirror by entering the `oc adm release mirror` command.
