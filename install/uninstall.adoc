...
Learn how to uninstall {product-title}. When you start to uninstall, you see two different levels of the uninstall process: A _custom resource removal_ and a _complete operator uninstall_, which are described in the following sections. *Note:* The uninstall process can take up to 20 minutes.

- The first level is the _custom resource removal_, which is the most basic type of uninstall that removes the `MultiClusterHub` custom resource instance, but leaves other required operator resources. This level of uninstall is helpful if you plan to reinstall by using the same settings and components.

- The second level is the _complete operator uninstall_, which removes most operator components, excluding components such as custom resource definitions. When you continue with this step, you remove all of the components and subscriptions that were not removed with the custom resource removal. After this uninstall, you must reinstall the operator before reinstalling the custom resource.
...
* If you use _Discovery_, you might see the following error when you attempt uninstall:
+
----
...
* If you use _Observability_, you might see the following:
...
// Somewhere near here, we need to add deletion of the local-cluster namespace. If it remains, it will block attempts to reinstall. Customer issues, see: https://issues.redhat.com/browse/ACM-10627
...
.. Copy the following script into a file, replacing `<namespace>` in the script with the name of the namespace where {product-title-short} was installed. Ensure that you specify the correct namespace, as the namespace is cleaned out and deleted. :
//we should not use scripts but I recongize this was pasted by another team member before peer reviews. I will check to make sure this is not in later versions and open the issue if it is.
...
When you use the {ocp-short} console to uninstall, you remove the operator. Complete the following steps to uninstall by using the console:
...
. Run the clean-up script according to the procedure in xref:../install/uninstall.adoc#removing-with-commands[Removing a MultiClusterHub instance by using commands].
+
