...
* If you use Discovery, you need to disable the function. From the console, navigate to the `Discovered Clusters` table and click *Disable cluster discovery*. Confirm that you want to remove the service. You can also use the terminal to disable Discovery with the following command:
...
* If you use Observability, disable and remove the `MultiClusterObservability`. *Note:* Your object storage is not affected after you remove the observability service.
...
Next you can uninstall the`MultiClusterHub` operator.
