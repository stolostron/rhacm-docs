...
The following table shows the configuration information for SNO clusters on bare metal environments that were used during this testing:
//Suggesting to add hardware - Alex 
//Need to confirm this w/Joydeep | MJ | 09/15/23
//What info can be provided for 100 clusters or 200 clusters
|===
| Node | Flavor | vCPU | RAM (GiB) | Disk type | Disk size (GiB) | Count | Region | Tested maximum SNO managed clusters
