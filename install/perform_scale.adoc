...
[#perf-1k]
== Provisioning 1000 Single Node OpenShift clusters

The provisioning of 1000 Single Node OpenShift (SNO) bare metal clusters with a standard 3-node {ocp} hub cluster completed in just over four hours. The provisioning of the clusters took approximately 3 hours and 45 minutes. Approximately 15 minutes after that, all of the clusters were under {product-title-short} management. 

Peak usage of the control plane nodes and worker nodes on the hub cluster during the provision is shown in the following table:

|===
|Node | Number | CPU: utilization | CPU: Logical cores | Memory: Utilization | Memory: GB consumed 

|Control plane | 3 | 42% | 9.6% | 7.8% | 30 
|Worker | 3 | 21.1% | 16.9% | 11.4% | 43.9
|===
...
For more information on role access, see https://docs.openshift.com/container-platform/4.8/authentication/using-rbac.html[Using RBAC to define and apply permissions] in the {ocp-short} documentation.
