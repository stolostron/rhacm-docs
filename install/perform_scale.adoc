...
{acm} is tested to determine certain scalability and performance data in areas of cluster scalability and search performance. You can use this information as you plan your environment. Data is based on the results from a lab environment at the time of testing.

{acm-short} is tested by using a three node hub cluster on bare metal machines. At testing, there is a sufficient amount of resource capacity, such as CPU, memory, and disk, to find software component limits. Your results might vary, depending on your environment, network speed, and changes to the product.
...
See the following query run time considerations:
...
For more information on role access, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/authentication_and_authorization/using-rbac[Using RBAC to define and apply permissions] in the {ocp-short} documentation.
