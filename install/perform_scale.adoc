...
The {product-title-short} hub cluster provided good performance when managing up to 250 managed clusters during testing. This is not an upper limit of the number of clusters that can be managed effectively by {product-title-short}, only the highest number of managed clusters that were tested.
