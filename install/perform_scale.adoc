...
By default, the data store is deployed with a memory limit of 4 GB.
If you are managing larger clusters, you might need to increase this limit by editing the `limit_memory` of the `redisgraph_resource` in the `searchoperator` hub cluster namespace. When the change is made, delete the `search-redisgraph` StatefulSet for the new limit to take effect.
