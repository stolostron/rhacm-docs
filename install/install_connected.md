...
There are two methods that you can use to install. Both of the methods require the same prerequisites and preparations. Method 1 is an easier method which leverages a `start.sh` installer script to automate the installation to get running quickly. Method 2 is more customizable if you want to use `oc` commands to complete each required step, but is not as easy as Method 1.   
...
There are some preparations that you need to complete before completing either of the  installation procedures. **Note:** Only run the steps in this section once.

1. Download the required files by cloning the `open-cluster-management/deploy` Git repository:
...
2. Generate a pull-secret
  ...
  1. Ensure you have access to the Red Hat Quay.io organization by signing in to the [open-cluster-management](https://quay.io/repository/open-cluster-management/multiclusterhub-operator-index?tab=tags).
  
    If you do not have access to the `open-cluster-management` org in Quay.io you can request access on the #forum-acm Slack channel.
    
  2. Replace *your_username* in the following link with your Quay.io username and visit the link:   https://quay.io/user/your_username?tab=settings.
...
3. Create the multiclusterhub-operator objects by applying the `yaml` definitions in the `multiclusterhub-operator` directory by entering the following command:
...
5. Edit the `example-multiclusterhub-cr.yaml` file in the `mulitclusterhub` directory by setting the `imageTagSuffix` to the snapshot value that you used in the `kustomization.yaml` file in the `multiclusterhub-operator` directory in step 2. Remove the `VERSION 1.0.0-,` from the `newTag` value taken from the `kustomization.yaml` file.

  The should look similar to the following example:
...
8. You can access your instance at: https://multicloud-console.apps.${HOST_URL} ????
