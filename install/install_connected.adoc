...
*Note:* Because {ocp-short} cannot be installed exclusively on an infrastructure node, the {product-title} hub cluster also cannot be installed on exclusively on an infrastructure node. 

By using `tolerations`, the {product-title} hub cluster allows most hub cluster components to be installed on an infrastructure node. To install the hub cluster on an infrastructure node, complete the following steps to prepare:
...
. Follow the steps to link:./install_connected.adoc#installing-from-the-cli[Install {ocp-short} via CLI] with the following specifications.

- When creating the `MulticlusterHub` subscription, include the following before applying the subscription:
...
- When creating the `MulticlusterHub` resource object, add the following `nodeSelector` entry:
