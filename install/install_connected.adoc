...
+
*Note:* For installing the {product-title-short} hub cluster on infrastructure nodes, the see xref:../install/install_connected.adoc#infra-olm-sub-add-config[{olm-long} Subscription additional configuration] section.
...
After adding infrastructure nodes to your {ocp-short} cluster, follow the xref:../install/install_connected.adoc#installing-from-the-cli[Installing from the {ocp-short} CLI] instructions and add the following configurations to the {olm} Subscription and `MultiClusterHub` custom resource.
