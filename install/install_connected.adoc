...
. Create the _MultiClusterHub_ custom resource.
 ...
 .. Update the default values in the YAML file. See options in the MultiClusterHub advanced configuration_ section of the documentation.
...
+
*Note:* For installing the {product-title-short} hub cluster on infrastructure nodes, see link:./install_connected.adoc#infra-mch-add-config[MultiClusterHub custom resource additional configuration]
