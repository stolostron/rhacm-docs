...
The secret requirements for {ocp-short} clusters are automatically resolved by {ocp-short} and {product-title-short}, so you do not have to generate the secret if you are not importing other types of Kubernetes clusters to be managed.
...
Your {ocp-short} pull secret is associated with your Red Hat Customer Portal ID, and is the same across all Kubernetes providers.
 .. Run the following command to compile your secret:
+
----
...
. If you plan to import Kubernetes clusters that were not created by {ocp-short} or {product-title-short}, generate a secret that contains your {ocp-short} pull secret to access the entitled content from the distribution registry. Secret requirements for {ocp-short} clusters are automatically resolved by {ocp-short} and {product-title-short}, so you do not have to generate the secret if you are not importing other types of Kubernetes clusters to be managed.
...
----
+
Add the secret that you created to the _imagePullSecret_ field on the console. 
