...
{product-title} is installed using an operator that deploys all of the required components. Before you get started, review the xref:../install/requirements.adoc#requirements-and-recommendations[Requirements and recommendations] section. Then see the following documentation:
...
* {ocp-short} version 4.6, or later, must be deployed in your environment, and you must be logged into with the CLI. See the following install doc for {ocp-short}: 
...
+
. *Optional:* Disable hub self management, if necessary. By default, the {product-title-short} hub cluster is automatically imported and managed by itself. This _managed_ hub cluster is named, `local-cluster`. 
