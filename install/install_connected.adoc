...
[role="_abstract"]
Install {acm} through {olm}, which manages the installation, update, and removal of the components that encompass the {acm-short} hub cluster. Because {acm-short} depends on and uses the {mce-short}, after you create the `MultiClusterHub` resource during installation, the {acm-short} operator automatically installs the {mce-short} and creates the `MultiClusterEngine` resource. 
...
+
*Important:* To update to the next minor release, you must return to the _{ocp-catalog}_ page and select a new channel for a more recent release.
