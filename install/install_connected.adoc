...
Tolerations exist in the deployment of the {product-title} version 2.2 hub cluster that allow the hub cluster to be installed on an infrastructure node. 

To install the hub cluster on an infrastructure node, complete the following steps to prepare:

. Configure your infrastructure nodes as infrastructure machine sets according to the procedure in link:https://docs.openshift.com/container-platform/4.6/machine_management/creating-infrastructure-machinesets.html[Creating infrastructure machine sets] in the {ocp} documentation. The toleration follows the pattern of the ones that are used to create infrastructure machine sets.
+
