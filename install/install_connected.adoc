...
- If you installed stand-alone MCE and changed the default name of your `local-cluster`, you need to use that same name for the ACM `local-cluster`. Alternatively, you can disable the MCE local cluster and enable again it with your chosen name before you install ACM. 
//what happens if you just keep in disabled.
...
* You need a supported {ocp-short} and the {ocp-short} CLI. See link:https://docs.redhat.com/en/documentation/openshift_container_platform_installation/4.16[{ocp-short} installing].
...
- The namespace, `<your-local-cluster>`, is reserved for the {acm-short} hub cluster when it is self-managed. This is the only `local-cluster` namespace that can exist in the product. 

- *Important:* For security reasons, do not give access to the `<your-local-cluster>` namespace to any user that is not a `cluster-administrator`.
