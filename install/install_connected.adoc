...
. Configure your infrastructure nodes as infrastructure machine sets according to the procedure in link:https://docs.openshift.com/container-platform/4.7/machine_management/creating-infrastructure-machinesets.html[Creating infrastructure machine sets] in the {ocp} documentation.
+
...
. Complete the steps in <<Installing from the OperatorHub>> or <<Installing from the CLI>> to finish installing the hub cluster. 
