...
+
*Important:* The `local-cluster` namespace is used for the imported self-managed hub cluster. You must create the `local-cluster` namespace, if necessary, and ensure that no clusters were previously imported into it.
+
...
+
*Important:* The `local-cluster` namespace is created during the installation for the imported self-managed hub cluster. The installation process fails if there is an existing namespace called `local-cluster` when the installation process is run. 
 .. *Optional:* Disable hub self management, if necessary. By default, the hub cluster is automatically imported and managed by itself, like any other cluster. If you want to manage the hub cluster with a different hub cluster, then change the setting for `disableHubSelfManagement` from `false` to `true`. If the setting is not included in the `.yaml` file that defines the custom resource, add it as shown in the example of the previous step.
