...
+
*Important:* A `ServiceAccount` with a `ClusterRoleBinding` automatically gives cluster administrator privileges to {product-title-short} and to any ID with access to the namespace.
For security reasons, do not give anyone access to the `local-cluster` namespace who does not already have cluster-administrator access.
+
* Channel: The channel that you select corresponds to the release that you are installing. When you select the channel, it installs the identified release and ensures that future updates are obtained from that channel.
+
* Approval strategy: The approval strategy identifies the human interaction that is required for applying updates. If you select *Automatic* updates, any updates within that release are automatically applied. If you have concerns about when the updates are applied, you can select *Manual*, and receive a notification when an update is available. 
+
*Note:* Automatic upgrades do not span across releases. To upgrade to the next release, you must return to the OperatorHub page and select a new channel for the more current release. 
