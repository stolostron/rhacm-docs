...
- If you configured your {ocp-short} cluster with infrastructure nodes, you can install the hub cluster on those infrastructure nodes by using the {ocp-short} CLI with additional resource parameters. See the _Installing the {product-title-short} hub cluster on infrastructure node_ section for more details.
...
* You need a supported {ocp-short} and the {ocp-short} CLI. See link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.14/html/installing/index[{ocp-short} installing].
...
You must have a supported {ocp-short} version, including the registry and storage services, installed and working.
//repetative
 For more information about installing {ocp-short}, see the {ocp-short} documentation.

. Verify that a {product-title-short} hub cluster is not already installed on your {ocp-short} cluster. You cannot have more than one hub cluster.{product-title-short} allows only one single {product-title-short} hub cluster installation on each {ocp-short} cluster. Continue with the following steps if there is no {product-title-short} hub cluster installed:
...
You can proceed to install {product-title-short} from the console or the CLI. 
...
//add example of the channel and update strategy
...
//Click the button, your operator is listed 
...
//need a description here to say the steps are installing the operator and the objects

. Create a {product-title-short} hub cluster namespace where the operator requirements are contained. Run the following command, where `namespace` is your hub cluster namespace. The value for `namespace` might be referred to as _Project_ in the {ocp-short} environment:
...
spec:
  ...
  - <namespace>
----
Replace `default` with the name of your operator group. Replace `namespace` with the name of your project namespace. See the following sample:
//use callouts here
...
+
If you receive the following error, the resources are still being created and applied. Run the `oc apply` command again in a few minutes when the resources are created:
