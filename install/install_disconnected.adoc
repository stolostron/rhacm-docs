...
You might need to install {product-title} on {ocp} clusters that are not connected to the Internet. You might want to upgrade an existing installation to the latest release in such a disconnected environment. To install or upgrade on a disconnected hub cluster, perform the following steps in addition to the usual install or upgrade steps for the connected network environment:
