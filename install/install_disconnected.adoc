...
. Create a yaml file that contains the `ImageContentSourcePolicy` with the name `icsp.yaml` . *Important:* If you modify this on a running cluster, it causes a rolling restart of all nodes.
+
----
