...
*Required access:* You need _luster administration access for all installation and upgrade tasks.
...
When you perform the previous procedures, note the the follow requirements that are also specific to {product-title}:

[#disconnect-include-acm-pkgs]
==== Include required operator packages in your mirror catalog 

* Include required operator packages in your mirror catalog. Red Hat provides the {product-title} operator in the Red Hat operators catalog, which is delivered by the `registry.redhat.io/redhat/redhat-operator-index` index image. When you prepare your mirror of this catalog index image, you can choose to either mirror the entire catalog as provided by Red Hat, or you can mirror a subset that contains only the operator packages that you intend to use.
...
+
* If instead you are populating the mirrored catalog or registry by using the `oc-mirror` plugin, include the following package names in the packages list portion of your `ImageSetConfiguration`, as displayed in the following example:
...
As described in the procedures in the {ocp} documentation, you need to add a `CatalogSource` resource to your disconnected cluster. *Important:* Take note of the value of the `metadata.name` field, which you need later.
...
In order to have your cluster obtain container images for the {product-title} operator from your mirror registry, rather than from the internet-hosted registries, you must configure an `ImageContentSourcePolciy` on your disconnected cluster to redirected image references to your mirror registry.  
...
After you have configured {olm} and {ocp} as described previously, you can install {product-title} by using either the OperatorHub console or a CLI. Follow the same guidance described in the xref:../install/install_connected.adoc#installing-while-connected-online[Installing while connected online] topic.
