...
* {ocp} version 46 or later, must be deployed in your environment, and you must be logged into it with the command line interface (CLI). 
+
*Note:* For managing bare metal clusters, you must have {ocp-short} version 46 or later.
+
...
* Your {ocp} CLI must be version 46 or later, and configured to run `oc` commands.
