...
[#disconnect-install-grc]
== Additional considerations when using governance policies

Complete the following additional steps when using governance policies with disconnected {ocp} clusters:

. TBD

. Edit your `ImageContentSourcePolicy` resource to specify additional mirrors for disconnected image registries if you need to use any of following governance policies:
+
|===
...
. Apply the changes to your `ImageContentSourcePolicy` resource.
