...
+
On the hub, all of the application lifecycle pods are installed in the `open-cluster-management` namespace, including the following pods:
...
** The namespace `open-cluster-management` needs access to `openshift-kube-apiserver` on port TCP/6443.
** The namespace `open-cluster-management` needs access to `openshift-dns` on port UDP/5353.
