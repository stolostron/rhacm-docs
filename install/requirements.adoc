...
** The namespace `open-cluster-management` needs to access the Kube API on port TCP/6443.
** The namespace `open-cluster-management` needs to access the OpenShift DNS on port UDP/5353.

+ 
On the managed cluster, the following access is required:

** The namespace `open-cluster-management-addon` needs to access the Kube API on port TCP/6443.
