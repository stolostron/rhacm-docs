...
By default, the {acm-short} hub cluster is automatically imported and the hub cluster manages by itself. A hub cluster that manages itself is designated as the `local-cluster`.

The setting that specifies whether a hub cluster manages itself is in the `MultiClusterEngine` custom resource. Changing this setting in {acm-short} automatically changes the setting in the `MultiClusterEngine` custom resource. 

*Note:* On a {acm-short} hub cluster that is managing a {mce-short} cluster, any earlier manual configurations are replaced by this configuration.

If you do not want the {acm-short} hub cluster to manage itself, you need to disable self management. Change the setting for `spec.disableHubSelfManagement` from `false` to `true`. 
