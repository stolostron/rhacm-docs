...
To define a set of node selectors for the {mce-short} cluster, see link:../clusters/install_upgrade/adv_config_install.adoc#advanced-config-mce[Advanced configuration {mce-short}] in the {mce-short} documentation.
...
To define a list of tolerations for the {mce-short} hub cluster, see link:../clusters/install_upgrade/adv_config_install.adoc#advanced-config-mce[Advanced configuration {mce-short}] in the {mce-short} documentation.
...
* If you plan to use `cnv-mtv-integrations`, do not set `disableHubSelfManagement: true` in the `MultiClusterHub` custom resource.
* Do not disable `local-cluster` in the `MultiClusterEngine` custom resource.
* If `disableHubSelfManagement` is set to `true`, `cnv-mtv-integrations` does not function correctly.
...
Verify that the local-cluster is enabled by running the following command. The output must show that the `local-cluster` exists and has `Available=True` status:
