...
By default, the {acm-short} hub cluster is automatically imported and manages itself as the `local-cluster`. This _managed_ hub cluster is named `<your-local-cluster>`. The setting that specifies whether a hub cluster manages itself is in the `multiclusterengine` custom resource. Changing this setting in {acm-short} automatically changes the setting in the `multiclusterengine` custom resource. 
