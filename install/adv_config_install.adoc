...
{product-title} is installed by using an operator that deploys all of the required components. The operator works to deploy the following sub-components:

== Components configuration overrides
...
The following example shows the `spec.overrides` default template used to enable a sub-component. Replace `namespace` with the name of your project and replace <component-name> with the name of the sub-component:
