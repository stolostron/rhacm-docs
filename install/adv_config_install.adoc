...
//this setting? this option? do we mean disableHubSelfManagement?
You can change the `spec.localClusterName` field value from `local-cluster` to a different name. The `ManagedCluster` resource and the namespaces reflect the change.
