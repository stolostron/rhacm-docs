...
You can change the `spec.localClusterName` name if the `disableHubSelfManagement` field is set to `true`. You must use 34 or fewer characters for the `<your-local-cluster-name>` value. The `local-cluster` cannot be renamed if it is set as `disableHubSelfManagement` field is set to `false`. The `local-cluster` resource and the namespaces reflects the change.

If the `disableHubSelfManagement` is not included in the YAML file that defines the custom resource, you need to add it. The hub cluster can only be managed with this option.
