...
{product-title} is installed by using an operator that deploys all of the required components. The operator works to deploy the following components:

|===
| Name                       | Description | Enabled
| app-lifecycle              | Enables unified and simplified options for constructing and deploying applications and application updates. | true
| cluster-backup             | Enables backup and restore support for all hub cluster resources such as managed clusters, applications, and policies restore. | false
| cluster-lifecycle          | Enables cluster management capabilities for {ocp-short} and {product-short-title} hub clusters. | true
| cluster-permission         | Enables administrators to automatically distribute RBAC resources to managed clusters and manage the lifecycle of those resources. | true
| console                    | Enables {product-short-title} web console plug-in. | true
| grc                        | Enables security enhancement for you clusters. | true
| insights                   | Enabled to help identify existing or potential problems in your clusters. | true
| multicluster-observability | Enables monitoring to gain further insights into the health of your managed clusters. | true
| search                     | Enables visibility into your Kubernetes resources across all of your clusters. | true
...
| volsync                    | Enables asynchronous replication of persistent volumes within a cluster, or across clusters with storage types that are not otherwise compatible for replication. | true
|===

**Important:** When {product-short-title} is installed onto the cluster, not all of the listed components are enabled by default.

{product-title-short} can be further configured during or after installation by adding one or more of the following attributes to the `MultiClusterHub` custom resource:

== Console and component configuration

The following example displays the `spec.overrides` default template that you can use to enable or disable the component. Replace `namespace` with the name of your project and `name` with the name of the component:
...
**Note** The console is forced to stay off if the {ocp} console is disabled. Similarly, if another component is disabled, that resource will not be deployed to the cluster until it is enabled.
