...
Some components are not enabled by default and must be enabled in the {product-title-short} hub configuration to use. See how to configure the following components:

[#cluster-proxy-addon]
== ClusterProxyAddon (Technical Preview)
