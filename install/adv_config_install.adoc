...
See the following instructions in link:../clusters/rel_img_conn.adoc#maintaining-a-custom-list-of-release-images-when-connected[Maintaining a custom list of release images when connected] to manage your available release images and to set the `spec.disableUpdateClusterImageSets` attribute, which stops the custom image list from being overwritten.
...
ClusterProxyAddon has been migrated to multicluster engine for Kubernetes and is installed there. See link:../multicluster_engine/install_upgrade/adv_config_install.adoc#cluster-proxy-addon[MCE Documentation] to learn more.
