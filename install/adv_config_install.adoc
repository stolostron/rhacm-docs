...
The setting that specifies if a hub cluster is a `local-cluster` is in the `MultiClusterEngine` custom resource. Changing this setting in {acm-short} automatically changes the setting in the `MultiClusterEngine` custom resource.
//this was here, but if "this setting"--are we trying to say changing disableHubSelfManagement changes mce? If so, we should specify.
Setting the `disableHubSelfManagement` option to `true` and attempting to manage the hub manually leads to unexpected behavior.
//Do we need to say this? Setting it to true leads to expected behavior--you can't manage hub
...
//do we mean disableHubSelfManagement?
You can change the `spec.localClusterName` field value from `local-cluster` to a different name. When you configure the `local-cluster` name, you must use 34 or fewer characters. The `ManagedCluster` resource and the namespaces reflects the change. 
//should this be MCE resource
