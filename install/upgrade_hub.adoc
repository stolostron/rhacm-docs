...
+
**Important:** You cannot skip channels when upgrading. For example, you cannot skip versions 2.2.z through 2.4. The {olm} `operatorcondition` resource checks for previous upgrades during the current upgrade process. You can check the status to see is upgradable is set to `true` or `false`.
