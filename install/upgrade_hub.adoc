...
* Upgrades are supported from the immediate previous version. You can upgrade to the next available feature release, but you cannot skip a release during upgrade.
//we should remove this, I assume?

* You can perform skip-level upgrades from `<2.x>` to `<2.x+2>` on odd versions of {acm-short}, starting with version 2.11.

* You can perform upgrades on even versions of {mce-short} starting with version 2.6. 
