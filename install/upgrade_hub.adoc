...
* If you are on a version with Extended Update Support (EUS), you can upgrade to the next version, or you can skip the the next EUS version. 

* You can perform skip-level upgrades from `<2.x>` to `<2.x+2>` on odd versions of {acm-short}, starting with version 2.11. For instance, when you upgrade {acm-short} 2.11, which us EUS, you can choose to upgrade to version 2.13.
...
.. Wait for the automatic updates to be applied to the operator. The updates automatically add the required updates to the latest version in the selected channel. When all of the updated updates are complete, the _Upgrade Status_ field indicates and `Up to date` status.
+
