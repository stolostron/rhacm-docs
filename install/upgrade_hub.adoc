...
+
**Important:** The {olm} `operatorcondition` resource checks for previous upgrades during the current upgrade process and prevents skipping versions. You can check the status to see is upgradable is set to `true` or `false`.
