...
* Kubernetes namespace (`Namespace`): Uses deployables to store Kubernetes resource templates. A subscription for this type of channel retrieves and deploys the template. The namespaces that the channel monitors for new or updated deployables must exist on the hub cluster. 

* ObjectStore (`ObjectBucket`): Stores Kubernetes resource YAML files. Each file includes the template portion for one resource, not the full deployable object. An Object storage can be populated with a deployable object that is on the hub cluster, or directly from a continuous integration pipeline, or by including the required YAML files into the Object storage. 

* Helm repository (`HelmRepo`): Stores Helm charts. For information about how to structure your charts, see https://helm.sh/docs/[Helm documentation].
...
For more information about deployables and other application resources, see xref:../manage_applications/app_resources.adoc#application-resources[Application resources].
