...
Channels are created on the hub cluster by pointing to the location where resources are stored. Channels are defined with a `Channel` custom resource (CR). Each channel namespace for role-based access control, and this is required for all channels except those of type `Git`.
