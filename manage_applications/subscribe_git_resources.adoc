...
By default, the `apps.open-cluster-management.io/reconcile-option: merge` annotation is added. See the following example:
...
With the `merge` annotation, entries from subscribed resource are either created or updated in the existing resource. No entry is removed from the existing resource.

*Important:* If the existing resource you want to overwrite with a subscription is automatically reconciled by another operator or controller, the resource configuration
