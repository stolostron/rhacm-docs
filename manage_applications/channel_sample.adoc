...
For {product-version}, specifying `insecureSkipVerify: "true"` in channel ConfigMap reference to skip Helm repo SSL certificate is deprecated:
...
See the replacement in the following current sample, with `spec.insecureSkipVerify: true` that is used instead in channel spec.
