...
For {product-version}, the following specifications are deprecated:

[source,yaml]
----
configMapRef:  # deprecated for 2.2 release
      name: insecure-skip-verify. # deprecated for 2.2 release
----

See the replacement in the following current sample, with `configRef.name: insecure-skip-verify` specifications that are used instead.
