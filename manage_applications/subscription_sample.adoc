...
| spec.placement
| Required. Identifies the subscribing clusters where deployables need to be placed, or the placement rule that defines the clusters. Use the placement configuration to define values for multi-cluster deployments.
...
| Optional. Defines the clusters where the subscription is to be placed. Only one of `clusters`, `clusterSelector`, or `placementRef` is used to define where your subscription is to be placed for a multi-cluster. If your cluster is a stand-alone cluster that is not your hub cluster, you can also use `local cluster`.
...
Use only one of `clusters`, `clusterSelector`, or `placementRef` to define where your subscription is to be placed for a multi-cluster. If your cluster is a stand-alone cluster that is not your hub cluster, you can also use `local cluster`.
