...
| 
`key: value` pair that are a kubernetes label and value found on the subscription(s) this application will be associated to. The label allows the application resource to find the related subscriptions by performing a label name and value match.
...
The spec for defining these applications is based on the Application metadata descriptor custom resource definition that is provided by the Kubernetes Special Interest Group (SIG). Only the values shown in the table above are required.
