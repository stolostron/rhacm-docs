...
Placement rules can be defined for placement rules and for deployables.
Define the placement rule at the placement rule level for multi-cluster deployments.
...
+
For more information about creating the definition for a deployable or placement rules, see:
...
Your new placement rule does not display in this list until the rule is referenced by a placement rule that is used by an application.
...
Include the following `placement` fields in your deployable or placement rule spec with the values to reference the placement rule:
...
If needed, you can select to view or edit the YAML for the placement rule, placement rule, and channel from this resource pipeline table.
