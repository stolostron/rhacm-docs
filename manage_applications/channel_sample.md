...
| spec.pathname | Required for `HelmRepo`, `GitHub`, `ObjectBucket`, `Namespace` channels. For a `HelmRepo` channel, set the value to be the URL for the Helm repository. For an `ObjectBucket` channel, set the value to be the URL for the Object store. For a `GitHub` channel, set the value to be the HTTPS URL for the GitHub repository. For a `Namespace` channel, set the value to be the namespace where the channel is included.|
