...
1. Prehook Ansiblejobs and the main subscription continuously reconcile and relaunch a new pre-Ansiblejob instance.

2. After the pre-Ansiblejob is done, re-run the main subscription. If there is any spec change in the main subscription, re-deploy the subscription. 

4. The main subscription status should be updated to align with the redeployment procedure. 

5. Reset the hub subscription status to `nil` and it will be refreshed along with the subscription deployment on target clusters. 
...
6. After the main subscription is done, relaunch a new post-Ansiblejob instance

7. Verify that the DONE subscription is updated:
