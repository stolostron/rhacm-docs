...
* NOTE FOR DEV: Version of {ocp-short} (They have OCP to install operator so we should mention what is tested here?)*
...
The application subscription operator is enhanced to define two subfolders: `prehook` and `posthook`. 

Both folders are in the Git repo resource root path and contain all prehook and posthook Anible jobs, respectively.
...
To get started, you need to install the Ansible Automation Platform Resource Operator to connect Ansible jobs to the lifecycle of Git subscriptions. For best results when using the AnsibleJob to launch Ansible Tower jobs, the Ansible Tower job template should be idempotent when ran. 
...
See the following YAML sample: *NOTE FOR DEV: Command `kubectl apply f` or something here??*
