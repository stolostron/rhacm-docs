...
Learn how to set up for fine-grained role-based access control (RBAC) and use the `ClusterPermission` resource from the console.
...
//If you changed this, need to change it back
...
. In your `MultiClusterHub` custom resource `spec.overrides.components` field, set `fine-grained-rbac-preview` to `true` to enable the feature. Change the `configOverrides` specification to `enabled: true` in the YAML editor. See the following example:
...
//user groups and cluster YAML process here
