...
When you create a Certificate Signing Request (CSR) by using the `csr.cnf` file, update the `DNS.1` field in the `subjectAltName` section to match the host name of the `rbac-query-proxy` route by completing the following steps:

. Run the following command to retrieve the host name:
...
- See link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/security_and_compliance/configuring-certificates#add-service-serving[Securing service traffic using service serving certificate secrets].
- See xref:../secure_cluster/cert_intro.adoc#cert-intro[Certificates].
//if we get rid of this topic, we need to ensure all links are removed.
