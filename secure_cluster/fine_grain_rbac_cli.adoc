...
*Technology Preview:* You can assign users to manage virtual machines with fine-grained role-based access control. 
...
. Create the following YAML file and name the file `acm-vm-rbac-required.yml` for later steps in the procedure. You can use a different name. If you choose a different name, you need ot use it throughout the procedure:
...
2. Add the names of the managed clusters that you want your users and groups to access.
3. This name must match the previous name that you used.
4. This name has to match the above ClusterRole name
5. This can be a user or a group.
6. Specify user or group name.
...
- apiGroup: rbac.authorization.k8s.io
  ...
  name: group1
----
//in the previous step but was also separated in the draft
...
----
1. Use the cluster name for this `ClusterPermission` resource.
2. Specify a managed cluster namespace.
3. Use any name.
