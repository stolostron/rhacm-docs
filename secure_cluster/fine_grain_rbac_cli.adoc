...
. Create the following YAML file and name the file `acm-vm-rbac-required.yml` for later steps in the procedure. You can use a different name. If you choose a different name, you need this name later in the procedure:
...
kind: ClusterRole
metadata:
  name: <cluster-role-name> <1> #match the UI instructions #use this for binding
rules:
...
roleRef:
  ...
  kind: ClusterRole
  name: acm-vm-rbac-required <4> #has to be what we say above
subjects:
  - kind: User <5>
    apiGroup: rbac.authorization.k8s.io
    name: user1 <6>
...
[source,bash]
----
oc apply -f acm-vm-rbac-required.yml #anything
----
...
metadata:
 ...
 namespace: cluster01 <2> #match the other instruction to use cluster
subjects:
 - kind: User <3>
   apiGroup: rbac.authorization.k8s.io
   name: <user-name> <4>
