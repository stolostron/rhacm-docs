[#certificates]
= Certificates
//we have certificates ,cert intro, cert managed, and some confusing headers in this book. I am keeping it this way for now because I had to get it in the build post-GA since it was not built before the issue closed. I am removing the intro part of the title, that is not needed. We only title the file so we know it contains all the links to that section.
...
//is this managed certificates or managed cluster certificates, at first I was confused, especially because there is a header below with no certs added to it, in fact, it's just a few paragraphs that don't really need a header...let's remove for UX purposes so the user does not expect a list of certs where a header is calling that section a list of certs like the one above. --brandi 4/28
...
=== Managed cluster certificates
//This should not be it's own header
