...
All certificates required by services that run on {acm-short} are created during the installation of {acm-short}. These certificates are {acm-short} hub cluster and managed certificates that are created and managed by the following components of {ocp}:
...
- See xref:../secure_cluster/cert_intro.adoc#cert-intro[Certificates introduction] for topics.
