...
* In earlier versions, only policy frameworks could resolve hub templates, requiring you to put the `ConfigurationPolicy` into a `Policy` on the hub cluster for distribution through the `Placement`. Now you can use the `governance-standalone-hub-templating` addon in the `ManagedClusterAddOn` to define the `ConfigurationPolicy` on your managed clusters.
