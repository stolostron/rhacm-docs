...
//thinking of moving this to cluster--will seek opinions-bcs 2/5

If you later installed {acm-short} after using stand-alone {mce-short} for cluster management, you get access to all {acm-short} features. See the following new features available for {mce-short} with {acm-short} integration.

You can now configure the {acm-short} hub cluster to change the naming convention for discovered and imported hosted clusters from {mce-short} to ensure all hosted clusters are named uniquely in your entire fleet. See xref:./discover_hosted/acm_discover_hosted.adoc#discover-hosted-acm[Discovering {mce-short} hosted clusters in {acm-short}].
