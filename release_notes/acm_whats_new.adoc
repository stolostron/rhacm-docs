...
- You can now use the `dryrun` subcommand in the `policytools` CLI to evaluate a `ConfigurationPolicy` locally using static resource files. This subcommand is now Generally Available.
- You can now view related resources for your policies across multiple clusters in the "Discovered policies" section in the console from one table, as opposed to going to the policy template for each cluster. This table works for all policy types except for Gatekeeper resources.
- You can now provide multiple operator versions in one entry of the `versions` list inside an OperatorPolicy, by separating them with commas.
