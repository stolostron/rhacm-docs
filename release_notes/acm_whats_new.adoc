...
* You can now use the `spec.versions` parameter of the Operator policy controller to list the installed `ClusterServiceVersion` names that are compliant. See the link:../governance/operator_pol_ctrl.adoc#operator-policy-yaml-table[Operator policy YAML table].
