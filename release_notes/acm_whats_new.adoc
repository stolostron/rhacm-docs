...
* You can now both filter and enforce an unnamed `objectDefinition` in a `ConfigurationPolicy` by providing a label selector in the `objectSelector` parameter to select the objects that you want to apply to the policy. For more information, see link:../governance/adv_template_process.adoc#object-selector[Configuration policy object selector].
