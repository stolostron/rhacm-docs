...
* An unnamed `objectDefinition` in a `ConfigurationPolicy` can now both be filtered and enforced by providing a label selector in the `objectSelector` parameter to select the objects that should apply to the policy. See the link:../governance/adv_template_process.adoc#object-selector[Configuration policy object selector].

* Templating for managed clusters in `ConfigurationPolicy` now supports `.ObjectName` and `.ObjectNamespace` variables. See the link:../governance/template_support_intro.adoc#template-comparison-table[Comparison of hub cluster and managed cluster templates].
