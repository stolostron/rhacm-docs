...
* To convert YAML strings, implement the `fromYaml` and `toYaml` Go template functions in the `ConfigurationPolicy` resource. You also have more Sprig functions in the list of available functions.
* With the `dryrun` subcommand being available, you can run it in the `policytools` CLI to evaluate a `ConfigurationPolicy` locally by accessing the static resource files. 
...
* To give different operator versions in one entry of the `versions` list inside an `OperatorPolicy`, separate these versions with a comma. 
