...
* You can now use the {acm-short} `GitOpsCluster` to register a non-{ocp-short} cluster to a GitOps cluster, giving you more ways to deploy your application. For more information, see: link:../gitops/gitops_registering.adoc#register-non-ocp[Registering non-_{ocp-short}_ clusters to GitOps].
