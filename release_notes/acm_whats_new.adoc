...
{acm-short} version 2.13 uses the {mce-short} version 2.8 for cluster management. See the link:../clusters/release_notes/mce_release_notes.adoc#mce-release-notes[Release notes for Cluster lifecycle with {mce-short}].
...
* In earlier versions, the policy framework on the hub was required to resolve hub templates, requiring you to put the `ConfigurationPolicy` into a `Policy` on the hub cluster for distribution through the `Placement`. Now you can use the `governance-standalone-hub-templating` addon to resolve hub templates directly on managed clusters, when the `ConfigurationPolicy` is created outside of the policy framework.
