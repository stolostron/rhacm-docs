...
- You can now use the `.Object` Go template context variable in the `ConfigurationPolicy` resource to reference values from the target object on the cluster, alongside the `.ObjectName` and `.ObjectNamespace` context variables.

- You can now use the `fromYaml` and `toYaml` Go template functions in the `ConfigurationPolicy` resource to convert YAML strings. You also have more Sprig functions in the list of available functions.
