...
. Resolved a few issues with the subscription `timewindow` function that sometimes prevented it from transitioning to and from `blocking` and `unblocking` at the scheduled times. (GitHub 7337)
...
. Fixed an issue that retained user accounts that were requested to be removed, which delays reaching the maximum number of user accounts. (GitHub 5948) 
