...
. Fixed an issue with the hub subscription crashing when a Helm subscription is created for subscribing resources from a private Helm repo channel, where only `spec.SecretRef` is defined. The private Helm repo channel secret must be defined in the same channel namespace. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1925281[Bugzilla 1925281])
