...
The `multicluster-observability-addon-manager` pod is in a `OOMKilled` state because there are too many resources being received by the pod. By default, the `multicluster-observability-addon-manager` pod is configured to watch all config maps and secrets on the hub cluster. When you enable the multicluster observability add-on, the metric collection fails. 
