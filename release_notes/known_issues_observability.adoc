...
A proxy configuration in a managed cluster that you configure by using the `addonDeploymentConfig` is not detected by the metrics collector. HTTPS proxy configuration with a custom CA bundle is not supported. HTTP proxy configuration is supported with manual configuration.

You can enable HTTP and HTTPS proxies that do not require a custom CA bundle by the removing the managed cluster `ManifestWork`. Removing the `ManifestWork` forces the changes to apply in the `addonDeploymentConfig`.