...
The `MultiClusterObservability` (MCO) uses an earlier version of the Cluster Monitoring Operator (CMO). When MCO uses an old version of CMO, it can cause the following symptom: it prevents the `cluster-monitoring-config` config map from updating to the other configurations within {product-title-short} observability. The configurations then get reset to their default values in the CMO. 
