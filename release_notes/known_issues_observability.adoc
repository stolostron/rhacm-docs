...
== Metrics are only displayed from {ocp-short} 3.11 managed clusters 
//2.11:ACM-12773

If a managed hub cluster with Observability enabled imports an {ocp-short} 3.11 cluster (deprecated) as managed cluster, the metrics are only displayed from the managed cluster.

//are users able to make updates to change this outcome?
