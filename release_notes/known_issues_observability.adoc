...
If you disabled automatic configuration of alert forwarding by setting `mco-disable-alerting: true` in the `MultiClusterObservability` custom resource, and manually configured alert forwarding, alert forwarding fails. Alert forwarding fails after you upgrade because the name of the `observability-alertmanager-accessor` and `hub-alertmanager-router-ca` secrets were changed to include a postfix of the hub cluster name.

To fix this issue, you must update your manually configured alert forwarding, to use the new secretsnames.
