...
[#configurations-losing-value]
== Configurations losing value
//2.11:12213

The `MultiClusterObservability` (MCO) uses an old version of the Cluster Monitoring Operator (CMO). When MCO uses an old version of CMO, it can cause the following symptom: it prevents the `cluster-monitoring-config` config map from updating to the other configurations within {product-title-short} observability. When the CMO tries to turn the configurations into text, it causes the following symptom: the `cluster-monitoring-config` config maps might get reset to their type's value of `0`. For example, the `retentionSize` configuration gets a value of `0` and no longer exists in the current version of MCO.

To workaround these symptoms, complete the following steps:

. Create a local cluster within your {product-title-short} deployment.
. Go to your CMO stack inside your `cluster-monitoring-configuration` config map.
. Customize the `retentionSize` configuration so its value is not `0`.
