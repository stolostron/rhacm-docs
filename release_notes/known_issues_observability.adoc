...
The Prometheus `AdditionalAlertManagerConfig` of the Observability add-on does not support proxy settings. You must disable the Observability alert forwarding feature.

In 2.9, the HTTPS proxy with a self-signed CA certificate is not supported. 
