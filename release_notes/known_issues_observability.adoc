...
. To fix the dashboard failure, redeploy Grafana by scaling the number of replicas to `0`. The `multicluster-observability-operator` automatically scales the number of replicas to the desire amount afterwards. Run the following command:
