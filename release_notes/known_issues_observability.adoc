...
[#configurations-getting-overwritten]
== Configurations getting overwritten
//2.11:11724

When you import an {ocp-short} managed cluster onto the hub cluster, {product-title-short} overwrites the `retentionSize` configuration under the `prometheusK8s` section of the config map. There is currently no workaround for this issue. 
