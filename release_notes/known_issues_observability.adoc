...
After you upgrade to version 2.15, alert forwarding fails, if you have manually configured alert forwarding. In {acm-short} 2.15, the name of the `observability-alertmanager-accessor` and `hub-alertmanager-router-ca` secrets were changed to include a postfix of the hub cluster name. See the following example of how the name is displayed: `observability-alertmanager-accessor-hub1-region1`. 

To fix this issue, you must update the configurations where you disabled automatic alert forwarding with the annotation, `mco-disable-alerting: true` in the `MultiClusterObservability` custom resource, and where you manually configured alert forwarding to forward alerts to your hub cluster.

For platform alerts, update the `cluster-monitoring-config` config map in the `openshift-monitoring` namespace. For user workload alerts, update the `user-workload-monitoring-config` config map in the `openshift-user-workload-monitoring` namespace. You can find the new secret names in the `openshift-monitoring` and `openshift-user-workload-monitoring` namespaces.
