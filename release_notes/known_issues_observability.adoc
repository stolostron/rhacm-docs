...
The `MultiClusterObservability` uses an earlier version of the Cluster Monitoring Operator. The earlier version prevents the `cluster-monitoring-config` config map from updating to the other configurations within observability. The configurations then get reset to their default values.  
