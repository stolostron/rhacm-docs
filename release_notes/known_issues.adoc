...
Only OpenShiftSDN is supported as a CNI network provider. In particular, OVN is currently not supported.

[#submariner-no-rhel]
=== Submariner currently does not support Red Hat Enterprise Linux nodes as worker nodes
...
=== Submariner currently does not support all infrastructure providers ACM can manage

Not all infrastructure providers ACM can manage are supported with Submariner. Please refer to the support matrix page for more details.
...
=== Submariner currently does not support preparing OpenStack infrastructure from the ACM UI

Automatic cloud preparation for OpenStack clusters is currently not supported from the ACM UI. However, it works fine if the ACM APIs are used.

[#submariner-headless-services-globalnet]
=== Submariner currently does not support Headless Services with Globalnet

When using Headless Services with Globalnet, DNS resolution for local pods doesn’t work correctly.

[#submariner-airgap]
=== Submariner currently does not support air-gapped clusters

Submariner is currently not validated for air-gapped clusters.
