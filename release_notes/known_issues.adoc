...
[#submariner-vxlan]
=== Submariner does not support VXLAN with NAT enabled
//2.5:24258

Using Submariner with the VXLAN cable driver is not supported with NAT enabled. Submariner with the VXLAN cable driver is currently only supported in non-NAT deployments.
