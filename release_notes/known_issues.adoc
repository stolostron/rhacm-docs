...
After you upgrade {produc-title-short} from {product-version-prev} to {product-version}.x, `MultiClusterObservability` is deployed successfully, and the metrics data is displayed on the Grafana console, the `MultiClusterObservability` custom resource (CR) continues to display `Installing`.
