...
When a restore operation is run on a new hub cluster, using data backed up from another hub cluster, the application and policy shows no status for resources on remote clusters. This happens because the search and policy add-ons are not reset to point to the new hub cluster. 

From the new hub cluster, you must restart the `addon-certpolicyctrl` and `addon-search` for all spoke clusters.

//Does remote cluster mean offline cluster or local cluster? 
// I believe spoke should be updated to managed
