...
=== Ansible Automation Platform operator access {aap-short} outside of a proxy

The Ansible Automation Platform (AAP) operator cannot access {aap-short} outside of a proxy-enabled {ocp-short} cluster. To resolve, you can install the {aap-short} within the proxy. See install steps that are provided by {aap-short}.
