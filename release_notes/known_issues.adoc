...
Submariner supports headless services with Globalnet. However, when you access the exported headless service from a client that resides in the same cluster using `clusterset.local` domain name, the `globalIP` that is associated with the headless service is returned to the client, which is not routable in the cluster. 
