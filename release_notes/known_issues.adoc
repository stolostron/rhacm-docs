...
=== _klusterlet-addon-search_ pod crashing
//2.5:27173

The `klusterlet-addon-search` pod crashes. You must update the memory request and limit by customizing the `klusterlet-addon-search` deployment on your managed cluster. Edit the `ManagedclusterAddon` custom resource named `search-collector`. For example , if you have a managed cluster named `foobar`, run the following command:

----
oc edit managedclusteraddon -n foobar search-collector
----

Add the following annotation to the `search-collector` custom resource to change the memory request and limit. View the following example, where the request is set to `512Mi` and limit is set to `1024Mi`:
