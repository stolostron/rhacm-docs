...
[#submariner-disconnected]
=== Disconnected clusters are not supported

Submariner is not validated for clusters that are provisioned in a disconnected environment.
...
Globalnet is not supported with Red Hat OpenShift Data Foundation disaster recovery solutions. Make sure to use a separate range of private IP addresses in each cluster for multi-cluster disaster recovery scenarios.
