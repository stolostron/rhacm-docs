...
=== Ansible Automation Platform (early access) 2.0.1 and later job fail

When the Ansible Automation Platform (early access) 2.0.1 and later is installed, `AnsibleJobs` fails to run. If you want to submit prehook and posthook `AnsibleJobs` through {product-title-short}, use the Ansible Automation Platform Resource Operator 0.1.1.

Use the `cluster-scoped` option to resolve the issue.
//Please clarify what this option is, the issue does not have anymore details than this.

[#ansible-automation-operator-access]
=== Ansible Automation Platform operator access Ansible tower outside of a proxy

The Ansible Automation Platform (AAP) operator cannot access Ansible tower outside of a proxy-enabled {ocp-short} cluster. To resolve, you can install the Ansible tower within the proxy.
//Does user need steps for this
