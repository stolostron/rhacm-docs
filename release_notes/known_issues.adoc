...
When the Ansible Automation Platform is installed, `AnsibleJobs` fails to run. To submit prehook and posthook `AnsibleJobs` through {product-title-short}, use the `-cluster-scoped` option. The option is available in Ansible Automation Platform version `2.0.1+0.1635279521` and later.

The previously mentioned restrictions affect all components that need to perform Ansible jobs.
...
[#grc-ansible-automation-platform-fail]
=== Ansible Automation Platform (early access) job fail
...
The configuration of ansible automation job for governance policies are not functional.
