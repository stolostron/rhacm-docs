...
[#scrolling-firefox-hosted]
=== Scrolling does not work on Mozilla Firefox after provisioning hosted control plane cluster
//2.7:27107

After provisioning a hosted control plane cluster, you might not be able to scroll horizontally if the `ClusterVersionSucceeding` parameter is too long and you are accessing the {product-title-short} console from Mozilla Firefox. You are not able to view the hidden data as a result. To work around the issue, use a different browser. See link:../install/requirements.adoc#supported-browsers[Supported browsers] for a list of supported browsers.
...
[#azure-cluster-name-limitations]
=== Azure cluster name length limitations
//2.7:26928

The length of cluster names for Azure deployments cannot exceed 13 characters.
