...
After provisioning a hosted control plane cluster, you might not be able to scroll horizontally in the {product-title-short} console if the `ClusterVersionSucceeding` parameter is too long. You are not able to view the hidden data as a result. To work around the issue, zoom out by using the zoom control in your browser or increase the window size of the {product-title-short} console.
