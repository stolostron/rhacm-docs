...
Before you upgrade your cluster, disable cluster backup and restore by setting the `cluster-backup` flag to `false`. You can reenable the backup and restore component when the upgrade is complete.
