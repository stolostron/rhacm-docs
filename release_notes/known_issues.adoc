...
=== Two cluster curator controllers running at the same time after upgrade
// 2.5:22382

After you upgrade from 2.4.x to 2.5.0, there might be two cluster curator controllers running at the same time. The symptom could be that two or more AnsibleJobs are created for some of the pre/post hooks for Cluster Lifecycle Ansible integration. See the following procedure to work around this issue:

. Check to see if there are two cluster curator controllers running.
...
. If there are two cluster curator controllers running, delete the cluster-curator-controller in the open-cluster-management namespace.
