...
=== _appliedmanifestwork_ not removed and hub cluster placement rule does not have a fixed cluster set
//2.7+:ACM-7588
...
=== appliedmanifestwork not removed and agentID is missing in the spec
//2.7+:ACM-7588

When you are using {product-title-short} 2.6 as your primary hub cluster, the `agentID` is missing in the specification of `appliedmanifestworks` because the field is introduced in the 2.7 release. This results in the extra `appliedmanifestworks` for the primary hub on the managed cluster.

To avoid the issue, upgrade the primary hub to {

{product-title-short}
  2.7, then restore the backup on a new hub cluster.

Fix the managed clusters by setting the `spec.agentID` manually for each `appliedmanifestwork` as you see in the following example:

# get the agent ID
oc get klusterlet klusterlet -o jsonpath='{.metadata.uid}'

# set the spec.agentID for each appliedmanifestwork
