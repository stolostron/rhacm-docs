...
Ansible Automation Platform only works when the `-cluster-scoped` channel options are chosen. Otherwise, `AnsibleJobs` fail to run when you select an incompatible option. This affects all components that need to perform Ansible jobs.
