...
[#mtu-issue]
=== Submariner Globalnet can fail on MTU between on-premises and public clusters
//2.5:22261
...
----
iptables -t mangle -I POSTROUTING 1  -d <remote-ep-cidr>  -p tcp -m tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss <default-iface-mtu-120>
----

Replace `remote-ep-cidr` with the the CIDR of the endpoint.

Replace `default-iface-mtu-120` with a numeric value that is equal to 120 less than the default interface MTU value. 
