...
When the managed cluster is not detached successfully, the `ObservabilityAddon` resource `_observability-addon_` in the cluster namespace is stuck in the _Terminating_ status and cannot be removed. Also, the cluster namespace cannot be deleted.

To fix this problem, you can update the `ObervabilityAddon` resource in the cluster namespace. Update the resource by deleting the `_finalizers_` parameter in the metadata. Run the following command to remove the `_finalizers_` parameter:

//IS THERE A COMMAND TO DO THIS DIRECTLY? IS THERE A COMMAND TO ACCESS THE ADD-ON
//RESOURCE?
