...
Argo Application set from the console cannot be deployed on 3.x {ocp-short} managed clusters because the `Infrastructure.config.openshift.io` API is not available on  on 3.x.
...
=== Local pods cannot access exported headless services residing in same cluster when using Globalnet

Submariner supports headless services with Globalnet. However, when the exported headless service is accessed
from a client that resides in the same cluster using `clusterset.local` domain name, the globalIP associated with the headless service is returned to the client, which is not routable in the cluster.
This will be fixed in the next release. In the meantime, clients can use `cluster.local` domain name to access the local headless services.
