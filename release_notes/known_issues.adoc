...
[#cannot-edit-namespace-bindings-for-cluster]
=== Cannot edit namespace bindings for cluster
//2.6:25389

When you edit namespace bindings for a cluster set with the `admin` role or `bind` role, you might encounter the following error:
...
To work around the issue, make sure you also have permission to create or delete a `ManagedClusterSetBinding` resource in the namespace you want to bind. The role bindings only allow you to bind the cluster set to the namespace.
