...
=== Creating clusters with a disconnected installation console is invalid
//2.5:22808
...
When creating a credential for the bare metal, VMware vSphere, or Red Hat OpenStack Platform providers, the _Additional trust bundle_ field in the _Proxy and Configuration for disconnected installation_ contains the same value since the installer does not distinguish between the certificates. You can still use these features independently, and you can enter multiple certificates in the field if different ones are required for proxy and disconnected installation.
