...
This occurs because a URL query parameter must be passed through the Thanos Query front end data source, to perform additional queries for other downsampling levels when data is missing.

You must manually update the Thanos Query console configuration. Complete the following steps:

. Navigate to the Query Frontend data source in Grafa by going to the _Misc_ section.

. Find the _Custom query parameters field and select *`max_source_resolution=auto`*.

. Refresh your Grafana page to verify that the data is displayed.
