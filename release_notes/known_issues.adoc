...
When there is a new managed cluster where the `insight-client` pod cannot retrieve CCX insights, policy violations for the new maaged cluster are not displayed in the policy report. CCX insight policy violations appear after the `insights-client` restarts.
