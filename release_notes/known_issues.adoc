...
Both OpenShift SDN and OVN Kubentetes CNI networks are supported with Submariner, unless you are using Globalnet. Only OpenShift SDN is supported when you use Globalnet.
...
Submariner deployments with the VXLAN cable driver are currently only supported in non-NAT deployments.
