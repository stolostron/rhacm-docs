...
=== Importing a managed cluster with custom CA certificates to a restored hub cluster might fail
//2.4:19481

After you restore a hub cluster that managed a cluster with custom CA certificates, you must add the custom CA certificate information of the managed cluster to the `<managed_cluster>-admin-kubeconfig` secret in the namespace of your managed cluster on the restored hub cluster. If you do not, the cluster import to the restored hub cluster will fail. 
