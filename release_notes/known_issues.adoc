...
Ansible hook stand-alone mode is not supported. The Ansible hook can work with the hub cluster mode with the following procedure: 

. Create a placement rule that deploy to `local-cluster`. See the following sample:
