...
If you detach an online cluster immediately after it was attached, the Klusterlet starts to run on the detached cluster before the `manifestwork` syncs. Removal of the managed cluster from the hub cluster does not uninstall the Klusterlet.

Run the following clean-up script from the deploy repo to fix the issue:
