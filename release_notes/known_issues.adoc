...
After you upgrade from {product-title-short} version 2.0.4 to version 2.1 and run the `oc get csv` command, the _PHASE_ of your {product-title-short} ClusterServiceVersion (CSV) is `Pending`. The output resembles the following content:

----  
NAME                                 DISPLAY                                      VERSION   REPLACES                             PHASE
advanced-cluster-management.v2.1.0   Advanced Cluster Management for Kubernetes   2.1.0     advanced-cluster-management.v2.0.4   Pending
----

To workaround this issue, complete the following steps to find and create the missing `clusterRole` custom resource:
...
See the following steps to follow in the console:

. Click the `Subscription` node in the resource Topology diagram in the console.
. Click the `View Resource YAML` button in the subscription details pop-up.
...
For product technology preview 1.0, all channels need an individual namespace, except Git channels, which can share a namespace with another Git channel.
