...
[#operator-lifecycle-manager-causes-role-change]
===  Operator Lifecycle Manager causes a role change 
// 2.0.0:3693

The Operator Lifecycle Manager (OLM) causes a role change for placement rule. Placement rules editing should be disabled for the editor role and user should only have read access.
