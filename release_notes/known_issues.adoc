...
[#insight-client-not-enlisting-ks-clusters]
=== Insight-client not enlisting Kubernetes clusters
//2.4:19407

When there are new managed clusters where the `insight-client` pod cannot retrieve CCX insights, policy violations for the new managed clusters are not displayed in the policy report. CCX insight policy violations appear after the `insights-client` restarts.

Restart the `insight-client` pod by deleting the pod with the following command:

----
oc get pods -n open-cluster-management --no-headers=true | awk '/insights-client/{print $1}'| xargs oc delete -n open-cluster-management pod
----
