...
[#vmware-cluster-might-not-be-destroyed]
=== Process to destroy a VMware vSphere cluster does not complete

// 2.1.0:4748

When you destroy a VMware vSphere managed cluster, the status continues to display `Destroying`, and the cluster is not destroyed. To resolve this issue, manually remove the namespace of the managed cluster by running the following command:
