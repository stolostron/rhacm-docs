...
=== Adding observability add-on fails
//2.6:24512

The observability add-on is not deployed to the managed cluster if the size of the manifest exceeds 50 thousand bytes. When this happens, only the local cluster appears in grafana after observability is deployed.
