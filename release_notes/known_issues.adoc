...
When you upgrade to {product-version}.2 where observability is enabled, some clusters are degraded because the observability add-on is not ready. and deploy observability, you might receive the following error statement in the `MultiClusterObservability` CR: `Failed to find expected deployment observability-grafana`. 
