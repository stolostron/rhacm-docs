...
Due to the URL length limitation in some DHCP servers, you might receive the following error message in the console when booting a node:

`no bootable devices`

To work around the issue, complete the following steps:

. Apply an infrastructure environment custom resource definition when using an assisted installation to expose the `bootArtifacts`, which might resemble the following file:
...
. Copy the `bootArtifacts` and add them them to the proxy.
