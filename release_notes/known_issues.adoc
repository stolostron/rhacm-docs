...
You cannot create a managed cluster on a different architecture than the architecture of the hub cluster without creating a release image that contains files for both architectures. For example, you cannot create an `x86_64` cluster from a `ppc64le` hub cluster. The cluster creation fails because the {ocp-short} release registry does not provide a multi-architecture image manifest. 
...
. Create a release image on the hub cluster.

.. Create a file that contains information similar to the following example:
+
----
$ kubectl login
...
.. Run the file on your hub cluster to apply the changes.

. Select this release image when you create your {ocp-short} cluster. 
