...
When you delete a managed cluster that was created with the {product-title-short} console, the `clusterdeployment` of the managed cluster might get stuck in a terminating state. To delete this `clusterdeployment`, manually clean the finalizers of this `clusterdeployment`.
