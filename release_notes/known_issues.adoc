...
[#appliedmanifestwork-not-removed-after-restoring]
=== The _appliedmanifestwork_ is not removed from the managed cluster after restoring the hub cluster
//2.7:27129

The `appliedmanifestwork` is not removed from managed clusters that have a placement rule for an application subscription that is not a fixed cluster set when the hub cluster data is restored on the new hub cluster.
...
This results in the application being orphaned when the managed cluster is detached from the restored hub cluster.
