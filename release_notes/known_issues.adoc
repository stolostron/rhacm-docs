...
=== Deployable resource with empty specification crashes the pod
// 2.2:9953 app team plans to fix in 2.3

Applying a deployable resource with empty specification crashes the pod `multicluster-operators-application` container `multicluster-operators-deployable`.
...
A deployable resource without specification is not needed. Delete the unnecessary deployable and restart the `multicluster-operators-application` pod.
