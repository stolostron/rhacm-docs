...
Any annotations defined in the `prepareForBackup` parameter is not functioning for resources created after the schedules creation. This affects Hive and AI secrets that are being annotated before the backup is initiated.
...
Update the `BackupSchedule`, `veleroSchedule`, or `veleroTTL` values to start a new set of schedules. Then use the resulted backup for a restore, which is defined to annotate the latest resources for the backup.
//What's AI? Assisted installer? If yes, that is currently listed as Infrastructure Operator for Red Hat OpenShift for our conref {assist-install}
