...
When deploying Submariner on a cluster that includes Red Hat Enterprise Linux worker nodes, application work-loads fail to communicate with remote clusters. Submariner only supports {ocp-short} clusters that use Red Hat Enterprise Linux CoreOS nodes.
