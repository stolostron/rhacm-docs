...
The IAM policy controller only checks for users with cluster administrator permissions. Users who are in groups and have cluster administrator privileges, are not recognized by the controller. 

//would it be more accurate to say, "The IAM policy
//controller only recognizes standard Kubernetes
//resources. `Group` and `User` resources are not
