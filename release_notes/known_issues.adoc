...
+
----
oc edit clusterdeployment/<mycluster>
----
+
Replace _mycluster_ with the name of the managed cluster that you are destroying.

. Ensure that the `clusterdeployment` is stopped by editing the content to remove the `hive.openshift.io/deprovision` finalizer.
