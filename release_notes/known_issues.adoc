...
When you detach a managed cluster, the _Create resources page_ might temporarily break and displays the following error:

----
Error occured while retrieving clusters info. Not found.
----

Wait until the namespace automatically gets removed, which takes about 5-10 minutes after you detach the cluster, or manually delete the namespace. You can return to the page to see if the error resolved.
