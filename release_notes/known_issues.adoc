...
After creating or modifying a policy, the placement rule and the policy binding might be empty in the policy details of the {product-title-short} console. This is generally because the policy is disabled, or some other change was made to the policy. Ensure that the settings are set correctly for the policy in the YAML view.
