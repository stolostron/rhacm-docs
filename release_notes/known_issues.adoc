...
[#git-kustomize-inline-patch]
== Git subscription kustomize inline patch error

// 2.0:3792

When you subscribe to a Git repository that contains kustomization, kustomization build does not work if the `kustomization.yaml` contains an inline patch where the patch content inside the file as a single string. 

To resolve this issue, use patch YAML files instead of inline patch. 
