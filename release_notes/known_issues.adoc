...
=== Upgrade from version 2.2.x to 2.3.1 fails with ImagePullBackOff error
// 2.3.1: 15059

When you upgrade your {product-title-short} from version 2.2.x to 2.3.1, the upgrade fails to progress.

When you upgrade from version {product-title-short} 2.2.x to 2.3.1, your upgrade might fail. The `Multiclusterhub` status displays: `failed to download chart from helm repo` in the error messages and status shows `failed to download chart from helm repo` in the component error messages. You may also see errors that reference a problem with `no endpoints available for service "ocm-webhook"`.
...
*Note:* The errors resolve, but the reconciliation process may not happen immediately. This can be accelerated by restarting the `multicluster-operators-standalone-subscription` in the namespace the product is installed.
