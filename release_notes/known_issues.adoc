...
If you upgrade your cluster from {product-version-prev} to {product-version} with the `cluster-backup` parameter set to `true`, the following message appears:

----
When upgrading from version 2.4 to 2.5, cluster backup must be disabled
----

Before you upgrade your cluster, disable cluster backup and restore by setting the `cluster-backup` flag to `false`. The `components` section in your `MultiClusterHub` resource might resemble the following YAML file:
...
You can reenable the backup and restore component when the upgrade is complete.
