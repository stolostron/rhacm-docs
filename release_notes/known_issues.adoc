...
When the Ansible Automation Platform is installed, `AnsibleJobs` fails to run if the `-cluster-scoped` options are not chosen. The option is available in Ansible Automation Platform version `stable-2.1` and later.
