...
Any labels defined in the `prepareForBackup` function is not added to resources created after the schedules creation. This affects Hive and {assist-install} secrets that are being annotated before the backup is initiated.
