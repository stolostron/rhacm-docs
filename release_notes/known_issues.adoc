...
. Copy the link that is not resolving to the correct section and paste it in your browser address bar. For example, it might be: `https://access.redhat.com/documentation/en-us/red_hat_advanced_cluster_management_for_kubernetes/2.5/html/clusters/index#volsync`.

. In the link, replace `html` with `html-single`. The new URL should read: `https://access.redhat.com/documentation/en-us/red_hat_advanced_cluster_management_for_kubernetes/2.5/html-single/clusters/index#volsync`
...
=== Dark mode is not supported on {product-title-short} version 2.5.x
//2.5.2:25554

While {product-title-short} version 2.5.2, and later 2.5.x versions are supported on {ocp} version 4.11, dark mode is not supported on {product-title-short} 2.5.x. Either disable dark mode in the settings, or upgrade to {product-title-short} version 2.6 to enable dark mode. 

[#dark-mode-not-supported-mce]
=== Dark mode is not supported on multicluster engine for Kubernetes operator version 2.0.x
//2.5.2:25554

While multicluster engine for Kubernetes operator version 2.0.2, and later 2.0.x versions are supported on {ocp} version 4.11, dark mode is not supported on multicluster engine for Kubernetes operator 2.0.x. Either disable dark mode in the settings, or upgrade to multicluster engine for Kubernetes operator version 2.1 to enable dark mode.
...
When you edit namespace bindings for a cluster set with the `admin` role, you might encounter an error that resembles the following message:
...
=== lookup template function error

As a reminder, when you use the `lookup` function to request a Kubernetes resource and it does not exist, an empty map is returned. If the resource does not exist and the value is added to another template function, you might get the following error, `invalid value; expected string`.
...
[#preparedforbackup-called-when-velero-schedules-are-created]
=== prepareForBackup is only called when Velero schedules are first created
//2.5.1:24012

Any labels defined in the `prepareForBackup` function are not added to resources created after the schedules creation. This affects Hive and {assist-install} secrets that are being labeled before the backup is initiated.

View the list of affected resources:

* Secrets that are used by `clusterDeployments` and created by cluster claims
* Cluster pool secrets
* Secrets with the labels, `agent-install.openshift.io/watch` and `environment.metal3.io`

Update the `BackupSchedule`, `veleroSchedule`, or `veleroTTL` values to start a new set of schedules. Then use the resulted backup for a restore, which is defined to label the latest resources for the backup.
