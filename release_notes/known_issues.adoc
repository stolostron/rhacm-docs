...
The summary card on the console *Overview* page shows the total number of nodes across all clusters, even when you select an infrastructure provider tile.  All other values are filtered according to the selected tile. To see the correct number of nodes for the selected infrastructure provider, click on the total number of nodes in the summary card and check the related node count in the search results.
