...
Any labels defined in the `prepareForBackup` function is not added to resources created after the schedules creation. All other occurrences of the annotation must be updated with the label. This affects Hive and {assist-install} secrets that are being annotated before the backup is initiated.
