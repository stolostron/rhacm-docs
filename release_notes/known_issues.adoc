...
- From the _Applications_ table on the _Overview_ page, the _Clusters_ column on each table displays a count of clusters where application resources are deployed. Since applications are defined by _Application_, _Subscription_, _PlacementRule_, and _Channel_ objects on the local cluster, the local cluster is included in the search results, whether actual application resources are deployed on the local cluster or not.
