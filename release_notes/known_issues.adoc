...
When the number of users with permissions to a given `ClusterRole` is determiined, the IAM policy controller only checks for Kubernetes `User` resources and does not consider users in Kubernetes `Group` resources.
