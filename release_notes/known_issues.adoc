...
. If search is disabled on the hub cluster, which means the `search-redisgraph-pod` is not deployed, reduce memory usage by reducing the `search-collector` to `0` replicas.

. If search is enabled on the hub cluster, which means the `search-redisgraph-pod` is deployed, increase the memory usage by editing the `search-collector` deployment.
