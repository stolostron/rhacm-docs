...
When you remove the `config-policy-controller` addon from a managed cluster by disabling the policy controller in the `KlusterletAddonConfig or by detaching the cluster, the `ConfigurationPolicy` CRD might get stuck in a terminating state.

This can cause new policies to not be added to the cluster if the addon is re-installed later. You can also receive the following error:
