...
OpenShiftSDN is the only supported CNI network provider when Globalnet is enabled. OVN is currently not supported.
...
Headless services with Globalnet is supported in many cases. However, when you access the exported headless service from a client that resides in the same cluster by using `clusterset.local` domain name, the `globalIP` that is associated with the headless service is returned to the client. In this case, the `globalIP` is not routable in the cluster. 
...
Submariner deployments with the VXLAN cable driver is currently only supported in non-NAT deployments.
