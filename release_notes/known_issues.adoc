...
=== The _appliedmanifestwork_ is not removed after restoring the hub cluster
//2.7:27129

If the placement rule for an application subscription is not a fixed cluster set, the `appliedmanifestwork` from the old hub cluster remains on the new hub cluster after restoring.
