...
[#unsupported-{ocp-short}-versions-listed-after-upgrade]
=== Unsupported {ocp-short} versions listed after hub cluster upgrade
// 2.6:25467

After you upgrade your hub cluster from versions before {previous-version} to {product-version}, some unsupported {ocp-short} versions are listed on the _Cluster_ page from the console.

//i assume orphan is similar to 'stale'. What are your thoughts about using stale instead of orphan?
The orphan `clusterImageSet` resources deployed by earlier versions before {previous-version} subscription controller are not deleted after the upgrade. To resolve this, manually delete the `clusterImageSet` resources that have unsupported {ocp-short} versions. For example, run the following command to delete the `img4.7.0-x86-64-appsub` `clusterImageSet`:
