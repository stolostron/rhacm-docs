...
When you restore the hub cluster data to a new hub cluster, the the existing application subscription on the managed cluster is not deleted, even after the managed cluster is removed from the placement cluster decision list.

You can work around the issue by completing to following steps:
