...
[#namespace-channel-and-subscription-failed]
=== Namespace channel and subscription failed
// 2.0.0:3581
...
To get out of `FAILED` state, complete the following steps:

. Log in to your hub cluster.
. Manually add a label to the subscription using the following command:

----
kubectl label subscriptions.apps.open-cluster-management.io the_subscription_name reconcile=true
----
