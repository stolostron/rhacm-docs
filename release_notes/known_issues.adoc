...
You cannot use Ansible Tower credentials with a host URL containing an insecure protocol (http) to create automation templates. When you use an Ansible Tower credential with an insecure protocol in the automation wizard, the job list cannot be populated in the dropdown. To work around this issue, use a secure protocol (https) in the host URL.

*Note:* The console prevents you from creating Ansible Tower credentials with a host URL containing an insecure protocol, but you can still manually create or patch a resource using an insecure protocol.
