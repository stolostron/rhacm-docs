...
//how does the user upgrade from the console? Do they navigate to the cluster page and select the Action icon? 
. Upgrade to 2.2.5.
//How would the user check the redisgraph statefulset? What page do they need to go to?
. Log in as an administrator and check the `redisgraph` StatefulSet.
. Notice that the StatefulSet and pod did not restart.
//Is there an action icon with a list of actions and the user can select delete?
. Delete the `redisgraph` StatefulSet.
. Verifiy if the `redisgraph` pod is updated and running successfully.
