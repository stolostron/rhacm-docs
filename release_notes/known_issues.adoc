...
After provisioning a hosted control plane cluster, you might not be able to scroll horizontally if the `ClusterVersionSuceeding` parameter is too long and you are accessing the {product-title-short} console from Mozilla Firefox. You are not able to view the hidden data as a result. To work around the issue, use a different browser. See link:../install/requirements.adoc#supported-browsers[Supported browsers] for a list of supported browsers.
