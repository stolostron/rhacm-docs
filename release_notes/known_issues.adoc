...
//is ClusterID the parameter name on the form? Is it required to use the camel case or is my suggestion sufficient?
When the cluster ID is missing for a newly added managed cluster, the `insight-client` pod does not enlist Kubernetes clusters. Restart the `insight-client` pod by deleting the pod with the following command:

----
oc delete po -n open-cluster-management insight-client
----
