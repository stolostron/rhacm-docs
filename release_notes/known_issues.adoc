...
When the Ansible Automation Platform (early access) is installed, AnsibleJobs fails to run. If you want to submit prehook and posthook AnsibleJobs through {product-title-short}, use the `early-access-cluster-scoped` option to resolve the issue. The options is available in the Ansible Automation Platform (early access) version `2.0.1+0.1635279521` and later.
