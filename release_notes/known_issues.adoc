...
[#submariner-no-rhel]
=== Submariner network loses conenection with clusters that ues Red Hat Enterprise Linux nodes

When you connect a Submariner network with at least one {ocp-short} cluster that uses Red Hat Enterprise Linux nodes, the connection fails when the cluster communicates with another cluster. Submariner only supports {ocp-short} clusters that use Red Hat Enterprise Linux CoreOS nodes.
