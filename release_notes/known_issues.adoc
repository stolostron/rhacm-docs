...
When you remove the `config-policy-controller` addon from a managed cluster by disabling the policy controller in the `KlusterletAddonConfig` or by detaching the cluster, the `ConfigurationPolicy` CRD might get stuck in a terminating state. If the `ConfigurationPolicy` CRD is stuck in a terminating state, new policies might to not be added to the cluster if the addon is re-installed later. You can also receive the following error:

----
NonCompliant; Failed to create policy template create not allowed while custom resource definition is terminating
----
...
If a deletion timestamp is present on the resource, the CRD is stuck. To resolve the issue, remove all finalizers from configuration policies that remain on the cluster. Use the following command on the managed cluster and replace '<cluster-namespace>' with the managed cluster namespace:
...
The configuration policy resources are automatically removed from the cluster and the CRD will exit its terminating state. If the addon has already been re-installed, the CRD will be recreated automatically and it will not have a deletion timestamp.
...
See link:../backup_restore/backup_and_restore.adoc#hub-backup-and-restore[Cluster backup and restore operator] for more information.
...
You must manually apply any configuration changes that are related to the `local-cluster` resource on the restored cluster. See link:../backup_restore/backup_and_restore.adoc#prepare-new-hub[Prepare the new hub cluster] for more details. 
