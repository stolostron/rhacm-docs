...
In rare cases with disconnected environments, some pods of the observability `StatefulSet` never reach the `Running` and `Ready` status because the pods cannot pull the images. The images defined in those pods are different from the ones defined in the related `StatefulSets`. To fix this problem, you need to delete the pods that use the wrong images. The newly launched pods should use the correct images.

// do the poods restart automatically? 
// I think it would be best to say what status the pods are stuck in instead

// In rare cases with disconnected environments, some pods of the observability `StatefulSet` are stuck in the following status, `ErrImagePull` status because the pods cannot pull the images.
