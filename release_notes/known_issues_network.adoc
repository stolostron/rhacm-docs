...
To preserve your source IP for hub cluster traffic on the OVN Kubernetes versions, enable the Submariner SNAT handler. Complete the following steps:

. Create a `ConfigMap` resource named `submariner-global` in the `submariner-operator` namespace with the following configuration. Create the following resource to configure Submariner to manage SNAT and override the default behavior of OVN-Kubernetes:
...
. Restart the `route-agent` pods apply the `ConfigMap` resources changes. Run the following command:
