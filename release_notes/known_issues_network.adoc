...
To preserve your source IP for inter-cluster traffic on the OVN Kubernetes versions, enable the Submariner SNAT handler on all managed clusters. Complete the following steps:

. Create a `ConfigMap` resource named `submariner-global` in the `submariner-operator` namespace to configure Submariner to manage SNAT and override the default behavior of OVN-Kubernetes. See the following YAML file example:
...
data:
  {{ enable-snat-handler: "true"}}
----

. Restart the `route-agent` pods for the `ConfigMap` resource to save changes. Run the following command:
