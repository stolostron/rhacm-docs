...
If you are using {ocp-short} 4.18 with OVN-Kubernetes, the source IP is not retained when packets reach the destination pod. As a result, applications that rely on the source IP, such as NetworkPolicy, might not work correctly.
