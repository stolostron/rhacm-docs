...
For versions 2.8 and earlier, when you install {product-title-short}, you also deploy the submariner-addon component with OLM. If you did not create a `MultiClusterHub` custom resource, the `submariner-addon` pod sends an error and prevents the operator from installing. 

The following occurs because the `ClusterManagementAddon` custom resource definition is missing:
...
When the `MultiClusterHub` custom resource is created, if there is not a `MultiClusterEngine` resource that is already available on the cluster, the `MultiClusterHub` operator deploys the `MultiClusterEngine` instance and the operator that is required, resolving the previous error.
...
[#submariner-no-support-acm28-and-ocp414]
=== Submariner version 0.15 is not supported when using {product-title-short} version 2.8 with {ocp-short} version 4.14
//2.9:ACM-8276

Submariner version 0.15, which was released with {product-title-short} version 2.8, is not supported with {ocp-short} version 4.14, or later. You must either upgrade your Submariner version to 0.16 when using {ocp-short} version 4.14, or later, or continue using {ocp-short} version 4.13 with Submariner version 0.15 and {product-title-short} version 2.8.
