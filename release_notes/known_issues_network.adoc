...
If you are using {ocp-short} 4.18 with OVN-Kubernetes for Submariner, the source IP is not retained when packets reach the destination pod. As a result, applications that rely on the source IP, such as `NetworkPolicy`, might not work correctly.

[#cluster-management-addon-needed]
=== Without _ClusterManagementAddon_ submariner add-on fails
//2.8:24159
//2.8 is EOL so this should really be removed. Much of this is old and Submariner team should have reviewed this as the cross-squad effort is intiated via story/tasks -- bcs 7/12
