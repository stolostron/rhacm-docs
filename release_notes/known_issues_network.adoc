...
=== Submariner addon resources not cleaned up properly when `managedclusters` are imported 
//2.9:ACM-8549

The `SubmarinerAddon` finalizers are not cleaned up properly for the managed clusters resources. Since the finalizers are not cleaned up properly, they prevent the `submariner-addon` component from being disabled within the hub cluster. 
