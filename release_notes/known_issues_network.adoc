...
[#submariner-sdn-cni]
=== Submariner only supports OpenShift SDN CNI or OVN Kubernetes

Submariner only supports OpenShift clusters that use the OpenShift SDN CNI or the OVN Kubernetes CNI network provider.
