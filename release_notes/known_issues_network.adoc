...
Submariner only supports OpenShift clusters that use the OpenShift software-defined networking (SDN) or the OpenShift virtualized network (OVN)-Kubernetes Container Network Interface (CNI) network provider.
