...
The Submariner add-on installation fails on VMware vSphere running an {ocp-short} 4.15 and later hub cluster because of a missing broker secret for `ManagedClusters` on the hub cluster. Only the `submariner-addon` pod is created in the `submariner-operator` namespace on `ManagedClusters` and the console shows gateways that are not labeled.

You can work around the issue by creating the secret manually for each `ManagedCluster` in the `ClusterSet` broker namespace. To create the secret manually, complete the following steps:
