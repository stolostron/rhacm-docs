...
During the `kube-apiserver` restart, your cluster table in the console might show clusters as `offline`, then `online` randomly for an extended period of time, then stabilizes. Currently there is no workaround for this issue.
