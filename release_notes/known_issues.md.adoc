...
A user performing in an `Editor` role should only have read or update authority on an application, but can also create and delete an application. Red Hat Open Shift Operator Lifecycle Manager default settings change the setting for the product.
