...
[#No-longer-supporting-Argo-CD-pull-model]
== Application console does not support Argo CD pull model
// 2.8.6365

When you deploy `ApplicationSet` applications using the Argo CD pull model, the application is displayed incorrectly from the _Topology_ page. Deploying `ApplicationSet` applications using the Argo CD pull model is not supported. 
