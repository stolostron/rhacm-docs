...
When you deploy `ApplicationSet` resources using the Argo CD pull model, the application is displayed incorrectly from the _Topology_ page. Deploying `ApplicationSet` applications using the Argo CD pull model is not supported. 
