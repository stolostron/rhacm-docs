...
Review the known issues for Red Hat Advanced Cluster Management for Kubernetes. 
  ...
  - [User `kubeadmin` gets extra browser tab opens with blank page](#user-kubeadmin-gets-extra-browser-tab-with-blank-page)
...
## User `kubeadmin` gets extra browser tab opens with blank page

When you log in as `kubeadmin` with the logout option in the drop-down menu, the console returns to the login screen, but a browser tab opens with a `/logout` URL. The page is blank and you can close the tab without impact to your console.
