...
* *Technology Preview:* You can configure fine-grain role based access control (RBAC) to restrict metrics access. For an example on how to use RBAC, see link:../observability/customize_observability.adoc#configure-fine-grain-rbac[Configuring fine-grain RBAC (Technology Preview)].
