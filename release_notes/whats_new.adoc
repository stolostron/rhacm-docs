...
* You can now use the external API for metrics to be queried through the OpenShift route, `rbac-query-proxy`. See link:../observability/observability_enable.adoc#external-metric-query[Using the external metric query] for more information.
...
* You can now create a managed cluster from the {product-title-short} console on the Red Hat OpenStack Platform. See link:../create_openstack.adoc#creating-a-cluster-on-openstack[Creating a cluster on Red Hat OpenStack Platform] for more information.  
...
* *Technology Preview:* You can test the ability to host the hub cluster on IBM Power systems.
...
* *Technology Preview:* You can schedule AnsibleJob templates to run before or after cluster creation or installation by using the console. See link:../clusters/ansible_config_cluster.adoc#ansible-config-cluster[Setting up Ansible Tower tasks with cluster installation and upgrades (Technology Preview)] for more information.
