...
You can now subscribe to a secondary channel if you have a mirrored channel (application source repository). Secify a `secondaryChannel` in the subscription YAML when an application subscription fails to connect to the repository server using the primary channel.
