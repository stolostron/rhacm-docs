...
* Partial matching for search is supported. See link:../observability/search_intro.adoc#search-operations[Search operations and data types].
...
* You can now use `object-templates-raw` to generate a `ConfigurationPolicy`. See link:../governance/policy_generator.adoc#policy-generator-configuration[Policy Generator configuration structure] for more details.
