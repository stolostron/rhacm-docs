...
{product-title} is now a generally available product. See what is available in version {product-version}.
...
With operator-based installation, you can install a {ocp} cluster on a configured cloud provider, such as Amazon Web Services, in less than 10 minutes.
...
You can provision and manage {ocp} clusters on selected Kubernetes cloud service providers.
...
* Manage all of your {ocp} cluster upgrades in one interface.
You can upgrade imported and provisioned {ocp} clusters either individually or in groups by using the console.
