...
* Dynamic metric collection for single node OpenShift (SNO) clusters is supported. For more information, see link:../observability/observability_enable.adoc#dynamic-metrics-for-sno[Dynamic metrics for single-node OpenShift clusters].
...
* The `hypershift` add-on helps you host {ocp-short} control planes at scale, separating management from workloads. The component is disabled by default.
  ...
  ** See the {product-title-short} documentation at link:../install/adv_config_install.adoc#hypershift-addon-intro[Hypershift add-on (Technology Preview)] and link:../clusters/hosted_control_planes_intro.adoc#hosted-control-plane-intro[Using hosted control plane clusters (Technology Preview)] to learn more.
    
  ** Use HyperShift to manage and provision hosted control plane clusters. See link:../clusters/hosted_control_planes_intro.adoc#hosted-control-plane-intro[Using hosted control plane clusters (Technology Preview)] for more information.
