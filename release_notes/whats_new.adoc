...
* Configure a debug log for your policy controllers. See link:../governance/policy_ctrl_adv_config.adoc#configure-debug-log[Configure debug log]
//will update what's new (#27147) | MJ | 01/23 
* The policy engine now supports ordering how policies are applied through dependencies, creating a hierarchy. See link../governance/

* Policy generator now references local and remote Kustomize configurations for enhanced flexibility. See link:../governance/
