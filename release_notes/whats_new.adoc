...
* The `MultiClusterObservability`, `SubmarinerConfig`, and `SubmarinerDiagnoseConfig` tabs are now removed from the _Installed Operators_ page for {acm-short}. To access the _CustomResources_, go to the _CustomResourceDefinitions_ page and create or update the `CustomResources` property.
