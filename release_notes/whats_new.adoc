...
// - Visual Web Terminal: keep the updates here (new function only that he wants to highlight) status grid and scrolling
  - You can now view a status grid of your pods. As you view multiple pods during the session, you can scroll the split screen to see a history of the pods that you viewed. See link:../console/vwt.adoc#starting-a-session[Starting a session] for more information.
