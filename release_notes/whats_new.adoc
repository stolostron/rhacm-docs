...
Observability is now enabled by default on the hub cluster and is not an add-on anymore. As a result, corresponding pods in the `open-cluster-management-addon-observability` namespace are removed. The multicluster observability operator now launches and manages the endpoint operator and metrics collector in the `open-cluster-management-observability` namespace. See link:../observability/observability_arch.adoc#observability-arch[Observability architecture] to learn more.
