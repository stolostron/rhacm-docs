...
MultiClusterHub now accepts `ClusterPermission`, `MultiClusterObservability`, and `SubmarinerAddon` as toggleable components within the MultiClusterHub CR. This means that users can choose to enable or disable these components functionality by default when installing {product-title-short}.
