...
* You can now use a restore hub as a cluster with resources that a user created before the restore operation. You can tag user created resources on the restore hub with the `velero.io/backup-name: backupName` label. See link:../business_continuity/backup_restore/backup_existing_clusters.adoc#backup-existing-clusters[Understanding outcomes for existing managed clusters during the restore process] and link:../backup_restore/backup_tag_resources.adoc#backup-tagging-resources[Tagging resources].
