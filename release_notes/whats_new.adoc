...
* You can now add recording rules in the observability service to specify which metrics are being collected by the `metrics-collector`. For more information, see link:../observability/customize_observability.adoc#adding-custom-metrics[Adding custom metrics].
