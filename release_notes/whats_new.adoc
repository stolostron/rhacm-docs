...
When you enable the backup component on the `MultiClusterHub`, the cluster backup and restore operator Helm chart installs the `backup-restore-enabled` policy. This policy informs you about issues with the backup and restore component. With {prod-short} 2.10 or later,  the `backup-restore-enabled` policy includes a new policy template named, `OADP-channel`. The `OADP channel` template checks if the installed OADP Operator version matches the version set by the {prod-short} cluster backup and restore operator. Use the `OADP-channel` template  to prevent your backup and restore operator from running with the wrong custom resource definitions (CRDs). 


OADP operator. The `OADP-channel` template checks if the installed Red Hat OADP Operator version matches the version set by the {prod-short} cluster backup and restore operator. The  `OADP-channel` template helps you prevent your operator from running with the wrong custom resource definitions (CRDs). 
