...
* To create and mount secrets to your `alertmanager` pods for access to authorization credentials, you can add the contents to your `MultiClusterObservability` resource. See link:../observability/observability_alerts.adoc#mount-secrets-alertmanager[Mounting secrets within the Alertmanager pods].
