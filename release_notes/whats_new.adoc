...
You can now edit the HiveConfig resource directly, and the MultiClusterHub Operator will not revert the changes. If the HiveConfig resource is deleted, the MultiClusterHub Operator will recreate it exactly as it was configured when the MultiClusterHub resource was first created.
