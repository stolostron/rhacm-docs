...
* When you create a managed cluster, a `ManagedClusterSet` called `global` is automatically created to ease management. You cannot edit the `global` managed cluster set. See link:../multicluster_engine/cluster_lifecycle/managedclustersets_intro.adoc##managedclustersets_global[Global ManagedClusterSet] for more details.
