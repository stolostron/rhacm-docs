...
* The `hypershift` add-on component allows you to create and delete a Service Account on a managed cluster. The component is disabled by default. 

  ** See the _multicluster engine operator_ documentation at link:../multicluster_engine/addon_managed_service.adoc#serviceaccount-addon-intro[ManagedServiceAccount add-on (Technical Preview)] to learn more.
