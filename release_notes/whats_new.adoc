...
* Previously if a policy were deleted and recreated with the same name, it would populate its status with the events from the deleted policy. Now, the status will only contain updates relevant to the current policy since its creation and won't consider the old events.
