...
* MultiClusterHub now accepts `cluster-permission`, `multicluster-observability`, and `submariner-addon` as toggleable components within the MultiClusterHub CR. Users can choose to enable or disable these components functionality by default when installing {product-title-short}.

* With the migration of multicluster-observability and submariner-addon outside of the {product-title-short} bundle, the `MultiClusterObservability`, `SubmarinerConfig`, and `SubmarinerDiagnoseConfig` tabs are now removed from the Installed Operators page for {product-title-short} on the {ocp-short} UI page.
