...
{product-title-short} now supports all providers that are certified through the Cloud Native Computing Foundation (CNCF) Kubernetes Conformance Program. Choose a vendor that is recognized by CNFC for your hybrid cloud multicluster management.

* Learn how providers are certified at link:https://www.cncf.io/training/certification/software-conformance/[Certified Kubernetes Conformance]. 

* For support information about CNFC third-party providers see [_https://access.redhat.com/third-party-software-support_]

* Run the RHOCP `oc` command for Red Hat supported cluster management. If you bring your own CNFC conformance certified cluster, change `oc` to the Kubernetes CLI, `kubectl`.
