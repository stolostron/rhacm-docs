...
* *Technology Preview:* Enable global search to include resources from all clusters managed by {global-hub}. Global search expands the search capabilities when you use {global-hub} to manage your environment. See, link:../observability/manage_search.adoc#customizing-search-console[Customizing the search console].
