...
* You can use the `configurationPolicyAnnotations` parameter in the policy generator resource to specify key-value pair annotations on configuration policies. See link:../governance/policy_generator.adoc#policy-gen-install-on-openshift-gitops[Install the policy generator on OpenShift GitOps (ArgoCD)] for more details.
