...
| Governance
| Replacing certificate management ingress is no longer supported.
| 2.7
| If you brought your own certificates to use with the management ingress, you must remove the certificates using the following command: `oc -n open-cluster-management delete secret byo-ca-cert byo-ingress-tls-secret`
| 
...
| See link:../clusters/cluster_lifecycle/ansible_config_cluster.adoc#ansible-template-run-cluster-console[Configuring an AnsibleJob template to run on a cluster by using the console] for more information.
