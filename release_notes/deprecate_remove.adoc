...
| 2.5
| Create a host inventory with the console
| See link:../multicluster_engine/create_bare.adoc#creating-a-cluster-on-bare-metal[Creating a cluster on bare metal] for the affected deprecated areas. See link:../cluster_lifecycle/create_cluster_on_prem.adoc#creating-a-cluster-on-premises[Creating a cluster in an on-premises environment] for the proceding process.
