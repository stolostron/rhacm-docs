...
| Search
| `SearchCustomization.open-cluster-management.io` custom resource definition is removed.
| 2.7
| Use custom resource definition `search.open-cluster-management.io/v1alpha1`. 
| 

| Search
| PostgreSQL replaced RedisGraph as the internal database.
| 2.7
| No change required. 
| The search component was reimplemented using PostgreSQL as the internal database.
