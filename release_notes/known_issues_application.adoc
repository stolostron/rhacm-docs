...
[#upgrading-permission-errors]
== Upgrading to {gitops} 1.17 causes permission errors 

When you upgrade your {gitops-short} operator to version 1.17, you get permission errors in your `ApplicationSet` resource status. 
...
Learn about {ocp} SCC at link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html-single/authentication_and_authorization/index#managing-pod-security-policies[Managing security context constraints], which is an additional configuration required on the managed cluster.
