...
If you create a `ClusterPermission` resource that contains multiple role bindings within the same namespace and do not set the optional `name` field, it fails. This error occurs because the same name is used for the role bindings in the namespace.
