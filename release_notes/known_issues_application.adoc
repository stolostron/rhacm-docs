...
Argo `ApplicationSet` from the console cannot be deployed on 3.x {ocp-short} managed clusters because the `Infrastructure.config.openshift.io` API is not available on 3.x.
...
Learn about {ocp} SCC at link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html-single/authentication_and_authorization/index#managing-pod-security-policies[Managing security context constraints], which is an additional configuration required on the managed cluster.
