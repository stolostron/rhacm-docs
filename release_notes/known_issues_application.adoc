...
When you create a service account in {ocp} 4.15 provisioned by some cloud providers, such as IBM VMware and Bare Metal, then the account does not automatically create a secret. Therefore, the {product-title-short} `gitopsCluster` controller fails to generate the managed cluster secret for the Argo CD push model. {ocp} 4.14 provisioned by IBM VMware and {ocp} 4.15 with any provider, do not observe this issue. This issue is delivered in {product-title-short} 2.10.3 and in {product-title-short} 2.9.4. 
