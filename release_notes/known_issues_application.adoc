...
In {ocp} 4.15 provisioned by IBM VMware, when you create a service account, it does not automatically create a secret. Therefore, the {product-title-short} `gitopsCluster` controller fails to generate the managed cluster secret for the ArgoCD push model. {ocp} 4.14 provisioned by IBM VMware and {ocp} 4.15 with any provider, do not observe this issue.  
