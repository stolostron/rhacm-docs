...
== _ClusterPermission_ resource fails when creating multiple role bindings
// 2.14:21342

The `ClusterPermission` resource fails when you create the resource that contains multiple role bindings within the same namespace and do not set the optional `name` field. This occurs because the same name is used for the role bindings in the namespace.
