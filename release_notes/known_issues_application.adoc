...
If you have {ocp-short} 4.17, the search `serviceaccount` is missing some permissions it needs to function like a user. Therefore, the Argo CD pull model resource sync controller fails to access the application resource list from the search API.
...
.  Restart the `search-api` pod by running the following command: 
+
[source,bash]
----
