...
When you use the Argo CD pull model to deploy `ApplicationSet` applications, the application resource names are being customized, and they might appear different for each cluster. When this happens, the topology does not display your application correctly. 
