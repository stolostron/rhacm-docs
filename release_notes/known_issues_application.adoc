...
When you deploy the subscription application, it will display a warning message for the subscription node in the application topology page. If you check the subscription node details, it incorrectly shows the local-cluster is offline. Check the real status of the local-cluster in the ACM console in menu:Infrastructure[Clusters page].
