...
Subscriptions that use `cluster-admin` privilege elevation on a managed cluster experience a delay in initial resource deployment. The subscription controller confirms cluster-admin eligibility by checking whether the subscription appears in `status.appliedResources` field of the `AppliedManifestWork`. The work agent populates the field at different times after you create the subscription.

If first local subscription reconcile runs before the work agent updates that status, the controller treats the `cluster-admin` status as unresolved and does not deploy resources during that reconcile cycle. As a result, deployment is delayed.

If the work agent status is current when the first reconcile runs, no delay occurs. Otherwise, resource deployment begins after one to three subscription reconcile loops complete. The default reconcile interval in appsub mode is 3 minutes per loop, so deployment can begin up to 9 minutes after you create the subscription.
