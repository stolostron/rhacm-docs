...
When you configure a `Restore` resource in passive sync mode by setting `syncRestoreWithNewBackups` to `true` and the `BackupStorageLocation` becomes temporarily unavailable, the Velero restore operation fais with a `FailedValidation` status.
