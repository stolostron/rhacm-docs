...
After you restore the `BackupStorageLocation` and create successsful backups, the {product-title-short} `Restore` resource remains in the `Error` phase. This occurs because failed Velero restore objects persist in the cluster, which prevents the controller from attempting new restores.
