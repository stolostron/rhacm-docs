...
To resume the backup sync operation, manually delete the failed Velero restore objects. Run the following command. Replace `<failed_restore_name>` with the name of the failed Velero restore object:
...
For more information, see the link:../business_continuity/backup_restore/backup_restore.adoc#restore-backup[Cleaning the hub cluster after restore] topic. 
