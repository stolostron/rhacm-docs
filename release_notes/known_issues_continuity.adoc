...
When you back up your {product-title-short} hub cluster, uninstall it, re-install it on the same cluster, then restore it, the `cluster-proxy-addon` does not function. You see the following error message from the pod log: 
