...
=== The _cluster-proxy-addon_ does not function 
//2.10:ACM-12113

When you backup your {product-title-short} hub, uninstall it, re-install it on the same cluster, then restore it, the `cluster-proxy-addon` does not function. You see the following error message from the pod log: 
...
. Disable the `cluster-proxy-addon` with the `MultiClusterEngine` (MCE) by running the following command: 
+
----
...
. Re-install the `cluster-proxy-addon` with the (MCE) by running the following command: 
+
----
oc patch mce multiclusterengine --type=merge -p '{"spec":{"overrides":{"components":[
