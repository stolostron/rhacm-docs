...
[#open-cluster-namespace-stuck-terminating-state-disabling-multiclusterhub]
=== The _open-cluster-management-backup_ namespace is stuck in the _Terminating_ state, disabling the component cluster backup in _MultiClusterHub_
//2.10:ACM-10292

The `open-cluster-management-backup` namespace is stuck in the `Terminating` state in the following instances: 

* If you have a cluster hub where you run a {product-title-short} restore operation. 
* If you disable the component cluster backup on the `MultiClusterHub` resource.
