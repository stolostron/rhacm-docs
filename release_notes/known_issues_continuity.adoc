...
When the `open-cluster-management-backup` namespace is stuck in the `Terminating` state, it disables the component cluster backup in `MultiClusterHub`. The namespace is stuck in the following instances: 

* If you have a hub cluster where you run a {product-title-short} restore operation. 
