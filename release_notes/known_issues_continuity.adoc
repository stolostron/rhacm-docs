...
After you restore the `BackupStorageLocation` and create successsful backups, the {product-title-short} `Restore` resource remains in the `Error` phase. This happens because failed Velero restore objects persist in the cluster, which prevents the controller from attempting new restores.
...
* Creates new Velero restore objects based on the latest available backups.
