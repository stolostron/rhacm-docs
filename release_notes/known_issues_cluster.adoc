...
. Manually update the `siteconfig-controller-manager` deployment manifest. Remove the `--http2-disable parameter` from the `kube-rbac-proxy` container. Run the following command:
