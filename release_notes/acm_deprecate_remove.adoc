...
[#global-hub-postgres-database-deprecation]
== {global-hub} deprecation of the built-in Postgres database

{global-hub} no longer includes the built-in Postgres database. You no longer need the Postgres database to automatically re-sync real-time data, such as `policies` and `clusters`. {global-hub} keeps data up to date without having a Postgres database. 
