...
[#global-hub-postgres-deprecation]
== {global-hub} deprecation of the Postgres upgrade documentation

In version 1.5 and later, {global-hub} automatically handles database upgrades. You no longer need to manually upgrade your data and databases with the Postgres database, so the Postgres upgrade documentation is removed.
