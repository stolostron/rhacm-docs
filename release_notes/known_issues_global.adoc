...
The {global-hub} operand is the API or resource of your system, and the {global-hub} operator is the controller. After upgrading {global-hub}, align the topics between your Kafka clusters and the {global-hub} operand. You can either create the  `gh-spec` and `gh-status` topics in your Kafka cluster, or you can modify the {global-hub} operand to use the existing topics, like: `spec` and `event`. 

Use the following YAML sample to modify your {global-hub} operand to use the Kafka topics:
