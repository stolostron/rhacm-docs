...
[#operator-hyperlink]
== {global-hub-cap} Operator installed on {ocp-short} 4.13 hyperlinks might redirect home
...
If the {global-hub} Operator is running on {product-title-short} 2.7.9 or earlier when the managed hub cluster is detached, all policies and applications that are created on the managed hub cluster are deleted.

[#policy-cannot-be-propagated-hub]
== Policy cannot be propagated to the managed hub cluster

If the {global-hub} Operator is running on {product-title-short} 2.8.3 or earlier, policy cannot be propagated to the managed hub cluster because the cluster namespace is not exist.
