...
When a hosted managed hub cluster gets detached from the {global-hub} operator, the hub cluster recreates the `addons` in the `open-cluster-management-agent-addon` namespace. 
