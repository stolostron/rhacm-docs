...
If {global-hub} is running on a FIPS-enabled {ocp-short} 4.16 environment, you cannot access the Grafana console due to the wrong `oauth-proxy` image. {product-title-short} 2.10.x supports {ocp-short} 4.16, enabling you to get the `oauth-proxy` image from {product-title-short} 2.10.x. 
