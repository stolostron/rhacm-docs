...
[#operator-hyperlink]
== The {global-hub} is installed on {ocp-short} {ocp-latest} hyperlinks might redirect home

If the {global-hub} Operator is installed on {ocp-short} {ocp-latest}, all hyperlinks that link to the managed clusters list and detail pages in dashboards might redirect to the {product-title-short} home page. 
