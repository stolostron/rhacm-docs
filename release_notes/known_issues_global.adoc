...
In the Federal Information Processing Standard (FIPS) environment, the Kafka operator keeps restarting because of its out-of-memory (OOM) state. To fix this issue, complete the following steps:

. Set the resource limit to at least `512M`.
. Complete the instructions at link:https://access.redhat.com/documentation/en-us/red_hat_amq_streams/2.6/html/deploying_and_managing_amq_streams_on_openshift/deploy-intro_str#assembly-fips-support-str[amq stream doc].
