...
----
. Restart the {global-hub} agent pod in each managed hub cluster. Run the following commands: 

+
[oc,bash]
----
oc delete pod multicluster-global-hub-agent-xxx -n agent_installed_namespace <1>
----
<1> By default, the `agent_installed_namespace` is the `multicluster-global-hub-agent`.
