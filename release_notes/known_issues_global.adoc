...
[#byo-kafka-topics-after-upgrade]
== The {global-hub} operand does not use BYO Kafka topics

There are three topics named `spec`, `status` and `event` in your "bring your own" (BYO) Kafka cluster. You can configure these BYO Kafka topics after you upgrade your BYO Kafka to {global-hub} 1.3 or later. After this upgrade, you only have two topics in the system: one for `spec` path and one for `status` path. The default external topic is `gh-status`. 

The {global-hub} operand is the API or resource of your system, and the {global-hub} operator is the controller. After upgrading {global-hub}, align the topics between your BYO Kafka clusters and the {global-hub} operand. You can either create the `gh-status` topic in your BYO Kafka cluster, or you can change the {global-hub} operand to use the existing topics, such as: `spec` and `status`. 

Use the following YAML sample to update your {global-hub} operand to use your BYO Kafka topics:
