...
[#granfa-cannot-open]
== The {global-hub} grafana console cannot open with the enabled FIPS environment

If {global-hub} is running on {ocp-short} 4.16 with the enabled Federal Information Processing Standard (FIPS) environment, you cannot access the grafana console due to the wrong `oauth-proxy` image. {product-title-short} 2.10.4 supports {ocp-short} 4.16, enabling you to get the `oauth-proxy` image from {product-title-short} 2.10.4. 
