...
If you re-create the {global-hub} custom resource (CR), you might be unable to create or deploy the Kafka cluster instance. To workaround this issue, you must manually restart the {global-hub} operator. In the {global-hub} operator log, you can see a message, informing you that the Kafka custom resource definition (CRD) is ready. For example, you might see a message such as, `wait for the Kafka CRD to be ready`. 
