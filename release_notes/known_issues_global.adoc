...
There are three topics named `spec`, `status` and `event` in your own Kafka that you brought. You can configure your own Kafka topics after you upgrade your Kafka to {global-hub} 1.3 or later. Then, you only have two topics in the system: one for `spec` path and one for `status` path. The default external topic is `gh-status`. 

The {global-hub} operand is the API or resource of your system, and the {global-hub} operator is the controller. After upgrading {global-hub}, align the topics between your specific Kafka clusters and the {global-hub} operand. You can either create the `gh-status` topic in your Kafka cluster, or you can change the {global-hub} operand to use the existing topics, such as: `spec` and `status`. 
