...
When a hosted managed hub cluster gets detached from the {global-hub} operator, the `detach logic` deletes the `open-cluster-management-agent-addon` namespace and all the `addons` within it. THe `local-cluster` addon manager finds these deleted `addons` and recreates them. 
