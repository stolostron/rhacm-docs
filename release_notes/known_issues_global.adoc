...
[#wait-for-kafka-crds]
== Cannot create the Kafka cluster if recreating the global hub CR

The Kafka cluster instances can be deployed successfully if recreating the global hub CR. And you should find this information `wait for the Kafka CRD to be ready` in the {global-hub} Operator.

You need to manually restart the {global-hub} Operator. The issue can be resolved.
