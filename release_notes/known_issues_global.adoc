...
[#kafka-pvc-remaining]
== kafka and zookeeper related pvc remaining when upgrade globalhub from 1.3.x to globalhub 1.4.x

When upgrade globalhub from 1.3.x to 1.4.x, kafka and zookeeper related pvc is remaning. You could mannually delete it after upgrade success.
They should be like:
```
