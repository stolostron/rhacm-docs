...
When a hosted managed hub cluster gets detached from the {global-hub} cluster, the `detach logic` deletes the `open-cluster-management-agent-addon` namespace and all the `addons` within it. The `local-cluster` addon manager finds these deleted `addons` and recreates them. 
