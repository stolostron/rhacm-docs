...
+
To resolve the issue, you must define the default fields in the object definition. For example, add the `type: Opaque` default value to a `Secret` resource, which is the default value set by the API.
