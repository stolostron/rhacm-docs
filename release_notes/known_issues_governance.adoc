...
When you create a policy with duplicate policy template names, it results in inconsistent outcomes you might not be aware of. For example, defining a policy with multiple configuration policies named `create-pod` might cause this issue. To resolve the issue, avoid using duplicate names for policy templates. 
