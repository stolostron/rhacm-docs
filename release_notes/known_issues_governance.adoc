...
When you create duplicate policy template names within a single policy, the policy is not detected and leads to inconsistent results. Defining a policy with multiple configuration policies named `create-pod` might cause this issue. To resolve the issue, avoid using duplicate names for policy templates. 
