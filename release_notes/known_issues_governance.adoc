...
[#gatekeeper-sync-missing]
== Gatekeeper operator exposes a cache setting but not sync setting
//2.9:8899

When you set `auditFromCache` to `Automatic` in the `operator.gatekeeper.sh/v1alpha1` API, the Gatekeeper operator will add `syncOnlys` to the config file.

See the following output example that is the result:
...
To get the explanation of the `sync` setting, run the following command:
