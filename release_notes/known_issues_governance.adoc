...
[#configurationpolicy-misaligns-objectselector]
== ConfigurationPolicy misaligns results from `objectSelector` and `namespaceSelector`
// 2.14.0:22676

When you use both the `objectSelector` and the `namespaceSelector` fields in a `ConfigurationPolicy` resource, the objects that are returned by the `objectSelector` are applied to all namespaces returned by the `namespaceSelector` rather than only the namespace the object is in. As a workaround, use `object-templates-raw` to iterate over the objects. This is fixed in {acm-short} v2.14.1.
