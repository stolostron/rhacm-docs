...
+
*Note:* If `pruneObjectBehavior` is set to something other than `none`, disabling the policy causes the objects to be cleaned up. In this case, set `prunObjectBehavior` to `none` so that the objects exist after the policy is disabled.
...
When disabling the Governance deployments, if Governance is disabled in the `MultiClusterHub` object, the deployments do not all get cleaned up properly. To disable Governance so that the deployments also get cleaned up:

. Disable the `policyController` in the `KlusterletAddonConfig` for the managed cluster. If doing this for all managed clusters, this loop will handle it:
+
[source,bash]
----
...
. (For self-managed hubs only) A self-managed hub's `governance-policy-framework-uninstall` pod will go into `CrashLoopBackOff` because it is in conflict with the `governance-policy-propagator` pod. If this is the case, delete the `ManifestWork` for the self-managed hub and remove the finalizer on the `ManagedClusterAddon`:
+
[source,bash]
----
...
. Now that Governance is disabled on the managed clusters, it can be disabled globally if required by setting the `grc` element under `spec.overrides` to `false` in the `MultiClusterHub` object:
+
[source,bash]
----
...
. (For self-managed hubs only) If there were any local-cluster policies, those can optionally be deleted:
+
[source,bash]
----
...
To re-enable Governance after doing this, re-enable the `grc` element under `spec.overrides` in the `MultiClusterHub` first and then run the loop with `"enabled":true` to re-enable Governance in the KlusterletAddonConfig. If the deployments don't come up successfully, the `governance-policy-addon-controller` might have a stale lease. Deleting the lease will allow it to function properly again:
