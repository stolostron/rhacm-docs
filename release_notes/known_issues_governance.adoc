...
When you create a policy with identical policy template names, it results in inconsistent outcomes and is not detected which means you are not made aware of the issue it can cause. For example, defining a policy with multiple configuration policies named `create-pod` leads to this problem. To resolve the issue, avoid using duplicate names for policy templates. 
