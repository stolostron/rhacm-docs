...
. To reenable governance in the `KlusterletAddonConfig`, reenable the `grc` element of the `spec.overrides` in the `MultiClusterHub` first and then run the loop with `"enabled":true`. If the deployments are unsuccessful, the `governance-policy-addon-controller` might have a stale lease. Delete the lease by using the following command:
