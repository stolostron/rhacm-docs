...
+
*Note:* {ocp} version 3.11 does not support dry run updates. A `ConfigurationPolicy` set to `mustonlyhave` might cause updates to repeat because the controller cannot determine default fields set by the API.
+
