...
== _ConfigurationPolicy_ misinterprets results from the _objectSelector_ and _namespaceSelector_
// 2.14:22676

When you use both the `objectSelector` and the `namespaceSelector` fields in a `ConfigurationPolicy` resource, the objects that the `objectSelector` return get applied to all the namespaces that the `namespaceSelector` return. The `ConfigurationPolicy` misinterprets the results of these objects. To workaround this issue, apply the `object-templates-raw` field to iterate over the objects.
