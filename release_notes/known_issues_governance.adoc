...
When you create a single policy with duplicate policy template names, it is not recognized and leads to inconsistent results. Defining a policy with multiple configuration policies named create-pod might cause this issue. To resolve this problem, avoid using duplicate names for policy templates.
