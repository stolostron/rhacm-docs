...
When you create a policy with identical policy template names, you recieve inconsistent outcomes that are not detected, so you might not knopw what is causing the results. For example, defining a policy with multiple configuration policies named `create-pod` leads to this problem. To resolve the issue, avoid using duplicate names for policy templates. 
