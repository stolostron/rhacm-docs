...
Governance resources are not being cleaned up properly. When the component is set to `false` or is disabled in the `MultiClusterHub` operator, the governance component is removed before it can clean up the add-ons that it manages. 
