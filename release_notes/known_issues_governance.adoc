...
Creating a policy with duplicate policy template names is not detected and will lead to inconsistent results. This might happen when defining a policy with multiple configuration policies named `create-pod`. To resolve this issue, it is required to not use duplicate names for policy templates.
