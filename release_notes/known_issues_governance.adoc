...
. Now that governance is disabled on the managed clusters, you can disable globally if required by setting the `grc` element under `spec.overrides` to `false` in the `MultiClusterHub` object. Run the following command:
+
[source,bash]
----
