...
When you create a policy with identical policy template names, it results in inconsistent outcomes you will not be aware of. For example, defining a policy with multiple configuration policies named `create-pod` won't allow you to detect your duplicate policy template names. YYour duplicate polocy template name will not be detectedYou To resolve the issue, avoid using duplicate names for policy templates. 
