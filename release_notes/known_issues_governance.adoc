...
When you create a policy with identical policy template names, it is not detected and results in inconsistent outcomes. For example, defining a policy with multiple configuration policies named `create-pod` leads to these results. Despite not being informed of the issue it can cause, it is required to avoid using duplicate names for policy templates. 
