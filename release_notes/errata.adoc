...
* Fixes an issue that displayed a status of `Creating` for a cluster, even though invalid information that was provided for the cluster was preventing it from being created. (Bugzilla #1995380)  

* Remove `skip-provider-button=true` flag to enable an extra login page before accessing the standard {ocp} login page and avoid log-off behavior after one hour. (Bugzilla #2061958)
