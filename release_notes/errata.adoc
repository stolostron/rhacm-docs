...
* Changes `Master` node to `Control Plane` node and `Worker` node to `Compute` node to fix a node types discrepancy. (link:https://issues.redhat.com/browse/ACM-10166[ACM-10166])

== Errata 2.9.8

* Fixes an issue with slow propagation of the Application subscriptions. Now the Application subscriptions get created on the managed cluster quicker and about the same time that the placement of the application subscription gets changed. (link:https://issues.redhat.com/browse/ACM-18885[ACM-18885])
