...
* Updates the InfraEnv `osImageVersion` field so that it scales clusters that have been upgraded since the initial install. (link:https://issues.redhat.com/browse/ACM-14601[ACM-14601])

* Changes the system so that the network time protocol (NTP) server specified in the InfraEnv gets used as a fallback.(link:https://issues.redhat.com/browse/ACM-16166[ACM-16166])
