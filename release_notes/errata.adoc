...
* *Important:* You must be runnning {ocp} version 4.6, or later, to upgrade to {product-title-short} version 2.1.7. If you cannot upgrade your {ocp} version 4.5 to a later version, you can continue to use {product-title-short} version 2.1.6.

* Fixed an issue that caused the Hive controller log to show the incorrect version information. (GitHub 12014)

* Added authorization for users with `View` permissions to create and delete the `ManagedClusterView` resource, which also enables users with `View` permissions to access to view the YAML file of the managed cluster resources. (GitHub 11243)
