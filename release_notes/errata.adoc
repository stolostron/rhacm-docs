...
*Note:* {ocp-short} version 4.7 is not supported. You cannot create bare metal managed clusters with the {product-title-short} hub cluster when the hub cluster is hosted on {ocp-short} version 4.7.
...
* Fixed an issue with hub cluster subscription crashing when a Helm subscription is used to subscribe resources from a private Helm channel with only `spec.SecretRef` defined. Now the hub cluster subscription does not crash for this type of Helm subscription. The private Helm repo channel secret must be defined in the same channel namespace. https://bugzilla.redhat.com/show_bug.cgi?id=1932430[Bugzilla 1932430]
