...
* Beginning with {product-title-short} version 2.5.2, {product-title-short} versions 2.5.x is supported on {ocp} version 4.11. 
...
* When the `NodePool.Release.Image` is not specified, its value defaults to the release image that is specified as the `HostedClusterSpec.Release.Image`. (https://bugzilla.redhat.com/show_bug.cgi?id=2105436[Bugzilla 2105436])
