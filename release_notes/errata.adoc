...
* Fixed an issue that caused bare metal clusters to fail to upgrade to 2.1.0 to fail due to memory errors. (GitHub 6898) (link:https://bugzilla.redhat.com/show_bug.cgi?id=1895799[Bugzilla 1895799])
...
* Fixed an issue that caused the GRC dashboard to take a long time to load. (GitHub 6925)

* Fixed selection and deleselection inconsistencies when creating a new Policy. (GitHub 6897)

* Fixed an issue that caused the configuration policy controller to crash when an object template checked a field for a list, but found an empty list or an object that wasn't a list that was set to that field. (GitHub 7135)

* Fixed an issue that occurred when the Git webhook was enabled on channel/subscription but the subscribed resources were not applied to the target clusters. (GitHub 6785)
