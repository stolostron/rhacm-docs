...
Adds support for `GET` cluster role for non-administrator user to access managed clusters. The clusters list returned by the `placementRule` that is created contains only the clusters that the user is allowed to access. ((https://bugzilla.redhat.com/show_bug.cgi?id=1949225)[Bugzilla 1946244])
...
* Resolves an issue with an existing policy in the `create policy` form displaying a blank page. (https://bugzilla.redhat.com/show_bug.cgi?id=1940588[Bugzilla 1940588])
...
* Fixes an _Application topology_ deployment status issue. The Helm resources chart now displays the resource deployment status when the custom alias doesn't match the package name. (GitHub 10401)
...
* Fixes an issue with custom certificate authority on hub cluster. Submariner agent can now connect. (GitHub 9894)
