...
*Note:* {ocp-short} version 4.7 is not supported. You cannot create bare metal managed clusters by using the {product-title-short} hub cluster when the hub cluster is hosted on {ocp-short} version 4.7.

* Fixed log errors that occurred when `packageOverrides` was incorrectly specified in a subscription CR.Errors are now logged while the incorrect `packageOverrides` specifications are ignored. (GitHub 10008)
...
* Fixed the hub cluster subscription crashing with a Helm subscription that is used to subscribe resources from a private Helm channel when `spec.SecretRef` only is defined. Now the hub subscription does not crash for this type of Helm subscription. The private Helm repo channel secret must be defined in the same channel namespace. https://bugzilla.redhat.com/show_bug.cgi?id=1932430[Bugzilla 1932430]
