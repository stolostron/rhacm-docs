...
* Fixes an issue that prevented managed clusters from upgrading when the observability resource was disabled because its `observabilityAddon` status was in a `degraded` state. This fix removes the `observabilityAddon` from the managed clusters when observability is disabled, which allows the upgrade to complete. (GitHub 19636) 
