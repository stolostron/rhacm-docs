...
{ocp} is designed for FIPS. When running on Red Hat Enterprise Linux or RHEL CoreOS booted in FIPS mode, {ocp-short} core components use the Red Hat Enterprise Linux cryptographic libraries submitted to NIST for FIPS Validation. See the additional resources sections for NIST and compliance documentation.
