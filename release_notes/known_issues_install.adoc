...
To see the details, run `oc get multiclusterhubs -n open-cluster-management -o yaml`. The hub cluster might report the following error message that shows an error with installing `multiclusterobservabilities.observability.open-cluster-management.io`:
...
- No known workaround exists for an upgrade with Observability enabled. You can unistall {acm-short} completely and reinstall without any CRDs
