...
[#local-cluster-upgrade-unknown]
== Upgrading an existing {acm-short} cluster with {ocp-short} 4.18 creates an error installing a CRD
//2.13:19151

If you upgrade an existing {acm-short} 2.13 cluster with {ocp-short} 4.18, your upgrade fails with an error from installing a CRD. If you install {acm-short) 2.13 with {ocp-short} 4.18 for the first time, you do not receive this error.

. To see the details, run `oc get multiclusterhubs -n open-cluster-management -o yaml`. The hub cluster reports the following error message that shows installing `multiclusterobservabilities.observability.open-cluster-management.io` failed:
