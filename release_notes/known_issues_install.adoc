...
When you uninstall {product-title-short} from {ocp-short}, but then install an earlier version of {product-title-short} and upgrade the version, the upgrade might fail. The upgrade fails because the `StorageVersionMigration` custom resources were not removed.
