...
When you uninstall {product-title-short} from {ocp-short}, but then install that same version of {product-title-short} and upgrade the version, the upgrade might fail because the `StorageVersionMigration` custom resources were not removed.
