...
== Upgrading {acm-short} with {ocp-short} 4.18 error installing CRDs
//2.13:19151

If you upgrade {acm-short} with with {ocp-short} 4.18, your upgrade fails with an error from installing the CRDs.

If you run `oc get multiclusterhubs -n open-cluster-management -o yaml`, the hub cluster might report the following error message:
...
For installations without the Observability component enabled, use the following workaround to resolve the error:
//really confused here--need to connect with engineer

*Important:* If the Observability component is installed on the cluster, the command causes the the Observability stack to be deleted. No known workaround exists for an upgrade with Observability enabled.
...
----
//I am confused about why we are saying the command is a workaround--if that is true, we should tell them what this is doing, I think?

Run the following command to check the progress of the {acm-short} upgrade after you enable the workaround. 
