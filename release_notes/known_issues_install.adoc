...
Review the known issues for installation and upgrading. The following list contains known issues for this release, or known issues that continued from the previous release. 
...
When you uninstall {product-title-short} from {ocp-short}, but then install an earlier version of {product-title-short} and upgrade the version, the upgrade might fail because the `StorageVersionMigration` custom resources were not removed.
