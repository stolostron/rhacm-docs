...
. Get the namespace that is specified in the `installer.namespace` label of the `kubevirt-hyperconverged` `ClusterManagementAddOn` resource. Run the following command and record the output to use in the next step:
...
. Apply the updated `ManifestWork` resource to the hub cluster. Run the following command:
