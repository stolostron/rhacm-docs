...
If you upgrade {acm-short} with with {ocp-short} 4.18, your upgrade fails because of an error installing the CRDs.

If you run `oc get multiclusterhubs -n open-cluster-management -o yaml`, the hub cluster might report the following error:
