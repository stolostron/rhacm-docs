...
[role="_abstract"]
Before you reinstall {acm} on a cluster where a previous version was installed and then deleted, you need to remove all artifacts that remain by running the clean-up script. You need to clean up artifacts if you plan to reinstall {acm-short} with an older version of {acm-short} on the same cluster. 
