...
. Start running commands in the status bar. For example, you can start running any `oc` or `kubectl` commands and get a visual output.
. Optional: Select the `getting started` command to pull up the _About_ page and more, or type the command in the command status.
. Optional: You can change your default namespace. Click `@default` to run the command that displays your namespaces. Click on a row in the table to set a different namespace.
