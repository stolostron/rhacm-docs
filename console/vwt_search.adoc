...
Visual Web Terminal by default includes the `oc`, `kubectl`, `helm` CLI binaries in versions that are equivalent to the supported {ocp-short} version. You do not need to install any CLI binaries to use Visual Web Terminal.
