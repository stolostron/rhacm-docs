...
By default, Visual Web Terminal includes the versions of the `oc`, `kubectl`, and `helm` CLI binaries that are equivalent to the versions that are supported by {ocp-short}. You do not need to install any additional CLI binaries to use Visual Web Terminal.
