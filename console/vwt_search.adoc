...
With Visual Web Terminal, many commands that return cluster resource data are rendered visually in interactive tabular format rather than remndered in just plain text. You can see the data, click on a row, and then see more details about that resource. 
...
. Optional: Select the `getting started` command to pull up the _About_ page and more.
