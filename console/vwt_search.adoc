...
Before you begin, check the versions of the CLI bundles. For instance, check the version of {ocp}, kubectl, and Helm CLIs. Run the following commands to ensure you have the latest supported versions:

- `oc version` returns client side version as 4.6.z
- `kubectl version` returns client side version as 1.19.z
- `helm version` returns 3.3.4 or greater
