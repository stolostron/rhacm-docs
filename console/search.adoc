...
By default, there is a limit of ten saved searches per user. Only a user with the `administrator` role can update the limit by adding the following key value to the `console-config` ConfigMap, `key:value: SAVED_SEARCH_LIMIT: x`.
