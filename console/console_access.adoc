...
Select `local-cluster` when you want to use {ocp-short} console features on the cluster where you installed {acm-short}. Select *Fleet Management* from the perspective selector when you want to use {acm-short} features to manage your fleet of clusters. 

*Important:* For {ocp} versions earlier than version 4.20, select *All Clusters* from the cluster switcher. If the cluster switcher is not present, the required console plug-ins might not be enabled.
...
+
*Important:* For {ocp-short} versions earlier than version 4.20, select *All Clusters* from the cluster switcher.
