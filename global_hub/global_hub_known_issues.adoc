...
If Global Hub is running on {product-title-short} 2.8.3 or earlier, policy cannot be propagated to the managed hub cluster due to https://issues.redhat.com/browse/ACM-8338. (not this)
//please summarize this issue, we cannot link to this bug and it does not tell me what the problem is.

Create a `<managed-hub-name>` namespace in the managed hub cluster.
//I think we need better explanation here?
