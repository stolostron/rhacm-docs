...
- Global hub agent needs to synchronize cluster information and policy information to Kafka transport. The global hub manager persists the Kafka transport data to PostgreSQL database.
...
. Mirror the imageset in a fully disconnected environment. Please refer to link:https://docs.openshift.com/container-platform/4.13/installing/disconnected_install/installing-mirroring-disconnected.html[here] for more details
...
. Mirror the images to the registry
+
[source,shell]
----
