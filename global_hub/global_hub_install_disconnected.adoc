...
. Verify that the required packages are available from your disconnected cluster by querying the available `PackageManifest` resource by running the following command: 
+
[source,bash]
----
...
----
+
. Verify that the displayed list includes entries showing that the `multicluster-global-hub-operator-rh` and `amq-streams` packages are supplied by the catalog source for your mirror catalog. 
. To change the build in the `amq-strimzi` catalog source, create a {global-hub} annotation by applying the following YAML: 
+
[source,yaml]
----
