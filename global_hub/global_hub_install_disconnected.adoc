...
. Create operator packages in the mirror catalog with the oc-mirror plug-in.
+
...
. Add the registry and catalog to your disconnected cluster.
+
...
.. Verify that the displayed list includes entries showing the `multicluster-global-hub-operator-rh` and `amq-streams` packages. Verify that the catalog source for your mirror catalog supplies these packages. 

.. To change the build in the `amq-strimzi` catalog source, create a {global-hub} annotation. Apply the following YAML: 
+
[source,yaml]
----
