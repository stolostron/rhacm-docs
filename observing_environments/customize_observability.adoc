...
+
----
cat observability-metrics-custom-allowlist
----

. Add the name of the name of the custom metric to the `metrics_list.yaml` parameter. For example, add `node_memory_MemTotal_bytes` to the metric list. Your ConfigMap might resemble the following content:
+
----
data:
  metrics_list.yaml: |
...
. Save and apply the `observability-metrics-custom-allowlist`. Run the following command:
+
----
