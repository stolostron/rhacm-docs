...
. Create a ConfigMap named `thanos-rule-custom-rules` in the `open-cluster-management-observability` namespace. The key must be named, `custom-rules.yaml` as shown in the following example. You can create multiple rules in the configuration:
+
