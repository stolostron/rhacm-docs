...
. Create the `observability-metrics-custom-allowlist.yaml` ConfigMap in the 
...
. Add the name of the name of the custom metric to the `metrics_list.yaml`. For example, add `node_memory_MemTotal_bytes` to the metric list. Your ConfigMap might resemble the following content:
+
----
data:
  metrics_list.yaml: |
