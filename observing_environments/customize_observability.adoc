...
. Create a ConfigMap named `thanos-rule-custom-rules` in the `open-cluster-management-observability` namespace. The key must be named, `thanos-ruler` for the `custom-rules.yaml` parameter, as shown in the following example. You can create multiple rules in the configuration:
+
...
----
data:
  custom_rules.yaml: thanos-ruler
    groups:
      - name: cluster-health
        rules:
