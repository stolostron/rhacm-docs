...
. Create network security groups rules to allow ESP and AH protocols. This needs to be created in both inbound and outbound directions for both the protocols.
+
----
az network nsg rule create --resource-group <res-group> \
--nsg-name <gw-nsg-name> --priority <priority> \
