...
. If you did not create the managed cluster with {product-title-short}, you must manually create a secret on your hub cluster in the namespace of your managed cluster that contains your Red Hat OpenStack Platform credential secret. If you created the cluster with {product-title-short}, then skip to step 2.

.. Create a file named `openstack_secret.yaml` and add the following example content: Run the command to apply the file:
+
[source,yaml]
apiVersion: v1
kind: Secret
...
type: Opaque
data:
   clouds.yaml: <rhos-clouds-yaml-file-content>
...
Replace `managed-cluster-namespace` with the namespace of your managed cluster.
+
Replace `rhos-clouds-yaml-file-content` with the contents of your encoded Red Hat OpenStack Platform `clouds.yaml: $(base64 -w0 )` file.
+
