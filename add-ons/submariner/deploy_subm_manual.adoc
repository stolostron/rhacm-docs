...
. Create a `ManagedClusterSet` resource on the hub cluster by using the instructions provided in the _Creating and managing ManagedClusterSets_ topic of the link:../clusters_mce/cluster_lifecycle/managedclustersets_intro.adoc#managedclustersets_intro[Creating and managing ManagedClusterSets] documentation. Make sure your entry for the `ManagedClusterSet` resembles the following content:
+
[source,yaml]
----   
