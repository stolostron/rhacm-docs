...
*Best practice:* Do not use the unencrypted VXLAN cable driver on public networks. Only use VXLAN to avoid unnecessary double encryption on private networks. For example, some on-premise environments handle the tunnel's encryption with a dedicated hardware device.