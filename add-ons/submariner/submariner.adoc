...
----
+
*Note:* You might have to change `subctl-v0.12.1-linux-amd64.tar.xz` to the version of Submariner that you are using.  
...
. Create a `submariner-broker` object that specifies the Globalnet configuration by creating a YAML file named `submariner-broker`. Add content that resembles the following content to the YAML file: 
+
[source,yaml]
----
