...
The Submariner service is an open source tool that can be used with {product-title} to provide direct networking between pods and multicluster service discovery across two or more clusters in an on-premises or cloud environment. The multicluster service discovery is based on the requirements of the multicluster services API that is available in the Kubernetes enhancement #1645 (KEP-1645). See https://github.com/kubernetes/enhancements/tree/master/keps/sig-multicluster/1645-multi-cluster-services-api[KEP-1645] for more information about the requirements.
