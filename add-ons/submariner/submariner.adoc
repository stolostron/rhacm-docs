...
Globalnet is a feature that can be applied to a cluster set that contains clusters that have the Submariner add-on enabled. Globalnet adds a global virtual network to the cluster set that resolves conflicts across clusters that are caused by overlapping CIDRs. Each cluster is given a subnet from the virtual global private network, configured as a new cluster parameter `GlobalCIDR`. The Globalnet controller uses the global CIDRs to support inter-cluster communication.

You must have cluster administrator privileges to enable Globalnet. You can enable Globalnet for a cluster set by selecting the option to *Enable Globalnet* in the console when you add the Submariner add-on to clusters in the cluster set.  
