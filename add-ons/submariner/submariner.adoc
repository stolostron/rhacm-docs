...
If there is a chance that your clusters running Submariner might have overlapping CIDRs, consider enabling Globalnet. Globalnet is configured by creating a `submariner-broker` in the the `<ManagedClusterSet>-broker` namespace. The `ClusterAdmin` role has the required permissions to create this object in the broker namespace. The `ManagedClusterSetAdmin` role, which is sometimes created as a proxy administrator to act as an administrator for the cluster set, does not have the required permissions.). The `ClusterAdmin` creates the role and assigns it to the `ManagedClusterSetAdmin` user. 

The `ClusterAdmin` role can enable Globalnet for a cluster set by selecting the option to *Enable Globalnet* in the console when enabling the Submariner add-on to clusters in the cluster set. After you enable it, you cannot disable it without removing Submariner.   
