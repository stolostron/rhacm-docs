...
| `Gateway count`
| The number of gateway nodes to be deployed on the managed cluster. For AWS, GCP, Azure and OpenStack clusters dedicated Gateway nodes are deployed whereas for VmWare clusters, existing worker nodes are tagged as Gateway nodes. The default value is `1`. If the value is greater than 1, the Submariner gateway High Availability (HA) is automatically enabled.
