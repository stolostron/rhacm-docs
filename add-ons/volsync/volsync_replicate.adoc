...
[#rsync-backup-volsync]
== Configuring an Rsync backup

You can create a 1:1 asynchronous replication of persistent volumes by using an Rsync backup. You can use Rsync-based replication for disaster recovery or sending data to a remote site.
...
+
Replace the value for `schedule` with how often to run the backup. This example has the schedule for every 30 minutes. See xref:../volsync_schedule.adoc#volsync-schedule[Scheduling your synchronization] for more information. 
+
...
[#rclone-backup-volsync]
== Configuring an Rclone backup
...
Complete the following steps to configure Rclone backups:
...
Replace `source` with the persistent volume claim that you are backing up.
+
Replace the value for `schedule` with how often to run the backup. This example has the schedule for every 6 minutes. This value must be within quotation marks. See xref:../volsync_schedule.adoc#volsync-schedule[Scheduling your synchronization] for more information.
+
Replace `intermediate-s3-bucket` with the path to the configuration section of the Rclone configuration file. This value must be within quotation marks. 
+
Replace `destination-bucket` with the path to the object bucket where you want your replicated files copied. This value must be contaned within quotation marks. 
+
Replace `rclone-secret` with the name of the secret that contains your Rclone configuration information. This value must be contained within quotation marks. 
+
...
+
Replace `my-vsc-name` with the name of the `VolumeSnapshotClass` to use, if you specified `Snapshot` as your `copyMethod`. This is not required for other types of `copyMethod`.
...
----
+
Replace the value for `schedule` with how often to move the backup to the destination. The schedules for the source and destination must be offset to allow the data to finish replicating before it is pulled from the destination. This example has the schedule for every 6 minutes, offset by 3 minutes. This value must be within quotation marks. See xref:../volsync_schedule.adoc#volsync-schedule[Scheduling your synchronization] for more information.
+
Replace `intermediate-s3-bucket` with the path to the configuration section of the Rclone configuration file. This value must be within quotation marks.
+
Replace `destination-bucket` with the path to the object bucket where you want your replicated files copied. This value must be contaned within quotation marks.
...
+
Replace `my-vsc-name` with the name of the `VolumeSnapshotClass` to use, if you specified `Snapshot` as your `copyMethod`. This is not required for other types of `copyMethod`. If not included, the system default `VolumeSnapshotClass` is used. 