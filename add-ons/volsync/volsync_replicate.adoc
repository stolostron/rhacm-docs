...
You can use three methods to replicate persistent volumes with VolSync, which depend on the number of synchronization locations that you have. 

[#rsync-backup-volsync]
== Comfiguring an Rsync backup

An Rsync backup creates a 1:1 asynchronous replication of persistent volumes. You can use Rsync-based replication for disaster recovery or sending data to a remote site.

The Rsync method is used for this example. For information about Rsync, see https://volsync.readthedocs.io/en/latest/usage/index.html[Usage] in the VolSync documentation.  
...
A restic-based backup copies a restic-based backup copy for the persistent volume in a location that is specified in your `restic-config.yaml` secret file.
...
+
Replace `my-secure-restic-password` with the encryption key that is required to access the repository. 
+ 
...
An Rclone backup copies a single persistent volume to multiple locations by using Rclone through an intermediate object storage location, like AWS S3.

Complete the following steps to configure Rclone backups:

. Create a `ReplicationSource` custom that resembles the following example: 
+
[source,yaml]
----
---
