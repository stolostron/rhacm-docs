...
. To ensure the pods, such as the cluster-proxy-proxy-agent, on your managed cluster gets configured with the add-on tolerations, verify that your `ClusterManagementAddon` looks like the following sample: 
+
[source,bash]
----
apiVersion: addon.open-cluster-management.io/v1alpha1
