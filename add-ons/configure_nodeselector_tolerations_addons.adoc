...
.  Use the `AddonDeploymentConfig` API to create a configuration to specify the `nodeSelector` and `tolerations` in the namespace that you used for your {product-title-short} installation. 

. To ensure the configuration changes get pushed to your managed cluster, create a `cluster-proxy addon supportconfig` by using the following template:  
+
[source,yaml]
----
