...
The Argo CD server on the hub cluster is responsible for deploying the application resources on the managed clusters. Pull model implementation applies {ocm} registration, placement, and `manifestWork` APIs so that the hub cluster. Secure communication channel between the hub cluster and the managed cluster. With this model, an Argo CD controller must be running on each target managed cluster, then the Argo CD  `ApplicationSet` can use a single manifest to deploy an application on multiple managed clusters. The same  `ApplicationSet` CRD is used to deploy the application to the managed cluster using the push or pull model. 
