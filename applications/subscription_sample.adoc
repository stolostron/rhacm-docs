...
| spec.placement
| Required
| Identifies the subscribing clusters where deployables need to be placed, or the placement rule that defines the clusters. Use the placement configuration to define values for multicluster deployments. *Deprecated:* `PlacementRules` is deprecated. Use Placement instead.
...
| Optional, but required for using a placement rule. 
| Set the value to `Placement` to indicate that a placement rule is used for deployments with the subscription. (PlacementRule is deprecated.)
