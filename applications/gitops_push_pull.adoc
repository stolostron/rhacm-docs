...
- Every managed cluster must have a cluster secret in the Argo CD server namespace on the hub cluster, which is required by the ArgoCD application set controller to propagate the Argo CD application template for a managed cluster. Create a `gitOpsCluster` resource that contains a reference to a `placement` resource. The `placement` resource selects all of the managed clusters that need to support the pull model, so that managed cluster secrets are created in the Argo CD server namespace.
...
spec:
  ...
  template:
    metadata:
      ...
      labels:
        apps.open-cluster-management.io/pull-to-ocm-managed-cluster: "true" <2>
...
<2> The `apps.open-cluster-management.io/pull-to-ocm-managed-cluster: "true"` is also needed for the pull model.
