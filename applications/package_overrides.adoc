...
. Specify the field within the Kubernetes resource `spec` to override as the value for the `path` field.
...
`packageName`:: Identifies the Helm chart.
`packageAlias`:: Indicates that you are overriding the release name.
