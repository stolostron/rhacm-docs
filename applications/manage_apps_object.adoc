...
//please check this entire new topic
...
//do we need anymore info here about how to do that?

. Navigate to your user name in the navigation bar. From the drop-down menu, select *My Security Credentials*.
...
. Add the URL. The URL identifies the channel in a AWS S3 bucket if the URL contains `s3://` or `s3 and aws` keywords. For example, see sll of the following bucket URLs will be regarded as a AWS s3 bucket.
...
+
*Note:* The AWS S3 object bucket URL is not necessary to connect the bucket with AWS S3 API. 
...
//does this mean just start at the beginning or see another topic?
