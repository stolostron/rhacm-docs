...
. Search for and install the _Ansible Automation Platform Resource Operator_. *Note:* To submit prehook and posthook `AnsibleJobs`, install Ansible Automation Platform Resource Operator with a corresponding version available on different {ocp-short} versions:
