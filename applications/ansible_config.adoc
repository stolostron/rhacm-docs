...
For a main-sub subscription with prehook and posthook `AnsibleJob`s, the main-sub subscription should be reconciled after all prehook and posthook `AnsibleJob`s or main subscription are updated in the Git repository. 

Prehook `AnsibleJob`s and the main subscription continuously reconcile and relaunch a new pre-`AnsibleJob` instance.
