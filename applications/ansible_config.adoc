...
. Search for and install the _Ansible Automation Platform Resource Operator_. *Note:* To submit prehook and posthook `AnsibleJobs`, install Ansible Automation Platform (AAP) Resource Operator with a corresponding version available on {ocp-short}. See the following versions:
