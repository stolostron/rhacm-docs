...
[#ghelm-keep-resource-after-sub-delete]
== Keeping deployed resources after deleting subscription with Helm

Helm provides an annotation keep specific deployed resources after you delete a subscription. See link:https://helm.sh/docs/howto/charts_tips_and_tricks/#tell-helm-not-to-uninstall-a-resource[Tell Helm Not To Uninstall a Resource] for more information.

*Note:* The annotation must be present in the Helm chart.