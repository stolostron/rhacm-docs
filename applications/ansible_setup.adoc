...
. Search for and install the _{aap-short} Resource Operator_. *Note:* To submit prehook and posthook `Ansible jobs`, install {aap} Resource Operator with corresponding version available on the following {ocp-short} versions: 
