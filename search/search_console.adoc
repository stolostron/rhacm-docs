...
| 
|
| Stores collected data from all managed clusters in an instance of the PostgreSQL database. Use the PostgreSQL database to improve the performance of the search service. The database is a cache and does not contain any unrecoverable information. If you recreate the database, consider the time it takes to rebuild the cache.
|===
