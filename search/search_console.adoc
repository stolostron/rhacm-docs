...
| 
|
| Stores collected data from all managed clusters in an instance of the PostgreSQL database. The database contains no historical data. When the database restarts, the data is overridden. If you recreate the database, consider the time it takes to rebuild the cache.
|===
...
`indexer`:: You can apply resources to an `indexer`, `database`, `queryapi`, or `collector` pod.
`database`::You can add multiple environment variables in the `envVar` section to specify a value for each variable that you name. 
`queryqpi.arguments`::You can control the log level verbosity for any of the previous four pods by adding the `- -v=3` argument.
