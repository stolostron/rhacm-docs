...
Discovered hosted clusters are not automatically imported into {acm-short} until the `spec.importAsManagedCluster` field is changed from `false` to `true`. Learn how to use a {acm-short} policy to automatically set this field to `true` for all `type.MultiClusterEngineHCP` in `DiscoveredCluster` resources so that discovered hosted clusters are immediately and automatically imported into {acm-short}.

Configure your Policy to import all your discovered hosted clusters. Log in to your hub cluster from the CLI to complete the following procedure:
