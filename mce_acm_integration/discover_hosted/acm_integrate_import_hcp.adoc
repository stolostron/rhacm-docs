...
Discovered hosted clusters from managed {mce-short} clusters are represented in `DiscoveredCluster` custom resources, which are located in the managed {mce-short} cluster namespace in {acm-short}. See the following `DiscoveredCluster` resource and namespace example. Discovered hosted clusters are not automatically imported into {acm-short} until the `spec.importAsManagedCluster` field is changed from `false` to `true`:
...
Learn how to use a {acm-short} Policy to automatically set this field to `true` for all `type.MultiClusterEngineHCP` in `DiscoveredCluster` resources so that discovered hosted clusters are immediately automatically imported into {acm-short}.
