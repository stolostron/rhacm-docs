...
When a discovered hosted cluster is auto-imported into the {acm-short} hub cluster, it becomes a managed cluster with the naming convention of the managed cluster, `<mce-cluster-name>`-`<hosted-cluster-name>`. If all the hosted clusters are named uniquely across the fleet, you can configure `discoveryPrefix` in the `hypershift-addon-deploy-config` `addondeploymentconfig` in the previous step so that no prefix is used to name the discovered cluster.
