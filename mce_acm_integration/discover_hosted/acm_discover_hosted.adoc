...
.. If you need to remove the discovery prefix entirely, ensure all hosted clusters are first detached from the respective clusters. *Important:* Using an empty string as the custom prefix can cause klusterlet naming collisions within the {mce-short} cluster. Run the following command:
