...
. *Optional:* You can set a naming convention by appending the previous command with `{"name":"discoveryPrefix","value": ""}]}}'`. When a discovered hosted cluster is automatically imported into the {acm-short} hub cluster, it becomes a _managed cluster_ with the following naming convention:`<mce-cluster-name>`-`<hosted-cluster-name>`. You can also set the `discoveryPrefix` to some other string to use it as a prefix that replaces `<mce-cluster-name>`. Run the following command with the prefix:
