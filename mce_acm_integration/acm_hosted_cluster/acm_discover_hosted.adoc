...
Create a _KlusterletConfig_ resource. {mce-short} has a local-cluster, which is a hub cluster that is managed. A resource named `klusterlet` is created for this local-cluster.
...
. Alternatively, extract the import manifest for your {mce-short} cluster. Run the following command. Replace `<mce-a>` with the name of your {mce-short} cluster:

+
[source,bash]
----
oc get secret mce-hosting-east-import -n <mce-a> -o jsonpath={.data.import\\.yaml} | base64 --decode > import.yaml
----
