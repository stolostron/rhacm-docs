...
* You need {acm-short} installed. See the {acm-short} link:../../install/install_overview.adoc#installing-acm-overview[Installing and updating overview] documentation.
...
----
Discovered hosted clusters are not automatically imported into {acm-short} until the `spec.importAsManagedCluster` field is changed from `false` to `true`. Learn how to use a {acm-short} policy to automatically set this field to `true` for all `type.MultiClusterEngineHCP` within `DiscoveredCluster` resources so that discovered hosted clusters are immediately and automatically imported into {acm-short}.
