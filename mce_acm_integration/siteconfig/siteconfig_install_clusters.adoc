...
* You have the default installation templates. To get familiar with the default templates, see xref:../../mce_acm_integration/siteconfig/siteconfig_cluster_templates.adoc#default-templates[Default set of templates]
* Install and configure the underlying operator of your choice. To install either the Image Based Install operator or the Assisted Installer, see _Image-based installations for {sno}_ or link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.17/html/installing_an_on-premise_cluster_with_the_agent-based_installer/index[Installing an on-premise cluster with the Assisted Installer].
...
== Applying the cluster and node templates
...
. Apply the cluster-level installation template by running the following command:
...
. Apply the node-level installation template by running the following command:
...
Reference your templates and supporting manifests in the `ClusterInstance` CR. Complete the following steps to render the installation manifests by using the default cluster and node templates:
