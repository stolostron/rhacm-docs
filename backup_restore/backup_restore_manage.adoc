...
//Procedure/task--however I suggest that we delete this file bc there may be repetitve info here. I just realized that i duplicated the section title: Enabling the backup and restore operator. I suggest that we move the content in the Enabling the backup and restore operator section and move it to the Enabling the backup and restore operator documention (backup_restore_enable.adoc) MJ, 10/11/22, 
Enable the cluster backup and restore operator to schedule back up and restore for your cluster resources.

**Required access**: Cluster administrator

//suggest that we move the contents of the prerequisites topic to the concept topic (backup_restore_enable.adoc)
* <<prereq-backup-restore-enable,Prerequisites>>
//duplicate name, this is also the title for the concept topic (backup_restore_enable.adoc), 
