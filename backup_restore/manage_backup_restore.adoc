...
. Use the backup and restore operator `backupschedule.cluster.open-cluster-management.io` to create a backup schedule and use the restore.restore.cluster.open-cluster-management.io` resources to restore a backup.

. Run the following command to create a `backupschedule.cluster.open-cluster-management.io` resource:
+
----
kubectl create -n <oadp-operator-ns>
-f cluster_v1beta1_backupschedule.yaml
----
+
...
metadata:
  name: schedule-acm
spec:
  veleroSchedule: 0 */2 * * * # Create a backup every 6 hours
  veleroTtl: 120h # deletes scheduled backups after 72h; optional, if not specified, the maximum default value set by velero is used - 720h
----
+
...
+
----
oc describe restore.velero.io -n <oadp-operator-ns>
oc get restore.velero.io -n 
----
