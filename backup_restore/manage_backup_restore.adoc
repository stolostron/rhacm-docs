...
Use the link:https://github.com/open-cluster-management-io/managed-serviceaccount[Managed Service Account] component on the primary hub cluster to automatically connect imported clusters to the new hub cluster.  You must create a token for each imported cluster, which is backed up in each managed cluster namespace. Update the `klusterlet-bootstrap-kubeconfig` `ClusterRole` to enable access to the token during the import operation. The `klusterlet-bootstrap-kubeconfig` `ClusterRole` can only get or update the `bootstrap-hub-kubeconfig` secret.
...
. Enable the Managed Service Account component by listing `managedserviceaccount-preview` in the `MultiClusterEngine` resource. See the following example:
+
[source,yaml]
----
