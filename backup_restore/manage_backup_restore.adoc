...
//Prerequisites from the older topic: Managing backup and restore operator (file that i deleted: backup_restore_manage.adoc])
...
----
//commenting this part out bc it is documented later in the Restore section
//+
...
+
//should this information be moved to the Restore section since they are all restore samples?
...
For descriptions of the parameters, see the <<restore-backup,Restoring a backup>> section.
...
//If we add the sample examples, is it necessary to add the link to the sample? How will the user return to the product doc? I have concerns about haing too many inline links. If we add a link here, it will disrupt the reader

//Even if we take the alternative: remove the sample and only point the user to the sample in stolostron, there are still concerns about disrupting the user with too many inline links

//Requested reference link: https://github.com/stolostron/cluster-backup-operator/blob/release-2.6/config/samples/cluster_v1beta1_backupschedule.yaml
...
[#restore-backup]
== Restoring a backup
//example of Restore yaml toward the end of the section
...
// Similar observation; i think it is unnecessary to add a reference link to this. 
// Requested link to add: https://github.com/stolostron/cluster-backup-operator/blob/release-2.6/config/samples/cluster_v1beta1_restore.yaml
**Notes**:
...
//removing the link, sample toward the end of the section 
//Use the link:https://github.com/stolostron/cluster-backup-operator/blob/release-2.5/config/samples/cluster_v1beta1_restore_passive_sync.yaml[`restore-passive-sync`] sample to restore passive data, while continuing to check if new backups are available and restore them automatically.
...
//Similar to other comments, i suggest to not add the reference link to this section, keep the sample

//Link to the Restore sample: https://github.com/stolostron/cluster-backup-operator/blob/release-2.6/config/samples/cluster_v1beta1_restore_passive_sync.yaml
...
//based on earlier comments, I suggest that we add the sample to this section and not add the link 
//Link to Restore sample: https://github.com/stolostron/cluster-backup-operator/blob/release-2.6/config/samples/cluster_v1beta1_restore_passive.yaml 

[#restore-activation-resources]
=== Restoring activation resources

//sample added
//Use the link:https://github.com/stolostron/cluster-backup-operator/blob/release-2.5/config/samples/cluster_v1beta1_restore_passive_activate.yaml[`restore-passive-activate`] sample when you want the hub cluster to manage the clusters.
...
//I suggest that we remove the link to the sample and provide the sample, this will also help prevent broken links the reference is branch specific

//Link to the Restore sample: https://github.com/stolostron/cluster-backup-operator/blob/release-2.6/config/samples/cluster_v1beta1_restore_passive_activate.yaml
