...
- *Optional:* Complete the following additional steps when you enable the backup and restore component with {ocp} in a disconnected environment:

. Update the `MultiClusterHub` resource with the follwing annotation to override the source from which the OADP operator is installed. Create the annotoation before the `cluster-backup` component is enabled on the `MultiClusterHub` resource:
+
[source,yaml]
----
