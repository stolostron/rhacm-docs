...
[source,yaml]
----
apiVersion: operator.open-cluster-management.io/v1kind:
MultiClusterHubmetadata:name: multiclusterhub namespace:
<namespace>spec:disableHubSelfManagement: true
----

*Note:* If the self managing option is not disabled on the restore hub cluster before the activation data is moved to the restore hub cluster, the `local-cluster` klusterlet and the klusterlet in the `<managed-cls>` namespace conflict. As a result, the restore hub cluster is managed by itself by using the `local-cluster` and by using the restored `<managed-cls>`. If you detach the `<managed-cls>` as part of the detach operation, the managed cluster receives a deprovision request, which results in the restore hub cluster rmeoving itself.