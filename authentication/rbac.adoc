...
| *cluster-admin*
| A user with cluster binding to the `cluster-admin` role is an {ocp-short} super user, who has all access.
...
| A user with cluster binding to the `managed-cluster-<managed_cluster_name>` role has administrator access to the `ManagedCluster` resource named,  `managedcluster-<managed_cluster_name>`.
