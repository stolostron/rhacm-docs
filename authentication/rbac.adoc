...
//not sure that this is needed in the description-->This cluster role is created by the `server-foundation-controller` when a `managed-cluster` is created.
...
| A user with a cluster binding to the `managed-clusterset-<managed_clusterset_name>` role has administrator access to `managedclusterset-<managed_clusterset_name>` resource. This role also gives a user administrator access to all resources that are in the `managedclusterset-<managed_clusterset_name>` resource. The user also has administrator access to `managedcluster.cluster.open-cluster-management.io/clusterclaim.hive.openshit.io/clusterdeployment.hive.openshit.io/clusterpool.hive.openshit.io` resources, which has the managed cluster set label: `cluster.open-cluster-management.io/clusterset=managed-clusterset-<managed_clusterset_name>`.
