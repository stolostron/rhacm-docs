...
| A user with a cluster binding to the `managed-clusterset-<managed_clusterset_name>` role has administrator access to `managedclusterset-<managed_clusterset_name>` resource. This role also gives a user administrator access to all resources that are in the `managedclusterset-<managed_clusterset_name>` resource.

| *open-cluster-management:managedclusterset:view:managed-clusterset-<managed_clusterset_name>*
| A user with cluster-wide binding to the `managed-clusterset-<managed_clusterset_name>` role has view access to `managedclusterset-<managed_clusterset_name>` resource. This role also gives the user view access to all resources that are in the `managedclusterset-<managed_clusterset_name>` resources. 
