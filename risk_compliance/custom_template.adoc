...
To conform to yaml syntax, templates needs to be set in the policy resource as strings using quotes or block char ( | or >), see examples below.  This causes the resolved template value to also be a String, to override this consider using toInt or toBool as the final function in the template to trigger further processing that will force the value to be interpreted as integer or bool.
...
----
func lookup (apiversion string, kind string, namespace string, name string) (value map[string]interface{}, err Error)
----
...
The `toInt` function casts and returns the integer value of the input value. Also, when this is the last function in the template, it triggers further processing of the source content to ensure the value is interpreted by yaml as an integer. View the following syntax for the function:

----
func toInt (input interface{}) (output int)
----

When you use the function, enter the data that needs to be casted to int. View the following example of the configuration policy that uses the `toInt` function:
...
The `toBool` function converts the input string into a boolean and  returns the bool. Also, when this is the last function in the template, it triggers further processing of the source content to ensure the value is interpreted by yaml as boolean. View the following syntax for the function:

----
func toBool (input string) (output bool)
----

When you use the function, enter the string data that needs to be converted to bool. View the following example of the configuration policy that uses the `toBool` function:
