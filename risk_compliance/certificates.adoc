...
//ACM external certs
// Add a section called BYO certificates to list the sections that are related: Replacing management ingress and Observability certificates
//new title or chapter name for Managing BYO certs or Hub cluster management certificates or Managing hub cluster certs 
...
// ACM component certs
//list and refresh managed certs is related to OCP managed certs; created by OCP Operator 

* <<list-ocp-managed-certificates,List {ocp-short} managed certificates>>
* <<refresh-ocp-a-managed-certificate,Refresh a {ocp-short} managed certificate>>
//ACM managed certificates, ACM operators are creating the certs
