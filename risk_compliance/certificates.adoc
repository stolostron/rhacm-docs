...
You can replace alertmanager certificates by updating the alertmanager route if you do not want to use the OpenShift default ingress certificate. The steps to use your own certificates for alertmanager route are similar to xref:../risk_compliance/cert_mgmt_ingress.adoc#replacing-the-management-ingress-certificates[Replacing the management ingress certificates] except the following three things:
...
*Note:* If you want to restore the default self-signed certificate for alertmanager route, you only need to delete two secrets in the `open-cluster-management-observability` namespace, the observability service operator will take care of everything.
