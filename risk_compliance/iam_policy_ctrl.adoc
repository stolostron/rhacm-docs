...
See https://github.com/open-cluster-management/policy-collection/blob/master/stable/AC-Access-Control/policy-limitclusteradmin.yaml[`policy-limitclusteradmin.yaml`] to view the IAM policy sample. Learn how to manage an IAM policy, see xref:../risk_compliance/create_iam_policy.adoc#creating-an-iam-policy[Managing IAM policies] for more details.
Refer to xref:../risk_compliance/policy_controllers.adoc#policy-controllers[Policy controllers] for more topics.
