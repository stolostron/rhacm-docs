...
For more information on roles, see xref:../risk_compliance/security_intro.adoc#role-based-access-control[Role-based access control]. View a sample of a role policy, see https://github.com/open-cluster-management/policy-collection/blob/master/stable/AC-Access-Control/policy-role.yaml[`policy-role.yaml`].

To learn how to manage role policies, refer to xref:../risk_compliance/create_role_policy.adoc#managing-role-policies[Managing role policies] for more information. See the xref:../risk_compliance/config_policy_ctrl.adoc#kubernetes-configuration-policy-controller[Kubernetes configuration policy controller] to view other configuration policies that are monitored the controller.
