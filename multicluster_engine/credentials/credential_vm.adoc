...
You need a credential to use {mce} console to deploy and manage a {ocp} cluster on VMware vSphere. *Note:* Only {ocp-short} versions 4.5.x, and later, are supported.
...
To create an opaque secret for VMware vSphere by using the API instead of the console, apply YAML content that is similar to the following example:
...
type: Opaque
data:
    vsphere_access_key_id: $(echo -n "${VSPHERE_KEY}" | base64 -w0)
    vsphere_secret_access_key: $(echo -n "${VSPHERE_SECRET}" | base64 -w0)
----
