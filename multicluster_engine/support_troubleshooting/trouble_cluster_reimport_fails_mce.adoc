...
. Run the following command tov set your `kubeconfig` file path. Replace `<path-to-kubeconfig>` with the path to your new `kubeconfig` file:
+
----
...
----
+
*Note:* On macOS, use the following command instead:
+
----
(cat ${kubeconfig_file} | base64)
----

. Run the following command to define the `kubeconfig` path:
+
----
