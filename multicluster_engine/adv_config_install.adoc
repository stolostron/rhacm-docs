...
See the following components and default configurations:
...
You can define a set of node selectors in the `MultiClusterEngine` to install to specific nodes on your cluster. The following examples shows a node selector to assign {product-title-short} pods to nodes with the label `node-role.kubernetes.io/infra`:
...
You can define a list of tolerations to allow the `MultiClusterEngine` to tolerate specific taints defined on the cluster. The following examples shows a custom toleration that matches a `node-role.kubernetes.io/infra` taint:
...
Some components are not enabled by default and must be enabled in the `MultiClusterEngine` specification to use. These components include the `ManagedServiceAccount` and Hypershift add-ons. These components are in a technical preview state and not fully supported as of {product-title-short} 2.0. 

[#serviceaccount-addon-intro]
=== ManagedServiceAccount add-on (Technical Preview)

By default, the `Managed-ServiceAccount` add-on is disabled. This component when enabled allows you to create or delete a service account on a managed cluster. To install with this add-on enabled, include the following in the `MultiClusterEngine` specification:
...
=== Hypershift add-on (Technical Preview)

By default, the `Hypershift` add-on is disabled. To install with this add-on enabled, include the following in the `MultiClusterEngine` specification:
