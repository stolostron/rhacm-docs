...
. Add additional hosts to create new Agent custom resources. See <<adding-hosts-to-an-infrastructure-environment, Adding hosts to an infrastructure environment>> for more details.
. Associate the new Agent custom resources with your `ClusterDeployment` custom resource by adding them to the `agentSelector` parameter.