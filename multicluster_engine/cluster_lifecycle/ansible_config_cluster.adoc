...
. Create or import your cluster by following the instructions in link:../multicluster_engine/cluster_lifecycle/create.adoc#creating-a-cluster[Creating a cluster] or xref:../clusters/import.adoc#importing-a-target-managed-cluster-to-the-hub-cluster[Importing a target managed cluster to the hub cluster], respectively.
+
