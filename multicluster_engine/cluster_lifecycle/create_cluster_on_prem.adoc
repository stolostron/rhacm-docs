...
If you are discovering new hosts with no existing infrastructure environment, complete the steps in xref:../cluster_lifecycle/scale_hosts_infra_env.adoc#scale-hosts-infrastructure-env[Scale hosts to an infrastructure environment] beginning with step 4 to define your hosts.   
