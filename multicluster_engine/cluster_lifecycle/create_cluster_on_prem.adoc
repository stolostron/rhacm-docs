...
If you are discovering new hosts with no existing infrastructure environment, complete the steps in xref:../cluster_lifecycle/add_hosts_infra_env.adoc#scale-hosts-infrastructure-env[Adding hosts to an infrastructure environment] beginning with step 4 to define your hosts.   
