...
. Access other services on the managed cluster. Here using prometheus-k8s as an example(this feature only feasible when the managed cluster is a OCP and the service must use service-serving-certificate to generate its server certificates)
