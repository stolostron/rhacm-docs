...
In some environments, a managed cluster is behind a firewall and cannot be accessed directly by the hub cluster. To gain access, you can set up a proxy add-on to access the `kube-apiserver` and other services of the managed cluster to provide a more secure connection. 
...
export NEW_SERVER=https://$(oc get route -n multicluster-engine cluster-proxy-addon-user -o=jsonpath='{.spec.host}')/$TARGET_MANAGE_CLUSTER
...
. Access other services on the managed cluster. See the prometheus-k8s as an example (this feature only feasible when the managed cluster is a {ocp-short} and the service must use service-serving-certificate to generate its server certificates)
...
----
+
From the hub cluster, convert the certificate authority to a file by using the following command:
+
----
