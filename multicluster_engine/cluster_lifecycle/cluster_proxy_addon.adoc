...
. Access other services on the managed cluster. This feature is available when the managed cluster is a {ocp-short} cluster. The service must use `service-serving-certificate` to generate server certificates:
