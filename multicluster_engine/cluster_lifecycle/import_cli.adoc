...
* If you are importing a cluster that was not created by {ocp-short}, you need a `multiclusterhub.spec.imagePullSecret` defined. This secret might be created when {mce} was installed. See xref:../../install_upgrade/install_connected.adoc#installing-from-the-operator-hub-mce[Installing from the OperatorHub web console interface] for more information about defining the secret. 
