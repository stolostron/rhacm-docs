...
* **Note:** If you are importing a cluster that was not created by {ocp-short}, you need a `multiclusterhub.spec.imagePullSecret` defined. This secret might be created when {mce} was installed. 
+
...
* Ensure the agent installed in the previous time is deleted on the cluster that you want to import. The `open-cluster-management-agent` and `open-cluster-management-agent-addon` namespaces must be removed to avoid errors.
...
For more information about Ansible Automation Platform, see link:../../applications/ansible_config.adoc
...
.. If necessary, configure a kubeconfig for your managed cluster.
...
. Configure a kubeconfig for your targeted managed cluster.
