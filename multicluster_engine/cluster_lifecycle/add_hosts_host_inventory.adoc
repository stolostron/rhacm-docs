[#adding-hosts-to-a-host-inventory]
= Adding hosts to a host inventory

You can use the console to add hosts to a host inventory. Adding the hosts makes it easier to select the already-configured hosts when you are creating a cluster. 
...
. Select the _Hosts_ tab to view the hosts that are already added to that host inventory and to add a host. Available hosts might take a few minutes to appear in the table. 
...
You can now create an on-premises cluster on this host inventory. See xref:../cluster_lifecycle/create_cluster_on_prem.adoc#creating-a-cluster-on-premises[Creating a cluster in an on-premises environment] for more information about creating a cluster.
