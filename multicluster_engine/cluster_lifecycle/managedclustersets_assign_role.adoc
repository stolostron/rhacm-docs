...
* Cluster set `bind`
** Permission to bind the cluster set to a namespace by creating a `ManagedClusterSetBinding`.
+
**Note:** The user or group must also have permission to create the `ManagedClusterSetBinding` in the target namespace.
