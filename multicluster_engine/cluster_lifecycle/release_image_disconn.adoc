...
+
  ...
  * For a managed cluster that is not an {ocp-short} cluster, use the `ManageClusterImageRegistry` CRD to override the location of the image sets. See xref:../cluster_lifecycle/specify_registry_img_on_managed_clusters_for_import.adoc#specify-registry-img-on-managed-clusters-for-import[Specifying registry images on managed clusters for import] for information about how to override the cluster for the mapping.  
