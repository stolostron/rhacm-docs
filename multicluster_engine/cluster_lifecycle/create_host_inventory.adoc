...
You can use the console to create a host inventory to manage your hosts and create clusters on those hosts.

* <<host-inv-prerequisites,Prerequisites>>
* <<enable-cim,Enable Central Infrastructure Management service>>
* <<creating-your-host-inv-with-the-console,Creating your host inventory with the console>>
* <<accessing-your-host-inv,Accessing your host inventory>>

Host inventories support the following features:
...
[#host-inv-prerequisites]
== Prerequisites

See the following prerequisites before creating a host inventory:
...
The CIM service is provided with multicluster engine and deploys {ocp-short} clusters. CIM is deployed when you enable the MultiClusterHub Operator on the hub cluster, but must be enabled.
...
[#creating-your-host-inv-with-the-console]
== Creating your host inventory with the console

To create a host inventory from the console, complete the following steps:

. From the navigation menu, navigate to *Infrastructure* > *Host inventory* and click *Create host inventory*.
. Add the following information to your host inventory settings: 
+
* Name: A unique name for your host inventory. 
* Network type: Specifies which types of hosts can be added to your host inventory. You can only use the static IP option when you are using bare metal hosts. 
...
* Labels: Optional field where you can add labels to the host inventory so you can more easily find and group the host inventory with other host inventories that share a characteristic. The selections that you made for the network type and location are automatically added to the list of labels.
...
You can now continue by adding hosts to your host inventory. 

[#accessing-your-host-inv]
== Accessing a host inventory

To access a host inventory, select *Infrastructure* > *Host inventory* in the console. Select your host inventory from the list to view the details and hosts for that host inventory.
