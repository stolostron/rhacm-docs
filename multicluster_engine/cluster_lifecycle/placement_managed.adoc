...
* `Placements` filter `ManagedClusters` from that working set by using label and claim selectors.
...
* `Placements` rank the clusters by the soft requirements and select a subset among them.
...
Next will use some examples to describe how to use the `Placement` API. Read the below content to get a general understanding of the API first.
...
- Select `ManagedClusters` with matching taints for a period of time.
...
- Select two clusters with the largest add-on score cpu ratio, and pin the placement decisions. 
+
[source,yaml]
----
apiVersion: cluster.open-cluster-management.io/v1beta1
...
Below are the explaination of each field under `prioritizerPolicy`:
  ...
  ** `configurations` represents the configuration of prioritizers.
    ...
    *** scoreCoordinate represents the configuration of the prioritizer and score source.
      ...
      **** `addOn` defines the resource name and score name. `AddOnPlacementScore` is introduced to describe addon scores. See https://open-cluster-management.io/scenarios/extend-multicluster-scheduling-capabilities/[Extend the multicluster scheduling capabilities with placement] to learn more about it.
