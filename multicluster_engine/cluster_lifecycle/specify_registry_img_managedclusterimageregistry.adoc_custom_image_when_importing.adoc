[#specify-registry-img-managedclusterimageregistry-when-importing]
= Specifying registry images with a _ManagedClusterImageRegistry_ when importing

You might need to override the registry images on the managed clusters that you are importing. You can do this by creating a `ManagedClusterImageRegistry` custom resource definition. 
