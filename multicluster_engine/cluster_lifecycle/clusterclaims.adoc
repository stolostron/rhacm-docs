...
A `_ClusterClaim_` is a cluster-scoped custom resource definition (CRD) on a managed cluster. A ClusterClaim represents a piece of information that a managed cluster claims. You can use the ClusterClaim to detemine the Placement of the resource on the target clusters. See xref:../cluster_lifecycle/placement_managed.adoc#placement-binding[Placement examples] for more information.
