...
**Note:** Cluster pools that are not specifically added to a managed cluster set are not added to the default `ManagedClusterSet` resource. After a managed cluster is claimed from the cluster pool, the managed cluster will be added to the default `ManagedClusterSet`.
