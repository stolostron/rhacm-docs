...
- Late binding: Delay binding a cluster to a host. An infrastructure administrator can boot the host, and the creator of a cluster can bind the host to the exisitng cluster at a later time. The cluster creator does not need administrator privileges to access the infrastructure when using late binding.
