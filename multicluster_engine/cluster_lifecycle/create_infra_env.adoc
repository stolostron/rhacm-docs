...
The CIM service is provided with the multicluster engine for Kubernetes operator and deploys {ocp-short} clusters. CIM is deployed when you enable the MultiClusterHub Operator on the hub cluster, but must be enabled.
