...
+
Include the optional `unauthenticated_registry` parameter if you are using a mirror reistry that does not require authentication. Entries on this list are not validated or required to have an entry in the pull secret. 
