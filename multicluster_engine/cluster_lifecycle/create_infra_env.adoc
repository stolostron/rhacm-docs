...
- Late binding: Add a node to an existing cluster. An infrastructure administrator can boot the host, and the creator of a cluster can bind the host to the existing cluster at a later time. The cluster creator does not need administrator privileges to access the infrastructure when using late binding.
