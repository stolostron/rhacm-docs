...
The Central Infrastructure Management service is provided with multicluster engine for Kubernetes and deploys {ocp-short} clusters. CIM is deployed when you enable the `MultiClusterHub` Operator on the hub cluster, but must be enabled.
