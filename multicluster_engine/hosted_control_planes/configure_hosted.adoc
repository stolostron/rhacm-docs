...
You can deploy hosted control planes by configuring an existing cluster to function as a hosting service cluster. The hosting cluster is the {ocp} cluster where the control planes are hosted, and can be the hub cluster or one of the {ocp-short} managed clusters. Complete the following steps on the cluster where the {mce} is installed to enable an {ocp-short} managed cluster as a hosting cluster:
...
After setting up the HyperShift binary and enabling your chosen cluster as the hosting service cluster, you can deploy a hosted cluster on AWS INSERT LINK