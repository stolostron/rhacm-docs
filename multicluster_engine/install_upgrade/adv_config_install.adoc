...
You can enable a hub cluster to be a _managed_ hub cluster that is named, `local-cluster`. By default, the hub cluster is not automatically imported and managed by itself. 

If you want your hub cluster to manage itself, see the following step and example:
...
A hub cluster that is managed by itself is designated as `local-cluster` in the list of clusters. 
