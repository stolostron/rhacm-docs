...
*Note*: `subctl` can only support commands relevant to {rhacm-short} deployments which are listed above. For more information about the `subctl` utility and its commands, see link:https://submariner.io/operations/deployment/subctl/[`subctl` in the Submariner documentation].
