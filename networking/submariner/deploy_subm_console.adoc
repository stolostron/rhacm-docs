...
* Red Hat OpenShift Service on AWS with hosted control planes (Technology Preview)
...
* Submariner deployments on VMware and vSphere environments that have a VMware network virtualization and security platform (NSX} enabled are not currently supported by {acm-short} {product-version}.
