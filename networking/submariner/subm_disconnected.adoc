...
[role="_abstract"]
Deploying Submariner on disconnected clusters can help with security concerns by reducing the risk of external attacks on clusters. To deploy Submariner with {acm} on disconnected clusters, you must first complete the steps outlined in link:../../get_started/install_disconnected.adoc#install-on-disconnected-networks[Installing in disconnected network environments]. 

After completing the steps in link:../../get_started/install_disconnected.adoc#install-on-disconnected-networks[Installing in disconnected network environments], configure Submariner during the installation to support deployment on disconnected clusters. 
