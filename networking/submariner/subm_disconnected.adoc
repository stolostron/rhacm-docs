...
. Mirror the images in the local registry. Make sure to mirror the `Submariner Operator bundle` image in the local registry before deploying Submariner on disconnected clusters.

. Choose the Submariner operator version that is compatible with {acm-short}. For instance, use `0.16.0` for {acm-short} version 2.9.
