...
*Note:* By default, Submariner uses IP security (IPsec) to establish the secure tunnels between the clusters on the gateway nodes. You can either use the default IPsec NATT port, or you can specify a different port that you configured. When you run this procedure without specifying an IPsec NATT port, 4500/UDP is used for the connections.

[#preparing-subm-manual-prereq]
== Prerequisites
...
[#preparing-ocp-ibm-cloud-subm]
=== Preparing Red Hat OpenShift on IBM Cloud for Submariner by using the API (Technology Preview)
