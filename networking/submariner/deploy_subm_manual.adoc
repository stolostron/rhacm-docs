...
*Note:* By default, Submariner uses IP security (IPsec) to establish the secure tunnels between the clusters on the gateway nodes. You can either use the default IPsec NATT port, or you can specify a different port that you configured. When you prepare or deploy clusters without specifying an IPsec NATT port, 4500/UDP is used for the connections.
