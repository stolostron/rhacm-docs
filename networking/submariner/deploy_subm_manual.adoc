...
*Note:* By default, Submariner uses IP security (IPsec) to establish secure tunnels between the clusters on the gateway nodes. You can either use the default IPsec NATT port, or you can specify a different port that you configure. When you prepare or deploy clusters without specifying an IPsec NATT port, 4500/UDP is used for the connections.
...
spec:    
    gatewayConfig:
      gateways: <num_of_gateways> <3>
    loadBalancerEnable: true  
----

<1> The name must be `submariner`, as shown in the example.
<2> Replace with the name of your managed cluster.
<3> Set the value higher than `1` if you need Submrminer gateway High Availability (HA).
