...
* <<deploy-subm-manual-prereq,Prerequisites>>
* <<preparing-bare-metal-submariner,Preparing bare metal for Submariner>>
* <<preparing-aro-submariner-cli,Preparing Microsoft Azure Red Hat OpenShift for Submariner by using the CLI>>
* <<preparing-rosa-submariner-cli,Preparing Red Hat OpenShift Service on AWS for Submariner by using the CLI>>
* <<deploy-ocp-ibm-cloud-subm,Deploying Red Hat OpenShift on IBM Cloud for Submariner by using the API (Technology Preview)>>
* <<deploying-submariner-mcaddon-api,Deploying Submariner with the ManagedClusterAddOn API>>
* <<add-resources-subm-manual,Additional resources>>

[#deploy-subm-manual-prereq]
== Prerequisites
...
metadata:
    name: submariner <1>
    namespace: <managed-cluster-namespace> <2>
spec:
    gatewayConfig:
      gateways: 1
----
+
<1> The name must be `submariner`, as shown in the example.
<2> Replace with the name of your managed cluster.
+
This configuration xref:../submariner/subm_customize.adoc#number-gateway-nodes[labels one of the worker nodes as the Submariner gateway] on your bare metal cluster.
...
To prepare Red Hat OpenShift on IBM Cloud clusters for deploying Submariner by using the API, customize and apply YAML content that is similar to the following example:
...
spec:    
    gatewayConfig:
      gateways: <num-of-gateways> <3>
    loadBalancerEnable: true  
----

<1> The name must be `submariner`, as shown in the example.
<2> Replace with the name of your managed cluster.
<3> Set the value higher than `1` if you need Submariner gateway with High Availability (HA).
