...
*Note:* By default, Submariner uses IP security (IPsec) to establish secure tunnels between the clusters on the gateway nodes. You can either use the default IPsec NATT port, or you can xref:../submariner/subm_customize.adoc#customizing-submariner#natt-port-custom[specify a different port] that you configure. When you prepare or deploy clusters without specifying an IPsec NATT port, 4500/UDP is used for the connections. See _Customizing Submariner deployments_ in the additional resources section to learn more.
...
+
This configuration xref:../submariner/subm_customize.adoc#customizing-submariner#number-gateway-nodes[labels one of the worker nodes as the Submariner gateway] on your bare metal cluster.
