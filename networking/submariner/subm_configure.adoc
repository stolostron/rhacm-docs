...
. Add the following annotations in the `metadata` field and save the file:
...
----
annotations:
  submariner.io/control-plane-sg-id: <your_group_ID> <1>
  submariner.io/subnet-id-list: <your_list> <2>
  submariner.io/vpc-id: <your_VPC_ID> <3>
  submariner.io/worker-sg-id: <worker-security-group-id> <4>
----
+
<1> Replace with your control plan security group ID. Typically, the name is similar to `<infra-id>-master-sg`.
<2> Replace with a comma-separated list of public subnet IDs in your custom VPC.
<3> Repalce with your custom VPC ID.
<4> Replace with your worker security group ID. Typically, the name is similar to `<infra-id>-worker-sg`.
