...
When using the console, the `ClusterAdmin` can enable Globalnet for a cluster set by selecting the option *Enable Globalnet* when enabling the Submariner add-on for clusters in the cluster set. After enabling Globalnet, you have to remove all managed clusters from your cluster before you can disable Globalnet.
