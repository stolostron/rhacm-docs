...
[role="_abstract"]
Review the following advanced networking requirements for infrastructure operators, Submariner, Hive, application deployment, and namespace connections.
...
When you are installing bare metal managed clusters with the Hive Operator, which includes using {cim}, you must configure a layer 2 or layer 3 port connection between the hub cluster and the `libvirt` provisioning host. This connection to the provisioning host is required during the creation of a base metal cluster with Hive. See the following table for more information:
