...
* <<importing-the-cluster-automated,Importing the cluster by using the automated method>>
* <<importing-the-cluster-manual,Importing the cluster by using the manual method>>
...
Continue with xref:../import_cli.adoc#importing-the-cluster-automated[Importing the cluster by using the automated method] or xref:../import_cli.adoc#importing-the-cluster-manual[Importing the cluster by using the manual method].

[#importing-the-cluster-automated]
== Importing the cluster by using the automated method

To use automated importing, you must create a secret that contains either a reference to the `kubeconfig` file  of the managed cluster, or server and token pair of the cluster to import. 

. If you are using the `kubeconfig` file, retrieve the `kubeconfig` file of the cluster that you are importing. See the documentation for your Kubernetes cluster to learn where to locate your `kubeconfig` file.

. Generate the import secret in the ${CLUSTER_NAME} namespace using the `kubeconfig` file of the cluster you are importing. Run the following command with your path to your `kubeconfig` file and `CLUSTER_NAME`:
+
----
...
+
**Tip:** Alternatively, you can create your secret by using a server and token, rather than a `kubeconfig` file.
+
. Complete the following steps to create a file that automatically creates the secret:
+
...
== Importing the cluster by using the manual method
