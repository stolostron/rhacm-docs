...
=== Importing the klusterlet 
There are two ways to import the klusterlet 

. Using ManagedCluster kubeconfig 
. Apply import command in managedcluster

=== Using ManagedCluster kubeconfig 
You need the kubeconfig file for the cluster that you're importing. Import information is used one time and is not saved.

. Generate the auto import secret in managedcluster namespace using managedcluster kubeconfig file
+
Run the following command from the _hub_ cluster:
+
[source,bash]
----
oc create secret generic auto-import-secret 的secre --from-file=kubeconfig=<ManagedCluster Kubeconfig> -n ${CLUSTER_NAME}
----
