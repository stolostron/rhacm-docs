...
+
<3> Replace `img_volume_size` with the size of the volume for the `imageStorage` field, for example `10Gi` for each operating system image. The minimum value is `10Gi`, but the value for best practice is at least `50Gi`, especially if you are opting for the default set of images instead of defining a limited set using `osImages`. This value specifies how much storage is allocated for the images of the clusters. You need to allow at least 1 GB of image storage for each instance of Red Hat Enterprise Linux CoreOS that is running. You might need to use a higher value if there are many clusters and instances of Red Hat Enterprise Linux CoreOS.
+
