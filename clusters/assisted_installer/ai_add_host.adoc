...
* <<add-host-ipxe-http,Hosting iPXE artifacts with HTTP or HTTPS>>
* <<additional-resources-add-host,Additional resources>>
...
+
*Note:* By default, the minimal ISO is provided. You can specify the full ISO by setting `ImageType` to `full-iso` in your `InfraEnv` resource. See _Creating a host inventory by using the command line interface_ in the additional resources section to learn more.
...
Set the field to `enabled` to use HTTP for for iPXE artifacts.

Set the field to `disabled` to use HTTPS for for iPXE artifacts.
