...
You can bind hosts to an {ocp} cluster by setting the `clusterDeploymentName` field in the `Agent` custom resource spec, or by setting the `{}bmac.agent-install.openshift.io/cluster-reference` bare metal host annotation.
...
* If you set the annotation and use a string value that follows `<cluster-namespace>/<cluster-name>`, the host becomes bound to the cluster that your `ClusterDeployment` custom resource represents.
