...
You can bind hosts to a {ocp} cluster by setting setting the `clusterDeploymentName` field in the `Agent` custom resource spec, or by setting the `{}bmac.agent-install.openshift.io/cluster-reference` bare metal host annotation.
