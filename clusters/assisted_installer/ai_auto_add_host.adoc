...
You can bind hosts to an {ocp} cluster by setting setting the `clusterDeploymentName` field in the `Agent` custom resource spec, or by setting the `{}bmac.agent-install.openshift.io/cluster-reference` bare metal host annotation.
...
You can use the `{}bmac.agent-install.openshift.io/cluster-reference` annotation in one of the following 3 ways:

* If you do not set the annotation in the bare metal host, no changes apply to the host.

* If you set the annotation with an empty string value, the host becomes unbound.

* If you set the annotation and use a string value that follows `<cluster-namespace>/<cluster-name>`, the host becomes bound to the cluster that the `ClusterDeployment` custom resource represents.
 
*Note:* If the `InfraEnv` that the host belongs to already contains `cluster-reference`, the `{}bmac.agent-install.openshift.io/cluster-reference` annotation is ignored.
