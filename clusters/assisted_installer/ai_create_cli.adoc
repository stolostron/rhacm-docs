...
| `ImageType`
| Optional
| Selects the ISO file type to download. Possible options are `minimal-iso` or `full-iso`. If you do not specify an ISO type, the default value, the minimal ISO is used by default. The minimal ISO does not contain the root file system, `RootFS`. If you select a minimal ISO, the `RootFS` is downloaded later.
|===
