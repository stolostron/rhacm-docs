...
| `ImageType`
| Optional
| Selects the ISO file type to download. Possible options are `minimal-iso` or `full-iso`. If you do not specify an ISO type, the minimal ISO is used by default. The minimal ISO does not contain the root file system, `RootFS`. If you select a minimal ISO, the `RootFS` is downloaded later. If you select a full ISO, you must also set `ISO_IMAGE_TYPE` to `full-iso` in the `AgentServiceConfig` resource for the `ImageType` parameter to take effect.
|===
