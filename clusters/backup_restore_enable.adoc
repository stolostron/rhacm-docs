...
The cluster backup and restore operator can be enabled when the `MultiClusterHub` resource is created for the first time. From the {ocp-short} console, select the *Enable Cluster Backup switch* to enable the operator. The `cluster-backup` parameter is set to `true`. When the operator is enabled, the operator resources are installed.
