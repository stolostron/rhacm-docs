...
** <<upgrading-backup-restore,Upgrading the backup and restore operator>>
...
//have a question about the comment made on line 89
[source,yaml]
----
...
. Set the `enableClusterBackup` parameter to `false` to upgrade the backup and restore operator.

// i personally suggest that we not create a new section bc there is only one task for this. Provided both options for discussion
[#upgrading-backup-restore]
=== Upgrading the backup and restore operator
