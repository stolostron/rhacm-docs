...
{ocp-short} supports multiple architectures by default.

.Procedure

. Use the following `clusterImageSet` to provision a cluster. Replace `4.x.0` with the current supported version:
...
.. From the link:quay.io/repository/openshift-release-dev/ocp-release[{ocp-short} release registry], create a link:docs.docker.com/registry/spec/manifest-v2-2/[manifest list] that includes `x86_64`, `s390x`, `aarch64`, and `ppc64le` release images.

.. Pull the manifest lists for both architectures in your environment from the link:quay.io/repository/openshift-release-dev/ocp-release?tab=tags[Quay repository] by running the following example commands. Replace `4.x.1` with the current version of the product:
...
+
*Note:* The creation process uses the merged release images to create the cluster. 
