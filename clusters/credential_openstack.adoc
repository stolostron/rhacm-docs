...
* Create a `clouds.yaml` file for accessing the CloudStack API. Within the `clouds.yaml` file:
** Figure out the cloud auth section name to use.
...
You can create a cluster that uses this credential by completing the steps in xref:../manage_cluster/create_openstack.adoc#creating-a-cluster-on-openstack[Creating a cluster on Red Hat OpenStack PLatform].
