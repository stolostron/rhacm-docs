[#troubleshooting-a-managed-cluster-import-failure-mce]
...
There can be a few reasons why an imported cluster is not available after an attempt to import. If the cluster import fails, complete the following steps until you find the reason for the failed import:
...
. On the hub cluster, if your managed cluster is a `local-cluster` that is provisioned by Hive, or has an auto-import secret, run the following command to check the import status of the managed cluster.
