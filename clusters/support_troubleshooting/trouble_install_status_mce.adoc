...
When installing the {mce-short}, the `MultiClusterEngine` remains in `Installing` phase, or multiple pods maintain a `Pending` status.
...
If you have this problem, then your cluster needs to be updated with either larger or more worker nodes. See link:../get_started_acm/cluster_size.adoc#sizing-your-cluster[Sizing your cluster] for guidelines on sizing your cluster.
