 ...
 * xref:../support_troubleshooting/trouble_install_rosa_hcp.adoc#trouble-install-stuck-rosa-hcp[Troubleshooting installation status stuck at `Installing` on a ROSA with HCP cluster]
 * xref:../support_troubleshooting/trouble_unknown_rosa_hcp.adoc#trouble-state-unknown-rosa-hcp[Troubleshooting all managed clusters become `Unknown` on a ROSA with HCP cluster]
