...
<<<<<<< HEAD
 * xref:../support_troubleshooting/trouble_install_rosa_hcp.adoc#trouble-install-stuck-rosa-hcp[Troubleshooting installation status stuck at `Installing` on a ROSA with hosted control planes cluster]
 * xref:../support_troubleshooting/trouble_unknown_rosa_hcp.adoc#trouble-state-unknown-rosa-hcp[Troubleshooting all managed clusters become `Unknown` on a ROSA with hosted control planes cluster]
=======
...
>>>>>>> 6d59aa5a3 (https://issues.redhat.com/browse/ACM-12663--catching other version)
