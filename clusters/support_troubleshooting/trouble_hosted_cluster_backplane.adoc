...
. Enter the following command to collect information about the hosted cluster.

+
In the command, the `hosted-cluster-namespace=HOSTEDCLUSTERNAMESPACE` parameter is optional. If you do not include the parameter, the command runs as though the hosted cluster is in the default namespace, which is `clusters`. Replace `<v2.x>` with the version that you are using:
...
. To save the results of the command to a compressed file, include the `--dest-dir=NAME` parameter, replacing `NAME` with the name of the directory where you want to save the results and replace `<v2.x>` with the version that you are using.
