...
. Learn about the `must-gather` command and install the prerequisites that you need at link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.13/html/support/support-overview#support-overview-gather-data-cluster[Gathering data about your cluster] in the {ocp-short} documentation.
...
. Add the multicluster engine for Kubernetes image that is used for gathering data and the directory. Run the following command, where you insert the image and the directory for the output, with the current supported version replacing `v2.x`:
+
----
