...
. Learn about the `must-gather` command and install the prerequisites that you need at link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/support/support-overview#support-overview-gather-data-cluster[Gathering data about your cluster] in the {ocp-short} documentation.
...
----
oc adm must-gather --image=registry.redhat.io/multicluster-engine/must-gather-rhel9:v2.17 --dest-dir=<directory>
----
...
. In a disconnected environment, mirror the Red Hat operator catalog images into their mirror registry. For more information, see xref:../get_started_mce/install_disconnected.adoc#install-on-disconnected-networks[Install on disconnected networks].
