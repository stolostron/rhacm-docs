...
Ensure that your VMware vCenter required account privileges are correct. See link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.17/html-single/installing_on_vmware_vsphere/index#registry-removed_installing-vsphere-installer-provisioned-network-customizations[Image registry removed during installation] for more information. 
...
Use the VMware vSphere client to give the user privileges* for _Profile-driven Storage Privileges_.
