...
- The Namespace-scoped `Placement` specifies a portion of `ManagedClusterSets` that select a working set of the potential `ManagedClusters`.

- The `Placement` selects from that working set by using label and claim selectors.
...
. Select the desired number of `ManagedClusters`.
