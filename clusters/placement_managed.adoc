...
. Select `ManagedClusters` with the `labelSelector`. See in the following sample where the `labelSelector` only matches clusters with label `vendor: OpenShift`.
...
. Select `ManagedClusters` with `claimSelector`. See in the following sample where `claimSelector` only matches clusters with `region.open-cluster-management.io` with `us-west-1.`
...
. Select `ManagedClusters` from particular `clusterSets`. See in the following sample where `claimSelector` only matches `clusterSets:` `clusterset1` `clusterset2`.
