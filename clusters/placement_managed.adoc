...
- Select a cluster with the largest allocatable memory.
  ...
  Note: Similar to Kubernetes [Node Allocatable](https://kubernetes.io/docs/tasks/administer-cluster/reserve-compute-resources/#node-allocatable), 'allocatable' here is defined as the amount of compute resources that are available for pods on each cluster.
