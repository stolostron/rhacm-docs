[#hosted-control-plane-configure]
...
. Because hosted control planes is a Technology Preview feature only on AWS, create an OIDC S3 credentials secret named `hypershift-operator-oidc-provider-s3-credentials` for the HyperShift operator. HyperShift operatorSave the secret in the managed cluster namespace, which is the namespace of the managed cluster that is used as the hosting service cluster. If you used `local-cluster`, then create the secret in the `local-cluster` namespace.
+
...
+
If you are deploying the `hypershift-operator` using mulicluster engine, you must have an S3 secret file available for the configuration on any platform. If you are not running AWS, complete the following steps to create a substitute file: 

.. Create a file named `s3-credentials-placeholder` that contains the following content:
+
----
[default]
...
.. Create the S3 secret file based on the `s3-credentials-placeholder` file by running the following command:
+
----
oc create secret generic hypershift-operator-oidc-provider-s3-credentials --from-file=credentials=s3-credentials-placeholder --from-literal=bucket=bucket --from-literal=region=us-east-1 -n local-cluster
----
