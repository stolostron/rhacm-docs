...
You can deploy hosted control planes by configuring an existing cluster to function as a hosting service cluster, and then deploying a cluster that is managed by the HyperShift hosting service cluster that you enabled as the hosting service cluster. The hosting service cluster is the {ocp-short} cluster where the control planes are hosted, and can be the {product-title-short} hub cluster or one of the {ocp-short} clusters that {product-title-short} manages.
...
*Note:* If you want your {product-title-short} hub cluster to be your hosting service cluster, you can configure `local-cluster` as your hosting service cluster. A hub cluster that is also managed by itself is designated as `local-cluster` in the list of clusters. 
...
. Create an OIDC S3 credentials secret named `hypershift-operator-oidc-provider-s3-credentials` for the HyperShift operator. Create the secret in the namespace of the cluster where the hosting service cluster is hosted. 
+
