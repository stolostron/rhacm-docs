...
You can deploy hosted control planes by configuring an existing cluster to function as a hosting service cluster, and then deploying a cluster that is managed by the HyperShift hosting service cluster that you enabled as the hosting service cluster. The hosting service cluster can be the {product-title-short} hub cluster or one of the {ocp-short} managed clusters.
