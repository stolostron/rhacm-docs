...
On VMware vSphere, Red Hat OpenStack, Red Hat Virtualization Platform, and in the bare metal environment, you can create a single node OpenShift (SNO) cluster. A SNO cluster contains only a single node which hosts the control plane services and the user workloads. This configuration can be helpful when you want to minimize the resource footprint of the cluster. 
...
* Credentials for accessing the where you want your SNO cluster.
