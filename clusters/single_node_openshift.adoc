...
You can create a single node OpenShift (SNO) cluster that contains only a single node in a bare metal environment. In a standard cluster, there are control plane nodes and separate worker nodes. This configuration can be helpful when you have many clusters that need to minimize their resource footprint. 
...
* Credentials for accessing the bare metal server where you want your SNO cluster.
...
Complete the steps in the {product-title-short} console for creating a cluster in the bare metal environment. Because the bare metal assets feature is deprecated, it is best practice to use the Central Infrastructure Management method of creating a SNO cluster. The procedure to create a SNO cluster is essentially the same as creating an on-premises cluster. See xref:../clusters/create_cluster_on_prem.adoc#creating-a-cluster-on-premises[Creating a cluster in an on-premises environment].
