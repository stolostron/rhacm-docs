...
In a typical cluster, there are control plane nodes and separate worker nodes. To optimize your deployment model and minimize the resource footprint, it can be useful to create a single node OpenShift (SNO) cluster that contains only a single node. The single node contains both the control plane and worker on one node. 

This procedure explains how to deploy a single SNO cluster. You can also use the zero touch provisioning procedure to provision multiple SNO clusters, which is suitable for large scale edge deployments. For more information about that procedure, see https://access.redhat.com/documentation/en-us/openshift_container_platform/4.10/html/scalability_and_performance/ztp-deploying-disconnected[Deploying distributed units at scale in a disconnected environment] in the {ocp-short} documentation.
...
Complete the steps in the {product-title-short} console for creating a cluster on one of the providers that are supported to run a SNO cluster in xref:../clusters/create.adoc#creating-a-cluster[Creating a cluster].

You must select {ocp-short} 4.9, or later In the _Release image_ field. If you enabled the feature, an option to select *Single node OpenShift* is displayed. 
