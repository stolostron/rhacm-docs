...
In a bare metal environment, you can create a single node OpenShift (SNO) cluster. A SNO cluster contains only a single node, which serves as the control plane node and the user workloads. This configuration can be helpful when you have many clusters that need to minimize their resource footprint. 
...
Complete the steps in the {product-title-short} console for creating a cluster in the bare metal environment. It is best practice to use the Central Infrastructure Management method of creating a SNO cluster. The procedure to create a SNO cluster is essentially the same as creating an on-premises cluster. See xref:../clusters/create_cluster_on_prem.adoc#creating-a-cluster-on-premises[Creating a cluster in an on-premises environment].
