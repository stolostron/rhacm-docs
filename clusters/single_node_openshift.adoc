...
Dynamic metric collection for SNO clusters is supported with a group of collection rules (`collection_rules`). Conditions for the collection rules are written as a PromQL expression. By default, the following conditions are defined: `HighCPUUsage` and `HighMemoryUsage`. When eiher of the conditions exceed 70 percent continuously, the observability component collects data. Observability automatically collects pod and container metrics for SNO clusters. 
