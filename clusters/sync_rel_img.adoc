...
. If the installer-managed `acm-hive-openshift-releases` subscription is enabled, disable the subscription by setting the value of `disableUpdateClusterImage` to `true`. You can remove the subscription by entering a command that is similar to the following command:
+
----
