...
| Outbound to the managed cluster
| The `ocm-controller` on the hub cluster 
| The `work-manager` add-on on the managed cluster 
| TCP
| 443
| Access the `work-manager` service route to view the pod logs on the managed cluster
...
https://issues.redhat.com/browse/ACM-18959
