...
----
+
This role is a superuser, which has access to all resources and actions. You can create cluster-scoped `managedcluster` resources, the namespace for the resources that manage the managed cluster, and the resources in the namespace with this role. You might need to add the `username` of the ID that requires the role association to avoid permission errors.
...
- Create a `ClusterPermission` resource and create multiple cluster role bindings. *Note:* You must have access to {acm-short}.

+
For example, run the following command to create `multi-crb-binding1` and `multi-crb-binding2` role bindings:

+
[source,bash]
----
oc create clusterpermission clusterpermission-multiple-clusterrolebindings -n clusterRoleBindings --user1=multi-crb-binding1 --user2=multi-crb-binding2
----
+
