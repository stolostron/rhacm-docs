...
+
The secret must contain 3 fields. The `bucket` field contains an S3 bucket with public access to host OIDC discovery documents for your HyperShift clusters. The `credentials` field is a reference to a file than contains the credentials of the `default` profile that can access the bucket. By default, HyperShift only uses the `default` profile to operate the `bucket`. The `region` field specifies the region of the S3 bucket.
+
