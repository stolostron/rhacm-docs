...
The above `HypershiftDeployment` custom resource provisions hosted control plane and its nodePools with default values. You can also customize the hosted control plane and the nodePools by specifying `hostedClusterSpec` and `nodePools` in the `HypershiftDeployment` custom resource.
+
.. Create a file that contains information that resembles the following example: 
+
[source,yaml]
