...
. Create an oidc s3 credentials secret for the HyperShift operator, named `hypershift-operator-oidc-provider-s3-credentials`. Create the secret in the same namespace where your `hypershift-management-cluster` is hosted. 
+
...
+
*Tip:* Add the label to ensure that the `hypershift-operator-oidc-provider-s3-credentials` secret is backed up for disaster recovery by entering the following command:
+
----
...
----
+
* To create this secret with the console, follow the credential creation steps by accessing *Credentials* in the navigation. 
+
...
----
+
*Note:* The value of `cluster.open-cluster-management.io/backup=true` is added to the secret so the secret is backed up for disaster recovery.
