...
. Create a `HypershiftDeployment` custom resource file in the cloud provider secret namespace. The `HypershiftDeployment` custom resource creates the infrastructure in the provider account, creates a hosted control plane on a management cluster, and configures the infrastructure compute capacity in the created infrastructure.
+
...
spec:
  ...
  hostingNamespace: clusters     # specify the namespace to which hostedcluster and nodepools belong on the hypershift management cluster.
  infrastructure:
    cloudProvider:
      name: <my-secret>
    configure: True
