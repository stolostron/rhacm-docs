...
+
The `spec.proxyConfig` is an optional section. If the {ocp-short} cluster is created with cluster wide proxy from the {product-title-short} configured, the cluster wide proxy configuration is added to the pods of the klusterlet add-ons as environment variables when the `proxyPolicy` in the `addon` section is set to `OCPGlobalProxy`.
+
The structure and values for the `spec.proxyConfig` section are automatically added to the pods of klusterlet add-on as environment variables when the `proxyPolicy` in the add-on section is set to `CustomProxy`.
+
The default of `proxyPolicy` in the `addon` section is `Disabled`.
+
