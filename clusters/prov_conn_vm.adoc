...
* <<vsphere_prov_conn_prerequisites,Prerequisites>>
* <<vsphere_provider_conn,Creating a provider connection by using the console>>
* <<vsphere_delete_provider_conn,Deleting your provider connection>>
...
You can create a cluster that uses this provider connection by completing the steps in xref:../clusters/create_vm.adoc#creating-a-cluster-on-vmware-vsphere[Creating a cluster on VMware vSphere].
