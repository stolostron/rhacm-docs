...
+
Replace `source-ns` with the namespace of the persistent volume claim where your source is located. See step 3f for instructions on how to replace this automatically. 
+
...
+
----
$ kubectl describe ReplicationSource -n <source-ns> <pvc-source>
----
+
...
+
----
$ kubectl get replicationdestination <destination-name> -n <destination-ns> --template={{.status.latestImage.name}}
----
Note the value of the latest snapshot for when you create your persistent volume claim.
+
Replace `destination-name` with the name of your replication destination. 
+
