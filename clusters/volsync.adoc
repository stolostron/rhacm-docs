...
+
Replace `destination_name` with the name of your replication destination CR.
+
...
+
----
$ ADDRESS=`kubectl get replicationdestination <destination_name> -n dest --template={{.status.rsync.address}}`
$ echo $ADDRESS
----
...
+
----
$ SSHKEYS=`kubectl get replicationdestination <destination_name> -n dest  --template={{.status.rsync.sshKeys}}`
$ echo $SSHKEYS
----
+
...
. Deploy your application. For this example, the application is: `application-destination`.

.. On the source cluster, `source-cluster`, enter the following commands to create and prepare the application: 
+
----
$ kubectl create ns source
$ kubectl create -n source -f application-destination
----

.. Run the following command to confirm that the application is running:
+
----
...
application-878097   1/1     Running   0          17s
----
...
----
+
Replace `source_name` with the name for your replication source CR. See step 3f for instructions on how to replace this automatically.
+
Replace `source` with the name of the namespace where your source is located. See step 3f for instructions on how to replace this automatically. 
+
...
+
----
$ kubectl describe ReplicationSource -n source application-source
----
+
...
.. Create a database in the `application` pod that is running in the source namespace by entering the following commands on the destination cluster:
+
----
$ kubectl exec --stdin --tty -n source `kubectl get pods -n source | grep mysql | awk '{print $1}'` -- /bin/bash
$ application -u root -p$APPLICATION_ROOT_PASSWORD
> show databases;
+--------------------+
| Database           |
+--------------------+
...
----
+
The `application` application is deployed to the `dest` namespace, which uses the replicated data. 
...
+
----
$ sed -i 's/snapshotToReplace/volsync-dest-database-destination-20201203174504/g' application/application-pvc.yaml
$ kubectl create -n dest -f <application>/destination-database/
----

.. Validate that the `application` pod is running in the environment by entering the following command:
+
----
$ kubectl get pods -n dest
----

.. Connect to the `application` pod and list the databases to verify that the synchronized database exists by entering the following command:
+
----
$ kubectl exec --stdin --tty -n dest `kubectl get pods -n dest | grep <application> | awk '{print $1}'` -- /bin/bash
$ <application> -u root -p$APPLICATION_ROOT_PASSWORD
> show databases;
+--------------------+
| Database           |
