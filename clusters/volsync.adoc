...
----
+
Replace `source` with the name for your replication source CR. See step 3f for instructions on how to replace this automatically.
+
...
----
+
*Note:* In this example, the source and the destination persistent volume claims are on the same server. If your persistent volume claims are not on the same server, then copy your secret file to the server of your source persistent volume claims. 
