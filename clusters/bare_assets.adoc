...
----
+
<1> When created, the `BareMetalHost` on the managed cluster gets the same name as the corresponding `BareMetalAsset` on the hub cluster. The `BareMetalHost` name should always match the corresponding `BareMetalAsset` name.
...
<4> Can be either "worker", "master", or "". The `role` setting is used to match a bare metal asset to specific machine roles types in the cluster. `BareMetalAsset` resources with a specified machine role type should not be used to fill another role. The `role` value is used as the value for a label with key `inventory.open-cluster-management.io/role`. This enables a cluster management application or user to query for inventory that is intended for a particular role.
...
[NOTE]
====
For the above curl command, it is assumed that the API server is served over HTTPS and is accessed securely. In a development or test environment, you can pass the `--insecure` parameter.
====
