...
You need a provider connection to use {product-title} console to create and manage a {ocp} cluster on Microsoft Azure.

*Note:* This procedure is a prerequisite for creating a cluster with {product-title}.

* <<azure_prov_conn_prerequisites,Prerequisites>>
* <<azure_provider_conn,Creating a provider connection by using the console>>
* <<azure_delete_provider_conn,Deleting your provider connection>>
...
You can create a cluster that uses this provider connection by completing the steps in xref:../clusters/create_azure.adoc#creating-a-cluster-on-microsoft-azure[Creating a cluster on Microsoft Azure].
