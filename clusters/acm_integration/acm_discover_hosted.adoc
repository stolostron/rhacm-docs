...
. Create the `addonsDeploymentConfig` resource to specify a different addon installation namespace. See the following example where `agentInstallNamespace` points to `open-cluster-management-agent-addons-discovery`:
...
. Run the following command to verify that the addons for the {product-title-short} `local-cluster` are re-installed into the namespace that you specified.:
...
Install those addons in a _different_ {mce-short} namespace so that the {mce-short} can self-manage with the `local-cluster` addons, while {product-title-short} manages {mce-short} at the same time. 
...
Add `backup=true` to enable backup. See the following steps for each addon:
...
After all your {mce-short} clusters are imported into {product-title-short}, you need to enable the hypershift addon for those managed {mce-short} clusters to discover the hosted clusters.
...
. Run the following command to set the `agentInstallNamespace` namespace of the addon to `open-cluster-management-agent-addons-discovery`:
...
Now you can visit xref:../acm_integrate_import_hcp.adoc#auto-import-hcp[Automating import for discovered hosted clusters] to learn how to automatically import clusters.
