...
Automate the import of hosted clusters by using the `DiscoveredCluster` resource for faster cluster management, without manually importing individual clusters. When you automatically import a discovered hosted cluster into {product-title-short}, all {product-title-short} addons are enabled so that you can start managing the hosted clusters with the available management tools.

The hosted cluster is also _auto-imported_ into {mce-short}. Through the {mce-short} console, you can manage the hosted cluster lifecycle. You cannot manage the hosted cluster lifecycle from the {product-title-short} console.
//We only have the integrated console, right? Are we saying you cannot manage from outside the "local cluster" console? The cluster switcher is a drop-down menu that initially displays `local-cluster`, so they cannot switch to all cluster view, right?

*Required access:* Cluster administrator

* <<auto-hcp-import,Prerequisites>>
* <<configure-hcp-autoimport,Configure settings for automatic import>>
...
You can _detach_ a hosted cluster from {product-title-short} using the *Detach* option in the {product-title-short} console, or by removing the corresponding `ManagedCluster` CR from the command line. For best results, detach the managed hosted cluster before _destroying_ the hosted cluster.
