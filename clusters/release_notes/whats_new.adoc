...
* You can now configure additional networks, request guaranteed CPU access for Virtual Machines (VMs), and manage scheduling of KubeVirt VMs for node pools. For more information, see link:../hosted_control_planes/managing_nodepools_kubevirt.adoc#manage-nodepools-hosted-kubevirt[Configuring additional networks, guaranteed CPUs, and VM scheduling for node pools].

[#acm-integration-wn]
== {acm-short} integration

If you enable Observability after you install {acm-short}, you can use Grafana dashboards to view your hosted control planes cluster capacity estimate, and existing hosted control planes resource utilizations. See more at xref:../install_upgrade/acm_integration.adoc#acm-integration[{acm-short} integration].