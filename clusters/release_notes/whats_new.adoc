...
You can now rely on the {olm} `operatorcondition` resource to enforce an efficient {mce-short} upgrade procedure. To prevent errors from attempts to skip releases, check the status of the `operatorcondition` when looking to upgrade.
