...
You can now automate the import of Red Hat OpenShift Service on AWS clusters by using {product-title-short} policy enforcement with  {mce-short}. Learn more at xref:../clusters/acm_integration/integration_import_rosa.adoc#import-discover-rosa[Automating import for discovered OpenShift Service on AWS clusters].