...
- You can now store and download HTTPS `osImages` that have TLS enabled by using a self-signed or third-party CA certificate for authentication. See xref:../cluster_lifecycle/cim_enable.adoc#enable-cim-disconnected[Enabling central infrastructure management in disconnected environments] to learn more.
