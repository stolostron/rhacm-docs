...
- You can now pass HTTP/HTTPS headers and query parameters in each request to fetch an OS image. See xref:../cluster_lifecycle/cim_enable.adoc#enable-cim-disconnected[Enabling central infrastructure management in disconnected environments] to learn more.
