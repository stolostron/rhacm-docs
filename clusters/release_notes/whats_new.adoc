...
- You can now run only the failed Ansible posthooks again that are part of cluster automation templates to save time. See xref:../clusters/cluster_lifecycle/ansible_config_cluster.adoc#ansible-rerun-job-cluster[Running a failed Ansible job again] for more information.
