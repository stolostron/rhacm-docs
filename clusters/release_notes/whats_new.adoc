...
- You can now enable a discovered cluster to automatically import supported clusters into your hub cluster with the `discovery-operator`. Import clusters for faster cluster management. See, link:../discovery/enable_discovery_management.adoc#enable-discovered[Enabling a discovered cluster for management].
