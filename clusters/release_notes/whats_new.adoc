...
You can now rely on the {olm} `operatorcondition` resource to enforce an efficient {mce-short} upgrade procedure, which will prevent errors from attempts to skip releases. See link:../install_upgrade/upgrade_cluster.adoc#upgrading[Upgrading] for details.
