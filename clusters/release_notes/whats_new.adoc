...
- ClusterCurator now supports specifying an Ansible inventory to use for all jobs, and this is available to {mce-short} users in the console as well. See xref:../clusters/cluster_lifecycle/ansible_config_cluster.adoc##specify-ansible-inventory[Specifying an Ansible inventory to use for all jobs] for more information. 
