...
You can now automatically apply configurations in the gloabl `KlusterletConfig` on every managed cluster, without binding a managed cluster to a `KlusterletConfig` by using an annotation. See xref:../cluster_lifecycle/adv_config_cluster.adoc#config-gloabl-klusterletconfig[Configuring the global _KlusterletConfig_] to learn more.
