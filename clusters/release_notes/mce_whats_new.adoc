...
You can now change the name of your local cluster. By default, the cluster that is running {mce-short} is the _local cluster_, which is a hub cluster that manages itself. When the `enabled` field is set to `false`, which disables the local cluster feature, you can change the name. Use 34 or fewer characters for the `<your-local-cluster-name>` value. The  resource and the namespaces reflects the change.
