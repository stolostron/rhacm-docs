...
- You can now import all cluster resources and continue using them after moving a managed cluster that was installed by the {ai} from one hub cluster to another hub cluster. To learn more, see xref:../cluster_lifecycle/import_ocp.ado#import-ocp-cluster-resources[Importing cluster resources].
