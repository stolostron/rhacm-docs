...
- You can now access the `kube-apiserver` of a managed by using a hub cluster user token. To learn more, see xref:../cluster_proxy_addon_use#access-kube-apiserver-hub-token[Accessing the `kube-apiserver` of a managed cluster by using a hub cluster user token].
