...
All other _Fixed issues_ in {mce-short} 2.7 after 2.7.10 are _container image updates_ only and do not require release notes. See the advisories for those details.
...
* Upgrades the Golang JSON web tokens that fixes an error where excessive memory was allocated during header parsing.
