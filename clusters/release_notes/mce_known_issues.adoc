...
The agent will not be removed from the hub automatically if you disable the local-cluster by setting the `disableHubSelfManagement` option to *true* in the `MultiClusterHub` resource. This issue appears only within 60 minutes after you upgrade the hub to version 2.13.
