...
The `clusterlifecycle-state-metrics` pod in the `multicluster-engine` namespace might crash and restart. The behvaior is inconsistent, but typically happens when managed cluster metrics are refreshed at the same time that you create, hibernate, or resume a Hive `ClusterDeployment`.
...
If a restart happens while Prometheus is scraping metrics, you might lose the data for that moment.
