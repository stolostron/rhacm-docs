...
When you enable and then disable the `cluster-api` and `cluster-api-provider-aws` components, custom resource definitions (CRDs) of a higher version might remain in the cluster.

You must manually remove the CRDs before you re-enable the `hypershift-local-hosting` component.

Manually remove the following CRDs:
