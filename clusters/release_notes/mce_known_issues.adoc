...
When cluster redistribution occurs across multiple `PlacementDecisions` resources, adding or removing clusters results in a shift between decisions. Because, the update mechanism is sequential, a cluster is temporarily not exist in any `PlacementDecision` resources during the update.
