...
The hosted control planes operator cannot manage infrastructure resources in AWS because of API conflicts between the `cluster-api-provider-aws` resource and the `hypershift` resource that is installed by the `hypershift-local-hosting` component. 
