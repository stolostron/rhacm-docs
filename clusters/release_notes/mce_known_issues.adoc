...
=== _`cluster-api-provider-aws`_ webhooks block hosted control planes deployments
//2.14:ACM-21708
...
To use the `hypershift` component for the AWS platform hosted clusters, _do not_ enable the `cluster-api` and `cluster-api-provider-aws` components, which are `enabled: false` by default. See the following default setting with the components disabled:
