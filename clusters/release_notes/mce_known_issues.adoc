...
[#cluster-curator-limit-hub-upgrade]
=== Cluster curator limitation when upgrading the hub cluster
...
To workaround the issue, manually upgrade the hub cluster. If you already used the `ClusterCurator` resource to upgrade the hub cluster, you must delete the `ClusterCurator` resource hub cluster namespace so that future upgrades appear in the console.
