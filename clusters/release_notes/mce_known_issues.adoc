...
When you use the `ClusterCurator` resource to upgrade the hub cluster, the `ClusterCurator` status becomes stuck, even if the upgrade is successful. During upgrade, the hub cluster reboots, which stops the `Job` resource pod monitoring and the upgrade.

To workaround the issue, manually upgrade the hub cluster. If you already used the cluster curator to upgrade the hub cluster, you must delete the `ClusterCurator` resource hub cluster namespace so that future upgrades appear in the console.
