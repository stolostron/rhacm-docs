...
When you add or remove clusters, cluster redistribution occurs across multiple `PlacementDecisions` resources, resulting in a shift between decisions. Because the update mechanism is sequential, a cluster temporarily does not exist in any `PlacementDecision` resources during the update.
