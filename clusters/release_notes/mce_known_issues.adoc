...
[#agent-not-removed]
=== Agent is not removed if the local-cluster is disabled
//2.13:ACM-17221

The agent is not be removed from the hub automatically if you disable the `local-cluster` by setting the `disableHubSelfManagement` option to `true` in the `MultiClusterHub` resource. This issue appears only within 60 minutes after you upgrade the hub cluster to version 2.13.

To work around the issue and remove the agent, complete the following steps:

. Delete the `klusterlet` resource manually:
+
[source,terminal]
----
...
. If the `klusterlet` resource does not exist, delete the agent and add-on namespaces:
+
[source,terminal]
----

oc delete namespace --ignore-not-found open-cluster-management-agent open-cluster-management-agent-addon
