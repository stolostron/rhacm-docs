...
=== Unclaimed Hive clusters might be recreated during ClusterPool CRD resource changes
//2.9:ACM-21686
//still editing
During an upgrade or modification to the ClusterPool custom resource definition (CRD)—such as those that may occur implicitly through an MCE upgrade—Hive may delete and recreate all unclaimed clusters. This is expected behavior and ensures that unclaimed clusters reflect the latest configuration defined in the updated ClusterPool specification.
