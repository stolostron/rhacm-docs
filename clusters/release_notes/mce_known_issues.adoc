...
=== Klusterlet add-on does not tolerate `NoExecute` taint
//2.13:ACM-15906

If you use the `NoExecute` taint on your infrastructure nodes, your pods might become stuck.

To work around the issue, remove the `node.kubernetes.io/unreachable:NoExecute` taint and remove the `node-role.kubernetes.io/infra=reserved:NoExecute` toleration.
