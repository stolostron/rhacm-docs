...
[#create-hosted-ocp-4193]
=== Creating a hosted cluster with {ocp-short} with 4.19.3 and earlier fails
//2.14:ACM-22342

If you create a hosted cluster on the BareMetal agent platform with {ocp-short} 4.19.3 and earlier, and use the MetalLB operator for the hosted cluster, the network cluster operator in the hosted cluster fails to start with the following error:

[source,bash]
----
oc get co network -ojson | jq ".status.conditions"[0].message
...
To work around the issue, upgrade to {ocp-short} 4.19.4 or later.
