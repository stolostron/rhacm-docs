...
=== Cluster provisioning with {ai} fails
//2.4:17411

When creating {ocp-short} clusters by using the {ai}, the file name of the ISO image might be too long. The long image name causes the image provisioning and the cluster provisioning to fail. To determine if this is the problem, complete the following steps: 
...
If this problem occurs, it is typically on the following versions of {ocp-short}, because the {ai} was not using image service:
