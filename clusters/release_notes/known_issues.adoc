...
The cluster proxy add-on does not support proxy settings between the {mce-short} hub cluster and the managed cluster. If there is a proxy server  between the {mce-short} hub cluster and the managed cluster, you must disable the cluster proxy add-on. Remove the `agent.open-cluster-management.io/klusterlet-config` annotation from the `ManagedCluster` resource.
