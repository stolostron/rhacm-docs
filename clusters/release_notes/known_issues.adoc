...
=== One way hub cluster communication limitations
//2.9:ACM-6292

The following limitations occur if the hub cluster is not able to reach the managed cluster:

- You cannot create a new managed cluster by using the console. You are still abel to import a managed cluster manually.
- If you deploy an Application or ApplicationSet by using the console, or if you import a managed cluster into ArgoCD, the hub cluster ArgoCD controller calls the managed cluster API server. You can use the AppSub or the ArgoCD puell model to work around the issue.
- The logs page for pod details does not work and an error message that resembles the following appears:
+
----
