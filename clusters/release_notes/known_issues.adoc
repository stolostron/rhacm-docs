...
The `ManagedServiceAccount` and cluster proxy add-ons are enabled by default in {product-title-short} version 2.10 and newer. If the add-ons are disabled after upgrading, you must enable the `ManagedServiceAccount` and cluster proxy add-ons manually to use the pod log feature on non-{ocp-short} managed clusters.
...
The converged flow is the default process of provisioning. When you use the `BareMetalHost` resource for the Bare Metal Operator (BMO) to connect your host to a live ISO, the ironic agent runs. First, the ironic agent runs the steps in the Bare Metal installer-provisioned-infrastructure. Then, the ironic agent starts the Assisted Installer agent, and the agent handles the rest of the install and provisioning process. 
...
*IMPORTANT:* When you disable the converged flow, only the Assisted Installer agent runs in the live ISO, reducing the number of open ports and disabling any features enabled by the ironic agent. 
...
----
<1> When you set the parameter value to "false", you also disable any features enabled by the ironic agent. 
...
In hosted control planes on the IBM Z platform, when you unbind the hosts with the cluster, the hosts restart in loop and are not ready to be used. For a workaround for this issue, see xref:../hosted_control_planes/destroy_hosted_cluster_x86bm_ibmz.adoc#destroy-hosted-cluster-x86bm-ibmz[Destroying a hosted cluster on x86 bare metal with IBM Z compute nodes].