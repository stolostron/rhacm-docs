...
When you deploy a hub cluster on bare metal by using the Assisted Service, the hub cluster installation requires an ironic agent image. The hub cluster {ocp-short} version determines which ironic agent image is used.
