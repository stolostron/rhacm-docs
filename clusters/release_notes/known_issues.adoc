...
[#delpoy-sno-managed-fails]
=== Deploying a single node OpenShift managed cluster fails
//2.8:ACM-5813

Deploying a single node OpenShift (SNO) managed cluster might fail when the Assisted Installer agent starts slowly. You can work around the issue by disabling converged flow. Complete the following steps:
