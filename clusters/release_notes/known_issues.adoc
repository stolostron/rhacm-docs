...
In a disconnected environment, you must mirror the {ocp-short} image manually. Make sure that the ironic agent image version matches the hub cluster {ocp-short} version by running the following command. Replace `<hub-release-image>` with the image for the hub cluster release:
...
Replace `<mirror>` with your mirror.
