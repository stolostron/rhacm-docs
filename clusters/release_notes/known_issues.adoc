...
=== Console lists nodes even after they are deleted
//2.9:ACM-83334
...
* On the command line interface, enter the following commands:

** Verify that 0 nodes are in the node pool by entering the following command:

+
----
oc get nodes --kubeconfig <hosted-cluster-kubeconfig>
----

** Verify that no agents are reported as bound to any cluster by entering the following command:
