...
[#vspher_deprovis_fails_when_folder_used]
=== Deprovisioning vSphere cluster fails when specific folder is used on {ocp-short} version 4.16.
//2.10:12371

Deprovisioning a vSphere cluster fails on {ocp-short} version 4.16 when multiple clusters are in the same folder.
