...
When a node pool is scaled down to 0 workers, the console still shows nodes in a Ready state in the list of hosts. In this situation, verify the number of hosts in the node pool by using the command line interface.