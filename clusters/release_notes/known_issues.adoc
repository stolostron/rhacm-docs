...
The `ManagedServiceAccount` and cluster proxy add-ons are enabled by default in {product-title-short} version 2.10 and newer. If they are disabled after upgrading, you must enable the `ManagedServiceAccount` and cluster proxy add-ons manually to use the pod log feature on non-{ocp-short} managed clusters.
