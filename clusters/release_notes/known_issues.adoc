...
In hosted control planes on the IBM Z platform, when you unbind the hosts with the cluster, the hosts restart in loop and are not ready to be used. For a workaround for this issue, see xref:../clusters/hosted_control_planes/destroying-hosted-clusted-x86bm-ibmz#destroying-hosted-clusted-x86bm-ibmz[Destroying a hosted cluster on x86 bare metal with IBM Z compute nodes].