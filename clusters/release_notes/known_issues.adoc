...
The cluster proxy add-on does not support proxy settings between the MCE hub cluster and the managed cluster. If there is a proxy server sitting between the MCE hub cluster and the managed cluster, you must disable the cluster proxy add-on. 
