...
If you deploy a hub cluster on bare metal by using the Assisted Service, installing managed clusters requires an ironic agent image. The hub cluster {ocp-short} version determines which ironic agent image is used.
