...
=== The`managed-serviceaccount` add-on limitation
//2.9:ACM-8586

The following are known issues and limitation for the `managed-serviceaccount` add-on:

- It does not support running on {ocp-short} 3.11. 
- The `installNamespace` field can only have one value. 
- The `tolerations` and `nodeSelector` settings do not support the `managed-serviceaccount` agent. 
