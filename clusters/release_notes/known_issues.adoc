...
=== _ClusterCurator_ does not support OpenShift Dedicated clusters
//2.10:ACM-10154

When you upgrade an OpenShift Dedicated cluster by using the `ClusterCurator`, the upgrade fails because the `ClusterCurator` does not support OpenShift Dedicated clusters.
