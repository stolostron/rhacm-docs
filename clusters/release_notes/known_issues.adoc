...
[#vsphere_deprovis_fails_when_folder_used]
=== Deprovisioning vSphere cluster might fail when specific folder is used on {ocp-short} version 4.16.
//2.10:12371

Deprovisioning a VMware vSphere cluster might fail on {ocp-short} version 4.16 when multiple VMware vSphere clusters are in the same folder. See **ocp-link or kcs link** for more information. 
