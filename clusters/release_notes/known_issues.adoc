...
Make your development quicker by configuring copy and paste features. To configure the `copy-from-mac` feature in the the `assisted-installer`, complete the following steps: 

. Go to the `nmstate` definition interface. 
. Copy the `mac-address` in the `nmstate` definition interface. 

If the `mac-address` is provided outside the `nmstate` definition, the `nmstate` cannot implement the `copy-from-mac` feature. This issue is the `nmstate` limitation. 
