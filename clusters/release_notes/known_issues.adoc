...
//check into this one, also we don't use tip, since 2020.
...
// 2.1:5636
//look into this one, if only for the ACM user, we should move
...
You cannot import IBM {ocp-short} Kubernetes Service version 3.11 (deprecated) clusters. Later versions of IBM OpenShift Kubernetes Service are supported.
...
=== Node information from the managed cluster cannot be viewed in search
// 2.0.2:4598
//hmmm, so Search is an ACM feature, this probably moves
...
//this looks like an OCP known issue that we should really not duplicate, no issue attached and likely added incorrectly
...
//2.9:ACM-6292
// applications is an ACM function
