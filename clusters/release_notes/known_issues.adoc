...
=== Hub cluster {ocp} image is not mirrored automatically
//2.8:ACM-6260

If you deploy a hub cluster on bare metal by using the Assisted Service, the {ocp} image is not mirrored automatically in disconnected environments. A matching ironic agent image of {ocp-short} must be used on your hub cluster to install managed clusters. The hub cluster {ocp-short} version detects which ironic agent image is being used.

In a disconnected environment, you must mirror the {ocp-short} image manually. Make sure that the ironic agent image version matches the hub cluster {ocp-short} version by running the following command:

----
oc adm release info --image-for=ironic-agent <hub-release-image>
----

If the image versions do not match, see link:https://docs.openshift.com/container-platform/4.12/installing/disconnected_install/installing-mirroring-disconnected.html[Mirroring images for a disconnected installation] in the {ocp-short} documentation to learn how to obtain a mirrored image.
