...
When you upgrade from {mce-short} from 2.4.0 to 2.4.1, you might find a `StorageVersionMigration` error in the log of the `cluster-manager` pods that run in the namespace of {mce-short}. These error messages have no impact on {mce-short}. Therefore, you do not need to take any action to resolve these errors. 
