...
+
This role is a superuser, with access to all resources and actions. You can create cluster-scoped `managedcluster` resources, the namespace for the resources that manage the managed cluster, and the resources in the namespace with this role. You might need to add the `username` of the ID that requires the role association to avoid permission errors. Run the following command:
...
. Administer a cluster role binding for a managed cluster named `cluster-name`, a role with read and write access to the cluster-scoped `managedcluster` resource. The `managedcluster` is a cluster-scoped resource and not a namespace-scoped resource. Run the following command:
...
. Create a namespace role binding to the cluster role `admin`, which has read and write access to the resources in the namespace of the managed cluster. Run the following command:
...
. Create a cluster role binding for the `open-cluster-management:view:<cluster-name>` cluster role to view a managed cluster named `cluster-name`. This role has read access to the cluster-scoped `managedcluster` resource. The `managedcluster` is a cluster-scoped resource. Run the following command:
...
. Create a namespace role binding to the cluster role `view`. This role has read-only access to the resources in the namespace of the managed cluster. Run the following command:
...
. View a list of the managed clusters that you can access. Administrators and users without cluster administrator privileges use the following command:
...
. View a list of the managed cluster sets that you can access. Administrators and users without cluster administrator privileges use the following command:
