...
+
**Note**: If you have any user defined private channels, you can include the channel secrets in this credentials backup if you set the `cluster.open-cluster-management.io/type` label selector to this secret. Without this, channel secrets are not retrieved by the `acm-credentials-schedule` backup and needs to be manually recreated on the restored cluster.
