...
Complete the following steps to create an instance of the DataProtectionApplication (DPA) CR.

. In the {:ocp:}, click `Operators`.
...
The CR values are mentioned for ease of use. Remember to mention `default: true` in backupStorageLocations if you intend on using the default backup storage location as shown below.
