...
//In a disaster scenario, the cluster where the backups are created becomes unavailable and data backed up needs to be moved to a new hub cluster. This is done by running the cluster restore operation on the hub where the backed up data needs to be moved to. In this case, the restore operation is executed on a different hub than the one where the backup was created.
