...
//still need to review the paragraphs for the new section 
...
For server-side encryption using AWS provider, you can use serverSideEncryption and kmsKeyId configurations as explained in link:https://github.com/vmware-tanzu/velero-plugin-for-aws/blob/main/backupstoragelocation.md[this sample AWS BackupStorageLocation]. The following sample specifies an AWS KMS key ID when setting up the DataProtectionApplication resource:
