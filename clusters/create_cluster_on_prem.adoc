...
A single-node OpenShift (SNO) cluster contains a single node which hosts the control plane services and the user workloads. This configuration can be helpful when you want to minimize the resource footprint of the cluster. If you want your cluster to be a SNO cluster, select the SNO option. SNO clusters are only supported on {ocp-short} version 4.11, and later. If you did not select a release image of {ocp-short} version 4.11, or later, the option for selecting a SNO cluster is not available. 
