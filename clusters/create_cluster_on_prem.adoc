...
*Note:* The single-node OpenShift control plane requires 4 additional CPU cores than a control plane node requires for a multinode control plane cluster. For example, the single-node OpenShift cluster requires 8 cores while a multinode cluster requires 4 cores.  
