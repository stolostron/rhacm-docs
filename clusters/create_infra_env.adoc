...
----
+
Replace `db_volume_size` with the volume size for the `databaseStorage` field, for example `10Gi`. This value is based on ???? and must be at least. 
+
Replace `fs_volume_size` with the volume size for the volume size for the `filesystemStorage` field, for example `20Gi`. This value is based on ??? and must be at least...
+
