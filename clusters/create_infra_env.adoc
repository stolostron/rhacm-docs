...
. If your {product-title-short} hub cluster is running on bare metal, Red Hat OpenStack, or VMware vSphere, modify the `Provisioning` resource to allow the Bare Metal Operator to watch all namespaces by running the following command:
+
----
