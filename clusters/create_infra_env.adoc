...
** <<enable-cim,Enable Central Infrastructure Management on Amazon Web Services>>
...
. Create a new `IngressController` with a unique domain using the `NLB` `type` parameter:
+
[source,yaml]
----
...
. In the `assisted-image-service` route, replace `spec` with `hosts` and replace `apps` with `nlb-apps`.
