...
+
Replace `img_volume_size` with the size of the volume for the `imageStorage` field, for example `2GB`. This value specifies how much storage is allocated for storing images for the clusters. You might need to use a higher value if there are many clusters.
+
