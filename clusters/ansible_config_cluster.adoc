...
== Ansible integration 
//***Do we need this section for cluster?***

You can integrate Ansible Tower jobs so they can be run before or after a cluster is installed.

***Do we need this for cluster?*** The application subscription operator is enhanced to define two subfolders: `prehook` and `posthook`. Both folders are in the Git repository resource root path and contain all prehook and posthook Ansible jobs, respectively.

***Do we need this for cluster?*** When the Git subscription is created, all of the pre and post AnsibleJob resources are parsed and stored in memory as an object. The application subscription controller decides when to create the pre and post AnsibleJob instances.
...
//***Does the content after this apply to cluster? Or any of it?***
