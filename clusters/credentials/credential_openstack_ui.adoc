...
[role="_abstract"]
You need a credential to use {mce-short} console to deploy and manage a supported {ocp} cluster on Red Hat OpenStack Platform. You must create a credential for Red Hat OpenStack Platform before you can create a cluster with {mce-short}. You can also create a credential by using the API. See xref:../credentials/credential_openstack_api.adoc#creating-a-credential-for-openstack-api[Creating a credential for Red Hat OpenStack Platform with the API] for those steps.

.Prerequisites

* A deployed hub cluster on a supported {ocp-short} version.
* Internet access for your hub cluster so that it can create the Kubernetes cluster on Red Hat OpenStack Platform.
