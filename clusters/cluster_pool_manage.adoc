...
. Optional: Select a cluster set name if you want the RBAC roles for your cluster pool to share the role assignments of an existing cluster set. The cluster set could only be set when you create the cluster pool and you could not change it after you create the cluster pool. All clusters claimed from this clusterpool will be added to this clusterset automatically.
+
