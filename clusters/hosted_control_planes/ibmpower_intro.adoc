...
* {mce} 2.4 and later installed on an {ocp-short} cluster. The {mce-short} is automatically installed when you install {acm-short}. You can also install {mce-short} without {acm-short} as an Operator from the {ocp-short} OperatorHub.
