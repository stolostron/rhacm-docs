...
- Each hosted cluster must have a unique name cluster-wide and cannot be the same as any existing managed cluster in order for {mce-short} to manage it.

- Do not name a hosted cluster `clusters`.
