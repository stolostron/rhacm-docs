...
The default `ControllerAvailabilityPolicy` option is `HighlyAvailable`. If you use that default option, schedule pods for each deployment within a hosted cluster across different failure domains by setting `topology.kubernetes.io/zone` as the topology key.
