...
. Set up https://docs.openshift.com/container-platform/4.13/networking/metallb/about-metallb.html[MetalLB] so that when you create a service of the LoadBalancer type, MetalLB adds an external IP address for the service.
+
----
...
.. Reach the {ocp-short} console by entering the following command:
+
----
...
. If you check the nodes by entering the following command, you can see a new node in the cluster. In this example, `ocp-worker-0` is added to the cluster.
+
----
oc --kubeconfig ${HOSTED_CLUSTER_NAME}.kubeconfig get nodes
----
