...
. Expose the Ingress controller to the L2 network by creating a .yaml file named `l2advertisement`:
+
[source,yaml]
----
apiVersion: metallb.io/v1beta1
...
. Enter the following command to apply the changes to the `l2advertisement.yaml` file:
+
----
oc --kubeconfig ${HOSTED_CLUSTER_NAME}.kubeconfig apply -f l2advertisement.yaml
----
+
