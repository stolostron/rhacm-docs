...
== Creating a hosted cluster on bare metal with CLI
...
. In the `<hosted-cluster-name>.kubeconfig` file, make the following changes so that the Ingress controller is exposed to the L2 network:
+
[source,yaml]
----
apiVersion: metallb.io/v1beta1
...
. Enter the following command to apply the changes to your `<hosted-cluster-name>.kubeconfig` file:
+
----
oc --kubeconfig <hosted-cluster-name>.kubeconfig apply
----
+
The {ocp-short} node starts to detect the IP from the host network, and the system becomes operational.
