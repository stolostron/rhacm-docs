...
+
After scaling the nodepool to 0 , the compute nodes will get detached from the hosted cluster. If the computed nodes are not detached from the hosted cluster or stuck in `Notready` state, delete the compute nodes manually by running the following command: 
...
Run the following command using the `hcp` command-line to destroy the hosted control plane:

+
[source,bash]
----
hcp destroy cluster agent --name ${hosted_cluster_name} <1> --namespace ${clusters_namepsace} <2>
----
<1> Specify the name of the hosted cluster.
<2> Specify the cluster namespace.