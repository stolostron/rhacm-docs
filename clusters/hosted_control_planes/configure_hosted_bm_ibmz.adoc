...
= Configuring the hosting cluster on IBM Z (Technology Preview)
...
- Each IBM Power and Z system host must be started with the PXE images that is provided by the CIM. After each host starts, it runs an Agent process to discover the details of the host and completes the installation. An Agent custom resource represents each host.
...
. xref:../hosted_control_planes/hosted_create_bare_metal.adoc#creating-a-hosted-cluster-bm[Create a hosted cluster]
