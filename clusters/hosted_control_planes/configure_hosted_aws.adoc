...
* <<hosted-install-cli,Installing the hosted control planes (hcp) command line interface>>
...
=== Deploying a cluster by using the hcp command line interface and external DNS
...
[#hosted-install-cli]
== Installing the hosted control planes (hcp) command line interface

The hosted control planes (`hcp`) command line interface is used to create and manage {ocp-short} hosted control plane clusters. 

[#hosted-install-cli-prereqs]
=== Prerequisites

* You must have enabled the hosted control planes feature. In the {ocp-short} console, the download for the `hcp` command line interface is visible only if you enabled the `hypershift-preview` feature.

[#hosted-install-hcp-cli]
=== Installing the hcp command line interface

You can install the hcp command line interface by completing the following steps:
