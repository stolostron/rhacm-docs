...
* <<hosted-install-cli,Installing the hosted control plane command line interface (hcp)>>
...
=== Deploying a cluster by using the hosted control plane command line interface and external DNS
...
. If the `autoImportDisabled` variable does not exist yet in the `AddonDeploymentConfig` resource, you can patch it by entering the following command:
...
For instructions to manually import a hosted cluster, see xref:../hosted_control_planes/importing_hosted_cluster_aws.adoc#importing-hosted-cluster-aws[Importing a hosted control plane cluster].

[#hosted-install-cli]
== Installing the hosted control plane command line interface (hcp)

The hosted control plane command line interface (`hcp`) is used to create and manage {ocp-short} hosted control plane clusters. 

[#hosted-install-cli-prereqs]
=== Prerequisites

* You must have enabled the `hypershift` feature. In the {ocp-short} console, the download for the hosted control plane command line interface (`hcp`)  is visible only if you enabled that feature.

[#hosted-install-hcp-cli]
=== Installing the hosted control plane command line interface (hcp)
