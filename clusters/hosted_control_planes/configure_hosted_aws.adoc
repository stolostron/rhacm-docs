...
Hosted clusters are automatically imported into {mce-short} after the control plane becomes available. All {product-title-short} add-ons are also enabled if {product-title-short} is installed.

However, if needed, you can disable the automatic import of hosted clusters. When automatic import is disabled, only newly created hosted clusters are not automatically imported. Hosted clusters that were already imported are not affected. You can still manually import clusters by using the UI or the `hcp` command line interface. For more information, about manual import, see _Importing a hosted control plane cluster on AWS_.
...
For instructions to manually import a hosted cluster, see xref:../hosted_control_planes/importing_hosted_cluster_aws.adoc#importing-hosted-cluster-aws[Importing a hosted control plane cluster on AWS].
