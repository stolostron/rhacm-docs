...
. On the hub cluster, open the `hypershift-addon-deploy-config` specification that is in the `AddonDeploymentConfig` resource in the `hypershift` namespace by entering the following command:
...
. To re-enable automatic import, remove the `autoImportDisabled` variable from the `AddonDeploymentConfig` resource.
