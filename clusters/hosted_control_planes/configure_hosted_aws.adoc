...
For {product-title-short} 2.7 users, you can use the managed hub cluster, also known as the `local-cluster`, as the hosting cluster. See the following topics to learn how to configure the `local-cluster` as the hosting cluster.
...
If you plan to create and manage hosted clusters on AWS, complete the following steps to create a secret:
...
. Run the following command to verify that the `hypershift-preview` and `hypershift-local-hosting` features are enabled in the `MultiClusterEngine` custom resource, as the YAML example demonstrates.
...
== Enabling the hypershift-addon managed cluster add-on for local-cluster
...
. Apply the file by running the following command. Replace `filename` with the name of the file that you created:
+
----
oc apply -f <filename>
----
