...
* Ingress for workloads within the hosted cluster, such as `*.apps.service-consumer-domain.com`
* Ingress for service endpoints within the management cluster, such as API or OAUTH endpoints through `*.service-provider-domain.com`

The input for the `hostedCluster.spec.dns` dictates the Ingress for workloads within the hosted cluster. The input for `hostedCluster.spec.services.routePublishingStrategy.hostname` dictates the Ingress for service endpoints within the management cluster.

External DNS is useful for setups with public endpoints, such as `Public` or `PublicandPrivate`. For a private setup, all endpoints must be accessible through `.hypershift.local`, which contains the CNAME records to the private link endpoint services.
...
You can create the public DNS hosted zone in the AWS Route 53 management console:

. In the Route 53 management console, click *Create hosted zone*.
// LH: Need to verify this step to ensure that it matches the UI
...
When the control plane operator creates the `Services` and `Routes`, it annotates them with the `external-dns.alpha.kubernetes.io/hostname` annotation. The value is the `hostname` field in the `servicePublishingStrategy` for that type. The control plane operator uses that name for the service endpoints, and it expects that if the hostname is set,  
The CPO uses this name blindly for the service endpoints and assumes that if the hostname is set, there is some mechanism external-dns or otherwise, that will create the DNS records.
//LH: add sentence here about assumptions
...
To deploy the `hypershift` and `external-dns` Operators when the external public hosted zone already exists, take the following steps:

. Ensure that the public hosted zone exists. In the following example, the zone is `service-provider-domain.com`.
...
. Ensure that the `external-dns` Operator is running and that the internal flags point to the public hosted zone by entering the following commands:
+
----
export KUBECONFIG=<path_to_management_cluster_kubeconfig>
export AWS_CREDS=~/.aws/credentials
