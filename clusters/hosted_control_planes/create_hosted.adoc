...
[#create-hosted]
== Creating a hosted control plane cluster with the console on BareMetal
...
. Create your `KlusterletAddonConfig` resource by using the following sample YAML file. This is applicable only in {product-title-short}. If you have installed {mce} only, this is not applicable.
+
[source,yaml]
----
