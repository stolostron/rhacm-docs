...
= Configuring image mirroring for a dual stack network (Technology Preview)
...
* If you are working in a virtual environment, after you configure mirroring, ensure that you meet the xref:../hosted_control_planes/managing_hosted_kubevirt.adoc#create-hosted-clusters-prereqs-kubevirt[prerequisites for hosted control planes on OpenShift Virtualization].
