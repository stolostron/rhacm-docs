...
For example, imagine that your {ocp-short} cluster has a default Ingress DNS entry of REDACTED1. The default Ingress of a KubeVirt guest cluster that is named `guest` and that runs on that underlying {ocp-short} cluster is REDACTED2.
