...
For example, imagine that your {ocp-short} cluster has the following default Ingress DNS entry:
----
*.apps.mgmt-cluster.example.com
----

A KubeVirt guest cluster that is named `guest` and that runs on that underlying {ocp-short} cluster would have the following default Ingress:
