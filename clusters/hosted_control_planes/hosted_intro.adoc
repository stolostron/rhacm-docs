...
Hosted control planes for {ocp-short} are available on bare metal and {ocp-virt}, and as a *Technology Preview* feature on Amazon Web Services (AWS). You can host the control planes on {ocp-short} version 4.14 only. The hosted control planes feature is enabled by default.
