...
Hosted control planes for {ocp-short} are available on bare metal and {ocp-virt}, and as a Technology Preview feature on Amazon Web Services (AWS). You can host the control planes on {ocp-short} version 4.14.

The following table indicates which {ocp-short} versions are supported for each platform:

|===
| Platform | HyperShift Operator versions | Minimum hosted {ocp-short} version
| Bare metal | 4.11, 4.12, 4.13, 4.14 | 4.14
| {ocp-virt} | 4.14 | 4.14
| AWS (Technology Preview) | 4.11, 4.12, 4.13, 4.14 | 4.14
|===
