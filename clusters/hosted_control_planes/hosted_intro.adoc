...
Hosted control planes for {ocp-short} are available on Amazon Web Services (AWS), bare metal, and {ocp-virt}. You can host the control planes on {ocp-short} version 4.13.
...
* xref:../hosted_control_planes/configure_hosted_aws.adoc#hosting-service-cluster-configure-aws[Configuring the hosting cluster on AWS]
