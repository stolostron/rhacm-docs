...
You can deploy hosted control planes by configuring an existing cluster to function as a hosting cluster. The hosting cluster is the {ocp} cluster where the control planes are hosted. The hosting cluster can be the hub cluster (also known as the `local-cluster`) or one of the {ocp-short} managed clusters. See the following topics to learn how to configure the `local-cluster` as the hosting cluster.
...
* The hosted control planes command line interface. See <<Installing the hosted control planes CLI,hosted-install-cli>> for more information.
...
The hosted control planes CLI is an `oc` command line plugin that allows you to create and manage {ocp-short} hosted control plane clusters. After <<hosted-enable-feature,Enabling the hosted control planes feature>>, you can install the hosted control planes CLI by completing the following steps:
...
You can now start using the `hypershift create cluster`command to create and manage hosted clusters. You can use the following command to list the available parameters:
