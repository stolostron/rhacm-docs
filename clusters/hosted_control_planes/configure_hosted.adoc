...
The hosted control planes CLI is an `oc` command line plugin that allows you to create and manage {ocp-short} hosted control plane clusters. After <<Enabling the hosted control planes feature,hosted-enable-feature>>, you can install the hosted control planes CLI by completing the following steps:
