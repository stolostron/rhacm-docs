...
. Create an OIDC S3 https://github.com/stolostron/rhacm-docs/pull/4628 secret named `hypershift-operator-oidc-provider-s3-credentials` for the HyperShift operator.
...
* You need to enable the Central Infrastructure Management service. The Central Infrastructure Management service is included as part of {product-title}. To enable the service see xref:../cluster_lifecycle/create_infra_env.adoc#enble-cim[Enabling the Central Infrastructure Management service] for the procedure. 
