...
A _hosted cluster_ is an {ocp-short} cluster with its control plane and API endpoint hosted on a management cluster. The hosted cluster includes the control plane and its corresponding data plane.
...
+
//lahinson - sept 2023 - In the upstream docs, this step includes a warning about additional steps needed before a day-1 patch. Should that warning be included in the official docs?
...
//lahinson - sept 2023 - is the latter sentence necessary? I'm not sure if that's something most users need to know.
...
spec:
  ...
  platform:
    type: Agent
  release:
    image: registry.hypershiftbm.lab:5000/openshift/release-images:4.14.0-0.nightly-2023-08-29-102237 <1>
  replicas: 0
status:
  replicas: 0 <4>
----
...
+
//lahinson - sept 2023 - in the upstream docs, this section contains a warning about a day-1 patch. Do we need to include that warning in the official docs?

+
<1> All of the nodes included in this `NodePool` are based on the following {ocp-short} version: `4.14.0-0.nightly-2023-08-29-102237`.
...
If you are working with virtual machines, you can complete the following steps to create empty worker nodes that the Metal3 Operator will consume. To do so, you use the `kcli` tool.
...
+
//lahinson - sept 2023 - is "sleep 2" a command that the user will enter?
