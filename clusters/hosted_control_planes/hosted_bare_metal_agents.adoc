...
= Adding agents to hosted control planes on bare metal (Technology Preview)
...
. Download the live ISO and use it to start a node (bare metal or VM). At startup, the node communicates with the Assisted Service and registers as an agent in the same namespace as the `InfraEnv` resource.
...
//lahinson - June 2023 - We need to better clarify what the following command does
+
----
...
----
+
// lahinson - June 2023 - We need to better clarify what the following commands do
----
...
+
//lahinson - June 2023 - Would it be clearer to add "by entering the following command:" to the end of line 44?
----
...
// lahinson - June 2023 - We need to better clarify this step. It says to "Wait", but then it provides a command. Is the user supposed to enter the command, or is the following text output?
+
----
...
. Create your BaremetalHost objects by entering the following commands. You need to configure a few variables that are required to start your bare-metal nodes.
+
----
