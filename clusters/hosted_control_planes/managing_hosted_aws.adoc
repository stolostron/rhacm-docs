...
To enable private hosted clusters, you must meet the following prerequisites:

* You completed the xref:[Getting started prerequisites].
* You created the private cluster IAM policy document by taking one of the following steps:
** If you use shell, enter the following commands:
+
----
cat << EOF >> policy.json
{
...
----
** If you use JSON, enter the following commands:
+
----
{
  "Version": "2012-10-17",
...
----
* You created the IAM policy in AWS by entering the following command:
+
----
aws iam create-policy --policy-name=hypershift-operator-policy --policy-document=file://policy.json
----
* You created a `hypershift-operator` IAM user by entering the following command:
+
----
...
=== Creating a private HostedCluster


[#access-aws-private-hosted-cluster]
=== Accessing a private HostedCluster
