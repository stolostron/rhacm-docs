...
* To access the hosted cluster by getting the `kubeconfig` file and credentials directly from resources, you need to be familiar with the access secrets for hosted control plane clusters. The secrets are stored in the `hypershift-management-cluster` namespace. Learn about the following secret name formats:
...
//lahinson - June 2023 - Does the user need to take any steps with the secrets to access the hosted cluster? The latter section seems incomplete.
...
//lahinson - June 2023 - Can we point the user to where they can find out about adding nodes to the cluster, or will that be obvious?
