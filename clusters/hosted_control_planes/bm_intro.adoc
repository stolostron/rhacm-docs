...
** The `Konnectivity` agent establishes a reverse tunnel to allow bi-directional communication on the hosted cluster. The agent establishes an egress connection to the `Konnectivity` server, which is exposed by using either a route on port 443 or a manually assigned `NodePort`.
