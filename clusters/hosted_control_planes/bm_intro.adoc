...
The {mce-short} 2.4 supports only the default `local-cluster`, which is a hub cluster that is managed, and the hub cluster as the hosting cluster. On {acm-short} 2.9, you can use the managed hub cluster, also known as the `local-cluster`, as the hosting cluster.
