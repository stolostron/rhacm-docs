...
The {mce-short} 2.5 supports only the default `local-cluster`, which is a hub cluster that is managed, and the hub cluster as the hosting cluster. On {acm-short} 2.10, you can use the managed hub cluster, also known as the `local-cluster`, as the hosting cluster.
...
* You need the {mce} 2.2 and later installed on an {ocp-short} cluster. The {mce-short} is automatically installed when you install {acm-short}. You can also install {mce-short} without {acm-short} as an Operator from the {ocp-short} OperatorHub.
...
* For a hub cluster that has a proxy configuration to reach the hosted cluster API endpoint, add all hosted cluster API endpoints to the `noProxy` field on the `Proxy` object. For more information, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/networking/enable-cluster-wide-proxy[Configuring the cluster-wide proxy].
