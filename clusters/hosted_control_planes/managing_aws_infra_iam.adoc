...
----
hcp create cluster aws \ <1>
    --infra-id INFRA_ID \ <2>
    --name CLUSTER_NAME \ <3>
    --aws-creds AWS_CREDENTIALS \ <4>
    --iam-json OUTPUT_IAM_FILE \ <5>
    --pull-secret PULL_SECRET_FILE \ <6>
    --generate-ssh \ <7>
...
<5> Replace `PULL_SECRET_FILE` with the name of the file that contains a valid {ocp-short} pull secret.
<6> The `--generate-ssh` flag is optional, but is good to include in case you need to SSH to your workers. An SSH key is generated for you and is stored as a secret in the same namespace as the hosted cluster.
