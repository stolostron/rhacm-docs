...
. View the populated table with the following information:
    ...
    - _Cluster types_ is the platform type of the cluster, such as {ocp-short} or {rosa}, for example.
    - _Infrastructure providers_ filters by where the cluster is running, such as Amazon Web Services (AWS) or Microsoft Azure, for example.
