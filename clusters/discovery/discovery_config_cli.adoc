...
* Create a credential to connect to {ocm}.

[#setup-discovery]
== Discovery set up and process

*Note:* The `DiscoveryConfig` resource must be named `discovery` and must be created in the same namespace as the selected `credential`. See the following `DiscoveryConfig` sample:
...
. Enter the versions of {ocp-short} clusters to discover as a list of strings. *Note:* Every entry in the `openshiftVersions` list specifies the major and minor version. For example, specifying `4.x` includes all errata releases, such as `4.x.1`, `4.x.2`.

[#view-discovered-cluster]
== View discovered clusters 

View discovered clusters by running `oc get discoveredclusters -n <namespace>` where `namespace` is the namespace where the discovery credential exists. You can use the same command to view the region if you have a region set.
