...
. Enter the versions of {ocp-short} clusters to discover as a list of strings. *Note:* Every entry in the `openshiftVersions` list specifies the {ocp-short} major and minor version. For example, specifying `4.x` includes all Errata releases, such as `4.x.1`, `4.x.2`.
