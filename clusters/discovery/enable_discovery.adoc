...
* Import from the console. Automatically import the `DiscoveredCluster` resource, you must modify the resource and set the `importAsManagedCluster` field to `true` in the console.
...
* Import from the CLI. Automatically import the `DiscoveredCluster` resource from the command line interface. Set the `importAsManagedCluster` paramater to `true` by using the following command after you log in. Replace `<name>` and `<namespace>` with your name and namespace:
