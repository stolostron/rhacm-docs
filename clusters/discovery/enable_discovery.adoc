...
. To automatically import the `DiscoveredCluster` resource from the command-line interface. Set the `importAsManagedCluster` parameter to `true` by using the following command after you log in. Replace `<name>` and `<namespace>` with your name and namespace:
