...
- You cannot install {mce-short} on a cluster that has a `ManagedCluster` resource configured in an external cluster. You must remove the `ManagedCluster` from the external cluster before you can install {mce-short}.
