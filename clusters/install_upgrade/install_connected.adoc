...
- You cannot install {mce-short} on a cluster that has a `ManagedCluster` resource. You must remove the `ManagedCluster` if it exists on the cluster before you can install {mce-short}.
