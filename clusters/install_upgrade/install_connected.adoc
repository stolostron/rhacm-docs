...
- You cannot install {mce-short} on a cluster that has a `ManagedCluster` resource. You must remove the `ManagedCluster` if it exists on the cluster before you install {mce-short}.
...
Before you install {mce-short}, see the following requirements:
...
* Your cluster must not be managed as a `local-cluster`.
