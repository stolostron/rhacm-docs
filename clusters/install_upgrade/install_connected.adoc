...
The {mce-short} is installed with {olm}, which manages the installation, upgrade, and removal of the components that encompass the {mce-short}.
...
* Your cluster does _not_ have a `ManagedCluster` resource configured in an external cluster.

* You must have a supported version of {ocp-short} deployed in your environment, and you must log in from the {ocp-short} command-line interface. For help with installing {ocp-short}, see the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/installation_overview/index[Installation overview] in the {ocp-short} documentation.

* Your {ocp-short} command-line interface must be configured to run `oc` commands. For help with the command-line interface, see link:https://docs.redhat.com/documentation/en-us/openshift_container_platform/4.16/html/cli_tools/openshift-cli-oc#cli-getting-started[Getting started with the CLI] for information.
...
* If you plan to create managed clusters by using the {ai} that is provided with {ocp}, see link:https://docs.redhat.com/en/documentation/assisted_installer_for_openshift_container_platform/2024/html/installing_openshift_container_platform_with_the_assisted_installer/prerequisites[Preparing to install with the {ai}] topic in the {ocp-short} documentation for the requirements. 
...
You can proceed to install {mce-short}.

[#installing-from-the-operatorhub-mce]
== Installing from the OperatorHub web console interface

From the _Administrator_ view in your {ocp-short} navigation, install the OperatorHub web console interface that is provided with {ocp-short}. Complete the following steps: 
...
+
After the `MultiClusterEngine` resource is created, the status for the resource is `Available` on the _MultiCluster Engine_ tab.
...
{mce-short} namespace. The value for `namespace` might be referred to as _Project_ in the {ocp-short} environment:
...
{mce-short} namespace that you created in step 1.
