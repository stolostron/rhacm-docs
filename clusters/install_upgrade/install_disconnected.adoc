...
* A supported {ocp-short} version must be deployed in your environment, and you must be logged in with the command-line interface. For managing bare metal clusters, you need a supported {ocp-short} version. +
See link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/installation_overview/index[{ocp-short} Installing].
...
+
*Important:* For bare metal only, you need to provide the certificate information for the disconnected registry in your `install-config.yaml` file. To access the image in a protected disconnected registry, you must provide the certificate information so the {mce-short} can access the registry.
