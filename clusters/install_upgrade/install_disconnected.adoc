...
*Important:* Install {mce-short} on a cluster that does not have {acm-short} installed because the two products provide some of the same management components. Install {mce-short} on a cluster that has never had {acm-short} previously installed. If you are using {acm-short} version 2.5.0 or later, {mce-short} is already installed on your cluster.
...
* {ocp} version 4.13 or later must be deployed in your environment, and you must be logged in with the command line interface (CLI). 
...
+
*Note:* For managing bare metal clusters, you must have {ocp-short} version 4.13 or later.
+
See the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.13/html/installation_overview/index/[Installation overview] in the {ocp-short} documentation.

* Your {ocp} CLI must be version 4.13 or later, and configured to run `oc` commands.
...
* You must have a supported {ocp-short} version, including the registry and storage services, installed and working in your cluster. For information about {ocp-short} version 4.13, see link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.13/[{ocp-short} documentation].
