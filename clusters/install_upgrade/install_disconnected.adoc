...
If you deploy a hub cluster on bare metal by using `assisted-service`, the {ocp} image is not mirrored automatically in disconnected environments. A matching ironic agent image of {ocp-short} must be used on your hub cluster to install managed clusters. The hub cluster {ocp-short} version detects which ironic agent image is being used.
...
You can set a different default ironic agent image by overriding the following environment variables:

- x86_64: `IRONIC_AGENT_IMAGE`
- arm64: `IRONIC_AGENT_IMAGE_ARM`

Alternatively, you can also use the `infraenv.agent-install.openshift.io/ironic-agent-image-override` InfraEnv annotation to override the ironic agent image.
