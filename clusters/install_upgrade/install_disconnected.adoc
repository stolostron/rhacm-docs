...
If you deploy a hub cluster on bare metal by using `assisted-service`, the {ocp} image is not mirrored automatically in disconnected environments. You must use a matching {ocp-short} ironic agent image to install managed clusters. The hub cluster {ocp-short} version determines which ironic agent image to use.

In a disconnected environment, you must mirror the {ocp-short} image manually. The ironic agent image used is based on the hub cluster release version set in the `ClusterVersion` resource. If the image version set in the resource is not available, one of the following default images is used, depending on your CPU architecture:
...
You can set a different default ironic agent image by overriding the following environment variables. See to link:https://github.com/openshift/assisted-service/blob/ceb709686799699f088892fa72994a5a60aa2c02/docs/operator.md#specifying-environmental-variables-via-configmap[Specifying Environmental Variables via ConfigMap] to learn how to override the environment variables:

- x86_64: `IRONIC_AGENT_IMAGE`
- arm64: `IRONIC_AGENT_IMAGE_ARM`

Alternatively, you can use the `infraenv.agent-install.openshift.io/ironic-agent-image-override` `InfraEnv` annotation to override the ironic agent image.

Make sure that the ironic agent image version matches the hub cluster {ocp-short} version by running the following command. Replace `<hub-release-image>` with the image for the hub cluster release:

----
oc adm release info --image-for=ironic-agent <hub-release-image>
----

If the image versions do not match, run the following command to mirror the {ocp-short} image manually:

----
skopeo copy <image-from-oc-adm-output> <mirror>
----

Replace `<image-from-oc-adm-output>` with the image from the output of `oc adm`.
