...
[#select-ironic-agent-image]
== Selecting an ironic agent image

When you deploy a hub cluster on bare metal by using the Assisted Service, the hub cluster runs an ironic agent image. The image version is selected based on the hub cluster {ocp} version.

If the {ocp-short} version does not match the version of the `InfraEnv` environment, or if the release image is not available, the default ironic agent image is used. 

See the following list of default images for the x86_64 and arm64 architecture:
...
You can change the default ironic agent image by replacing one of the following values:

- x86_64: `IRONIC_AGENT_IMAGE`
- arm64: `IRONIC_AGENT_IMAGE_ARM`

You can also use the `infraenv.agent-install.openshift.io/ironic-agent-image-override` `InfraEnv` annotation to replace the default ironic agent image.

*Important:* Make sure that you mirror the correct ironic agent images. The ironic agent image from the hub cluster {ocp-short} version must be accessible in the hub cluster installation environment. Run the following command to see the version:
