...
| image-based-install-operator-preview | Provides site configuration to {sno} clusters to complete installation | False
| local-cluster | Enables the import and self-management of the local hub cluster where the {mce-short} is deployed | True
| managedserviceacccount | Synchronizes service accounts to managed clusters then collects tokens as secret resources to give back to the hub cluster | True
...
To define a set of node selectors for the {acm-short} hub cluster, see link:../../install/adv_config_install.adoc#node-selector[nodeSelector] in the {acm-short} documentation.
