...
The `local-cluster` cannot be renamed if it is `enabled`. With a standalone `MultiClusterEngine` resource, you must disable `local-cluster`, even though `spec.localClusterName` does exist in the {mce-short}.
