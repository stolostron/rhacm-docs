...
By default, the cluster that is running {mce-short} is a local cluster, which is a hub cluster that manages itself and is designated as the `local-cluster` in the list of clusters.

In the `MultiClusterEngine` resource, the `name` value identifies the hub cluster as a `local-cluster`. 
...
You can change the `local-cluster` name if the `enabled` field is set to `false`. When you configure the `local-cluster` name, you must use 34 or fewer characters.

The `local-cluster` cannot be renamed if it is set as `enabled: true`. With a standalone `MultiClusterEngine` resource, you must disable `local-cluster`, even though the `spec.localClusterName` field does exist in the {mce-short}.
...
See the following YAML sample, where the `local-cluster` is disabled with the `enabled: false` setting:
