...
The {mce-short} is installed using an operator that deploys all of the required components.
//this line?
 The {mce-short} can be further configured during or after installation. Learn more about the advanced configuration options.
...
| cluster-api-preview | Provides capabilities to handle the Cluster API lifecycle from within a management cluster | False
| cluster-api-provider-aws-preview | Provides Kubernetes-style APIs for cluster creation, configuration, and management for AWS | False
...
By default, the cluster that is running {mce-short} manages itself. A hub cluster that is managed by itself is designated as the `local-cluster` in the list of clusters. 
...
The `name` value identifies the hub cluster as a `local-cluster`. If you changed the default name from `local-cluster` to another value, you need to use that same name in the `MultiClusterEngine` resource. The `MultiClusterEngine` `local-cluster` name cannot differ from the {acm-short} name.

When you configure or change the `local-cluster` value, the name must be 34 or fewer characters.
//check this spot before adding to MCH

The local-cluster cannot be renamed if local-cluster is enabled. You must set `disableHubSelfManagement` to true to make this change after installation.

To rename with a a standalone {mce-short} resource, you must disable `local-cluster`, even though s`pec.localClusterName` does exist in the MCE CR, if it's managed by ACM, that change must be made in the MCH CR.
//confused--waiting for meeting

To install {mce-short} without the cluster managing itself, specify the values in the `spec.overrides.components` settings in the `MultiClusterEngine` resource. 

See the following YAML sample, where `<your-local-cluster-name>` is the name of your `local-cluster`:
