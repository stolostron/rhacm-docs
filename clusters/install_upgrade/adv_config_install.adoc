...
By default, the cluster that is running {mce-short}, identified as the `local-cluster`, manages itself. To install {mce-short} without the `local-cluster` managing itself, specify the following values in the `spec.overrides.components` settings in the `MultiClusterEngine` section:
