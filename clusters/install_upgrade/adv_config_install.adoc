...
The `name` value identifies the hub cluster as a `local-cluster`. If you changed the default name from `local-cluster` to another value, you need to use that same name in the `MultiClusterEngine` resource.
