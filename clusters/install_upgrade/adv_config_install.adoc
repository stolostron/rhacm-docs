...
A _managed_ hub cluster is named a `local-cluster`. For a hub cluster to manage itself, the settings in the  `spec.overrides.components` must be set as shown in the following example:  
...
* The `enabled` setting specifies whether the feature is enabled or disabled. When the value is `true`, the hub cluster manages itself. When the value is `false`, the hub cluster does not manage itself. The default value is `true`.`
...
You must import the existing hub cluster as a `local-cluster` by completing the following steps:
