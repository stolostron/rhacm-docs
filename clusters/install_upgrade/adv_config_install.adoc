...
With a standalone `MultiClusterEngine` resource, you must disable `local-cluster` to rename, even though the `spec.localClusterName` field does exist in the {mce-short}.

See the following example where the local cluster feature is disabled by the `enabled: false` setting:
