...
The local-cluster cannot be renamed if local-cluster is enabled. You must set `disableHubSelfManagement` to true to make this change after installation. (MCH)

To rename with a standalone `MultiClusterEngine` resource, you must disable `local-cluster`, even though `spec.localClusterName` does exist in the {mce-short}.

if it's managed by ACM, that change must be made in the MCH CR. (move to MCH)
