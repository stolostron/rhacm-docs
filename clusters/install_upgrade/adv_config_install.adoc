...
By default, the cluster that is running {mce-short} manages itself. To install {mce-short} without cluster managing itself, specify the following values in the `spec.overrides.components` settings in the `MultiClusterEngine` section:
