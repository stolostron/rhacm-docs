...
You can change the `local-cluster` name if the `enabled` field is set to `false`. You must use 34 or fewer characters for the `<your-local-cluster-name>` value. 
...
See the following example where the local cluster feature is disabled:
