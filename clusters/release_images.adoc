...
For example, you might need to create an `x86_64` cluster from a hub cluster that is running on the `ppc64le`, `arm64`, or `s390x` architecture. If you create the release image with both sets of files, the cluster creation succeeds because the new release image enables the {ocp-short} release registry to provide a multi-architecture image manifest. 

To create the release image, complete steps similar to the following example for your architecture type:

. From the https://quay.io/repository/openshift-release-dev/ocp-release[{ocp-short} release registry], create a https://docs.docker.com/registry/spec/manifest-v2-2/[manifest list] that includes `x86_64`, `s390x`, `arm64`, and `ppc64le` release images.
...
podman pull quay.io/openshift-release-dev/ocp-release:4.10.1-arm64
----
...
podman push quay.io/openshift-release-dev/ocp-release:4.10.1-arm64 <private-repo>/ocp-release:4.10.1-arm64
----
+
Replace `private-repo` with the path to your repository.
...
podman manifest add mymanifest <private-repo>/ocp-release:4.10.1-arm64
----
+
Replace `private-repo` with the path to your repository.
