...
You can select which `ManagedClusters` objects to filter by using `labelSelector`, `claimSelector`, or `celSelector` fields. See the following examples to learn how to use the filters:
...
- The following example shows five different ways of how you can use the `celSelector` field with Common Expression Language. You can use one or several expressions to filter your `ManagedClusters` objects:
...
<1> Select clusters by Kubernetes version listed in the `managedCluster.Status.version.kubernetes` field.
<2> Select clusters by informtion stored in the `clusterClaims` field.
...
<5> Use Open Cluster Management customized function scores to select clusters by the `AddonPlacementScore` field.
