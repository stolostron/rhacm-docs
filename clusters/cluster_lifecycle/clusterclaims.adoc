...
`ClusterClaims` that you manage:: Without the `open-cluster-management.io` or `openshift.io` suffix. By default, up to 20 `ClusterClaims` are synchronized to the `ManagedCluster` status on the hub cluster.
