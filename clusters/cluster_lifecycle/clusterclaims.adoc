...
After the managed cluster joins the hub, the ClusterClaims that are created on a managed cluster are synchronized with the status of the `ManagedCluster` resource on the hub. A managed cluster with ClusterClaims might look similar to the following example, replacing `4.x` with a supported version:
...
The length of `spec.value` field must be 1024 or lower. The `create` permission on resource `clusterclaims.cluster.open-cluster-management.io` is required to create a `ClusterClaim` resource.
