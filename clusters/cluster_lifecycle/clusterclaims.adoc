...
After the managed cluster joins the hub, any `ClusterClaim` that is created on a managed cluster is synchronized with the status of the `ManagedCluster` resource on the hub cluster. See the following example of `clusterClaims` for a `ManagedCluster`:
