...
After the managed cluster joins the hub cluster, the following two types of `ClusterClaim` created on the managedcluster are synchronized with the status of the `ManagedCluster` resource on the hub cluster. `ClusterClaims` with label `open-cluster-management.io/spoke-only` label are not synchronized:
