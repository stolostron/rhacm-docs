...
`ClusterClaim` resources that {mce-short} manages:: Includes `id.k8s.io`, or labels with the `open-cluster-management.io` or `openshift.io` suffix. These `ClusterClaim` resrouces are synchronized to the `ManagedCluster` status on the hub cluster.
