...
*Important:* The `cluster-proxy-addon` does not work when another proxy configuration is on your hub cluster or managed clusters. The following scenarios cause include:
//are these the only 2 scenarios? 
//with this list, it seems more like a known issue 
    * There is a cluster-wide-proxy configuration for your hub cluster or managed cluster.

    * The agent supports proxy configuration in `AddOnDeploymentConfig`.
