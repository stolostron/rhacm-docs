...
You can change the value of the hub cluster to not import automatically by changing the `disableHubSelfManagement` value to `true`, as described in xref:../get_started_mce/install_connected.adoc#installing-while-connected-online-mce[Installing while connected online].
...
* The default setting of `disableHubSelfManagement` is `false`. The `false` setting causes the hub cluster, which is the `local-cluster`, to reimport and manage itself when it is detached and it reconciles the `MultiClusterHub` controller.
+
...
----
+
You can change the value of the hub cluster to not import automatically by changing the `disableHubSelfManagement` value to `true`. See xref:../get_started_mce/install_connected.adoc#installing-while-connected-online-mce[Installing while connected online].
...
. Defragment the `etcd` database and clear any `NOSPACE` alarms as outlined in link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/scalability_and_performance/recommended-performance-and-scalability-practices-2#etcd-defrag_recommended-etcd-practices[Defragmenting `etcd` data].
