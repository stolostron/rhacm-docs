...
You can detach or destroy multiple clusters by selecting the check boxes of the clusters that you want to detach or destroy and selecting *Detach* or *Destroy*.

*Important:* If you attempt to detach the managed hub cluster, the `local-cluster`, check to see if the default setting of `disableHubSelfManagement` is `false`. This setting causes the hub cluster to reimport itself and manage itself when it is detached, and it reconciles the `MultiClusterHub` controller. It might take hours for the hub cluster to complete the detachment process and reimport. 
...
Having many managed clusters can affect the size of the `etcd` database in the hub cluster. When you delete a managed cluster, the `etcd` database in the hub cluster is not automatically reduced in size. 

In some scenarios, the `etcd` database can run out of space and sends the following error: `etcdserver: mvcc: database space exceeded`. To correct this error, reduce the size of the `etcd` database by compacting the database history and defragmenting the `etcd` database.

The etcd Operator automatically defragments disks and compacts the `etcd` history. No manual intervention is needed. The following procedure is for {ocp-short} version 4.8 and earlier.
//This should not have been added and the forgotten about :-). If we are versioning a procedure (4.8) in our areas, we need to go back and check with development to remove this. This is legacy content that has been moved to a different writer who will need to follow up. 4.8 is long past EOL. Potentially this whole section is removed-bcs 5/31:
...
. Defragment the `etcd` database and clear any `NOSPACE` alarms as outlined in link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/scalability_and_performance/recommended-performance-and-scalability-practices-2#etcd-defrag_recommended-etcd-practices[Defragmenting `etcd` data].
