...
When you create a cluster on a provider by using {mce-short}, you must specify a release image to use for the new cluster. The release image specifies what version of {ocp} to use for building the cluster. By default, {ocp-short} uses the `clusterImageSets` resources to get the list of supported release images. See the following topics to learn more:
...
- Option 3: Follow the `README.md` in the _cluster-image-set-controller_ GitHub repository to enable automatic updates of `ClusterImageSets` from a forked GitHub repository.
