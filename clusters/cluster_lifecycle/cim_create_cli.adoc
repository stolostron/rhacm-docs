...
You can create a host inventory (infrastructure environment) by using the command line interface. Use the command line interface instead of the console for advanced configuration or automated deployment. See the following information to learn more:
...
<5> A list of NTP sources (hostname or IP) to add to all hosts. They are added to NTP sources that are configured by using other options, such as DHCP.
