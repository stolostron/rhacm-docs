[#enable-cim-aws]
= Enabling Central Infrastructure Management (CIM) on Amazon Web Services

If you are running your hub cluster on Amazon Web Services and want to enable the CIM service, complete the following steps after xref:cim_enable.adoc#enable-cim[Enabling the Central Infrastructure Management (CIM) service]:
