...
* You need a `HiveConfig` file that references a credential within the Hive namespace that has access to create resources on the VPC that you created for the managed cluster service endpoints.
...
+
The following example shows how to you might subnets for VPCs that have 6 availability zones in the `us-gov-east-1` region:
----
