...
* The {ocp-short} CLI. See link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/cli_tools/openshift-cli-oc#cli-getting-started[Getting started with the OpenShift CLI] for information about installing and configuring the {ocp-short} CLI.
<<<<<<< HEAD
* A defined `multiclusterhub.spec.imagePullSecret` if you are importing a cluster that was not created by {ocp-short}. This secret might have been created when {mce} was installed. See link:../install_update/adv_config_install.adoc#advanced-config-hub[_MultiClusterHub_ advanced configuration] for more information about how to define this secret.
=======
* A defined `multiclusterhub.spec.imagePullSecret` if you are importing a cluster that was not created by {ocp-short}. This secret might have been created when {mce} was installed. See link:../../get_started/adv_config_install.adoc#advanced-config-hub[_MultiClusterHub_ advanced configuration] for more information about how to define this secret.
>>>>>>> 2.17_stage
...
----
+
When the values for `cloud` and `vendor` are set to `auto-detect`, {acm-short} detects the cloud and vendor types automatically from the cluster that you are importing. You can optionally replace the values for `auto-detect` with with the cloud and vendor values for your cluster. See the following example:
+
[source,yaml]
----
cloud: Amazon
...
metadata:
  ...
  namespace: <cluster_name> <1>
stringData:
  kubeconfig: |- <kubeconfig_file> <2>
  token: <token_cluster> <3>
  server: <cluster_api_url> <3>
type: Opaque
----

<1> Add your cluster name.
<2> If you are using the `kubeconfig` file, add the value for the `kubeconfig` file that has context set to the cluster you want to import.
<3> If you are using the token and server pair, add the cluster access token and cluster API URL instead of the `kubeconfig` file.
