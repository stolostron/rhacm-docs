...
The AWS private configuration information is used when you are creating an AWS GovCloud cluster. See
//xref:../cluster_lifecycle/create_ocp_govcloud.adoc#creating-a-cluster-on-amazon-web-services-govcloud[Creating a cluster on Amazon Web Services GovCloud]
 for information about creating a cluster in that environment. 
