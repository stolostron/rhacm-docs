...
. Create a script file that contains content that resembles the following example, with the supported version replacing `<4.x.z>`:
+
----
UPSTREAM_REGISTRY=quay.io
PRODUCT_REPO=openshift-release-dev
RELEASE_NAME=ocp-release
OCP_RELEASE=<4.x.z>-x86_64
...
+
See link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/images/image-configuration#images-configuration-cas_image-configuration[Configuring additional trust stores for image registry access] in the {ocp-short} documentation for more information.
