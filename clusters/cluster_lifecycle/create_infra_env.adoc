...
You can import an existing {ocp-short} cluster so that you can add additional nodes. Complete the following steps to import an {ocp-short} cluster without a static network and without bare metal host and prepare it for adding additional nodes:
...
. Create an `AgentClusterInstall` by applying the following YAML content. Replace values where needed:
+
[source,yaml]
----
...
<3> You can use the optional `sshPublicKey` field to log into nodes for troubleshooting purposes.
...
. Add an `InfraEnv` to discover new hosts to add to your cluster by applying the following YAML content. Replace values where needed:
+
