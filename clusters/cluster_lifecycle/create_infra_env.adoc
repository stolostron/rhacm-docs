...
= Managing host inventory
//iniline links need to be reduced in this file, need to add Additional resources section, plus modularize| MJ | 07/27/23
Your host inventory consists of pools of bare-metal and virtual machines that you can use to create on-premises {ocp-short} clusters. These clusters can be standalone, with dedicated machines for the control plane, or <<hosted-control-planes-intro,hosted control planes>>, where the control plane runs as pods on a hosting cluster.

The host inventory management, together with the standalone on-premises cluster installation, are parts of {mce-short}'s Central Infrastructure Management (CIM) feature. CIM runs the link:https://access.redhat.com/documentation/en-us/assisted_installer_for_openshift_container_platform/2022/html/assisted_installer_for_openshift_container_platform/index[Assisted Installer] as an operator on the hub cluster.

A machine joins the host inventory once it is booted with a Discovery Image. The Discovery Image contains:

- an agent that performs discovery, validation, and installation tasks
- the necessary configuration for reaching the service on the hub, including the endpoint, token, and static network configuration if applicable

You would generally have a single Discovery Image for each "infrastructure environment", which is a set of hosts sharing a common set of properties. This infrastructure environment and associated Discovery Image is represented by the InfraEnv custom resource definiition.

Once the host boots and the agent contacts the service, the service creates a new Agent custom resource on the hub that represents that host. The Agent resources make up the host inventory.

The high-level flow is summarized in the following diagram.
image:../../images/infraenv-agent-flow.png[InfraEnv and Agent flow]
