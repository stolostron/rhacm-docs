...
*Important:* If you are using the late binding feature, the {ocp-short} release images that you use when creating your clusters must be version 4.13 or later when the `spec.osImages` releases specified in the the `AgentServiceConfig` custom resource are version 4.13 or later. The Red Hat Enterprise Linux CoreOS images for version 4.13 and later are not compatible with images earlier than version 4.13.   
