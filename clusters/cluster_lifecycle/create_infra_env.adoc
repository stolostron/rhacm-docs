...
** <<add-nodes-ocp-infra-env,Adding production environment worker nodes to {ocp-short} clusters>>
...
You can import an existing {ocp-short} cluster so that you can add additional nodes. Complete the following steps to import an {ocp-short} cluster and prepare it for adding additional nodes:

. Create a namespace for the {ocp-short} cluster you want to import by applying the following YAML content. Replace :
+
[source,yaml]
----
apiVersion: v1
...
. Make sure that the infrastructure owner has made the ClusterImageSet available by applying the following YAML content:
+
[source,yaml]
----
apiVersion: v1
...
. Create an `AgentClusterInstall` by applying the following YAML content:
+
[source,yaml]
----
...
----
+
<1> You can use the optional `sshPublicKey` field to log into nodes for troubleshooting purposes.

. Create a ClusterDeployment by applying the following YAML content:
+
[source,yaml]
----
apiVersion: hive.openshift.io/v1
...
metadata:
  ...
  namespace: managed-cluster
spec:
  baseDomain: redhat.com <1>
  installed: true <2>
  clusterMetadata:
      adminKubeconfigSecretRef:
        name: some-other-cluster-admin-kubeconfig <3>
      clusterID: "" <4>
      infraID: "" <4>
  clusterInstallRef:
...
----
+
<1> Make sure `baseDomain` matches the domain you are using for your {ocp-short} cluster.
<2> Your {ocp-short} cluster is only automatically imported as a production environment cluster if `installed` is set to `true`.
<3> Make sure that the `kubeconfig` you created in step 4 is referenced in `adminKubeconfigSecretRef`.
<4> In production environments, you can leave `clusterID` and `infraID` empty.


. Add an `InfraEnv` by applying the following YAML content:
+
[source,yaml]
----
apiVersion: agent-install.openshift.io/v1beta1
...
<2> You can use the optional `sshPublicKey` field to log into nodes for troubleshooting purposes.
...
=== Adding production environment worker nodes to {ocp-short} clusters
...
+
*Note:* If the agent is not listed, exit the `watch` command by pressing Ctrl and C, then log into the worker node to troubleshoot.
...
+
----
oc get agent -n managed-cluster -ojson | jq -r '.items[] | select(.spec.approved==false) |select(.spec.clusterDeploymentName==null) | .metadata.name'| xargs oc -n spoke-cluster patch -p '{"spec":{"clusterDeploymentName":{"name":"some-other-cluster","namespace":"spoke-cluster"}}}' --type merge agent
----
