...
. Make sure that value of the `spec.domain` parameter of the new `IngressController` is not in conflict with an existing IngressController by completing the following steps:
...
.. Run the following command on each of the `IngressControllers`, except the one that you just created (`ingress-controller-with-nlb`):
+
----
