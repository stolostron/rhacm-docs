...
+
*Important:* You cannot delete the `global` managed cluster set, or update its `spec.clusterSelector`. The `global` managed cluster set includes all managed clusters. See the following example:
+
[source,yaml]
----
