...
Before adding production environment worker nodes to {ocp-short} clusters, you must check if the you have a valid `kubeconfig` by completing the following steps:
...
----
+
If the `kubeconfig` is valid, the workers report a `Ready` status.
