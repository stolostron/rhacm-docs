...
[#add-nodes-ocp-infra-env-prereqs]
== Prerequisite

You must have the new CA certs required to trust the managed cluster API.

[#add-nodes-ocp-infra-env-kubeconfig]
== Creating a valid `kubeconfig`

Before adding production environment worker nodes to {ocp-short} clusters, you must check if the you have a valid `kubeconfig`.  If the API certs in your managed cluster have changed, following procedure updates the `kubeconfig` with new CA certs. Complete the following steps:
...
. If you receive the following error message, you must update your `kubeconfig` secret. If you receive no error message, continue to <<add-nodes-ocp-infra-env-kubeconfig-steps,Adding production environment worker nodes to {ocp-short} clusters>>:
...
----
+
If you receive an output, the `kubeconfig` is valid.

. Update the `kubeconfig` secret in the {product-title-short} hub cluster by running the following command. Replace `<new_kubeconfig_name>` with the name of your new `kubeconfig` file:
...
[#add-nodes-ocp-infra-env-kubeconfig-steps]
== Adding worker nodes
