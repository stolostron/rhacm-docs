...
Before adding production environment worker nodes to {ocp-short} clusters, you must check if the you have a valid `kubeconfig`. If the API certificates in your managed cluster have changed, complete the following steps to update the `kubeconfig` with new CA certificates:
