...
. Make sure that a `ClusterImageSet` matching the {ocp-short} cluster you are importing exists by applying the following YAML content. Replace `<4.x.0>` with the version of {ocp-short} that you are using: :
+
[source,yaml]
----
