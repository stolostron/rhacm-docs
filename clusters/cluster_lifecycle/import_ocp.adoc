...
You can import an {ocp-short} managed cluster that was installed by the {ai} on one hub cluster to another hub cluster along with all cluster resources. You can import the following resources and continue to manage your cluster with them:
...
You can move management by creating a copy of the original resources to load balance, scale down, and scale up your managed cluster in the new hub cluster.

*Important:* This feature is only available if the managed cluster you are importing was installed by the {ai}.

To create a copy of the `Agent` resource on the target hub cluster, complete the following steps. You can use the following example to create copies of your other resources as well:

. Get the `Agent` from your source hub cluster by running the following command. Replace values where needed:
