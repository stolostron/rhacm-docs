...
You might need to override the image registry on the managed clusters that you are importing. You can do this by creating a `ManagedClusterImageRegistry` custom resource definition. The `ManagedClusterImageRegistry` custom resource definition is a namespace-scoped resource.

The `ManagedClusterImageRegistry` custom resource definition specifies a set of managed clusters for a `placement` to select, but needs different images from the custom image registry. 

After the managed clusters are updated with the new images, the following label is added to each managed cluster for identification: `open-cluster-management.io/image-registry=<namespace>.<managedClusterImageRegistryName>`.
...
== Accessing _ClusterImageSets_ in a disconnected environment

If you cannot access the repository, you might need to take additional steps. {acm-short} retrieves {ocp-short} `ClusterImageSets` from a GitHub repository that is managed by Red Hat. 

These `ClusterImageSets` define {ocp-short} versions that can be deployed. 

Access to this repository requires an internet connection to GitHub unless you configure your setup to use a mirrored or internal repository​. 

See xref:../cluster_lifecycle/release_image_disconn.adoc#release-images-disconnected[Maintaining a custom list of release images while disconnected].