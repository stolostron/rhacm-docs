...
. Fork the link:https://github.com/stolostron/acm-hive-openshift-releases/tree/backplane-2.8[acm-hive-openshift-releases GitHub].
...
. Create a `ConfigMap` file in the `multicluster-engine` namespace that is named `cluster-image-set-git-repo`. See the following example, but replace `mce-version` with you current version of {mce-short}, for example, `2.9`:
