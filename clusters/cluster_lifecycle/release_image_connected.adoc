...
data:
  ...
  caCerts: <4> 
----
<1> Use the URL that you received when you forked the `acm-hive-openshift-releases` repository. 
<2> Choose to use either of the supported channels values, `fast` or `stable`.
<3> Set the `insecureSkipVerify` specification to `true` to build the insecure connection. The supported values are `true` or `false`.
<4> List the certified authority (CA) certificates to build a secure HTTPS connection.
