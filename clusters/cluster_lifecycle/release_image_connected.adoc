...
data:
  ...
  caCerts: <3> 
  ----
  <1> Choose to use either of the supported values `fast` or `stable`.
  <2> Specify `insecureSkipVerify` to `true` for building the insecure connection. The supported values are `true` or `false`.
  <3> List the certified authority (CA) certificates for building a secure HTTPS connection.
...
. If the `acm-hive-openshift-releases` repository URL requires TLS connection, complete the following actions in the `cluster-image-set-git-repo` `ConfigMap`: 
.. Specify `insecureSkipVerify` to `true` for building the insecure connection. 
.. Specify the `caCerts` for building the secure HTTPS connection. 
