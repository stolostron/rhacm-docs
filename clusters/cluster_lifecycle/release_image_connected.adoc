...
. Create a `ConfigMap` file in the `multicluster-engine` namespace that is named `cluster-image-set-git-repo`. See the following example, but replace `mce-version` with you current version of {mce-short}:
