...
You can create an `Ansible job` that is bound to a cluster when the cluster is created or imported to be managed.
...
----
+
By storing the file in the prehook or posthook directory, it creates a list of cluster names that match the placement rule. The list of cluster names can be passed as a value of `extra_vars` to the `Ansible job` `kind` resource. When this value is passed to the `Ansible job` resource, the Ansible job can determine the new cluster name and use it in the automation.
...
The {mce-short} automatically injects the cluster name into the `Ansible job.extra_vars.target_clusters` path. You can dynamically inject the cluster name into the definition. Complete the following procedure to create an Ansible job and configure it with a cluster that is already managed:
