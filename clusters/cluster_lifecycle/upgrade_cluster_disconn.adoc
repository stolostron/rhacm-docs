...
You can use {cincinnati} with {mce-short} to upgrade your clusters in a disconnected environment.
...
+
See https://docs.openshift.com/container-platform/4.11/registry/configuring-registry-operator.html#images-configuration-cas_configuring-registry-operator[Configuring additional trust stores for image registry access] in the {ocp-short} documentation for more information.
...
. In the console navigation, select *Infrastructure* > *Clusters* to check the status of the managed cluster. When the policy is applied, the cluster status is `ready`.
...
. Log in to your {mce-short} console.
