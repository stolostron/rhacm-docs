...
| `demo-nmstate-label`
| Optional
| The label name and the label value must match the values in the `label` section of the `NMStateConfig` custom resource that you created in <<on-prem-creating-your-cluster-with-the-cli-nmstateconfig,Optional: Create the NMStateConfig custom resource>>.
