...
metadata:
  ...
  labels:
    infraenvs.agent-install.openshift.io: <myinfraenv> <1>
spec:
  online: true
...
----
<1> Must match the InfraEnv name and exist in the same namespace.
<2> If you do not set a value, `metadata` is automatically used.
...
The following annotations are available for the agent custom resource:
|===
