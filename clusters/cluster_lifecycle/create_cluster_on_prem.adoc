...
spec:
  ...
  sshPublicKey: | <4>
    ...
    controlPlaneAgents: 1 <5>
  sshPublicKey: ssh-rsa <your-public-key-here> <6>
----
...
<4> You can specify multiple SSH public keys, which allow you to access the host after installing.
<5> Possible values are `1` and `3`. If you are using {ocp-short} version 4.18 or newer, the `platformType` field is set to `BareMetal` or `None`, and your CPU architecture is `x86_64`, you can also use `4` and `5`. Control plane nodes manage workloads, maintain cluster state, and ensure stability. You can increase fault tolerance and availability, and reduce downtime during failures, if you use more than three nodes.
<6> You can specify your SSH public key, which enables you to access the host after it is installed.
