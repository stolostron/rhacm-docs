...
- You must include the `demo-nmstate-label` label name and value in the `InfraEnv` resource `spec.nmStateConfigLabelSelector.matchLabels` field.
- You must add a mac address when when using the DHCP or static network protocol.
