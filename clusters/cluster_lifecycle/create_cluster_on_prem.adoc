...
*Note:* You need to create a multi-architecture `ClusterImageSet` if you install a managed cluster that has a different architecture than the hub cluster. To learn more, see xref:../get_started/release_images_cross_arch.adoc#manual-release-image-cross-arch[Create a release image to deploy a cluster on a different architecture].
...
- See xref:../get_started/release_image_intro.adoc#release-images-intro[Release images]
...
- See xref:../assisted_installer/ai_third_party_cni.adoc#config-third-party-cni-on-prem[Configuring third-party CNI plugins for on-premises cluster installation (Technology Preview)]
