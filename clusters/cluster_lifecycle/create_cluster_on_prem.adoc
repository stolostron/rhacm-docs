...
The `NMStateConfig` custom resource is only required if you have a host-level network configuration, such as static IP addresses. If you include the `NMStateConfig` custom resource, you must complete this step before you create an `InfraEnv` custom resource. The values for `spec.nmStateConfigLabelSelector` in the `InfraEnv` custom resource refer to the `NMStateConfig` custom resource.
