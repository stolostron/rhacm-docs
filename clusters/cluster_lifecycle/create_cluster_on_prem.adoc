...
<4> Possible values are `1` and `3`. If you are using {ocp-short} version 4.18 or newer, your platform is `BareMetal` or `None`, and your CPU architecture is `x86_64`, you can also use `4` and `5`. Control plane nodes manage workloads, maintain cluster state, and ensure stability. You can increase fault tolerance and availability, and reduce downtime during failures, if you use more than three nodes.
