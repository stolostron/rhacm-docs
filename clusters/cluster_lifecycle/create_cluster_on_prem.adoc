...
If you are discovering new hosts with no existing infrastructure environment, complete the steps in xref:../cluster_lifecycle/cim_add_host.adoc#add-host-host-inventory[Adding hosts to the host inventory by using the Discovery Image].
...
<1> Specify the platform type of the environment where the cluster is created. Valid values are `BareMetal`, `None`, `VSphere`, or `External`.
