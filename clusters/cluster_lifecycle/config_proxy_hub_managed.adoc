...
..  A certificate authority (CA) bundle is required for HTTPS proxy. To create a `KlusterConfig` resource for HTTPS proxy where the required certificate authority bundle is defined, run the following command:
...
----
+
See the following configuration where the CA bundle refers to a config map that contains one or multiple CA certificates with HTTPS proxy:  
...
If your development changes, you might need to disable the HTTP or HTTPS proxy. Complete the following steps:
...
When you create a cluster using {acm}, you can specify which nodes you want to run the managed cluster klusterlet by configuring the `nodeSelector` and `tolerations` annotation for the managed cluster. Complete the following steps to configure these settings: 
