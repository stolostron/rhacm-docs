...
- A hub cluster cannot manage a different hub cluster. A hub cluster is set up to automatically import and manage itself as a `local-cluster`. You do not have to manually import a hub cluster to manage.

- The namespace for the imported cluster must match the name of the cluster.
...
*Important:* If you attempt to detach the hub cluster, which is named `<your-local-cluster-name>`, be aware that the default setting of `disableHubSelfManagement` is `false` and the hub cluster is the `local-cluster`, which manages itself. This setting causes the hub cluster to reimport itself and manage itself when it is detached and it reconciles the `MultiClusterHub` controller. It might take hours for the hub cluster to complete the detachment process and reimport. If you want to reimport the hub cluster without waiting for the processes to finish, you can run the following command to restart the `multiclusterhub-operator` pod and reimport faster:
...
By default, the cluster namespace is deleted after you detach a managed cluster.

To prevent {mce-short} from deleting the namespace after detaching a managed cluster, complete the following step:

. Add the `open-cluster-management.io/retain-namespace` annotation to the managed cluster namespace.

If you remove the `open-cluster-management.io/retain-namespace` annotation after detaching a managed cluster, {mce-short} deletes the cluster namespace.

The `cluster.open-cluster-management.io/managedCluster` and `open-cluster-management.io/cluster-name` labels identify the annotation as a cluster namespace. If you remove the annotation and labels at the same time, {mce-short} does not manage the cluster namespace anymore.

[#import-cluster-additional-resources]
=== Additional resources

<<<<<<< HEAD
- See link:../install_update/adv_config_install.adoc#advanced-config-hub[_MultiClusterHub_ advanced configuration] for more information about how to define a custom image pull secret.
=======
- See link:../../get_started/adv_config_install.adoc#advanced-config-hub[_MultiClusterHub_ advanced configuration] for more information about how to define a custom image pull secret.
>>>>>>> 2.17_stage
[]
- See the link:../../get_started/adv_config_install.adoc#disable-hub-self-management[disableHubSelfManagement] topic.
