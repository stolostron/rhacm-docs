...
|===
| Run import commands manually | After completing and submitting the information in the console, including any Red Hat {aap-short} templates, run the provided command on the target cluster to import the cluster. If you are importing a cluster in the {ocp-short} Dedicated environment and running the import commands manually, complete the steps in xref:../cluster/cluster_lifecycle/import_gui.adoc#run-import-commands-manually[Running import commands manually in an {ocp-short} Dedicated environment] before providing the information in the console.
