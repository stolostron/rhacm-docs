...
* <<run-import-commands-manually,Additional steps for running import commands manually in the Red Hat OpenShift Dedicated environment>>
...
* A deployed hub cluster. If you are importing bare metal clusters, the hub cluster must be installed on {ocp} version 4.12.
...
|===
| Run import commands manually | After completing and submitting the information in the console, including any Red Hat {aap-short} templates, run the provided command on the target cluster to import the cluster. If you are importing a cluster in the {ocp-short} Dedicated environment and running the import commands manually, complete the steps in xref:../import_gui.adoc#run-import-commands-manually[Additional steps for running import commands manually in an {ocp-short} Dedicated environment] before providing the information in the console.
...
=== Additional steps for running import commands manually in an {ocp-short} Dedicated environment
