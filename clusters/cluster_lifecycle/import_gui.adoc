...
You can also use a `KlusterletConfig` to configure the `nodeSelector` and `tolerations` for the managed cluster. If you use a `KlusterletConfig`, the managed cluster uses the configuration in the `KlusterletConfig` settings instead of the settings in the managed cluster annotation. See the following example:
+
[source,yaml]
----
...
Make sure to add the `agent.open-cluster-management.io/klusterlet-config: `<klusterletconfig_name>` annotation to the managed cluster, replacing `<klusterletconfig_name>` with the name of your `KlusterletConfig`
