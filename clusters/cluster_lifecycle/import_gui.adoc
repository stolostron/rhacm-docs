...
[#retain-namespace-managed-cluster]
== Retaining the namespace after removing a managed cluster

By default, the cluster namespace is deleted after you detach a managed cluster.

To prevent {mce-short} from deleting the namespace after detaching a managed cluster, add the `open-cluster-management.io/retain-namespace` annotation to the managed cluster namespace.
