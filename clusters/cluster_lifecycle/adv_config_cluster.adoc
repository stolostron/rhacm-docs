...
+
*Note:* You can use the {ocp-short} command-line interface or the {product-title-short} search feature to edit the managed cluster YAML definition.on. The key for this annotation is: `open-cluster-management/nodeSelector`. The value of this annotation is a string map with JSON formatting.
