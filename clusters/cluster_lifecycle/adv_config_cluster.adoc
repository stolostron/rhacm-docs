...
If you create a `KlusterletConfig` resource and set the name to `global`, the configurations in the gloabl `KlusterletConfig` are automatically applied on every managed cluster.
