...
. To set a custom strategy, run the following command. Replace <mce_install_namespace> with your {mce-short} install namespace:
...
----
+
*Note:* The default {mce-short} install namespace is `multicluster-engine`.

. If you want to include the updated strategy in the cluster backup, add the `backup=true` label to the `ConfigMap` by running the following command. Replace <mce_install_namespace> with your {mce-short} install namespace:
