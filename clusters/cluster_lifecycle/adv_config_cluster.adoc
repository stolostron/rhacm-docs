...
- You have a manged cluster that can reach the HTTP or HTTPS proxy server, and the proxy server can access the MCE hub cluster. 
...
. When creating a managed cluster, chose the `KlusterletConfig` by adding an annotation that refers to the `KlusterletConfig`. See the following example:
+
[source,yaml]
----
