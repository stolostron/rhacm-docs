...
[#config-cluster-proxy-addon-hub-managed]
...
+
*Note:* The `spec.installNamespace` field is deprecated. See link:../../clusters/release_notes_mce/mce_deprecate_remove.adoc#mce-deprecation-removal[Product deprecations and removals] to learn more.
...
. Verify the proxy settings by checking if the cluster proxy agent pod in the `open-cluster-management-addon` namespace has `HTTPS_PROXY` or `NO_PROXY` environment variables on the managed cluster.
