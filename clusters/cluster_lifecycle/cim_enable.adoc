...
//should the conref be used here in the title? "{assist-install}" | MJ | 07/03
...
When you install a cluster that is in FIPS mode, you must specify a Red Hat Enterprise Linux (RHEL) version in the `AgentServiceConfig` resource. When you install {ocp-short} version 4.15 and earlier, the installer must run on a RHEL host.
...
. Log in to you managed cluster
//is there a command to complete is? 
