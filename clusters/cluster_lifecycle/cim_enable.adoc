...
. Write the key pairs representing the headers and query parameters that you want to send with every `osImage` request. If you don't need both parameters, you can write key pairs for only headers or query parameters.

*Important:* Headers and query parameters are only encyrpted if you use HTTPS. Make sure to use HTTPS to avoid security issues.
