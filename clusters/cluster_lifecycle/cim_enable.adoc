...
Modify the `Provisioning` resource if your hub cluster is installed on one of the following platforms:
...
If you need to modify the `Provisioning` resource, complete the following steps:
...
. If you receive a `No resources found` error, create the `Provisioning` resource by adding the following YAML content:
+
[source,yaml]
----
