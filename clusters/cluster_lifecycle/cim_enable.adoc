...
. If you want to use HTTPS `osImages` with a self-signed or third=party CA certificate, make sure to add the certificate to the `image-service-additional-ca` `ConfigMap`. You can run the following command to create a certificate:
+
----
...
If you are using HTTPS `osImages` with self-signed or third-party CA certificates, make sure to reference the certificate in the `OSImageCACertRef` spec.
