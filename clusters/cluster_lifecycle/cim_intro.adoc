...
The host inventory management and on-premises cluster installation are available using the {mce-short} Central Infrastructure Management (CIM) feature. CIM runs the Assisted Installer (also called infrastructure operator) as an operator on the hub cluster.
