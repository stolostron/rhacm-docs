...
See link:https://docs.openshift.com/container-platform/{ocp-version}/openshift_images/managing_images/using-image-pull-secrets.html[Using image pull secrets].
...
. While you create your cluster pool, you must select {ocp-short} {ocp-latest} or later in the _Release image_ field. If you enabled the feature gate, an option to select *Single node OpenShift* is displayed. 
+
