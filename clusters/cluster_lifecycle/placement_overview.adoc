...
* Placements sort the clusters by `Prioritizers` scores and select the top `n` clusters from that group. You can define `n` in `numberOfClusters`.
...
- See xref:../cluster_lifecycle/placement_filter.adoc#placement-tolerations[Filtering `ManagedClusters` by using taints and tolerations] for more information.
- To learn more about the API and `Prioritizers`, see link:../api/placement.json.adoc#placements-clusters-api[Placements API].
