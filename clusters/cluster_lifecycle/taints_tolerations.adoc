...
If the taints section does not exist, you can add a taint to a managed cluster by entering a command that resembles the following example:
...
Alternatively, you can append a taint to existing taints by running a command similar to following example:
