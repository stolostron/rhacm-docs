...
When you install the {mce-short} version 2.5 or later, the `ManagedServiceAccount` add-on is enabled by default. In {mce-short} version 2.4 and earlier, the `ManagedServiceAccount` add-on was disabled by default. If you upgrade your hub cluster from {mce-short} version 2.4 and did not enable the `ManagedServiceAccount` add-on before upgrading, you have to enable the add-on manually.
