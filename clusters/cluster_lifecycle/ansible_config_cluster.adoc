...
You can use the `ClusterCurator` resource to push custom labels to the automation job pod created by the `ClusterCurator`. You can push the custom labels on all curation types. See the following example:
