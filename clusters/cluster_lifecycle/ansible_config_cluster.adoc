...
To save time, you can also only run the failed Ansible posthooks again that are part of cluster automation templates. Complete the following steps to run only the posthooks again, without retrying the entire upgrade:

. Add the following content to root of the `ClusterCurator` resource to run the install posthook again:
...
. Add the following content to root of the `ClusterCurator` resource to run the upgrade posthook again:
