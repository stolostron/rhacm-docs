...
. Add `spec.upgrade.intermediateUpdate` to the `ClusterCurator` resource with the intermediate release value. See the following sample, where the intermediate release is `4.15.x`, and the `desiredUpdate` is `4.16.x`:
...
spec:
  desiredCuration: upgrade
  upgrade:
    intermediateUpdate: 4.15.x
    desiredUpdate: 4.16.x
    monitorTimeout: 120
----
