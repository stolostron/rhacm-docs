...
* <<eus-updates, Using the _ClusterCurator_ for Extended Update Support (EUS) updates>>
* <<update-ocp-cluster-curator,Updating {ocp-short} clusters by using the _ClusterCurator_>>
...
You can use the `ClusterCurator` resource to specify an Ansible inventory to use for all jobs. See the following example. Replace `channel` and `desiredUpdate` with the correct values for your `ClusterCurator`:
...
*Note:* To use the example resource, the inventory must already exist in Ansible.

You can verify that the inventory is created by checking the list of available Ansible inventories from the console.
...
[#eus-updates]
== Using the _ClusterCurator_ for Extended Update Support (EUS) updates

You can use the `ClusterCurator` resource to perform an easier, automatic upgrade between EUS releases. 

. Add `spec.upgrade.intermediateUpdate` to the `ClusterCurator` resource with the intermediate release value. See the following sample, where the intermediate release is `4.17.x`, and the `desiredUpdate` is `4.18.x`:
...
[#update-ocp-cluster-curator]
== Updating {ocp-short} clusters by using the _ClusterCurator_

You might want to upgrade your {ocp-short} cluster to a version with known issues to test new features quickly.

*Important:* Upgrading to an {ocp-short} cluster version with known issues can reduce performance and compatibility.

To upgrade to an {ocp-short} cluster version with known issues, complete the following steps:

. Create a file named `clustercurator.yaml`.

. Add the following annotation to the file:
...
spec:
  desiredCuration: upgrade
  upgrade:
    desiredUpdate: <4.x.x> <3>
    monitorTimeout: 120
----
<1> Add your cluster name.
<2> Add your cluster namespace.
<3> Add the {ocp-short} cluster version with known issues that you want to upgrade to.
...
The `ClusterCurator` checks the `desiredUpdate` image digest version at startup in the conditional updates list. If the image digest is not found, the `ClusterCurator` uses the image tag instead.
