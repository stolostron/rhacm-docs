...
* A {mce-short} 2.5 or later, hub cluster that is running on {ocp} version {ocp-latest} or later.
...
. Select {ocp-short} {ocp-latest} or later In the _Release image_ field. If you enabled the feature, an option to select *{sno}* is displayed. If you select an {ocp-short} version that is earlier than 4.11, the option to select _{sno}_ is not displayed. When you select *{sno}* during the creation process, the _Worker pools_ option is removed from the procedure because there are no worker nodes in a {sno} cluster. 
