...
* A {mce-short} 2.5 or later, hub cluster that is running on {ocp} version 4.13 or later.
...
To create a {sno} cluster, complete the following steps in the {acm-short} console for creating a cluster on one of the providers that are supported to run a {sno} cluster in xref:../cluster_lifecycle/create_intro.adoc#create-intro[Cluster creation]:

. Select {ocp-short} 4.13 or later In the _Release image_ field. If you enabled the feature, an option to select *{sno}* is displayed. If you select an {ocp-short} version that is earlier than 4.11, the option to select _{sno}_ is not displayed. When you select *{sno}* during the creation process, the _Worker pools_ option is removed from the procedure because there are no worker nodes in a {sno} cluster. 
