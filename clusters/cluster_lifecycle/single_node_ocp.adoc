...
To create a {sno} cluster, complete the following steps in the {product-title-short} console for creating a cluster on one of the providers that are supported to run a {sno} cluster in xref:../cluster_lifecycle/create_intro.adoc#create_intro[Cluster creation introduction]:
