...
* You must configure cluster proxy add-ons.
...
== Accessing the managed cluster `kube-apiserver` by using a hub cluster user token
...
To list pods in the default namespace by using the the hub cluster `developer ` user token, you must first create the appropriate `Role` and `RoleBinding` for the `developer` on the managed cluster.
