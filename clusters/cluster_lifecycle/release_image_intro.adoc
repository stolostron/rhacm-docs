...
In {mce-short}, a cluster image set controller that runs on the hub cluster. This controller queries the link:https://github.com/stolostron/acm-hive-openshift-releases[acm-hive-openshift-releases GitHub repository], at set intervals, for new `clusterImageSets` YAML files. By default, the controller synchronizes with the `fast` channel in the most recent `backplane-{mce-version}` branch.
