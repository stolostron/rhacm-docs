...
. Create the `HostedCluster` and `NodePool` resources, including the `pausedUntil: true` field. If you use the `hcp create cluster` command line interface command, you can specify the `--pausedUntil: true` flag. For more information about the hosted control plane command line interface (`hcp`), see _Installing the hosted control plane command line interface (hcp)_.
