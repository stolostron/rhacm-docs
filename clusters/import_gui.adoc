...
You can specify an Ansible Automation template to run when the cluster is imported, or when it is upgraded. You must have the Ansible Automation Platform Resource Operator installed from OperatorHub to create and run an Ansible job. 
