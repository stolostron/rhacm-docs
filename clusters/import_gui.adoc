...
* **Note:** If you are importing a cluster that was not created by {ocp-short}, you need a `multiclusterhub.spec.imagePullSecret` defined. This secret might be created when {product-title-short} was installed. 
+
If you need to create a new one, complete the following steps:
+
...
. Run the following command to create a new secret in the namespace of your hub cluster: 
+
----
...
+
See link:../install/adv_config_install.adoc#custom-image-pull-secret[Custom image pull secret] for more information about how to define this secret.
