...
* Run import commands manually: After completing and submitting the information in the console, including any Red Hat Ansible Automation Platform templates, run the provided command on the target cluster to import the cluster. Continue with the steps in xref:../cluster_lifecycle/import_gui.adoc#run-import-commands-manually[Additional steps for running the import commands manually].
...
+
You can specify a Red Hat Ansible Automation Platform template to run when the cluster is imported or upgraded. You must have the Red Hat Ansible Automation Platform Resource Operator installed from OperatorHub to create and run an Ansible Automation Platform job.
+
