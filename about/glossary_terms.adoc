...
Created and imported clusters are maanaged by `klusterlet` and `klusterlet-addon`, that is, the agent that initiates a connection to the Red Hat Advanced Cluster Management for Kubernetes hub cluster, and the controller which monitors klusterlet applications within the managed cluster, respectively.
