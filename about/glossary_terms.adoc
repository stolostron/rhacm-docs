...
Create and import clusters with the `klusterlet` and `klusterlet-addon`, that is, the agent that initiates a connection to the Red Hat Advanced Cluster Management for Kubernetes hub cluster, and the controller which oversees klusterlet applications within the managed cluster, respectively.
...
The controller on the managed cluster which oversees klusterlet applications.
