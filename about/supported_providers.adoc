...
You must have `kubectl` installed when you import a managed cluster to a hub cluster. See link:../multicluster_engine/cluster_lifecycle/import.adoc#importing-a-target-managed-cluster-to-the-hub-cluster[Importing a target managed cluster to the hub cluster] for details.
