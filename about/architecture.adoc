...
For example, managed clusters send metrics to the hub cluster if the observability service is enabled. See [Observing environments](../observability/observe_intro.adoc#observing-environments) to receive metrics and optimize the health of all managed clusters.
