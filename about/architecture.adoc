...
The {acm-short} hub cluster uses the `MultiClusterHub` operator to manage, update, and install hub cluster components and runs in the `open-cluster-management` namespace. The hub cluster aggregates information from multiple clusters by using an asynchronous work request model and search collectors. The hub cluster maintains the state of clusters and applications that run on it.
...
<<<<<<< HEAD
* link:../install/install_overview.adoc#installing-acm-overview[Installing and updating overview]
=======
* link:../get_started/get_started_acm.adoc#get-started-acm[Get started with {acm-short}]
>>>>>>> 2.17_stage
