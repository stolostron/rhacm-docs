...
A fleet is a group of devices that is governed by a common device template and management policies.
...
Device template:: Defines the configuration the devices in the fleet.
...
When you update the device template for the fleet, the {rhem} service rolls out the new configuration to all of the devices in the fleet.
The {rhem} agent updates the device as soon as the agent detects the new target configuration.
The update process is the same when you add an individual device to the fleet or when a device changes fleets.

//TODO Add link: For more information, see xref:../edge_manager/edge_mgr_manage_organize.adoc#labels[Labels and label selectors].
