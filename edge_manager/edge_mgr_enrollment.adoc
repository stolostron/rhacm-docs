...
You need to enroll devices to a {rhem} service before you can start managing them.
The {rhem} agent that runs on a device handles the device enrollment.
...
The enrollment request must be approved by an authorized user.
After the request is approved, the device becomes trusted and managed by the {rhem} service.
...
Late binding:: You can define the enrollment endpoint and certificate at provisioning time instead of including them in the operating system image.
...
*Note:* The enrollment certificate is only used to secure the network connection for submitting an enrollment request.
