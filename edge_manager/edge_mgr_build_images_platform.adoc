...
For optimized provisioning and integration with {ocp-virt} and VMware vSphere, enrollment certificates and agent configurations can be provided with `cloud-init`, rather than embedding them in the image. Additionally, include appropriate guest tools for better platform integration. This process generates platform-specific image formats, such as `QCOW2` for {ocp-virt} and `VMDK` for vSphere.
