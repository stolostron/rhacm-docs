...
*Technology Preview:* You can include an operating system-level host configuration in the image to provide maximum consistency and repeatability.
...
[#os-config-additional-resources]
== Additional resources

* For more information about device lifecycle hooks and the default rules used by the {rhem} agent, see xref:../edge_manager/edge_mgr_device_lifecycle_hooks.adoc#device-lifecycle-hooks[Device lifecycle hooks].
* For more information about granting {rhem} permissions, see xref:../edge_manager/edge_mgr_rbac.adoc#rhem-rbac[{rhem} authorization].