...
To enable the {gitops-short} add-on for your selected managed clusters, reference your `Placement` and use the `GitOpsCluster` custom resource as the interface for enabling. To enable and manage the {gitops-short} add-on, complete the following sections: 
...
If you want to enable the {gitops-short} add-on with the `ArgoCD` agent, use the `Agent` mode. This mode extends the basic {gitops-short} add-on with the `ArgoCD` agent component, enabling a pull model architecture. 

If you want to enable the {gitops-short} add-on without the `ArgoCD` agent, use the `Basic` mode. This mode enables the {gitops-short} add-on to the managed clusters that you selected with the `Placement`. 
