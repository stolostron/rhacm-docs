...
. Customize your container images for the {gitops-short} components by applying the following YAML with the `GitOpsCluster`:
...
. Finish applying and save the YAML sample by running the following command: 
...
. Customize the namespaces where you deploy the the {gitops-short} components by applying the following YAML with the `GitOpsCluster`:
...
. Specify if the `ArgoCD` agent can reconcile application in all namespaces by applying the following YAML with the `GitOpsCluster`:
...
. After giving a value for the `reconcileScope` field, finish applying and save the YAML sample by running the following command: 
