...
* `uninstall`: Uninstall the GitOps add-on and all components. Requires `overrideExistingConfigs: true` as a safety confirmation. The default is `false`.
...
* `uninstall`: Uninstall only the agent while keeping the GitOps add-on. Requires `overrideExistingConfigs: true` in the parent `gitopsAddon` spec as a safety confirmation The default is `false`. 
...
You can completely uninstall the GitOps add-on or only uninstall the `ArgoCD` agent component. You can remove the `ArgoCD` agent from managed clusters while preserving the {gitops-short}  operator and the `ArgoCD` instance.

The `ArgoCD` custom resource definitions (CRDs) are preserved even after uninstallation to prevent accidental deletion of existing `ArgoCD` applications.

To uninstall only the `ArgoCD` agent while keeping the GitOps add-on, complete the following steps: 

. Set both `uninstall: true` and `overrideExistingConfigs: true` as a double confirmation by applying the following YAML:
...
spec:
  ...
  gitopsAddon:
    enabled: true
    overrideExistingConfigs: true <1>
    argoCDAgent:
      enabled: false
      uninstall: true
----
<1> The `overrideExistingConfigs: true` field is required as a safety mechanism to confirm the uninstall operation.
...
. Set both `uninstall: true` and `overrideExistingConfigs: true` as a double confirmation:
