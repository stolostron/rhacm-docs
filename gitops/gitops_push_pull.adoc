...
- The Argo CD controller pushes resources from the centralized hub cluster to the managed cluster. For every managed cluster, there is an application manager add-on that runs on the managed cluster. By default, the Argo CD controller uses the service account application manager to push these resources to the managed cluster. 

- Now, you can specify the different service account you want the Argo CD push model to use, based on your individual developer needs. Therefore, when the Argo CD  controller pushes resources from the hub to the managed cluster, you can use a different service account than the default application manager. 
