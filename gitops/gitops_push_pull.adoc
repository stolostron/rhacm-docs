...
* <<crd-pull-model,Creating the _ApplicationSet_ CRD>>
...
- For every managed cluster, there is an application manager add-on that runs on the managed cluster. By default, the Argo CD controller uses the service account application manager to push these resources to the managed cluster. 

- The application manager service account has a broad set of permissions because the application subscription add-on uses the application manager service to deploy applications on the managed cluster. If you prefer to use a service account with a limited set of permissions, do not use the application manager service account.

- You can specify a different service account you want the Argo CD push model to use. Therefore, when the Argo CD controller pushes resources from the centralized hub cluster to the managed cluster, you can use a different service account than the default application manager. By using a different service account, you have full control over the permissions that are granted to this service account. 
