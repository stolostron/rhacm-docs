...
. Add the following environment variables to the `spec.config.env` file by applying the following YAML sample:
...
. Replace the existing `ArgoCD` custom resource with the compatible `Agent` mode configuration by applying the following YAML sample:
...
. On your managed cluster, create a `GitOpsCluster`` resource with the `ArgoCD` agent enabled by applying the following YAML sample: 
...
. Create an `ArgoCD` application resource in the managed cluster namespace by applying the following YAML file: 
