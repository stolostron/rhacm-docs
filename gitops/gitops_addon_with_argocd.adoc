...
. Add the following environment variables under `spec.config.env` by applying the following YAML sample:
...
. Create a `GitOpsCluster resource with the `ArgoCD` agent enabled by applying the following YAML sample: 
