...
When you integrate with the GitOps operator, for every managed cluster that is bound to the GitOps namespace through the placement and `ManagedClusterSetBinding` custom resources, a secret with a token to access the `ManagedCluster` is created in the GitOps instance server namespace. The GitOps controller needs this secret to sync resources to the managed cluster. By default, the service account application manager works with the cluster admin permissions on the managed cluster to generate the GitOps cluster secret in the GitOps instance server namespace. The default namespace is `openshift-gitops`. 
