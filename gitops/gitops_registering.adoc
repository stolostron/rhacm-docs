...
The {gitops-short} controller needs this secret to sync resources to the managed cluster. By default, the service account application manager works with the cluster admin permissions on the managed cluster to generate the {gitops-short}  cluster secret in the {gitops-short} instance server namespace. The default namespace is `openshift-gitops`. 
