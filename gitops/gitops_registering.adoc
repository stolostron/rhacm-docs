...
. Enable the `ManagedServiceAccount` addon to rotate the token used for the Argo CD push and pull model to connect to the managed cluster. For help enabling the addon, see link:../add-ons/configure_klusterlet_addons.adoc#configure-klusterlet-addons[Configuring klusterlet add-ons].
