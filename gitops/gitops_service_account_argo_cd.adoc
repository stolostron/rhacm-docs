...
[#creating-cluster-permission-existing-roles]
== Creating a cluster permission using an existing role 
...
For example, when you apply the following YAML sample, you create a `clusterpermission` resource that uses an existing `cluster` role: 
...
spec:
  ...
  clusterRoleBinding:
      ...
      subject:
        ...
        name: group1 
----
<1> An optional name to create the `roleBindings` resource instead of using the default `clusterpermission` name. 
<2> Refers to an existing role on the managed cluster. 
<3> An optional name to create the `clusterRoleBindings` resource instead of using the default `clusterpermission` name. 
<4> Refers to an existing `cluster` role on the managed cluster. 
