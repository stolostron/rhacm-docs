...
. Deploy applications by using an `ApplicationSet` resource on the hub cluster. In the advanced pull model, use `destination.name` so the principal routes the application to the correct agent. See the following YAML sample:
