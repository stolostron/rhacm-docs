...
You can use {gitops-short} with the `ArgoCD` resource to define a custom logic that sets the health status of a resource based on the resource state. For example, you can define custom health checks that only report a policy as healthy if your policy is compliant.

You must add health checks for resources to the `resourceHealthChecks` field. 
...
. Apply the `argocd-policy-healthchecks.yaml` policy by navigating to *Governance* > *Create policy* in the console and pasting the content into the YAML editor.
+
*Note:* You can add Placement information to determine which clusters have the policy active in the YAML editor.
