...
You can use {gitops-short} with the `ArgoCD` resource to define a custom logic that sets the health status of a resource based on the resource state. For example, you can define custom health checks that only reports a policy as healthy if your policy is compliant.

You must add health checks for resources as a `group` in the `resourceHealthChecks` field. 
...
. To apply the `argocd-policy-healthchecks.yaml` policy, navigate to *Governance* > *Create policy*, and paste it into the YAML editor. There, you can add Placement information to determine which clusters have the policy active.
