...
Use {gitops-short} with the `ArgoCD` resource to define a custom logic that sets the health status of a resource based on the resource state. For example, you can define custom health checks that only report a policy as healthy if your policy is compliant.
