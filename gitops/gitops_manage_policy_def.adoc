...
//what other background information do we need to add? 
To define additional health checks for different resource kinds, configure your `ArgoCD` resource. You can customize and view health details for your resource kinds . When you add a health check for a resource, you must add it as a `group` in the `resourceHealthChecks` field. Complete the following steps to define health checks for your resource kinds:
...
//why does the user need to view the policy before they apply it?
. Review the policy before you apply it to your policy with the following command:

+
[source,bash]
----
less ./argocd-policy-healthchecks.yaml
----
+
...
//^should the message say "At least one certificate does not comply..." instead of once?
