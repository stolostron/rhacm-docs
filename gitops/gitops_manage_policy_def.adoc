...
- <<gitops-pol-def-prereq,Prerequisites>>
...
//we likely need to remove this section, we need to alert the engineering team to remove the FIPS annotation/doc for 4.14 and later-bcs 5/2025
...
Use {gitops-short} with the `ArgoCD` resource to define a custom logic that sets the health status of a resource based on the resource state by adding to the `resourceHealthChecks` field.  For example, you can define custom health checks that only report a policy as healthy if your policy is compliant.
...
. Add a health check to your `CertificatePolicy`, `ConfigurationPolicy`, `OperatorPolicy`, and `Policy` resources by downloading the `argocd-policy-healthchecks.yaml`. Run the following command:
...
. Apply the `argocd-policy-healthchecks.yaml` policy by going to *Governance* > *Create policy* in the console and pasting the content into the YAML editor.
+
*Note:* You can add Placement information in the YAML editor to determine which clusters have the policy active.
