...
You are currently viewing the 2.0_stage branch, which is the _in-progress_ branch of the documentation for the 2.0 release. We prioritize documentation changes to the newest published release and the previous 2 releases (n-2). Published release content that is in the n-2 range is updated weekly, as needed. If you need an update that is exclusive to a previous release, link:https://github.com/open-cluster-management/rhacm-docs/issues/new[open an issue] and propose that change. 
