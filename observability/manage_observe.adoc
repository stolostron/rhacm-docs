...
// Note, the metrics that are available in the system are outlined in the section: List of Metrics Collected 
// List of Metrics Collected from the Managed Clusters
//This is an immutable list. But by looking at this configmap, you can see the current list of metrics being collected from all the managed clusters: `kubectl describe cm observability-metrics-whitelist`Create rules only the metrics that are listed


In order to add Prometheus Recording Rules and Alerting Rules to the Observability installation, create them as usual using the Prometheus guidelines. You can customize the observability resource by configuring Prometheus rules. For more information, see https://prometheus.io/docs/prometheus/latest/configuration/configuration/[Prometheus configuration]. 
...
. Create a ConfigMap named `thanos-rule-custom-rules` in the `open-cluster-observability` namespace. The key must be named, `custome-rules.yaml`, as shown in the following example. You can create multple rules in the configuration.
+
