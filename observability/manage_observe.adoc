...
. Create a ConfigMap named `thanos-rule-custom-rules` in the `open-cluster-management-observability` namespace. The key must be named, `thanos-ruler-custome-rules.yaml`, as shown in the following example. You can create multiple rules in the configuration:
+
...
** Add the `exclude.observability: disabled` label to the custom resource, `managedclusters.cluster.open-cluster-management.io`.
** From the {product-title-short} console, access a cluster YAML file from the _Clusters_ page to add the `exclude.observability: disabled` label.
