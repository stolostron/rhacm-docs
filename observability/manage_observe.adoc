...
* <<restricting-configuration-observability,Restricting configuration changes>>
...
Add metrics to the `metrics_list.yaml` file, in order to view collected metrics that are from managed clusters. *Note*: When you update your custom rules, `observability-observatorium-thanos-rule` pods are restarted automatically.
...
[#restricting-configuration-observability]
== Restricting configuration changes

Complete the following steps to restrict configuration changes for the observability service:
