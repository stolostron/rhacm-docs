...
[#view-issues-console]
= Viewing identified {light-short} issues from the console

[role="_abstract"]
View {light-short} identified issues from the console by using the cluster issues summary card and the `PolicyReport` resource. The `PolicyReports` are used to define and configure alerts that are sent to incident management systems.

*Required access*: Cluster administrator

.Prerequisites

* Ensure that {light-short} is enabled. For more information, see link:https://docs.redhat.com/en/documentation/red_hat_lightspeed/1-latest/html-single/connecting_your_rhel_systems_through_the_red_hat_lightspeed_proxy/index#connecting-through-your-own-proxy[Connecting to {light-short} through your own proxy].
...
+
*Note:* When you resolve the issue, you receive the {light-short} every 30 minutes, and {light-short} is updated every two hours.
...
* xref:../observability/obs_insights_risks_console.adoc#obs-insights-risks-pred[Viewing {light-short} updated risk predictions]
* xref:../observability/obs_insights.adoc#obs-insights[Using observability with {light-short}]
