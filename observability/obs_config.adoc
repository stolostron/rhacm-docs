...
Metrics and alerts for the hub cluster appear in the `<your-local-cluster-name>` namespace. The `local-cluster` is only available if hub self-management is enabled. You can query the `local-cluster` metrics in the Grafana explorer. Continue reading to understand what metrics you can collect with the observability component, and for information about the observability pod capacity.
