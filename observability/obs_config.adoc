...
Metrics and alerts for the hub cluster appear in the `local-cluster` namespace. The `local-cluster` is only available if hub self-management is enabled. You can query the `local-cluster` metrics in the Grafana explorer. Continue reading to understand what metrics can be collected with the observability compnent, and for information about the observability pod capacity.
