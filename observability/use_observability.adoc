...
//is this a substep if the user chooses to access the `rbac-query-proxy`?
...
.. Run the following command to create `proxy-byo-cert` secrets using the generated certificates:
+
[source,bash]
----
