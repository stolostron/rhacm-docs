...
. Run the following command to add the access permission to you script file:
...
+
Your script might resemble the following file:
...
echo "${crt}" > "${BASE_PATH}"/ca.crt

PROXY_ROUTE_URL=$(oc get route rbac-query-proxy -n open-cluster-management-observability -o jsonpath='{.spec.host}')
QUERY_EXPRESSION="up"
curl -vv  --location --cacert "${BASE_PATH}"/ca.crt -H "Authorization: Bearer ${TOKEN}" https://${PROXY_ROUTE_URL}/api/v1/query\?query\=${QUERY_EXPRESSION}
----
...
//are the steps on line 87-99 related to querying at all ? 
...
//should this section be a subsection of the previous section, Querying metrics using the observability API?
