...
. If you want to configure custom certificates for the `rbac-query-proxy` route, access an endpoint of the default CA certificate from your hub cluster through mutual TLS, which verifies the identities of both parties in a network connection. Create the `proxy-byo-ca` TLS secret by running the following command:
