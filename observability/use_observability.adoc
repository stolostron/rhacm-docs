...
You can view the {ocp-virt} dashboard to see comprehensive insights for each cluster with the {ocp-virt-short} operator installed. The state of the operator is displayed, which is determined by active {ocp-virt-short} alerts and the conditions of the Hyperconverged Cluster Operator. Additionally, you can view the number of running virtual machines and the operator version for each cluster. 
