...
[#view-issues-console]
= Viewing identified Red Hat Lightspeed issues from the console

[role="_abstract"]
View Red Hat Lightspeed identified issues from the console by using the cluster issues summary card and the `PolicyReport` resource. The `PolicyReports` are used to define and configure alerts that are sent to incident management systems.

*Required access*: Cluster administrator

.Prerequisites

* Ensure that Red Hat Lightspeed is enabled. For more information, see link:https://docs.redhat.com/en/documentation/red_hat_lightspeed/1-latest/html-single/connecting_your_rhel_systems_through_the_red_hat_lightspeed_proxy/index#connecting-through-your-own-proxy[Connecting to Red Hat Lightspeed through your own proxy].
...
+
*Note:* When you resolve the issue, you receive the Red Hat Lightspeed every 30 minutes, and Red Hat Lightspeed is updated every two hours.
...
* xref:../observability/obs_lightspeed_risks_console.adoc#obs-lightspeed-risks-pred[Viewing Red Hat Lightspeed updated risk predictions]
* xref:../observability/obs_lightspeed.adoc#obs-lightspeed[Using observability with Red Hat Lightspeed]