...
First, clone the link:https://github.com/open-cluster-management/multicluster-monitoring-operator/[`open-cluster-management/multicluster-monitoring-operator/`] repository, so that you are able to run the scripts to set up the instance. Complete the following steps to set up the Grafana developer instance:
...
. Switch the user role to Grafana administrator with the `switch-to-grafana-admin.sh` script. You must navigate to Grafana URL, `https://$ACM_URL/grafana-dev/` and log in. Then run the following command:
+
----
...
. From the Grafana console, create a dashboard by selecting the *Add* icon from the navigation panel. Select *Create Dashboard*, and then click *Add new panel*.
