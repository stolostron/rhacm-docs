...
- <<enabling-adding-managed-cluster-labels,Enabling and adding managed cluster labels>>
...
When Observability is enabled in the hub cluster, the `observability-managed-cluster-label-allowlist` ConfigMap is createed in the `open-cluster-management-observability` namespace. The ConfigMap contains a list of managed cluster labels that are maintained by the `observabilty-rbac-query-proxy` pod, to populate a list of label names to filter from within the _ACM - Cluster Overview_ Grafana dashboard. By default, Observability ignores out-of-the-box labels in the `observability-managed-cluster-label-allowlist` ConfigMap.
...
The labels that are enabled are displayed in the drop-down filter on the _ACM - Clusters Overview_ Grafana dashboard. The values are from the `managed_cluster_labels` metric, depending on the `label` key value that is selected.
...
[#enabling-adding-managed-cluster-labels]
=== Enabling and adding managed cluster labels

When you add a new managed cluster label to the `observability-managed-cluster-label-allowlist` ConfigMap, you are enabling the cluster. Add a unique label to the hub cluster, or managed cluster object that is associated with the managed cluster fleet. For example, if you add the label, `department=finance` to a managed cluster, the ConfigMap is updated and might resemble the following changes:
...
From the Grafana dashboard, verify that the label is listed as a valiue in the _Label_ drop-down menu.
