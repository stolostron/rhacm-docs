...
. Switch the user role to Grafana administrator with the `switch-to-grafana-admin.sh` script. You must navigate to Grafana URL, `https://$ACM_URL/grafana-dev/` and log in. Then run the command below to add the logged-in user as Grafana administrator. e.g. After log in using kubeadmin, run following command:
+
----
