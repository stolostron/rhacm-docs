...
. Clone the link:https://github.com/stolostron/multicluster-observability-operator[`open-cluster-management/multicluster-observability-operator/`] repository, so that you are able to run the scripts that are in the `tools` folder.
