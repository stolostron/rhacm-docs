...
. Clone the link:https://github.com/stolostron/multicluster-observability-operator[`stolostron/multicluster-observability-operator/`] repository, so that you are able to run the scripts that are in the `tools` folder.
...
//@subbarao, can you remind me why users should reference this doc. I recall you said something along the lines of continuous configuration or something
