...
First, clone the link:https://github.com/open-cluster-management/multicluster-observability-operator[`open-cluster-management/multicluster-observability-operator/`] repository, so that you are able to run the scripts that are in the `tools` folder. Complete the following steps to set up the Grafana developer instance:
