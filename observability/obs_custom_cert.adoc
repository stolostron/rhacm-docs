...
metricObjectStorage:
  ...
  tlsSecretMountPath: /etc/<s3-data>/certs <2>
----
<1> The value for `tlsSecretName` is the name of the `Secret` object that you previously created.
<2> The `/etc/<s3-data>/certs/` path defined for the `tlsSecretMountPath` parameter specifies where the certificates are mounted in the Observability components. This path is required for the next step.
...
thanos.yaml: |
   ...
   config:
     ...
     http_config:
       tls_config:
         ca_file: /etc/<s3-data>/certs/ca.crt <2>
...
----
 thanos.yaml: |
    ...
    config:
      ...
      http_config:
        tls_config:
          ca_file: /etc/<s3-data>/certs/ca.crt <1>
          cert_file: /etc/<s3-data>/certs/public.crt
          key_file: /etc/<s3-data>/certs/private.key
          insecure_skip_verify: false
----
