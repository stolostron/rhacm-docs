...
For more details, see _Replacing certificates for alertmanager route_ in the additional resources section.
...
----
 thanos.yaml: |
    ...
    config:
      ...
      http_config:
        tls_config:
          ca_file: /etc/<s3-mount-directory>/certs/ca.crt <1>
          cert_file: /etc/<s3-mount-directory>/certs/public.crt
          key_file: /etc/<s3-mount-directory>/certs/private.key
          insecure_skip_verify: false
----
