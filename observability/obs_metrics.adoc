...
You can monitor Platform metrics by creating a `ConfigMap` on the hub cluster in the `open-cluster-management-observability` namespace. Use `observability-metrics-custom-allowlist` as the name. Complete the following steps:

. *Optional:* Update the `names` parameter of the `metrics_list.yaml` section to add the names of the custom metrics you want to collect from your managed cluster.

. *Optional:* Update the `recording_rules` section by adding one value for the `expr` and `record` parameter pair to define the query expression.
+
See the following `ConfigMap` example that you can use to monitor Platform metrics:
...
----
*Note:* The name for the metrics you collect is the same name that you define in your managed cluster `record` parameter. 

. Run the query expression to get the metric value results. You can use one section or both sections, which applies to every cluster with monitoring enabled:
...
. If you only want to collect custom metrics from a single managed cluster, run the following command to create the `observability-metrics-custom-allowlist` config map:

+
[source,bash]
----
oc create -n observability-metrics-custom-allowlist
----
//command needs to be verified^


. Apply the ConfigMap on your managed cluster in the `open-cluster-management-addon-observability` namespace with the following command:

+
[source,bash]
----
-n open-cluster-management-observability -f observability-metrics-custom-allowlist.yaml
----
+
*Note:* Similar to the previous sample, the `names` parameter of the `metrics_list.yaml` section and the `recording_rules` section are optional fields. 
+
See the following `ConfigMap` example that you can use to monitor Platform metrics:
...
You can monitor User workload metrics by setting the configuration on the managed cluster in namespace where you want to capture the metric. The name must be `observability-metrics-custom-allowlist`. Complete the following steps:

. Add the namespace where you want to capture the metric in the `namespace` section of the `metadata` section.
. Add the value of the config map data in YAML format. Add list of metric names that you want to collect from the `test` namespace to the `names` section.

+
After you create the config map, the observability collector collects and pushes the metrics from the target namespace to the hub cluster.
