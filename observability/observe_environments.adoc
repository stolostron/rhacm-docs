...
**Important:** Avoid using the local storage operator for persistence. this can lead to data loss situations. The local storage on a node is not accessible by a pod if it is ever relaunched on a different node on a restart. The persistent volumes must be recoverable, especially for receive pods and rules pods if you want to avoid data loss.
//should we point the user to backup and restore topic? 
