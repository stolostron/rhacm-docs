...
**Important:** Avoid using the local storage operator for persistence. This can lead to data loss. The local storage on a node is not accessible by a pod if it is ever relaunched on a different node during restart. The persistent volumes for the `receive` and `rules` pods must be recoverable to avoid data loss.
