...
== Observability pod capacity requests
//add sizing column, request from issue 25916; Observability requirements give us the details for each observability workload expected resources but does not give the sizing for the nodes.

//The sizing documentation is not clear.
//The documentation mention capacity if installed on worker nodes but no in infra nodes. We are still not capable to estimate the CPU/RAM for ACM components in the infra nodes. MJ, 10/24
