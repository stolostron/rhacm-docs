...
- Create a custom alert rule. Create a config map named `observability-thanos-rule-custom-rules` in the `open-cluster-management-observability` namespace. You must name the key, `custom_rules.yaml`, as shown in the following example. You can create multiple rules in the configuration. 
...
- Create a custom recording rule within the `observability-thanos-rule-custom-rules` config map. Create a recording rule that gives you the ability to get the sum of the container memory cache of a pod. Your YAML might resemble the following content:
