...
To customize certificates for accessing the object store, complete the following steps:

. Create the secret with the `ca.crt` certificate. If you want to enable mutual TLS, you need to add the `public.crt` and `private.key` keys in the earlier secret. View the following example:
...
metricObjectStorage:
      ...
      tlsSecretMountPath: /etc/minio/certs <1>
----
<1> The `/etc/minio/certs/` is the path defined in `tlsSecretMountPath` parameter of the `MultiClusterObservability` custom resource.

. Update the `thanos.yaml` definition in the `thanos-object-storage` secret by adding the `http_config` section and add the certificate in the object store secret. View the following example:
