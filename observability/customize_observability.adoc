...
.. In the `rules` section, enter a value for the `expr` parameter to define the query expression. The metrics are collected as the names that are defined in the `record` parameter from your managed cluster. For more information, see the documentation for https://prometheus.io/docs/prometheus/latest/configuration/recording_rules/#recording-rules[recording rules]. 
