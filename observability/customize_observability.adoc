...
- Create a custom alert rule by creating a config map named `observability-thanos-rule-custom-rules` in the `open-cluster-management-observability` namespace.

 ** Add the alert in the `custom_rules.yaml` section as shown in the following example.
