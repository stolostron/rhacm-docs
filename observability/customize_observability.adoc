...
. Create an `AddOnDeploymentConfig` with the proxy settings by adding a `spec.proxy.Config`. View the following YAML example:
+
[source,yaml]
----
...
spec:
  ...
  proxyConfig:
    httpsProxy: "https://<username>:<password>@<ip>:<port>" <1>
    noProxy: ".cluster.local,.svc,172.30.0.1" <2> 
----
+
<1> Specify the same name for the HTTP and HTTPS proxy.
<2> Include the IP address of the `kube-apiserver`. To get the IP address, run following command on your managed cluster: `oc -n default describe svc kubernetes | grep IP:`. 
. Go to the `ManagedClusterAddOn` and reference the `AddOnDeploymentConfig` you made by adding an item in `spec.configs`. View the following YAML example: 
