...
When you disable the `metrics-collector`, the replica of the deployment is scaled to zero and all managed clusters are disabled. Update the `multicluster-observability-operator` resource by setting `enableMetrics` to `false`. Your updated resource might resemble the following change:
...
Disable the `metrics-collector` by removing observability components on specific managed clusters. Add the `observability: disabled` label to the `managedclusters.cluster.open-cluster-management.io` custom resource.
