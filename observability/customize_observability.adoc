...
Increase and decrease the persistent volume and persistent volume claims to change the amount of storage your storage class. Complete the following steps:
...
.. Look for the statefulsets or deployments of the desired component. Change their replica count to `0`. This initiates a shutdown, which involves uploading local data when applicable to avoid data loss. For example, Thanos receiver.

.. Delete the persistent volumes and persistent volume claims used by the desired component. 
//How would the entry look in the policy?
