...
You can configure the `alertmanager-config` YAML file to forward alerts from managed clusters to the Alertmanager in the hub cluster. For example, configure the `alertmanager-config` YAML to receive alerts in Alertmanager on the hub cluster. Complete the following steps to set up forwarding:
...
//are users directed to the OCP console once they select the managed cluster that they want to view? 
//should we instruct the users to select OCP from the Application icon? 
. To view and update the `alertmanager-config` YAML, select the *Applications* icon from the console header and click {ocp-short}.
. From the {ocp-short} menu navigation, select *Secrets*. Select the `alertmanager-config` secret to view the YAML file.
//Evaluation interval when a instance is stopped is about 1 minute; not sure if we need to mention this 
...
. Configure the `alertmanager-config` YAML file to send alerts to an external system. For example send alerts to Slack. Alerts are listed in a Slack message with the following information: `Alerts`, `Description`, and `Details`. 

. Select the *Grafana* button at the end of the Slack message to view the specified alert. The information for `Details` can also be located in the Grafana table of the alert.

. Use the Alertmanager CLI tool (`amtool`) to query alerts by features. Run the following command to query _severity=critical_:
+
----
amtool --alertmanager.url http://localhost:9093 alert query severity=critical
----

. If you do not want to receive specific alerts, silence the alert with the following command:
+
----
amtool silence --alertmanager.url http://localhost:9093 add alertname=etcdMembersDown -c "testing." dd400765-4ab3-49d4-837f-dd337b19f5f8
----

. Expire the silence feature from the alert with the following command:
+
----
amtool silence --alertmanager.url http://localhost:9093 silence expire dd400765-4ab3-49d4-837f-dd337b19f5f8
----

. Reload the instance used in your managed cluster to get an updated list of alerts sent to Slack. The alert listed is updated after two minutes. Your Slack message displays the following message with information previously mentioned:
+
----
[RESOLVED]etcdMembersDown (critcal)
Alerts:
Description:
Details:
----
//is the instance a node?
