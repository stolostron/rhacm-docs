...
.. In the _Metrics_ exploration bar, type in `ALERTS` and run the query. All the `ALERTS` that are currently in pending or firing state in the system are displayed.
