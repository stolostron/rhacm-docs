...
To verify that the alert rules are functioning appropriately, query `ALERTS` from the Grafana Explre page.
...
Then create a file named `observability-metrics-custom-allowlist.yaml` to add the name and recording rule of the custom metric to the `metrics_list.yaml` parameter. Your YAML for the ConfigMap might resemble the following content:
+
----
kind: ConfigMap
apiVersion: v1
...
Data from your custom metric is collected. If the updated metrics is used in the Grafana dashboard, see xref:../observability/design_grafana.adoc#designing-your-grafana-dashboard[Designing your Grafana dashboard] to update your dashboard.

[#removing-default-metrics]
== Removing default metrics

If you want data to not be collected for a specific metric, you can remove the metric from the `observability-metrics-custom-allowlist.yaml` file. When you remove a metric, you are also deleting the metric. As mentioned previously, first verify that `mco observability` is enabled.
...
Verify that your default metric is not being collected from your managed clusters by querying the metric from the Grafana dashboard. Data from your default metric is no longer being collected.

[#adding-advanced-config]
== Adding _advanced_ configuration
//Can this be complete from the console?
...
If your workload increases, increase the number of replicas of your observability pods. Navigate to the {ocp} console from your hub cluster. Locate the `multiclusterobservability` custom resource definition and update the `replicas` parameter value.
...
From your hub cluster, select the managed cluster that you want to forward alerts. Select the {ocp-short} link to access and update the `alertmanager-config` secret. *Note*: If you make changes to the `alertmanager-config` secret, the evaluation interval is about one minute.
...
View the data from your managed clusters by accessing Grafana from the hub cluster. You can query specific metrics and add filters for the query. 
...
You can view the etcd table from the hub cluster dashboard in Grafana. 

Select the Grafana link from your hub cluster to view the _etcd_ table data, which is collected from your hub cluster. The _Leader election changes_ across managed clusters are also displayed.
...
You can view the cluster fleet Kubernetes API service-level overview from the hub cluster dashboard in Grafana.
...
You can view the Kubernetes API service-level overview table from the hub cluster dashboard in Grafana. 
...
When you disable the `metrics-collector`, the deployment is scaled to zero and all managed clusters are disabled. Update the `multicluster-observability-operator` resource by setting `enableMetrics` to `false`. Your updated resource might resemble the following change:
...
You can disable the `metrics-collector` on specific managed clusters by adding the `observability: disabled` label to the `managedclusters.cluster.open-cluster-management.io` custom resource.
