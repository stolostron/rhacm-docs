...
Customize the obseravbiility configuration to specify how you want to manage and view cluster fleet data. You can also collect logs about new information that is created for observability resources with the `must-gather` command. For more information, see the _Must-gather_ section in the _Troubleshooting documentation_. 

Read the following sections to learn more about customizing,  and managing data that is collected by the observability service:
//are there any prerequisites that we want to list here? | MJ | 09/29/23
//I moved some topics to Using observability, need to confirm if changes make sense
