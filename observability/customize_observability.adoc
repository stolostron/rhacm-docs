...
When you disable the `metrics-collector` deployment is scaled to zero, and all managed clusters with the `observability: disabled` label are disabled. View the following options to disable the `metrics-collector`:
