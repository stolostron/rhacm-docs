...
----
.. In `names` section, add the name of the custom metrics which will be collect from managed cluster.
.. In `rules` section, follow the same meaning of Prometheus https://prometheus.io/docs/prometheus/latest/configuration/recording_rules/[recording rules]. `expr` defines the query expression, the metrics will be collected as name defined in `record` from managed cluster.
