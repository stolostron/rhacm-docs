...
** Recording rules provide you the ability to pre-calculate, or computate expensive expressions as needed. The results are saved as a new set of time series.
** Alerting rules provide you the ability to specify the alert conditions based on how an alert should be fired to an external service.
...
+
By default, the out-of-the-box alert rules are defined in the ConfigMap named `thanos-ruler-default-rules` in the `open-cluster-management-observability` namespace. 
+
...
+
For example, you can create a recording rule that will allow you to get the sum of the container memory cache of a pod:
+
----
data:
  custom_rules.yaml: |
...
*Note*: If this is the first new custom rule, it is created immediately. For changes to the ConfigMap, the configuration will automatically be reloaded. The configuration is reloaded due to the `config-reload` within the `observability-thanos-ruler` sidecar.
