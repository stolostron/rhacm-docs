...
----
<1> The supported parameter values are `fixed` and ... "Im not sure what other parameter value is supported. What does fixed mean?"
<2> Represents the list of managed clusters.
<3> Represents the namespaces of the managed clusters.
...
. Define separate `ClusterRoles` and `ClusterRoleBindings` resources to represent the different managed clusters in the namespaces. 
