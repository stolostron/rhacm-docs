...
+
*Note*: Avoid setting `CPU.limits` in the `ObservabilitySpec.resources` YAML section if observability is enabled on OpenShift SNO clusters that are included as managed clusters.
