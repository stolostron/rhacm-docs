...
You can configure the `multiclusterobservability` CR to forward alerts from managed clusters to the Alertmanager in the hub cluster. For example, configure the `multiclusterobservability` CR to receive alerts in Alertmanager on the hub cluster. Complete the following steps to set up forwarding:


//You can query different types of alerts
//How to silence an alert

//After alerts are recieved in the hub cluster Alertmanager you can continue to silence specific alert

//Confirgure an external system like slack fo
