...
You can monitor Platform metrics creating a `ConfigMap` on the hub cluster in the `open-cluster-management-observability` namespace called `observability-metrics-custom-allowlist`. See the following `ConfigMap` example:
...
<1> *Optional:* Add the name of the custom metrics you want to collect from your managed cluster.
<2> *Optional:* Add one value for the `expr` and `record` parameter pair to define the query expression.

The metrics are collected as the name that you define in your managed cluster `record` parameter. The metric values you get are the results after you run the query expression. You can use one section or both sections, which applies to every cluster with monitoring enabled.

If you want to use the configuration for a single cluster, use the following configuration on your managed cluster in the `open-cluster-management-addon-monitoring` namespace:
[source,yaml]
----
kind: ConfigMap
apiVersion: v1
