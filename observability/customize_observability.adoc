...
. View the _Fleet Overview_ status panel to see the total number of clusters that are exceeding or meeting the targeted _service-level objective_ value for the past seven or 30-day period.
. View the _Top Cluster_ table to view the topk clusters _service-level objective_ within the cluster fleet.
. Select an _offending_ or _non-offending_ cluster from the topk cluster table to drill down into the individual cluster view dashboard.
...
. View the _Error Budget for 7 Days_ or _Error Budget for 30 Days_ gauge panels to see the _error budget_ for the past seven or 30-day period for the _service-level objective_. To see the _downtime remaining_ within the period for the _service-level objective_, view the downtime status panels.
