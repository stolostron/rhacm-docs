...
----
+
<1> For this field, you can specify either a HTTP proxy, `http://:@:`, or a HTTPS proxy, `https://:@:`.
...
. Go to the `ManagedClusterAddOn` resource and reference the `AddOnDeploymentConfig` you made by adding an item in `spec.configs`. View the following YAML example: 
+
[source,yaml]
----
apiVersion: addon.open-cluster-management.io/v1alpha1
