...
The previous example monitors the User workload metric `sample_metrics` from the namespace `monitored_namespace`. If you create the configuration in the `open-cluster-management-addon-monitoring` namespace instead, the metrics are collected from all the namespaces of the managed cluster.
