...
Configure your custom `remoteWrite` configurations to your `PrometheusAgents` resources. Metrics are directly sent by the managed cluster to the external endpoint when you configure your `remoteWrite` configuration. Filter metrics by using relabeling rules. Enhance resiliency during network partitions between your managed and hub clusters for up to two hours.
