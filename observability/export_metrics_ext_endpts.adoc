...
= Exportng metrics to external endpoints for the multicluster observability add-on

Configure your custom `remoteWrite` configurations to your `PrometheusAgents` resources. Metrics are directly sent by the managed cluster to the external endpoint when you configure your `remoteWrite` configuration. Filter metrics by using relabeling rules. Enhance resiliency during network partitions between your managed and hub clusters for up to 2 hours.
...
. Add the `remoteWrite` configuration to the `PrometheusAgent` resource. Your `PrometheusAgent` resource might resemble the following YAML file, where the `up` metric is exported to a custom endpoint:
