...
+
*Note:* When you use the `platform-metircs-collector` label, the multicluster observability add-on automatically set the `scrapeClass` and `targets` parameters to enable federation from the in-cluster Prometheus platform.
//what is in-cluster? also asked about YAML snippet
...
//please help with completing this yaml example
+
[source,yaml]
----
