...
[role="_abstract"]
To configure an external metrics endpoint, add your custom `remoteWrite` specification to your `PrometheusAgent` resources. When you configure your `remoteWrite` specification, the managed cluster sends metrics are directly to the external endpoint.
