...
Create the `search-collector-config` ConfigMap to define which Kubernetes resources get collected from the cluster by referencing the allow and deny list within a ConfigMap. Your ConfigMap might resemble the following YAML file: 
