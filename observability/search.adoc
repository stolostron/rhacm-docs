...
* By default, there is a limit of 2500 images in the search drop-down menu. To increase the image limit, edit the `search-api` deployment by adding the `defaultImageQueryLimit` environment variable. View the following example:
+
[source,yaml]
----
name: defaultImageQueryLimit
 value: x <1>
----
<1> `x` represents the number of images that you want to be displayed from the search drop-down mneu.
