...
- <<additional-resources-alerts,Additional resources>>
...
//still working on this section|MJ|05/11/23
Suppress {product-title-short} alerts to ...Suppress alerts by deining an `inhibit_rule` parameter section. 

An inhibition rule mutes an alert by matching a set of parameter matches when an alert exists that matches another set of matchers. In order for the rule to take effect, both the target and source alerts must have the same label values for the label names in the `equal` list. An `inhibit_rule` might resemble the following:
