...
If you use alert silencers, you can migrate observability storage while retaining the silencers from its earlier state. To do this, migrate your {product-title-short} observability storage by creating new `StatefulSets` and `Persistent Volumes` (PV)s that use your chosen `StorageClass`. 
...
- *Observatorium or Thanos:* Receives data then uploads it to object storage. Some of its components store data in PVs. For this data, it usually automatically regenerates it on a startup, so there is no consequence if you lose this data.  
