...
If you use alert silencers, you can migrate observability storage while retaining the silencers from its earlier state. To do this, migrate your {product-title-short} observability storage by creating new `StatefulSets` and `Persistent Volumes` (PV)s that use your chosen `StorageClass`. This way, you have more control over where to store your data and how to use it during storage. 
