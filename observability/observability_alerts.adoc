...
If you use alert silences, you should migrate your {product-title-short} observability storage by creating new `StatefulSets` and `Persistent Volumes` that use your chosen `StorageClass`, to retain these silences. By creating new `StatefulSets` and PVs, you have more control over where your data is stored and how it is used in storage.

*Note:* The storage for PVs is different from the object storage used to store the metrics collected from your clusters.  

When you use `StatefulSets` and PVs to migrate your observability storage, it stores the following data components:

- *Observatorium:* Uploads its data to the object storage. Other component areas do not store data that can be lost; instead, they automatically regenerate what they need during a reboot. 
...
- *Metrics collector:* Deploys as deployments on the managed clusters. 
