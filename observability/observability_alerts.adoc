...
When you use alert silencers, you can migrate your {product-title-short} observability storage by creating new `StatefulSets` and `Persistent Volumes` to use your chosen `StorageClass`. By creating new `StatefulSets` and PVs, you have more control over where your data is stored and how it is used in storage.
