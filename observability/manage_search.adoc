...
** <<querying-argo-app,Querying ArgoCD applications>>
...
To collect the Kubernetes objects from the managed clusters, the `search-collector` process is run on all the managed clusters where search is enabled. The deployment ran is the `klusterlet-addon-search` deployment in the `open-cluster-management-agent-addon` namespace. A managed cluster with high number of resources might require more memory requirements for the `klusterlet-addon-search` deployment to function. 

You can tune the resource requirements for the deployment.
Resource requirements for the `klusterlet-addon-search`  deployment in a managed cluster can be specified in the `ManagedclusterAddon` custom resource. Edit the `ManagedclusterAddon` custom resource with the following command:
