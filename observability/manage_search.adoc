...
You can define which Kubernetes resources are collected from the cluster by creating a `search-collector-config` config map for each managed cluster where you want to customize the resources that search collects.

Place the config map in the same namespace where the search add-on is deployed. BThe default namespace is `open-cluster-management-agent-addon`.
