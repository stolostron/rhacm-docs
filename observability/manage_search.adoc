...
. *Technology Preview:* To enable global search to view your hub and managed cluster resources, add the `global-search-preview=true` annotation to the `search-v2-operator` resource. Run the following command:

+
[source,bash]
----
oc annotate search search-v2-operator n opencluster-management global-search-preview=true
----
