...
----
+
<1> Apply to a indexer, database, queryapi, or collector pod.
<2> You can control the log level verbosity for any of the previous four pods by adding the `--v=3` argument. See the following sample:
+
You can apply the following to any of the four search pods:
[source,yaml]
----
       indexer:
         replicaCount: 3
