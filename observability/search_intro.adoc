...
----
+
You can also add customizations to the search pod by adding `queryapi` after `resources`. You can control the log level verbosity the adding the `--v=3` argument. See the following sample:
+
[source,yaml]
----
    queryapi:
