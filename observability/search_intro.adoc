...
----
+
<1> Apply to a component, indexer, database, or similar. See the following example:
+
[source,yaml]
----
apiVersion: search.open-cluster-management.io/v1alpha1
...
----
+
<1> You can also add customizations to the search pod by adding `queryapi` after `resources`. You can control the log level verbosity by adding the `--v=3` argument. See the following sample:
+
[source,yaml]
----
    queryapi:
