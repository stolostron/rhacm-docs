...
- Environment variables for search pods:
+
Add environment variables for search pods by defining the variables in the `env` section of the `Search` custom resource:
+
[source,yaml]
----
apiVersion: search.open-cluster-management.io/v1alpha1
