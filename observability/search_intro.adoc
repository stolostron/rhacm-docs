...
- `search-collector`: Watches the Kubernetes resources, collects the resource metadata, and sends the collected data to the `search-indexer`. The `search-collector` also computes relationships for resources among the managed cluster and sends it to the `search-indexer`.
