...
----
+
<1> Apply to a indexer, database, queryapi or collector pod. See the following example:
+
[source,yaml]
----
      resources:
...
----
+
You can control the log level verbosity for any of the previous 4 pods by adding the `--v=3` argument. See the following sample:
+
[source,yaml]
----
      arguments:
