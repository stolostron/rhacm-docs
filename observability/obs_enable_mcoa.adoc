...
When you enable `platform` and `userWorkloads` specifications, the `MultiClusterObservability` operator removes the `endpoint-observability-operator` and `metrics-collector-deployment` resources, and deploys the `multicluster-observability-addon-manager` in the `open-cluster-management-observability` namespace.
...
[source,bash]
----
oc patch MultiClusterObservability observability -n open-cluster-management-observability --type=merge -p '{"spec":{"platform":{"enabled": true},"userWorkloads":{"enabled": true}}}'
----
+
...
[source,bash]
----
oc get clustermanagementaddon -n multicluster-observability-addon
----
//^need to verify all commands
