...
The observability service can be used to get a status and metrics for your hub cluster and managed clusters. When you enable observability, the `multicluster-observability-operator` pod is deployed by the `multiclusterhub-operator` automatically to each imported or created hub cluster.
