...
- <<obs-component-versions,Observability component versions>>
...
** *Technology Preview*: Use the `workers` parameter to increase the number of internal workers in the metric collector process. The internal workers shard `/federate` endpoint requests made to Prometheus on your managed cluster and send smaller,separate remote-write requests to Thanos on your hub cluster. 
...
//should there be a description of the metrics-collector here
