...
Thanos:: A toolkit of components that you can use to perform global querying across multiple Prometheus instances. For long-term storage of Prometheus data, persist it in any S3-compatible storage. You can also compose a highly-available and scalable metrics system. The Thanos compactor completes more `PUT` and `DELETE` operations to create blocks with a larger duration.
