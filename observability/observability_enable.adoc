...
//Get the default CA certificate to store the content of the key, `tls.crt`, OR Get the default CA certificate and store the content of the key, `tls.crt`,
* Get the default CA certificate to store the content of the key, `tls.crt`, in a local file. Run the following command:
+
----
