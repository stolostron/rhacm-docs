...
. Verify that the `multicluster-observability-operator` pod is being deployed by the `multiclusterhub-operator` deployment by viewing the `labels` section of the `multicluster-observability-operator` deployment. Run the following command to view the deployment:
+
[source,bash]
----
need command here
