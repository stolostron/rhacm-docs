...
** For Amazon S3 or S3 compatible, you can also use short term, limited-privilege credentials generated with AWS Security Token Service (AWS STS). Refer to link:https://docs.aws.amazon.com/STS/latest/APIReference/welcome.html[AWS Security Token Service documentation] for more details.
+
Generating access keys using AWS Security Service require additional steps, such as creating a IAM policy that limits access to a S3 bucket, creating a IAM role with a trust policy to generate JWT tokens for {ocp-short} service accounts, and specifying annotations for Observability service accounts that require access to S3 bucket. Below is an example of how observability on
//Adding view if we used bullet points, will receive feedback in peer review
+
Generating access keys using AWS Security Service require the following additional steps:

- Create an IAM policy that limits access to a S3 bucket
- Create an IAM role with a trust policy to generate JWT tokens for {ocp-short} service accounts, and 
- Specify annotations for the observability service accounts that require access to S3 bucket. You can find an example of how observability on Red Hat OpenShift Service on AWS (ROSA) cluster can be configured to work with AWS STS tokens in the _Set environment_ step. See link:https://www.rosaworkshop.io/[Red Hat OpenShift Service on AWS (ROSA)] for more details, along with link:https://www.rosaworkshop.io/rosa/15-sts_explained/[ROSA with STS explained] for an in-depth description of the requirements and setup to use STS tokens.

Complete the following steps to generate access keys using AWS Security Service:
...
. Create a policy for access to your S3 bucket. Run the following command:
+
[source,yaml]
----
...
. Create a trust policy. Run the following command:
+
[source,yaml]
----
