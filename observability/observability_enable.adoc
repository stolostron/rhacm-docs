...
spec:
  ...
  observabilityAddonSpec: # The ObservabilityAddonSpec defines the global settings for all managed clusters which have observability add-on enabled
    ...
    interval: 1m # Interval for the observability addon push metrics to hub server
    resources: #WHAT SHOULD BE THE DEFAULT VALUE HERE??
  advanced:
    retentionConfig:
      ...
      retentionInLocal: 24h # How long to retain raw samples on local storage
      retentionResolution1h: 0d # How long to retain samples of 1 hour in bucket
      retentionResolution5m: 180d # How long to retain samples of 5 minutes in bucket
      retentionResolutionRaw: 30d # How long to retain raw samples in bucket
    rbacQueryProxy:
    grafana:
    alertmanager:
    observatoriumAPI:
  ...
  status: #Status contains the different condition statuses for this deployment
----
+
