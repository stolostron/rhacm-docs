[#managing-observability-service]
= Managing the observability service

When you enable the observability component, you can use metrics and configure Grafana dashboards to make cluster resource information visible, help you save cost, and prevent unnecessary events.

Monitor the status of your managed clusters with the observability component (`multicluster-observability-operator`).
...
+
You can also access the {ocp-short} 3.11 Grafana dashboards with the following URL: `https://$ACM_URL/grafana/dashboards`. Select the folder named _OCP 3.11_ to view the {ocp-short} 3.11 dashboards.
+
...
. Verify that the `multicluster-observability-operator` pod is being deployed by the `multiclusterhub-operator` deployment by viewing the `labels` section of the `multicluster-observability-operator` deployment. Run the following command to get the `multicluster-observability-operator` deployment and display the labels associated with the resource:
+
[source,bash]
----
...
----
+
The `labels` section might contain the following details:
+
[source,yaml]
----
 labels:
