...
- When you import an {ocp-short} managed cluster into the hub cluster, {product-title-short} removes all the configuration settings in that {ocp-short} managed cluster `cluster-monitoring-config` configmap. After removing the configuration settings, {product-title-short} replaces it with the settings passed down from the hub cluster.  
