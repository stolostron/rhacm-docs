...
// Confirmation from dev needed: Can these steps be completed from the console? How so? 
...
. To validate that the observability service is enabled, launch the Grafana dashboards to make sure the data is populated. Click the Grafana link that is near the console header.
+
...
To enable observability from the {ocp} console, create a project named `open-cluster-management-observability`. Complete the steps in the console. Be sure to create an image pull-secret named, `multiclusterhub-operator-pull-secret` in the `open-cluster-management-observability` project.
