...
* https://docs.openshift.com/container-platform/4.8/storage/persistent_storage/persistent-storage-ocs.html[Red Hat OpenShift Container Storage]
...
Disable the `metrics-collector` by removing observability components on all managed clusters. This stops data from being collected and sent to the observability service on the {product-title-short} hub cluster. Add the following cluster label to your `MultiClusterObservability` YAML file: `observability: disabled`.
