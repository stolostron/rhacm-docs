...
Now all the alerts from the managed clusters will be forwarded to the {product-title-short} hub cluster

Now all the managed clusters connected to the {product-title-short} hub cluster are enabled to send alerts back to the {product-title-short} Observability service, so you can config the {product-title-short} Alertmanager to take care of deduplicating, grouping, and routing these alerts to the correct receiver integration such as email, PagerDuty, or OpsGenie and take care of silencing and inhibition of these alerts.
*Note*: The alerts forward to the {product-title-short} hub cluster feature is only supported by the managed cluster with the {ocp} version 4.8 or later.
