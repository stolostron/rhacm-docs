...
- When you configure the `MultiClusterObservability` to the `cluster-monitoring-config`, then the only fields that persist are the fields inside the `cluster-monitoring-config`. You lose all other fields. 
