...
+
*Note:* Alert forwarding to the {acm-short} hub cluster feature is only supported by managed clusters with {ocp} version 4.12 or later. After you install {acm-short} with observability enabled, alerts from {ocp-short} 4.12 and later are automatically forwarded to the hub cluster. See xref:../observability/customize_observability.adoc#forward-alerts[Forwarding alerts] to learn more.
