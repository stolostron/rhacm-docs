...
Monitor the status of your managed clusters with the observability component, also known as the `multicluster-observability-operator` pod. Enable the observability service by creating a `MultiClusterObservability` custom resource instance. After you enable the observability service, the following functions are initiated:
...
+
*Important:* If you modify the global pull secret for your cluster by using the {ocp-short} documentation, be sure to also update the global pull secret in the observability namespace.

. Create a secret object storage for your cloud provider. Your secret must contain the credentials to your storage solution. Run the following command to create a secret for the following cloud providers:
...
.. Create the following secret for Amazon S3 or S3 compatible:
...
.. Create the following secret for Google Cloud Platform:
...
.. Create the following secret for Microsoft Azure:
...
+
*Note:* If you use Microsoft Azure as an object storage for a {ocp} cluster, the storage account associated with the cluster is not supported. You must create a new storage account.

.. Create the following secret for Red Hat OpenShift Data Foundation:
...
.. Create the following secret for Red Hat OpenShift on IBM:
...
* link:https://docs.microsoft.com/en-us/azure/storage/blobs/storage-blobs-introduction[Microsoft Azure storage]
...
* link:https://www.ibm.com/docs/en/baw/20.x?topic=storage-preparing-cloud-public-roks[Red Hat OpenShift on IBM]
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/images/managing-images#images-update-global-pull-secret_using-image-pull-secrets[Updating the global pull secret]
* link:https://cloud.ibm.com/objectstorage/create[Cloud Object Storage]
* link:https://cloud.ibm.com/objectstorage/create[Cloud Object Store]
* link:https://cloud.ibm.com/docs/cloud-object-storage/iam?topic=cloud-object-storage-service-credentials[Service Credentials]
