...
*Note:* Regardless of the `cluster.open-cluster-management.io/backup` label value, secrets and config maps are restored when passive resources are restored. Since the `cluster-activation` value is ignored in this case, set the `cluster.open-cluster-management.io/backup: cluster-activation` value to the resources using these secrets and config maps instead. This way, they are not restored until the activation step.
