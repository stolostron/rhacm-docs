...
*Note:* For any resource in the managed cluster namespace, you must restore it at the cluster activation step. Therefore, if you need to add to the backup resource created in the managed cluster namespace, then use the `cluster-activation` value for the `cluster.open-cluster-management.io/backup` label. After you create the managed cluster, restore the managed cluster namespace. 

Because of the absence of the `managedCluster` custom resource (CR), the import controller can delete your namespace, even after you have restored it. If you restore the `managedCluster` CR, then the registration controller automatically creates a namespace. To understand the restore process, see the following: 

* If you restore the namespace, then the import controller deletes the namespace. 
* If you restore the `managedCluster` CR, then the registration controller creates the namespace. 
