...
You can use an {product-title-short} schedule to generate backup files and can create a subsection to store and organize these backup files. 
...
| _Resources backup_
| Contains one backup for the {product-title-short} resources, `acm-resources-schedule-<timestamp> backup` and one for generic resources, `acm-resources-generic-schedule-<timestamp>`. These resources use the backup label, `cluster.open-cluster-management.io/backup`.
