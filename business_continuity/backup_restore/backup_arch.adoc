...
*Note:* For any managed cluster namespace, or any resource in it, you must restore either one at the cluster activation step. Therefore, if you need to add to the backup resource created in the managed cluster namespace, then use the `cluster-activation` value for the `cluster.open-cluster-management.io/backup` label. 
...
* If you restore the `managedCluster` custom resource (CR), then the `cluster-manager-registration-controller` creates the namespace. 
