...
[#policies-installed-hub-cluster]
== _PolicySets_ and policies installed on the hub cluster 

When you enable the backup operator, a set of {acm} policies and ConfigMaps are installed on the hub cluster. When you disable the policy or remove it from the cluster, all the resources that the policy created are removed. 
...
Creates the namespace where OADP gets installed, if this namespace is not found. Copies the OADP secret and installs OADP, but only for managed clusters.Creates the `DataProtectionApplication` resource as defined by the `acm-virt-config` label, but only for the managed clusters. Does not create the `DataPotectionApplication` . The user must manually create it on the hub cluster. 
|===

The backup policy backs up the `kubevirt.io.VirtualMachine`  resources that has the label, `cluster.open-cluster-management.io/backup-vm: schedule_cron_name`. 
