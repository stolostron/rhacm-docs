...
* If you need to back up your managed cluster namespace or the resources under it, you must restore the namespace or resources after you create the managed cluster resources. When you restore your namespace or resources, set the label value to `cluster-activation`. 
* Secrets and config maps are restored when passive resources are restored, regardless of the `cluster.open-cluster-management.io/backup` label value. Because the `cluster-activation` value is ignored in this case, set the `cluster.open-cluster-management.io/backup: cluster-activation` value to the resources using these secrets and config maps instead. This way, the resources are not restored until the activation step.
