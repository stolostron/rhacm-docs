...
- Back up all custom resource definitions with an API version suffixed by `.open-cluster-management.io` and `.hive.openshift.io`. These suffixes indicate that all {product-title-short} resources are backed up.

- Back up all custom resource definitions from the following API groups: `argoproj.io`, `app.k8s.io`, `core.observatorium.io`, `hive.openshift.io`. These API groups are under the resources backup, and all the CRDs from the group, `agent-install.openshift.io`, are under the managed clusters backup. 
