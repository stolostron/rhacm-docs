...
- Resources from the `acm-resources-generic-schedule` backup which are labeled with `cluster.open-cluster-management.io/backup:cluster-activation` are being restored at cluster activation time. For more information on activation resources, see xref:../business_continuity/backup_restore/backup_restore.adoc#restore-activation-resources[Restoring activation resources].  
- Resources from the `acm-resources-generic-schedule` backup that are labeled with `cluster.open-cluster-management.io/backup:value` and where the label value is not `cluster-activation`, are restored when the passive resources are restored. For more information on restoring passive resource, see xref:../business_continuity/backup_restore/backup_restore.adoc#restore-passive-resources-check-backups[Restoring passive resources while checking for backups].  