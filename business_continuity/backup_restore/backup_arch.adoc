...
The `backupschedule.cluster.open-cluster-management.io` resource creates a set of four `schedule.velero.io` resources. These `schedule.velero.io` resources generate the backup files. 

To view the list of scheduled backup files, run the following command: `oc get schedules -A | grep acm`. 

See the following table to view the descriptions of these scheduled backup files: 
...
For more information on backup files, see the following resources: 

- Resources from the `acm-resources-generic-schedule- backup` which are labeled with `cluster.open-cluster-management.io/backup:cluster-activation` are being restored at cluster activation time. For more information on activation resources, see link:https://access.redhat.com/documentation/en-us/red_hat_advanced_cluster_management_for_kubernetes/2.8/html/business_continuity/business-cont-overview#restore-activation-resources[restoring activation resources].  
- Resources from the `acm-resources-generic-schedule-` backup which are labeled with `cluster.open-cluster-management.io/backup:value` where the label value is not `cluster-activation`, are restored when the passive resources are restored. For more information on restoring passive resource, see link:https://access.redhat.com/documentation/en-us/red_hat_advanced_cluster_management_for_kubernetes/2.8/html/business_continuity/business-cont-overview#restore-passive-resources-check-backups[restoring passive resources]. 