...
- Resources from the `acm-resources-generic-schedule` backup which are labeled with `cluster.open-cluster-management.io/backup:cluster-activation` are being restored at cluster activation time. For more information on activation resources, see xref:../business_continuity/backup_restore.adoc#restore-activation-resources[Restoring activation resources].  
