...
- Exclude all of the following resources that are a part of the included API groups, but are either not needed or are being recreated by owner resources that are also backed up: `backupschedule.cluster.open-cluster-management.io`, `restore.cluster.open-cluster-management.io`, `clusterclaim.cluster.open-cluster-management.io`, `discoveredcluster.discovery.open-cluster-management.io`.

- *Note:* If you modify `ClusterManagementAddon`, add the label, `cluster.open-cluster-management.io/backup` to trigger the backup. 
