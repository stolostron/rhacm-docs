...
- Back up all custom resource definitions with an API version suffixed by `.open-cluster-management.io`. This suffix indicates that all {acm-short} resources are backed up.
...
- Back up secrets and ConfigMaps with one of the following labels: `cluster.open-cluster-management.io/type`, `hive.openshift.io/secret-type`, or `cluster.open-cluster-management.io/backup`. 
...
== Backup files created by an {acm-short} schedule 

You can use an {acm-short} schedule to backup hub resources, which are grouped in separate backup files, based on the resource type or label annotations.
