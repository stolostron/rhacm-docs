...
If you want to run a disaster recovery test and do not want the primary hub cluster to stop during the test, keep the primary hub cluster active during the entire hub cluster restoration.

If you keep the primary hub cluster active while restoring the hub cluster data on a different hub cluster, the initial hub cluster can access the managed clusters. In this case, both hub clusters try to take control of the managed clusters. If the initial hub cluster takes control of the managed clusters, you might lose policy or application changes that you made. 
