...
When you restore data to a new hub cluster while your primary hub cluster is still active, both of these hubs try to manage the same cluster fleet. If the primary hub cluster takes control of the managed clusters, it could override the policy or application changes that you made to your new hub cluster. 
