...
You are able to keep your hub data safe even when you keep your primary hub cluster active during a restore process. When you restore data to a new hub cluster while your primary hub cluster is still active, both hub clusters try to manage the same cluster fleet. If the primary hub cluster takes control of the managed clusters, it might override the policy or application changes that you made to your new hub cluster. 
