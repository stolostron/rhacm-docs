...
After you install a custom OADP version, you can add the parameter, `startingCSV`.  Install a specific version of the OADP operator by setting the `startingCSV` property on the `installer.open-cluster-management.io/oadp-subscription-spec` annotation. After you enable the `cluster-backup` option on the `MultiClusterHub` resource, the OADP operator is installed with the version that is specified by the `startingCSV` property.
