 ...
 * If you try to manually install the OADP  Operator on the hub cluster, your OADP Operator and Velero custom resource definition (CRD) versions must exactly match the `MultiClusterHub` OADP Operator and Velero CRD versions. If these versions do not exactly match one another, you encounter problems. 
 * CRDs are cluster-scoped, so you cannot have two different versions of an OADP Operator or Velero CRD installed on the same cluster. If you have two different versions, one version runs with the wrong CRD. If you have previously installed and used the OADP Operator on the hub cluster in a namespace different from the backup component namespace, uninstall this version.
