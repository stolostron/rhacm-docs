...
. Edit the `MultiClusterHub` resource by setting the `cluster-backup` parameter to `true`. This paramater installs the OADP operator in the same namespace with the backup component.
