...
. Complete the steps for link:https://docs.redhat.com/documentation/en-us/openshift_container_platform/4.16/html/backup_and_restore/oadp-application-backup-and-restore#oadp-creating-default-secret_installing-oadp-aws[Creating a default Secret] for the cloud storage where the backups are saved. 
...
- For a list of supported storage providers for Velero, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/backup_and_restore/oadp-application-backup-and-restore#about-installing-oadp[About installing OADP].
...
[#installing-startingcsv]
=== Installing a _startingCSV_ on the OADP operator 

Install a specific version of the OADP operator by setting the `startingCSV` property on the `installer.open-cluster-management.io/oadp-subscription-spec` annotation. After you enable the `cluster-backup` option on the `MultiClusterHub` resource, the OADP operator is installed with the version that is specified by the `startingCSV` property.

Complete the following steps: 

. On your `MultiClusterHub` resource, set the following annotation, 
+
[source,yaml]
----
installer.open-cluster-management.io/oadp-subscription-spec: ' {"source": <my-mirror-catalog-source>,"startingCSV": "<1>"}
----
<1> The OADP operator version that you specified to be installed from the source.
+ 
. To verify that the OADP operator uses the `startingCSV` property, confirm that the `subscription` YAML for the OADP operator includes the `startingCSV` property. 
...
- Learn more about the link:https://docs.redhat.com/documentation/en-us/openshift_container_platform/4.16/html/backup_and_restore/oadp-application-backup-and-restore#oadp-installing-dpa_installing-oadp-aws[_DataProtectionApplication_] resource.
