...
- Starting with {product-title-short} 2.11, when you create the `MultiClusterHub` resource, the Velero custom resource definitions (CRDs) do not automatically install on the hub cluster. When you install the OADP operator, that is when the Velero CRDs install on the hub cluster. As a result, the Velero CRDs resources are no longer reconciled and updated by the `MultiClusterHub` resource. 
