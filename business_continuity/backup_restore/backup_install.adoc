...
. *Optional:* To install a specific OADP version of the operator, set your annotation. For example, if you want to install OADP version 1.5 of the operator, set the `startingCSV` property on the `installer.open-cluster-management.io/oadp-subscription-spec` annotation. Your `MultiClusterHub` resource might resemble the following example: 
