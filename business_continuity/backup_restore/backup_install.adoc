...
When you enable the backup option on the `MultiClusterHub`, the backup option remains in the `Pending` state until you install the OADP operator in the `open-cluster-management-backup` namespace. If you use the chart, the OADP operator is automatically installed. If you use the STS option, you must manually install the OADP operator. 
