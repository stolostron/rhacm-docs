...
- Even if you have not enabled the cluster backup and restore operator, the MultiClusterHub resource installs on the hub for the OADP Operator and Velero CRD resources. The MultiClusterHub reconciles the OADP and Velero CRD resources with the version used by the OADP Operator that was installed when the cluster backup and restore operator was enabled. For this reason, you cannot install another version of OADP or Velero on the cluster hub, unless you use the same CRD resources used by the OADP Operator that was installed when the backup and restore operator was enabled.

- The backup component works with OADP installed in the component namespace. 
