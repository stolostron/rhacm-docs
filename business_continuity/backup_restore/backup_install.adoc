...
. Edit the `MultiClusterHub` resource by setting the `cluster-backup` parameter to `true`. This parameter installs the OADP operator in the same namespace with the backup component.

=== For passive hub clusters only

. Before you run the restore operation on the passive hub cluster, manually configure the hub cluster and install all operators on the active hub cluster. 
. Ensure the operators are in the same namespace as the active hub cluster.
...
=== For created secrets when you create a `DataProtectionApplication` resource
