...
- If you have previously installed and used the OADP Operator on the hub cluster in a namespace different from the backup component namespace, uninstall this version. Custom resource definitons are cluster-scoped, so you cannot have two different versions of OADP or Velero installed on the same cluster. If you have two different versions, one version runs with the wrong custom resource definitions.

- The backup component works with OADP installed in the component namespace. Before installing the OADP Operator in the backup component namespace, you might need to manually delete the OADP and Velero custom resource definitions installed by previous versions of the OADP Operator.
...
- Learn more about the link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.13/html/backup_and_restore/application-backup-and-restore#oadp-installing-dpa_installing-oadp-aws[_DataProtectionApplication_] resource.
