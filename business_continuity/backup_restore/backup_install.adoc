 ...
 * If you try to manually install the OADP Operator on the hub cluster, your OADP Operator and Velero custom resource definition versions must exactly match the `MultiClusterHub` OADP Operator and Velero custom resource definition versions. If these versions do not exactly match one another, you encounter problems. 
