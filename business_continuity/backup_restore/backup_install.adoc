...
Install the OADP operator at a specific version by setting the `startingCSV` property on the `installer.open-cluster-management.io/oadp-subscription-spec` annotation. After you enable the `cluster-backup` option on the `MultiClusterHub` resource, the OADP operator is installed by using the version specified by the `startingCSV` property.
