...
. The restore hub cluster must use the same {product-title-short} version that the backup hub cluster uses. You cannot restore a backup on a hub cluster with a {product-title-short} version older than the one used by the backup hub cluster. If you want to run the restore operation on a hub cluster that has a {product-title-short} version newer than the cluster hub where the backup was created, complete the following:
.. On the restore hub, install the {product-title-short} operator at the same version as the {product-title-short} hub where the backup was created. 
.. Restore the date on the restore hub. 
.. On the restore hub, use the {product-title-short} upgrade operation to upgrade {product-title-short} to the version you want to use. 
