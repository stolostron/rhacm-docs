...
- If you did not enable the cluster backup and restore operator, the `MultiClusterHub` resource installs on the hub cluster for the OADP Operator and Velero Custom Resource Definition (CRD) resources. The `MultiClusterHub` reconciles the OADP and Velero CRD resources with the version that is used by the OADP Operator that was installed when you enabled the cluster backup and restore operator. You cannot install another version of OADP or Velero on the clusterhub, unless you use the same CRD resources that is used by the OADP Operator that was installed when you enabled the backup and restore operator.
