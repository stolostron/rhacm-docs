...
After you enable the `cluster-backup` option on the `MultiClusterHub` resource, you can set the `startingCSV` property to make it easier to store and transfer data across your hub clusters. 
...
[source,yaml]
----
installer.open-cluster-management.io/oadp-subscription-spec: ' {"source": <my-mirror-catalog-source>,"startingCSV": "<1>.v<2>"}
----
<1> Your OADP operator. 
<2> The OADP operator version that you are using.
+ 
