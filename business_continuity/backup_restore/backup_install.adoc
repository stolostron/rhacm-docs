...
* <<dr4hub-install-setting-up-hub-clusters-for-backup-and-restore-operators,Setting up hub clusters for backup and restore operators>>
* <<dr4hub-install-enabling-backup-restore,Enabling the backup and restore operator>>

[#dr4hub-install-setting-up-hub-clusters-for-backup-and-restore-operators]
...
To create a storeage location secret, complete the following steps: 
...
. Ensure that the {product-title-short} operator is installed in the same namespace as the initial hub cluster. Then create the `DataProtectionApplication` resource and connect to the same storage location where the initial hub cluster backed up the data. 
...
[#dr4hub-install-enabling-backup-restore]
