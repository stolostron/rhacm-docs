...
. To install the OADP 1.5 version of the operator, set your annotation. Set the `startingCSV` property on the `installer.open-cluster-management.io/oadp-subscription-spec` annotation. Your `MultiClusterHub` resource might resemble the following example: 
