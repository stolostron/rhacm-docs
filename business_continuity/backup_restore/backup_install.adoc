...
- If you did not enable the cluster backup and restore operator on the MCH resource, the OADP Operator and Velero Custom Resource Definition (CRD) still installs on the hub cluster. The `MultiClusterHub` reconciles the OADP and Velero CRD resources with the version that is used by the OADP Operator which is installed when you enabled the cluster backup and restore operator. As a result, you cannot install another version of OADP or Velero on the hub cluster, unless your version uses the same CRD definitions as the OADP Operator installed when you enabled the backup and restore operator.
