...
After installing Velero, the Velero pod is set to the default CPU and memory limits. If your cluster backs up a large number of resources, you might need to update the Velero resource requests and limits. If you do not update the values, the Velero pod might fail due to an out-of-memory error.
