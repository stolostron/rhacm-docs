...
[#dr4hub-install-oadp]
= OADP operator installation on the hub cluster

[role="_abstract"]
If the OADP operator is not automatically installed after you enable the backup and restore operator, manually install the OADP operator. To learn how to install OADP, see _OADP operator installation on the hub cluster_ in the {ocp-short} documentation.
...
- If you manually install the OADP operator, the custom resource definition version of the OADP operator and Velero must exactly match. If these versions do not exactly match one another, you get problems.
...
- For a list of supported storage providers for Velero, see _About installing OADP_ from the _Additional resources_ section.

.Additional resources

* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/backup_and_restore/oadp-application-backup-and-restore#installing-operator-oadp_installing-oadp-operator[Installing the OADP Operator]
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/backup_and_restore/oadp-application-backup-and-restore[About installing OADP]