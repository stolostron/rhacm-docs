...
View the following YAML examples to use the advanced options with the `velero.io.restore` spec option to filter out resources you want to restore.

* Use the following sample with the `velero.io.restore` spec options to restore resources only from the `vb-managed-cls-2` managed cluster namespace and to exclude the global `MultiCluster` resources:
...
* Set the following `velero.io.restore spec` options when you create the {acm}restore resource:
...
** When you set any of these Velero restore filters, set the field, `cleanupBeforeRestore`, to `None`. Doing this, you prevent having to clean up hub cluster resources that are part of the restored backup but are not being restored because of the applied filter.
