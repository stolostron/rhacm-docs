...
To restore all the data and to make sure that the hub cluster manages the managed clusters, use the following `restore-acm` sample: 
...
After you create the `restore.cluster.open-cluster-management.io` resource on the hub cluster, you can get the status of the restore operation by running the following command: 
