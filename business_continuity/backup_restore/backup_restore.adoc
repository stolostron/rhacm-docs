...
If you restore data back to the initial primary hub cluster, your current hub cluster and the primary hub cluster both can access the managed clusters. The hub clusters compete for control, making the managed clusters constantly go back and forth between the hub clusters. 
...
In a disaster recovery test scenario, use the primary hub cluster as the passive cluster. In a disaster recovery test, you only validate the restore operation on a passive hub, then use the primary hub cluster. In this scenario, you only test the hub backup and do not use the primary hub cluster to create new resources. Instead, the backup data has temporarily moved from the primary hub cluster to the passive hub cluster. You can make the initial primary hub cluster a primary hub cluster again if you restore all resources. The operation runs the post restore operation, moving the managed clusters back to this hub cluster.
