...
* Use the advanced options with `velero.io.restore` to filer out resources you want to restore. Use the following sample with `velero.io.restore` `spec` options to restore resources only from the `vb-managed-cis-2` managed cluster namespace and to exclude global `MultiCluster` resources: 
