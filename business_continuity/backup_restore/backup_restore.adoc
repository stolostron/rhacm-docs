...
The restore operation is used to restore all 3 backup types that are created by the backup operation. Yu can choose to install only a certain type of backup (only managed clusters, only user credentials, or only hub cluster resources).
...
Before you restore the activation data on the passive hub, shut down the previous hub cluster where the backup was created. If the primary hub is still running, it will try to reconnect with the managed clusters when the reconciliation procedure running on this hub determines that the managed clusters are no longer available.
...
- Return to <<restore-backup,Restoring a backup>>.