...
* <<advanced-restore-options,Using advanced restore options>>
...
Use the restore operation to restore all three backup types that the backup operation creates. Choose to install only a certain type of backup, such as only managed clusters, only user credentials, or only hub cluster resources.
...
When you need to restore backup data on your cluster, create a new cluster instead of restoring data with the primary hub cluster or the hub cluster with resources that are user-created.

If you restore data back to the initial primary hub cluster, your current hub cluster and the primary hub cluster both can access the managed clusters. Both the hub clusters try to get control, making the managed clusters constantly go back and forth between the hub clusters. 

If you restore data on a hub cluster with resources that are user created before the restore operation, the operation cannot include the resources. The data on the hub cluster does not contain all the available data. 

In a disaster recovery test scenario, use the primary hub cluster as the passive cluster. In a disaster recovery test, you only validate the restore operation on a passive hub, then use the primary hub cluster. 

You only test the hub backup cluster and do not use the primary hub cluster to create new resources. Instead, the backup data temporarily moves from the primary hub cluster to the passive hub cluster. You can make the initial primary hub cluster a primary hub cluster again if you restore all resources. The operation runs the post restore operation, moving the managed clusters back to this hub cluster.

*Important:* In the disaster recovery test, stop the restore cluster before moving back to the initial primary hub cluster. Stopping the restore cluster prevents the managed clusters from bouncing back after the restore operation moves the data to the initial primary hub cluster. 

[#prepare-new-hub]
== Preparing the new hub cluster 

When you need to restore backup data on a hub cluster, create a new hub cluster instead of using the primary hub cluster or a hub cluster where an earlier user created resources before the restore operation.

In a disaster recovery scenario, when you restore the backup to the passive hub cluster, you want a hub cluster identical to the hub cluster that failed during the disaster. You do not want any other user resources, such as: applications, policies, and extra managed clusters. 

When you restore the backup on this hub cluster, if you have managed clusters managed by this hub cluster, then the restore operation might detach them and delete their cluster resources. The restore operation does this to clean up the resources that are not part of the restored backup, making the content in this hub cluster identical with the initial hub cluster. In this case, you might lose user data because the managed cluster workloads get deleted by detach action.  
...
. On the restore hub cluster, install the operator at the same version as the hub cluster where you created the backup. 
. Restore the backup data on the restore hub cluster. 
. On the restore hub cluster, use the upgrade operation to upgrade to the operator version that you want to use. 
...
- See link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.17/html/backup_and_restore/oadp-application-backup-and-restore[_DataProtectionApplication_].
