...
If you want to restore the data on the same hub cluster where the backup was collected, you can recover the data from an earlier snapshot. In this case, both restore and backup operations run on the same hub cluster.
...
+
*Important:* Always use the `cleanupBeforeRestore = None` option to avoid losing restored resources that you filter out with the `excludedResource`, `excludedNamespaces`, or `excludedResources` restore options, or other filter options. If you do not use the `cleanupBeforeRestore = None` option, you delete the filtered out backup data on restore hub cluster.
+
