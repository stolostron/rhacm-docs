...
Use the restore operation to restore all three backup types that the backup operation creates. Decide to install only a certain type of backup, such as only managed clusters, only user credentials, or only hub cluster resources.
