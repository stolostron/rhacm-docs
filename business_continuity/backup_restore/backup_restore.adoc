...
+
*Important:* Always use the `cleanupBeforeRestore = None` option to avoid losing restored resources that you filter out with the restore options `excludedResource`, `excludedNamespaces`, `excludedResources`, or other options that filter out the restored resources. If you do not use the `cleanupBeforeRestore = None` option, you delete the filtered out backup data on restore hub.
+
