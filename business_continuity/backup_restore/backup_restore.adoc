...
To filter out resources you want to restore, use the advanced restore option, `velero.io.restore` specification.  

To restore resources only from the `vb-managed-cls-2` managed cluster namespace and to exclude the global `MultiCluster` resources, complete the following steps:

. Create a file with the following YAML sample: 
...
. Set the following `velero.io.restore` specification options when you create the {acm} restore resource:
...
. Set the  `cleanupBeforeRestore` field  to `None` to prevent having to clean up hub cluster resources that are part of the restored backup but are not being restored because of the applied filter. 
