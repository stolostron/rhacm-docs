...
When you need to restore backup data on your cluster, create a new cluster instead of restoring data with the primary hub cluster or the hub cluster with user created resources. 
...
If you restore data on a hub cluster with user resources created before the restore operation, the operation cannot include the resources. The data on the hub cluster does not contain all the available data. 

One situation where you can use the primary hub cluster as the passive cluster is in a disaster recovery test. A disaster recovery test is when you only validate the restore operation on a passive hub, then move back to use the primary hub cluster. In this recovery test scenario, you are only testing the hub backup scenario. You do not use the primary hub cluster to create new resources. Instead, the backup data has temporarily moved from the primary hub cluster to the passive hub cluster. You can make the initial primary hub cluster a primary hub cluster again if you restore all resources to this hub cluster. The operation runs the post restore operation, which moves the managed clusters back to this hub cluster.
