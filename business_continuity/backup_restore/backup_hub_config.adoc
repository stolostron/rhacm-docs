...
When you update the `active-passive` configuration, the update does not automatically include {acm} in the backup. To ensure the {acm-short} version gets backed up with the `active-passive` configuration, update both of them at the same time. 
