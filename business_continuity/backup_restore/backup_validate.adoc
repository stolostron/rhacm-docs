...
+
* When you enable the backup component on the `MultiClusterHub`, the cluster backup and restore operator Helm chart installs the OADP operator. The `OADP-channel` template checks if the installed Red Hat OADP Operator version matches the version set by the {prod-short} cluster backup and restore operator.
* The template shows violations if it finds an installed Red Hat OADP Operator on the hub cluster, but the Red Hat OADP Operator does not match the version installed by the {prod-short} cluster backup and restore operator Helm chart. The violation shows that the OADP and Velero Custom Resource Definitions (CRDs) are `cluster-scoped`. 
* In the following examples, the backup and restore operator could run with wrong CRDs resulting in erroneous behavior: 
** {prod-short} has many installed versions of OADP. 
** The OADP version installed by `MultiClusterHub` is uninstalled then manually installed.
