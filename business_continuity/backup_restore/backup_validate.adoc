...
If a cluster hub is managed by a global hub cluster, or if it is installed on a managed cluster instance, the `disableHubSelfManagement=true` setting is disabled. In this instance, you can enable the `backup-restore-enabled` policy. Enable the policy by setting the `is-hub=true` label on the `ManagedCluster` resource that represents the managed hub. 
