...
*Note:* A hub cluster is set to `automanage` by default. If you disable it by setting `disableHubSelfManagement=true`, the `backup-restore-enabled` policy is not placed on the hub cluster and does report.
