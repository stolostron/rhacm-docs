...
If a cluster hub is managed by a global hub cluster, or if it is installed on a managed cluster instance, the self managing option might be disabled by setting the  `disableHubSelfManagement=true`. In this instance, you can still enable the `backup-restore-enabled` policy on the cluster hub. Do this by setting the `is-hub=true` label on the `ManagedCluster` resource that represents the managed hub cluster. 
