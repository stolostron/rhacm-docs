...
** `oadp-channel-validation`: Use this temmplate to ensure the OADP operator versino in the `open-cluster-management-backu`p` namespace matches or exceeds the version set by the {product-title-short} backup and restore operator.
