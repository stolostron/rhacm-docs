...
+
* `acm-cluster-backup-enabled` template checks if the `cluster-backup` component is enabled in the MultiClusterHub resource. The template validates that the backup functionality has the correct configuration for the MultiClusterHub. If the `cluster-backup` component is disabled with the `enabled: false` flag in the `spec.overrides.components` section of the MultiClusterHub resource, the policy reports a `noncompliant` status with instructions to enable it.
