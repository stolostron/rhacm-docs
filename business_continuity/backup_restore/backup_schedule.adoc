...
. The administrator tests a disaster scenario by making Hub2 a primary hub cluster, without stopping Hub1 first, causing the following conditions: 
- Hub1 is still running.
- Hub1 backup data is restored on Hub2, including managed clusters backup. Therefore, Hub2 is now the active cluster.
- Since the `BackupSchedule.cluster.open-cluster-management.io` resource is still enabled on Hub1, it writes backups at the same storage location which corrupts the backup data. For example, any hub cluster restoring the latest backups from this location might use Hub1 data instead of Hub2 data. To avoid data corruption, Hub1 `BackupSchedule` resource status automatically changes to `BackupCollision`. In this scenario, to avoid getting into this backup collision state, stop Hub1 first or delete the `BackupSchedule.cluster.open-cluster-management.io` resource on Hub1 before restoring managed clusters data on  Hub2.
+
