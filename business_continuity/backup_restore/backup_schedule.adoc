...
<2> *Optional:* Deletes scheduled backups after 120h. If not specified, the maximum Velero default value of 720h is used. View the following descriptions of the `backupschedule.cluster.open-cluster-management.io` `spec` properties:
+
...
<3> *Optional:* Enables the automatic import feature in the restore operation when you set this parameter to `true`.
