...
As a result, the latest backups are generated by a hub cluster that is no longer set as the primary hub cluster. This hub cluster still produces backups because the `BackupSchedule.cluster.open-cluster-management.io` resource is still enabled, even though this hub cluster is not supposed to write backup data because it is not a primary hub cluster anymore.
...
. The administrator tests a disaster scenario by making Hub2 a primary hub cluster, which includes the following conditions occuring:
** Hub1 is stopped.
