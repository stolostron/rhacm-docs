...
- Ensure that you can run a restore operation for backup types by completing the xref:../backup_restore/backup_restore.adoc[Using the restore operation for backup types] process.
...
. To preserve the tenant ID for maintaining continuity in the metrics ingestion and querying, restore the `observatorium` resource to the backup hub cluster. Run the following command:
...
. Verify metrics collection from your managed clusters by accessing Grafana.
