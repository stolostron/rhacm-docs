...
. Verify that the `observability-controller` `managedclusteraddon` does not have a `Degraded` status, and that the progressing status is not set to `false`. Run the following command:
...
. Shut down the Observability service on your old hub cluster by removing the resources. Run the following command:
