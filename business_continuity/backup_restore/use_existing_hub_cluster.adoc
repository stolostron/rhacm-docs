...
When you restore data on a hub cluster that includes data that the user created, and the `cleanupBeforeRestore` option is set to `CleanupRestored` for the restore operation, that data is not cleaned up. This is because the data is created by the user and not by an earlier restore. 

Even when the user-created resources do not get cleaned up, you can still use an existing hub cluster for a restore hub cluster configuration. You can also make this hub cluster have the same content as the backup hub cluster. To do this, tag all user resources with the backup label, making it look like an earlier restore generated these resources. 
