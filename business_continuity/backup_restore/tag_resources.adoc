...
[role="_abstract"]
Tag user-created resources that you want to include in the hub cluster when you use an existing hub cluster for a restore hub cluster configuration. Apply the `velero.io/backup-name: backupName` label to your chosen resources.

When you run a hub restore operation and set the `cleanupBeforeRestore` option to `CleanupRestored` on the restore resource, user resources that are not part of your current restored backup are cleaned. After you tag the resources, you can use the hub cluster as a restore cluster. 
