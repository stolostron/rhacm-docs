...
=== Updating VolSync ManagedClusterAddOn

Depending on what version of {product-title-short} you use, you might need to update your VolySync version. To update VolSync `ManagedClusterAddOn`, complete the following steps: 
...
----
+
. Choose the `operator-subscription-channel` where you want VolySync deployed from. 

*Note:* In the this example, the `operator-subscription-channel` is `stable-0.9`. You get to choose the channel that you want. 
