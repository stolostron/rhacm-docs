...
* A configured {ocp} environment running a {acm-short} version 2.10 or later hub cluster

* At least two configured clusters that are managed by the same {acm-short} hub cluster
...
Depending on what version of {acm-short} you use, you might need to update your VolSync version. To update VolSync `ManagedClusterAddOn` resource, complete the following steps: 
