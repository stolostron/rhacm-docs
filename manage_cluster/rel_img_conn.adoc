...
. If the `acm-hive-openshift-releases` subscription is enabled, either remove the subscription or disable it by setting the value of `disableUpdateClusterImage` to `true`. You can remove the subscription by entering a command that is similar to the following command:
+
----
