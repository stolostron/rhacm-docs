...
. If the `acm-hive-openshift-releases` subscription is enabled, disable it by setting the value of `disableUpdateClusterImage` to `true`.
...
----
+
*Note:* If you have not disabled the automatic updates of the `ClusterImageSets` by setting the value of `disableUpdateClusterImage` to `true`, any images that you delete are recreated automatically.
