...
*Note:* The default setting for the hub cluster is to import itself and manage itself. You can change this setting by following the steps to change the `disableHubSelfManagement` value to `true`, as described in link:install_connected.adoc#installing-while-connected-online[Installing while connected online]. If the value is left at its default value of `false`, the hub cluster reimports itself the next time it reconciles the `MultiClusterHub` controller after you detach it.
