...
+
**Note:** If you import a Red Hat OpenShift Dedicated cluster and do not add a label for `vendor=OSD`, or if you add a label for `vendor=autodetect`, a `managed-by=platform` label is automatically added to the cluster to identify it as a Red Hat OpenShift Dedicated cluster. 
