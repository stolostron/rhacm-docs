...
**Note:** You must have cluster administrator access to all of the managed clusters that you add to the `ManagedClusterSet`, and to the namespace where the `ManagedClusterSet` is hosted. 
...
. Find the definition of the managed cluster in the `yaml` file. The entry resembles the following content:
+
----
