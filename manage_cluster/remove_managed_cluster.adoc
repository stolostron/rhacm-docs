...
When you detach or destroy a cluster that is managed by a {product-title-short} hub cluster, you must wait an additional 12 minutes after the cluster is removed from the list before reimporting the cluster to a hub cluster or creating a new managed cluster with the same name. This allows time for cleaning up the resources and deleting the namespace.
