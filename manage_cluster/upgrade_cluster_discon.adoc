...
. Add content that is similar to the following content to the custom policy to add the settings for your ImageContentSourcePolicy: 
+
----
...
spec:
  ...
  clusterSelector:
    matchExpressions:
      []  # selects all clusters if not specified
----
. Add the following content to the `policy`:`spec` in the _policy_ section:
+
----
apiVersion: config.openshift.io/v1
  kind: ClusterVersion
