...
View a sample of a certificate policy, see link:https://github.com/open-cluster-management/policy-collection/blob/master/stable/SC-System-and-Communications-Protection/policy-certificate.yaml[policy-certificate.yaml]. Refer to xref:../security/cert_policy_ctrl.adoc#certificate-policy-sample[Certificate policy controller] for more details.
