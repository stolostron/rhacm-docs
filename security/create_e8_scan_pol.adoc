...
+ 
The following resources are created: ScanSettingBinding, a ComplianceSuite (`compliance-suite-e8`), and a ComplianceCheckResult (`compliance-suite-e8-results`).
+
*Note:* Enforce is supported, however it is best to set the remediation action to `inform`. 
