...
. Update the policy controller to watch for `SamplePolicy` kind. Run the following command:
+
----
...
. Navigate to your OpenShift Container Platform cluster. Hover your cursor on the *Applications* icon > select *Red Hat OpenShift Container Platform*.
. From your OpenShift Container Platform cluster, navigate to the _Deployment_ page. From the navigation menu, select *Workload* > *Deployments*.
. Scale the `grc-policy-propagator` by selecting to increase or decrease the number of pods the deployment is applied to.
