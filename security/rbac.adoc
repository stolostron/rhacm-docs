...
| *cluster-admin*
| An user with cluster-wide binding to the `cluster-admin` role is an {ocp-short} super user, who has all access.
...
| *admin, edit, view*
| Admin, edit, and view are {ocp-short} default roles. An user with a namespace-scoped binding to these roles has access to `open-cluster-management` resources in a specific namespace, while cluster-wide binding to the same roles gives access to all the `open-cluster-management` resources cluster-wide.
|===
...
To perform cluster lifecycle operations, users must have access to both the `managedcluster` custom resource and namespace, for consistent role behavior from the console. Access to only the cluster's namespace and not the custom resource will cause  the  user to see the cluster but  perform any actions on the managed-cluster.
...
+
Learn about creating ManagedClusterSet resources to group clusters, see link:../managed_cluster/custom_resource.adoc[Creating ManagedClusterSets].
