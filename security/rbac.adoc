...
(Mikela : perhaps a note or bold text here to make it more specific that  user with role has ability to Create ManagedCluster)
...
(Notes to Mikela: A little subsection / bullet  here for creation  - If a subsection here is not inline with the other subsections here, then may be a important again here for creation)
* To create a  `managedcluster x`, the following roles are required:
** A cluster-wide binding to the role open-cluster-management:cluster-manager-admin
