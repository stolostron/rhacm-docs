...
To use the observability features: access managed cluster metrics, use the search component, use the Visual Web Terminal (VWT), a user must have access to the managed cluster. To create,update, and delete the MultiClusterObservability custom resource, the user must have the `cluster-admin` role or the `open-cluster-management:cluster-manager-admin` role. View the following RBAC table:
