...
To create and admin all managed clusters

* Create a cluster role binding to the cluster role `open-cluster-management:cluster-manager-admin`. This role is a super user, who has access to all resources and actions. This allows creating of cluster-scoped managedclusters, the namespace for the managedcluster, and resources in the namespace. This also allows access to provider connections and bare metal assets used to create managed clusters.
+
----
oc create clusterrolebinding (role-binding-name) --clusterrole=open-cluster-management:cluster-manager-admin
----

To admin a managed cluster named (cluster-name)

* Create a cluster role binding to the cluster role `open-cluster-management:admin:(cluster-name)`. This role allows read/write access to the cluster-scoped managedcluster resource. This is needed because the managedcluster is a cluster-scoped resource and not namespace-scoped.
+
----
oc create clusterrolebinding (role-binding-name) --clusterrole=open-cluster-management:admin:(cluster-name)
----

* Create a namespace role binding to the cluster role `admin`. This role allows read/write access to the resources in the namespace for the managed cluster.
+
----
...
* Create a cluster role binding to the cluster role `open-cluster-management:view:(cluster-name)`. This role allows read access to the cluster-scoped managedcluster resource. This is needed because the managedcluster is a cluster-scoped resource and not namespace-scoped.
+
----
oc create clusterrolebinding (role-binding-name) --clusterrole=open-cluster-management:view:(cluster-name)
----

* Create a namespace role binding to the cluster role `view`. This role allows read-only access the resources in the namespace for the managed cluster.
+
----
