...
To perform cluster lifecycle operations, users must have access to both the `managedcluster` custom resource and namespace, for consistent role behavior from the console. A user that has access to only the cluster namespace and not the custom resource can see the cluster, but is not permitted to perform any actions on the managed cluster.
...
To use the observability features: access managed cluster metrics, using search, visual web terminal, a user must have access to the managed cluster.
To create update and delete the MultiClusterObservability custom resource, the user must have the cluster admin role or the open-cluster-management:cluster-manager-admin role. View the following RBAC table: 
