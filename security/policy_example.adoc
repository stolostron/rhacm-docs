...
The parameter values are `enforce` and `inform`. If specified, the `remediationAction` value that is defined in the `policy-templates` section overrides the `remediationAction` in the child policy. For example, if `spec.remediationAction` value in the `policy-templates` section is set to `enforce`, then the `spec.remediationAction` in a configuration policy is set to `enforce` during runtime.
