...
See https://github.com/open-cluster-management/policy-collection/blob/main/stable/SI-System-and-Information-Integrity/policy-scc.yaml[`policy-scc.yaml`] to view the policy sample. To learn how to manage an SCC policy, see xref:../security/create_scc_policy.adoc#managing-security-context-constraints-policies[Managing Security Context Constraints policies] for more details.
