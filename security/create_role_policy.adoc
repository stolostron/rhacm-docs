...
See the `policy-role.yaml` sample from the link:https://github.com/open-cluster-management/policy-collection/tree/master/stable/AC-Access-Control[AC-Access-Control folder]. You can view a list of other configuration policy samples, see xref:../security/policy_sample_intro.adoc#policy-samples[Policy samples].
