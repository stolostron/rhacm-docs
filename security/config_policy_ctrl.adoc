...
See configuration policies samples that are supported by {product-title-short} from the link:https://github.com/open-cluster-management/policy-collection/blob/master/stable/SC-System-and-Communications-Protection/policy-certificate.yaml[CM-Configuration-Management folder]. Learn about how policies are applied on your hub cluster, see xref:../security/policy_sample_intro.adoc#policy-samples[Policy samples] for more details.
