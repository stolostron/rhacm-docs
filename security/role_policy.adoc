...
For more information on roles, see xref:../security/security_intro.adoc#role-based-access-control[Role-based access control]. View a sample of a role policy, see https://github.com/open-cluster-management/policy-collection/blob/master/stable/AC-Access-Control/policy-role.yaml[`policy-role.yaml`].
