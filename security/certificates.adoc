...
You can refresh all certificates that are issued by the {product-title-short} CA. During the refresh, the Kubernetes secret that is associated with each `cert-manager` certificate is deleted. The service restarts automatically to use the certificate. Run the following command:
