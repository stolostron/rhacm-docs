...
CA certificates can be associated with Git channel that are a part of the {product-title-short} application management. See link:../manage_applications/configuring_git_channel.adoc[Configuring application channel and subscription for a secure Git connection] for more details.

Helm channels allow you to disable certificate validation. Helm channels must be configured in development environments because disabling certificate validation introduces security risks.

//i decided not to reference the specific section bc there are no task in the section. Will ask Brandi to update her file to point the user to the doc that explains how to create a secret manage_applications/secret_sample.adoc
