...
Gatekeeper is a validating webhook that enforces CustomResourceDefinition (CRD) based policies that are run with the Open Policy Agent (OPA). You can install Gatekeeper to integrate a gatekeeper policy with {product-title}. You can use Gatekeeper to define specific constraints with ConstraintTemplates in your gatekeeper policy. 

Gatekeeper policy can be used to evaluate Kubernetes resource compliance. You can leverage an OPA as the policy engine, and use Rego as the policy language. The gatekeeper policy is created as a Kubernetes configuration policy.
