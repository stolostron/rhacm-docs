...
| Spec.NamespaceSelector| Required. Namespace where you want the policy to be applied to. Enter parameter values for `Include` and `Exclude`. **Note**: When you create multiple certificate policie and apply them to the same managed cluster, each policy `NamespaceSelector` must be assigned a different value.|
