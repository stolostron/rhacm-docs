...
|*nodePlacement* +
__required__|Enables explicit control over the scheduling of the add-on agents on the managed cluster. |<<_rhacm-docs_apis_clustermanagementaddon_jsonclustermanagementaddon_nodePlacement,nodePlacement>>
|===
...
|Name|Description|Schema
|*nodeSelector* +
__optional__|Defines which nodes the pods are scheduled to be enabled. If the selector is an empty list, the selector matches all nodes.|map[string]string
|*tolerations* +
__optional__|Connected by pods and used to list any taint that matches the triple <key,value,effect> using the matching operator <operator>.|[]corev1.Toleration
|===
