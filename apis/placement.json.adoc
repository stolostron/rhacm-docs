...
|**metadata** +
__required__|The metadata of the Placement. |object
|**spec** +
...
|**tolerationSeconds** +
__optional__|The length of time that a taint is tolerated, after which the taint is not tolerated. The default value is nil, which indicates that there is no time limit on how long the taint is tolerated. | int64
|===
