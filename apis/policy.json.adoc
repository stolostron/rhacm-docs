= Policy (policy.open-cluster-management.io/v1) API


[[_rhacm-docs_apis_policy_jsonoverview]]
== Overview
This documentation is for the Policy resource for Red Hat Advanced Cluster Management for Kubernetes. Policy resource has four possible requests: create, query, delete and update. 
// not sure if this last sentence applies
...
=== Create a policy
....
POST /policy.open-cluster-management.io/v1/namespaces/{namespace}/policy
....
