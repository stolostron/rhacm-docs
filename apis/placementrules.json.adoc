...
*Deprecated:* This API is deprecated. The _documentation_ for all remaining APIs is also deprecated. Use the _API Explorer_ in the console, or an `oc` command, to view current and supported APIs. 

[[_rhacm-docs_apis_placementrules_jsonoverview]]
== Overview
This documentation is for the PlacementRule resource for {acm}. The PlacementRule resource has four possible requests: create, query, delete and update.
