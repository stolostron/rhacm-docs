...
|*cpuArchitecture* +
__required__|Specify the type of software architecture to use for nodes that do not have an architecture defined. This parameter must be defined in the `ClusterInstanceSpec` and `NodeSpec` specifications. The architecture from the `ClusterInstanceSpec` is applied to nodes with undefined software architectures.

If the `cpuArchitecture` parameter is not defined at the `ClusterInstanceSpec` level, the default value is `x86_64`.|string 
|*cpuPartitioningMode* +
