...
|*cpuArchitecture* +
__required__|Specify the type of software architecture to use for nodes that do not have an architecture defined. For nodes with undefined software architectures, the architecture from the `ClusterInstanceSpec` is applied.
|*cpuPartitioningMode* +
