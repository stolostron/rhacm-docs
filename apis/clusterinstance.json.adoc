...
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/installing_an_on-premise_cluster_with_the_agent-based_installer/index[Installing an on-premise cluster using the {ai}]
...
|*cpuArchitecture* +
__optional__|Specify the type of software architecture to use for nodes that do not have an architecture defined. The following software architectures are supported: `multi`, `x86_64`, and `aarch64`.

If the `cpuArchitecture` parameter is not defined in the `ClusterInstanceSpec`, the default value is `x86_64`.|string 
|*cpuPartitioningMode* +
...
|*hostRef* +
_optional_|Specify a reference to a `BareMetalHost` node that is in a different namespace. Use `hostRef` to link a resource to a specific `BareMetalHost` instance.|
|===
...
|*cpuArchitecture* +
__optional__|Specify the type of software architecture to use for a specific node that does not have an architecture defined. The `x86_64` and `aarch64` software architectures are supported.



If the `cpuArchitecture` parameter is not defined for your nodes, the value in the `ClusterInstance` specification is used. The default value for the `ClusterInstance` specification is `x86_64`.|string
|*installerArgs* +
...
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/deploying_installer-provisioned_clusters_on_bare_metal/ipi-install-installation-workflow#bmc-addressing_ipi-install-installation-workflow[BMC addressing]