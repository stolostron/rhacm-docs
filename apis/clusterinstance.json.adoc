...
|*cpuArchitecture* +
__optional__|Specify the type of software architecture to use for nodes that do not have an architecture defined. The following software architectures are supported: `multi`, `x86_64`, and `aarch64`. The architecture from the `ClusterInstanceSpec` is applied to nodes with undefined software architectures.
...
|*cpuArchitecture* +
__optional__|Specify the type of software architecture to use for a specific node that does not have an architecture defined. The `x86_64` and `aarch64` software architectures are supported. The architecture from the `ClusterInstanceSpec` is applied to nodes with undefined software architectures.

If the `cpuArchitecture` parameter is not defined in `ClusterInstanceSpec`, the default value is `x86_64`.|string
|*installerArgs* +
...
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.17/html/deploying_installer-provisioned_clusters_on_bare_metal/ipi-install-installation-workflow#bmc-addressing_ipi-install-installation-workflow[BMC addressing]