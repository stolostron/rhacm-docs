...
1. Remove old _What's new_ entries so that the team can start over for the new release. Open a task for this.
2. Remove deprecation and removal items that are older than staged-N minus two (the squads help with this via the cross-squad issues that you open, you need a task for this, as well)
...
5. Update any other versions, such as channel, RHEL, etc.... Get guidance from a tech lead on this. Also change `must-gather` commands.
