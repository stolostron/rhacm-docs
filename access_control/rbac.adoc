...
* <<overview-of-roles,Overview of roles>> MCE only
* <<rbac-implementation,RBAC implementation>> MCE only
* <<cluster-lifecycle-RBAC,Cluster lifecycle RBAC>> MCE only (leave artifact)
...
| A user with the `open-cluster-management:managedclusterset:view:<managed_clusterset_name>` role has view access to the managed cluster resource called `<managed_clusterset_name>`. The user can bind `<managed_clusterset_name>` to a namespace. The user also has view access to `managedcluster.cluster.open-cluster-management.io`, `clusterclaim.hive.openshift.io`, `clusterdeployment.hive.openshift.io, and `clusterpool.hive.openshift.io resources, which have the following managed cluster set labels: `cluster.open-cluster-management.io`, `clusterset=<managed_clusterset_name>`. See link:../multicluster_engine/cluster_lifecycle/managedclustersets_intro.adoc#managedclustersets-intro[Creating and managing ManagedClusterSets] to learn how to manage the resource.
|===
