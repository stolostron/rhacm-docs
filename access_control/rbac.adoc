...
| *open-cluster-management:admmin:managed-cluster-<managed_cluster_name>*
| A user with cluster binding to the `managed-cluster-<managed_cluster_name>` role has administrator access to `managedcluster-<managed_cluster_name>` resource. 
//when a user has a managed cluster, this role is automatically created

| *open-cluster-management:view:managed-cluster-<managed_cluster_name>*
| A user with cluster-wide binding to the `managed-cluster-<managed_cluster_name>` role has view access to `managedcluster-<managed_cluster_name>` resource. The user also has administrator access to `managedcluster.cluster.open-cluster-management.io/clusterclaim.hive.openshit.io/clusterdeployment.hive.openshit.io/clusterpool.hive.openshit.io` resources, which has the managed cluster set label: `cluster.open-cluster-management.io/clusterset=managed-clusterset-<managed_clusterset_name>`.

//not sure if this is needed in the description-->This cluster role is created by the `server-foundation-controller` when a `managed-cluster` is created.
...
| A user with a cluster binding to the `managed-clusterset-<managed_clusterset_name>` role has administrator access to `managedclusterset-name` resource. This role also gives a user administrator access to all resources that are in the `managedclusterset-<managed_clusterset_name>` resource. The user also has administrator access to `managedcluster.cluster.open-cluster-management.io/clusterclaim.hive.openshit.io/clusterdeployment.hive.openshit.io/clusterpool.hive.openshit.io` resources, which has the managed cluster set label: `cluster.open-cluster-management.io/clusterset=managed-clusterset-<managed_clusterset_name>`.

| *open-cluster-management:managedclusterset:view:managed-clusterset-<managed_clusterset_name>*
| A user with cluster-wide binding to the `managed-clusterset-<managed_clusterset_name>` role has view access to `managedclusterset-<managed_clusterset_name>` resource. This role also gives the user view access to all resources that are in the `managedclusterset-<managed_clusterset_name>` resource. The user also has view access to `managedcluster.cluster.open-cluster-management.io/clusterclaim.hive.openshit.io/clusterdeployment.hive.openshit.io/clusterpool.hive.openshit.io` resources, which has the managed cluster set label: `cluster.open-cluster-management.io/clusterset=managed-clusterset-<managed_clusterset_name>`.
//if we agree with adding the managed cluster lifecycle, be sure to add reference to the section
//no need to move the content over, add a reference link to the ManagedClusterSets page
...
//Not creating managed cluster section, content is doc'ed already
