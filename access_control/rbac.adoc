...
To use the observability features, you must be assigned the `view` role. View the following list of observability features:

* Access managed cluster metrics.
+
Users are denied access to managed cluster metrics, if they are not assigned to the `view` role in the same namespace of the managed cluster.
