...
The {mce} is a prerequisite and the cluster lifecycle function of {product-title-short}. To manage RBAC for clusters with the {mce-short}, use the RBAC guidance from the cluster lifecycle xref:../multicluster_engine/about/mce_rbac.adoc/mce_rbac.adoc#mce-rbac[Role-based access control] documentation. 
