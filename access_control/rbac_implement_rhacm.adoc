...
{product} RBAC is validated at the console level and at the API level. Actions in the console can be enabled or disabled based on user access role permissions. 

The {mce} is a prerequisite and the cluster lifecycle function of {product}. To manage RBAC for clusters with the {mce}, use the RBAC guidance from the cluster lifecycle xref:../multicluster_engine/about/mce_rbac.adoc/mce_rbac.adoc#mce-rbac[Role-based access control] documentation. 

View the following sections for more information on RBAC for specific lifecycles for {product-short}:
