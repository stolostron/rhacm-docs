...
- To create a policy in the `rhacm-policies` namespace, create a namespace `RoleBinding` (e.g. `rhacm-edit-policy`) to the `rhacm-policies` namespace using the `ClusterRole` created previously. Run the following command:
+
----
