...
. On the RHACM cluster, run the following command to apply the file: 
...
. In the `open-cluster-management` namespace, edit the `advanced-cluster-management.v2.1.0` CSV. Run the following command to edit:
...
. Check that the new pods are running with the new docker image. Run the following command, then look for the new docker image:
