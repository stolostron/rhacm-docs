...
+
----
oc adm must-gather --image=registry.redhat.io/rhacm2/acm-must-gather-rhel8:v2.10 --dest-dir=<directory>
----
...
. Run the following commands to collect all of the information, replacing `<2.x>` with the supported version for both `<acm-must-gather>`, for example `2.10`, and `<multicluster-engine/must-gather>`, for example `2.4`.
+
----
