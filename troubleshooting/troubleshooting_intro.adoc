...
View troubleshooting topics for {acm-short} components. Before using the _Troubleshooting_ guides, you can run a `must-gather` command to gather details, logs, and take steps in debugging issues.

For `must-gather` information, see the following sections:

* xref:../troubleshooting/must_gather_acm.adoc#running-the-must-gather-command-to-troubleshoot[Running the must-gather command to troubleshoot {acm-short} components]
...
For more information about the `must-gather` command, see the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/support/gathering-cluster-data[Gathering data about your cluster] in the {ocp-short} documentation.
...
- xref:../troubleshooting/trouble_cluster_curator_fail.adoc#trouble-cluster-curator-template[Troubleshooting cluster curator automatic template failure to deploy]
...
- xref:../troubleshooting/must_gather_global_hub.adoc#global-hub-must-gather[Troubleshooting with the _must-gather_ command]

*Application management*

To view the main documentation about application management, see link:../applications/app_management_overview.adoc#managing-applications[Manage applications].
...
Console observability includes _Search_, along with header and navigation function. To view the Observability guide, see link:../observability/observe_environments_intro.adoc#observing-environments-intro[Observability service].
...
- link:../troubleshooting/trouble_submariner_degraded.adoc#trouble-submariner-degraded[Troubleshooting Submariner add-on status is degraded]

- xref:../troubleshooting/trouble_submariner_tests.adoc#trouble-submariner-test-failure[Troubleshooting Submariner end-to-end test failures]

*Secure cluster*

- xref:../troubleshooting/trouble_multicluster_roleassign.adoc#troubleshooting-multicluster-roleassign[Troubleshooting _MultiClusterRoleAssignment_ size limits]
- xref:../troubleshooting/trouble_multicluster_roleassign_upgrade.adoc#troubleshooting-multicluster-roleassign-upgrade[Troubleshooting _MultiClusterRoleAssignment_ resources after upgrade]
