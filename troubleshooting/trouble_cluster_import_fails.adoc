...
After you complete the procedure for importing a cluster, you cannot access it from the {acm} console.
...
. On the {acm-short} hub cluster, run the following command to ensure that the {acm-short} import controller is running. 
+
----
...
. On the {acm-short} hub cluster, run the following command to determine if the managed cluster import secret was generated successfully by the {acm-short} import controller:
+
----
...
. On the {acm-short} hub cluster, if your managed cluster is the `local-cluster`, which is provisioned by Hive, or has an auto-import secret, run the following command to check the import status of the managed cluster.
+
----
