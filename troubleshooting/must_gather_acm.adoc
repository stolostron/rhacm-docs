...
. Learn about the `must-gather` command and install the prerequisites that you need at link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.17/html-single/support/index#gathering-cluster-data[Gathering data about your cluster] in the {ocp} documentation.
...
. Run the following commands to collect all of the information, replacing `<2.x>` with the supported version for both `<acm-must-gather>`, for example `2.13`, and `<multicluster-engine/must-gather>`, for example `2.8`.
