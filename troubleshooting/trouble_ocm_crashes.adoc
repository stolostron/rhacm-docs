...
= Troubleshooting ocm-controller errors after {acm-short} upgrade
...
== Symptom: Troubleshooting ocm-controller errors after {acm-short} upgrade 
...
== Resolving the problem: Troubleshooting ocm-controller errors after {acm-short} upgrade
...
. Restore the `cluster-manager` back to {acm-short} {product-version}. It might take several minutes. Run the following command:
+
[source,bash]
----
...
To verify that {acm-short} is recovered run the following commands:
