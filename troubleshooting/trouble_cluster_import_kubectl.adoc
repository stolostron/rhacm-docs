...
+
. Download the latest version of the `kubectl` tool from link:https://kubernetes.io/docs/tasks/tools/install-kubectl/[Install and Set Up kubectl] in the Kubernetes documentation.
. Import the cluster again after upgrading your `kubectl` tool.

* Run a file that contains the import command.
+
. Start the procedure in link:../clusters/cluster_lifecycle/import_cli.adoc#importing-a-managed-cluster-with-the-cli[Importing a managed cluster with the CLI].
