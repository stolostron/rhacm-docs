...
The Thanos compactor halts because there is no space left on your persistent volume claim. You receive the following message:
...
To resolve the problem, increase the storage space of the Thanos compactor persistent volume claim. Complete the following steps:
...
. To check the storage for your persistent volum claims of the `data-observability-thanos-store-shard-` pod, run the following command:
...
. Restart the `observability-thanos-compact` pod by scaling the pod to `0`. The pod is automatically scaled to `1`.
//is there a command to run for the user to edit this scale?

. When the Thanos compactor is healthy, the `acm_thanos_compact_todo_compactions` metric is low, and you can attempt to decrease the persistent volume claim size.
