...
If this is the case, the worker nodes resources are not sufficient in the cluster to run the product.

[#resolving-the-problem-adjust-sizing]
== Resolving the problem: Adjust worker node sizing

If you have this problem, then your cluster needs to be updated with either larger or more worker nodes. See link:../install/installing#sizing-your-cluster[Sizing your cluster] for guidelines on sizing your cluster.