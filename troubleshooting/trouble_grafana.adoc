...
If you hit the `Gateway Time-out` error when you query some time-consuming metrics in the Grafana explorer, it is possible that the timeout is caused by the Grafana in the `open-cluster-management` namespace.
...
queryTimeout: 300s
----
. If you verified the default configuration of Grafana has expected timeout settings, then you can configure the Grafans in the `open-cluster-management` namespace by running the following command:
+
----
oc annotate route grafana -n open-cluster-management --overwrite haproxy.router.openshift.io/timeout=300s
----
