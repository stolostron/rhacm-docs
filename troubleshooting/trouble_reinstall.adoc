...
If your pods do not start after you install {product-title}, it is likely that {product-title-short} was previously installed, and not all of the pieces were removed before you attempted this installation. 
...
. Ensure that your {ocp} CLI is configured to run `oc` commands. See https://docs.openshift.com/container-platform/4.5/cli_reference/openshift_cli/getting-started-cli.html[Getting started with the CLI] in the Red Hat OpenShift documentation for more information about how to configure the `oc` commands. 
