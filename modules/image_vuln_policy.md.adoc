...
The image vulnerability policy is checked by the Configuration policy controller.
...
See xref:managing-an-image-vulnerability-policy[Managing an image vulnerability policy] for more information.
View other configuration policies on the xref:kubernetes-configuration-policy-controller[Kubernetes configuration policy controller].
