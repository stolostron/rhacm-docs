...
See xref:setting-up-a-provider-connection-for-microsoft-azure[Setting up a provider connection for Microsoft Azure] for more information.
...
If you do not have one configured, or want to configure a new one, see xref:creating-a-provider-connection-on-azure[Creating a provider connection on Azure].
