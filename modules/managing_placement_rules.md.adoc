...
NOTE: The custom resource definition (CRD) and controller for placement rules replaces the placement policies that were used in previous versions of Red Hat Advanced Cluster Management for Kubernetes for deployables.
...
For more information about the YAML structure, including the required fields, see <<placement-rule-definition,Placement rule definition>>.
