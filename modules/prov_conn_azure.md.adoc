...
You can create a cluster that uses this provider connection by completing the steps in xref:creating-a-cluster-with-red-hat-advanced-cluster-management-for-kubernetes-on-microsoft-azure[Creating a cluster on Microsoft Azure].
