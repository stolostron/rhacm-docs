...
Red Hat Advanced Cluster Management for Kubernetes is tested to determine some scalability and performance data.
...
IMPORTANT: This data is based on the results from a lab environment at the time of testing.
...
The memory required is proportional to the number of resources (ConfigMaps, Deployments, Pods, etc) and their relationships in the cluster.
...
Table: Physical memory data By default, we deploy the datastore with a memory limit of 1Gi.
If you are managing larger clusters, you may need to increase this limit by editing the deployment named `search-prod-xxxxx-redisgraph` in the open-cluster-management namespace.
