...
NOTE: You must create a placement policy and placement binding policy to apply your policies on to your cluster.
For more information, see link:../security[Red Hat Advanced Cluster Management for Kubernetes policy example].
     ...
     object-templates:
       - complianceType: musthave
         objectDefinition:
           ...
           metadata:
             name: open-cluster-management-policies-limit-range
           spec:
             limits:
             - default:
                 memory: 512Mi
...
----
   ...
   metadata:
     ...
     annotations:
       ...
       policy.open-cluster-management.com/categories: DE.CM Security Continuous Monitoring
       policy.open-cluster-management.com/controls: DE.CM-8 Vulnerability scans
   spec:
     complianceType: musthave
     remediationAction: inform
     disabled: false
...
For more information on roles, see link:../security_intro.md.adoc[Role-based access control (RBAC)].
...
----
   ...
   spec:
     ...
     namespaces:
       ...
       include: ["default"]
     role-templates:
       - apiVersion: policy.open-cluster-management-policies # role must follow defined permissions
         metadata:
...
----
   ...
   - name: policy-scc
     kind: Policy
     apiGroup: policy.open-cluster-management.com
   ---
   apiVersion: apps.open-cluster-management.io/v1
   kind: PlacementBinding
   metadata:
...
. From the navigation menu, click *Govern risk*.
. Click *Create policy*.
. Select *SecurityContextConstraintPolicy* from the _Specifications_ field.

See link:../security[Red Hat Advanced Cluster Management for Kubernetes Governance and risk] for more information about policies.
