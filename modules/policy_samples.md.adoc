...
For more information, see xref:../governance/policy_example.adoc[Red Hat Advanced Cluster Management for Kubernetes policy example].

[#memory-usage-policy]
== Memory usage policy

Apply the limit range policy to limit your memory usage.
...
For more information on roles, see xref:../governance/security.adoc[Role-based access control (RBAC)].
