...
You can create a cluster that uses this provider connection by completing the steps in xref:creating-a-cluster-with-red-hat-advanced-cluster-management-for-kubernetes-on-amazon-web-services[Creating a cluster on Amazon Web Services].
