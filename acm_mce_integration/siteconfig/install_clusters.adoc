...
* You have the default cluster templates. To get familiar with the default templates, see link::../../acm_mce_integration/siteconfig/cluster_templates.adoc#default-templates[Default set of templates]
//link
* Install and configure the underlying operator of your choice. To install either the Image Based Install operator or the {ai}, see _Image-based installations for {sno}_ or _Installing an on-premise cluster using the {ai}.
