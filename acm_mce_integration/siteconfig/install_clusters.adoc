...
* If using {ztp}, configure your {ztp} environment. For more information, see link::https://docs.redhat.com/en/documentation/openshift_container_platform/4.17/html/edge_computing/ztp-preparing-the-hub-cluster[Preparing the hub cluster for GitOps ZTP].
* You have the default cluster templates.
//link
* Install and configure the underlying operator of your choice. For more information, see _Image-based installations for {sno}_ or _Installing an on-premise cluster using the {ai-full}.
