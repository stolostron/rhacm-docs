...
- *Required access:* What's the required access level? 
- You must enable the ManagedServiceAccount add-on. See link:../../clusters/install_upgrade/adv_config_install.adoc#serviceaccount-addon-intro[ManagedServiceAccount add-on].
...
[source,bash]
----
console-config -n open-cluster-management -p '{"data": {"VIRTUAL_MACHINE_ACTIONS": "enabled"}}'
----

. To run the actions, create and configure a `ManagedServiceAccount` resource for each managed cluster. Save the following YAML file:
...
. Apply the `ManagedServiceAccount` resource to your managed cluster by running the following command:
...
[source,bash]
----
"Virtual Machine tech preview cleanup complete."
----
