...
Use the `pruneObjectBehavior` parameter to clean up resources that are creted by policies. 


//As I wait for the playback recording or feedback from dev
//Can you enable/disable this feature?
//How is it displayed from the console?
//are there any notes that need to be added about this? 
//do all policies support this feature? 

If a policy that installs an operator with the `pruneObjectBehavior` field deleted, additional clean up is needed to complete the operator uninstall. Additional clean up might include deleting the operator `ClusterServiceVersion` object.
