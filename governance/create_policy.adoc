...
- As you disable the `config-policy-addon` and the Kubernetes configuration policy custom resource is deleted on the managed cluster, you must first remove the resources from the managed cluster. 
