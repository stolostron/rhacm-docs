...
Be sure to change the `PlacementRule` to specify the clusters where the policies need to be applied by `clusterNames`. View link:../applications/placement_sample.adoc#placement-rule-samples[Placement rule samples overview]
+
...
*Note:* If a policy that installs an operator with the `pruneObjectBehavior` parameter undefined, additional clean up is needed to complete the operator uninstall. Additional clean up might include deleting the operator `ClusterServiceVersion` object.
