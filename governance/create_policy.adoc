...
View more descriptions of the policy YAML files in the xref:../governance/policy_intro.adoc#policy-overview[Policy overview].
...
- If a policy that installs an operator uses the `pruneObjectBehavior` parameter defined, then additional clean up is needed to complete the operator uninstall. Additional clean up might include deleting the operator `ClusterServiceVersion` object.

- As you disable the `ManagedClusterAddOn` and the Kubernetes configuration policy custom resource is deleted on the managed cluster, you must first remove the resources from the managed cluster. 
