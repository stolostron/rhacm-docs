...
For both root and propagated policies, a metric of `0` is displayed if the policy is compliant, and `1` if it is non-compliant.
...
*Note*: If the metrics are not needed, and there are any concerns about performance or security, this feature can be disabled. Set the `DISABLE_REPORT_METRICS` environment variable to `true` in the propagator deployment.

//is there a yaml example that can be displayed? 
// Do we want to reference the user to the observability doc?
//is it appropriate or accurate to mention the policy_governanace_info metric in the observability doc?