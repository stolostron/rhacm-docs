...
- If you create a Policy dependency with a period `.` in the name, you must also add the namespace to the start of the name in the dependency section and follow with a period. For example, a Policy in the `default` namespace that is named, `my-4.16-policy` must be written as it is in the following example: `default.my-4.16-policy`.
