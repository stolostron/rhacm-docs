...
`dryrun`:: The `dryrun` is a subcommand for `policytools` that runs a `ConfigurationPolicy` resource locally by using static input resources for the cluster state.
