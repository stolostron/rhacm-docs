...
With the `policytools` command line, you can interact with policies locally to help with creating and debugging.  The `policytools` command line is available for download from the hub cluster console. See link:../console/console.adoc#command-line-tools[Command line tools].
...
+
The `template-resolver` connects to the managed cluster and hub cluster to retrieve information that is necessary to resolve the templates. Provide paths by using the `--save-resources` and `--save-hub-resources` arguments to view the resources that the `template-resolver` retrievs. Use the `template-resolver` files as inputs for the `dryrun` subcommand.

`dryrun`:: The `dryrun` is a subcommand for `policytools` that runs a `ConfigurationPolicy` locally by using static input resources for the cluster state.

+
To execute a policy, you must add the path to the `ConfigurationPolicy` YAML by using the `--policy` flag. You can provide additional resource YAML files, which are used as the current cluster state. For example, if your policy defines a specific configuration for a namespace, you provide the current state of the namespace to see what the policy does. Multiple resources can be in the same resource file.

+
By default, `dryrun` prints the resource diffs for any objects defined in the policu and the compliance messages that the `ConfigurationPolicy` releases. To save the compliance messages to a separate file, use the `--messages-path` argument. To save the full policy status, use the `--status-path` argument.
