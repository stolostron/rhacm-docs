...
+
By default, `dryrun` prints the resource diffs for any objects defined in the policy and the compliance messages that the `ConfigurationPolicy` releases. To save the compliance messages to a separate file, use the `--messages-path` argument. To save the full policy status, use the `--status-path` argument.
