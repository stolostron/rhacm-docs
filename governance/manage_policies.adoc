...
+
To resolve a policy with hub cluster templates, you must provide the `--cluster-name` argument with the name of a managed cluster that is imported into {acm-short}, and you must provide the `--hub-kubeconfig` argument with the path to a `kubeconfig` file that points to the hub cluster.

The `policytools` CLI is available for download from the hub cluster console. See link:../console/console.adoc#command-line-tools[Command line tools].
