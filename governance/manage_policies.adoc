...
+
*Technology Preview:* The `policytools` command-line interface introduces a `--lint` argument for the `template-resolver` subcommand to print any linting issues, along with the resolved Go templates from the input policy. To generate a SARIF report of the linter results, which may be used by other tools, provide a path with the `--sarif` argument.
