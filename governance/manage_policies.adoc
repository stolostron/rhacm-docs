...
+
To evaluate a policy against the current state of a live cluster, use the `--from-cluster` flag or set the `DRYRUN_FROM_CLUSTER` environment variable to `true`. When this option is enabled, the `dryrun` subcommand reads the current state of resources directly from the cluster configured in your kubeconfig (or specified by the `KUBECONFIG` environment variable), rather than from input files. Any YAML resource files provided as input are ignored when `--from-cluster` is enabled. This allows you to test how a policy would behave against your actual cluster state without needing to manually export and maintain resource files.
