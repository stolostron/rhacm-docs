...
See a sample of the link:https://github.com/open-cluster-management-io/policy-collection/blob/main/stable/CM-Configuration-Management/policy-compliance-operator-e8-scan.yaml[`policy-compliance-operator-e8-scan.yaml`]. See xref:../governance/create_policy.adoc#managing-security-policies[Managing security policies] for more information. *Note:* After your E8 policy is deleted, it is removed from your target cluster or clusters.
