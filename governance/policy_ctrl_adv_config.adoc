...
When you configure and collect debug logs for each policy controller, the default log level might take up more space than you want. In {product-title-short} 2.9, you can reduce the volume of debug logs. You can reduce the volume below the default setting. 

*Note:* Reducing the volume of debug logs can make issues more difficult to debug. 

You can reduce the debug logs emitted by the policy controllers to be `error-only`. To reduce the debug logs, set the debug log value to `-1` in the annotation.
