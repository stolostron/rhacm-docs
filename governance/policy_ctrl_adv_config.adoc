...
- <<configure-gov-framework,Configure the governance framework>>
...
== Configure the governance framework
//need to confirm accuracy of this statment
...
You can configure the rate and burst by setting the `client-qps` and `client-burst` annotations with non-zero integers within quotation marks. Set the value on the `ManagedClusterAddOn` object name to `config-policy-controller` in the managed cluster namespace of the hub cluster.
