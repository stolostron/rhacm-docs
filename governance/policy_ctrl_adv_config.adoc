...
You can configure the rate and burst by setting the `client-qps` and `client-burst` annotations with non-zero integers within quotation marks. You can set the value on the `ManagedClusterAddOn` onbject name, `config-policy-controller`, in the managed cluster namespace of the hub cluster.
