...
Install the {gate} from the {ocp} catalog to install Gatekeeper on your cluster. See _Adding Operators to a cluster_ in the {ocp-short} documentation and go to the _Additional resources_ section. With {acm-short} you can use a policy to install the {gate} by using the governance framework.
