...
. For Gatekeeper version 3.20 and later, specify `audit.podAnnotations` that are specific to the audit pod by providing a list of annotation names and values to add to the pod. An omitted value is treated as `true`. View the following example:
...
. Define `operations` for your webhook. Use one or more of the following supported values: `CREATE`, `UPDATE`, `CONNECT`, and `DELETE`. See the following example:
...
. For version 3.20 and later, specify `rules` to overwrite the `rules` configuration in the `mutatingWebhookConfig` specification. The `rules` parameter preceeds the `operations` field. See the following example:
...
. Use the `config` section to exclude namespaces from certain processes for all constraints on your cluster. See the following example:
