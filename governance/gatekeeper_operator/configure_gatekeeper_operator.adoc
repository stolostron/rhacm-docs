...
To install Gatekeeper on your hub cluster, install the {gate} from the {ocp} catalog. Configure the {acm-short} Gatekeeper policy by using the governance framework. Then, you can start the Gatekeeper installation on your cluster.

After you install the {gate}, to install Gatekeeper you must configure the Gatekeeper custom resource.
...
See _Adding Operators to a cluster_ in the link:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.16/html/operators/administrator-tasks#olm-adding-operators-to-a-cluster[{ocp-short} documentation]. 
...
For versions 3.14 or later, the {gate} exposes a setting in the {gate} custom resource for the audit configuration with the parameter `auditFromCache`, which is disabled by default. If you enable `auditFromCache`, then you need to set `config.gatekeeper.sh` with the objects to sync to the cache.
