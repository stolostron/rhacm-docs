...
- <<uninstalling-gk,Uninstalling Gatekeeper policy, Gatekeeper, and {gate} policy>>
...
. Change the value for `complianceType` of the `ConfigurationPolicy` template to `mustnothave`. Changing the value deletes the {gate} custom resource and cleans up the Gatekeeper deployment.
