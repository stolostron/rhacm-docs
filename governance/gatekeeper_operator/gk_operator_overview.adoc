...
//// 
Recommend not having the important notes here and to add in to the related sections instead.
*Important:*
...
Default fail-open behavior:: By default, the `failurePolicy` parameter for Gatekeeper is set to `Ignore`. If the Gatekeeper webhook becomes unavailable, constraints are not enforced. To learn about changing the `failurePolicy` when you configure the {gate}, see _Configuring the {gate}_ in the list of resources. For more information about configuring fail-open and fail-closed behavior, see link:https://open-policy-agent.github.io/gatekeeper/website/docs/failing-closed[Gatekeeper documentation].
////
