...
//we can possibly add a comment to say PlacementRule is deprecated and the add Placement before PlacementRule (the info that is listed in the YAML example. PlacementRule is listed as a kind after `apiVersion: apps.open-cluster-management.io/v1`
+
[source,yaml]
----
