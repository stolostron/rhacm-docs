...
- A `ScanSettingBinding` resource (`_compliance-cis-scan_`) to identify which profiles to scan:
...
- A `ComplianceSuite` resource (`_compliance-suite-cis_`) to verify if the scan is complete by checking the `_status_` field:
...
- A `ComplianceCheckResult` resource (`_compliance-suite-cis-results_`) which reports the results of the scan suite by checking the `ComplianceCheckResult` custom resources (CR):
