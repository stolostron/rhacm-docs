...
The OpenShift CIS scan policy is created as a Kubernetes configuration policy in {acm-short}. {ocp-short} supports the OpenShift CIS scan policy. For more information, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/security_and_compliance/compliance-operator#understanding-compliance-operator[Understanding the Compliance Operator] in the {ocp-short} documentation for more details.
...
For more information on creating policies, see xref:../governance/manage_policies.adoc#creating-security-policies[Creating security policies].
