...
To create and manage policies, gain visibility and remediate configurations to meet standards, use the {acm} security policy framework. {acm} governance provides an extensible policy framework for enterprises to introduce their own security policies.

You can create a policy in any namespace on the hub cluster except the managed cluster namespaces. If you create a policy in a managed cluster namespace, it is deleted by {acm-short}. Each {acm-short} policy can be organized into one or more policy template definitions. For more details about the policy elements, view the _Policy YAML table_ section on this page.

[#hub-policy-policy-reqs]
== Requirements
...
- xref:../governance/compliance_history#configure-compliance-history[Configuring policy compliance history (Technology Preview)]
- Return to the xref:../governance/policy_deployment.adoc#policy-deployment[Policy deployment]
