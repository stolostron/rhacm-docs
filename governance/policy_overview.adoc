...
* You can create a policy in any namespace on the hub cluster except the cluster namespace. Every managed cluster already has a namespace on the hub cluster. If you create a policy in the cluster namespace, it is deleted by {product-title}. Instead, use a namespace such as `policy`.
