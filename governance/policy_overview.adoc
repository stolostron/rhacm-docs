...
* You can create a policy in any namespace on the hub cluster except the managed cluster namespaces. Every managed cluster has a namespace on the hub cluster that matches the managed cluster's name. If you create a policy in a managed cluster namespace, it is deleted by {product-title}. Instead, use a namespace such as `policy`.
