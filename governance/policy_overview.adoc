...
The policy requires a _Placement_ that defines the clusters that the policy document is applied to, and a _PlacementBinding_ that binds the {product-title} policy. For more on how to define a `Placement` see link:../clusters/cluster_lifecycle/placement_overview.adoc#placement-overview[Placement overview] in the Cluster lifecycle documentation.
...
//What does null mean for the user in this case
//bindingOverrides and subFilter are these new or did I miss these in a previous release? If these are new, we need to also add a what's new entry
| `bindingOverrides.remediationAction`
| Optional
| When this parameter is set to `enforce`, it provides a way for you to control the remediation action of the related `PlacementBinding` resources for configuration policies. The default value is `null`.

| `subFilter`
| Optional
| Use this parameter to enable filtering for the `bindingOverrides` parameter.
//is subfilter a subsection of bindingOverrides? 
