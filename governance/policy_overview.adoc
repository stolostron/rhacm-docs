...
* If you do not select a cluster set, all clusters are selected from all cluster sets that are bound to the namespace. 
...
//should the tolerations example be added to the previous empty sample ? 
[#policy-yaml-table]
== Policy YAML table
...
----
//or should it be added here: tolerations:- key: cluster.open-cluster-management.io/unreachableoperator: Exists- key: cluster.open-cluster-management.io/unavailable operator: Exists
