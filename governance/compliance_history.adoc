...
----
//Is this info about success and failure important to the user? If we only provide 1 verification step to check if the password is specified correctly, can the user skip this? 
+
...
----
+
.. To address the connection failure, view the Kubernetes events in the `open-cluster-management` namespace with the following command:
+
[source,bash]
----
...
----
+
.. Instructions are displayed for you to view the `governance-policy-propagator` logs. Run the following command:
+
[source,bash]
----
...
----
+
.. You might see a similar error message, which indicates that the password is not correctly specified:
+
----
...
----
//where can the user update the password specification? Do they update the compliance history API YAML? 
