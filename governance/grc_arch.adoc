...
* *Open source community:* Supports community contributions with a foundation of the {acm-short} policy framework. Policy controllers and third-party policies are also a part of the `open-cluster-management/policy-collection` repository. You can contribute and deploy policies using GitOps. 
