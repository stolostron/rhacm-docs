...
- *Policy propagator controller:* Runs on the {product-title-short} hub and generates the replicated policies in the managed cluster namespaces on the hub based on the placements bound to the root policy. It also aggregates compliance status from replicated policies to the root policy status and initiates automations based on policy automations bound to the root policy.

- *Governance policy addon controller:* Runs on the {product-title-short} hub and manages the installation of policy controllers on managed clusters.
