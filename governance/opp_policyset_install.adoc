...
Complete the following steps before you apply the policy set:app-name:
//added the full YAML bc this policy is in the community folder. Avoid linking to community policies 
...
. Policies are installed to the `policies` namaespace. Make sure the placement bindings match the `policies` namespace for the hub cluster and other managed clusters. The following example YAML is a `ManagedClusterSetBinding` apply a ManagedClusterSetBinding in the `policies` namespace:
+
[source,yaml]
----
