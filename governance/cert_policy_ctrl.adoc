...
You can use the certificate policy controller to detect certificates that are close to expiring, time durations (hours) that are too long, or contain DNS names that fail to match specified patterns. You can add the certificate policy to the `policy-templates` field of the policy on the hub cluster, which propagates to the selected managed clusters by using the governance framework. See the xref:../governance/hub_policy_framework.adoc#hub-policy-framework[Hub cluster policy framework] documentation for more details on the hub cluster policy.
...
When your certificate policy controller is created on your hub cluster, a replicated policy is created on your managed cluster. See link:https://github.com/open-cluster-management-io/policy-collection/blob/main/stable/SC-System-and-Communications-Protection/policy-certificate.yaml[`policy-certificate.yaml`] to view the certificate policy sample.
