...
* Install the Ansible Automation Platform Resource Operator from the Operator Lifecycle Manager. In the _Update Channel_ section, select `stable-2.3-cluster-scoped`. Select the *All namespaces on the cluster (default)* installation mode.
+
*Best practice*: Ensure that the Ansible Tower job template is idempotent when it is run. If you do not have Ansible Automation Platform Resource Operator, you can find it from the {ocp} _OperatorHub_ page. 
