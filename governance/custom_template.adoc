...
* <<managed-templates,Support for managed cluster templates in configuration policies>>
...
[#managed-templates]
== Support for managed cluster templates in configuration policies

{product-title-short} also supports managed cluster templates to define configuration policies that are dynamically customized to the target cluster. This reduces the need to create separate policies for each target cluster or hardcode configuration values in the policy definitions. 

Managed cluster templates are based on Golang text template specifications, and the `{{managed … managed}}` delimiter indicates a managed cluster template in a configuration policy.

*Important:* If you use managed cluster templates to propagate secrets or other sensitive data, the template content is expanded in the policy, and policies are not encrypted by the {ocp-short} ETCD encryption support.
