...
When you use this function, enter the namespace (must be the same as the policy when a <<hub-templates,hub>> cluster template), name, and data key of a Kubernetes `Secret` resource. Note that when using this with <<hub-templates,hub>> cluster templates, the output is automatically encrypted using the the <<protect-function,protect function>>. You receive a policy violation if the Kubernetes `Secret` resource does not exist on the target cluster. If the data key does not exist on the target cluster, the value becomes an empty string. View the following configuration policy that enforces a `Secret` resource on the target cluster. The value for the `PASSWORD` data key is a template that references the secret on the target cluster:
