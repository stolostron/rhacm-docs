...
The `protect` function enables you to encrypt a string in a hub cluster policy template. View the following example of the configuration policy that uses the `protect` function:
...
----
+
In the previous YAML example, there is an existing hub cluster policy template that is defined to use the `lookup` fucntion. On the replicated policy in the managed cluster namespace, the value might resemble the following syntax: `$ocm_encrypted:okrrBqt72oI+3WT/0vxeI3vGa+wpLD7Z0ZxFMLvL204=`
...
[#sensitive-data-hub]
=== Secure sensitive data in hub cluster policy templates

// Add info on how to do this 
[#template-processing]
=== Template processing
