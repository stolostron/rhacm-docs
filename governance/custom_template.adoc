...
When you use this function, enter the namespace, name, and data key of a Kubernetes `Secret` resource. You must use the same namespace that is used for the policy within the hub cluster template. See, <<hub-templates,Support for hub cluster templates in configuration policies>> for more details.
