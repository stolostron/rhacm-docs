...
The `lookup` function returns the Kubernetes resource as a JSON compatible map. Note that if the requested resource does not exist, an empty map is returned. If the resource does not exist and the value is added to another template function, you might get the following error, `invalid value; expected string`.
