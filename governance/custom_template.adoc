...
*Note*: If you use hub cluster templates to propagate secrets or other sensitive data, be aware that the template content is expanded in the policy that exists in the managed cluster namespace, on the hub cluster and on the managed clusters where that policy is distributed. Policies are not encrypted by the {ocp-short} etcd encryption support.

// OR

// If you use hub cluster templates to propagate secrets or other sensitive data, be aware that the template content is expanded in the policy, which exists in the managed cluster namespace on the hub cluster and on the managed clusters where that policy is distributed. Policies are not encrypted by the {ocp-short} etcd encryption support.
