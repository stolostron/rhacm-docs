...
----
//or should i replace, "You receive a policy violation if the data key does not exist on the target cluster."
//with, "If the data key does not exist on the target cluster, the value becomes an empty string."
When you use this function, enter the namespace, name, and data key of a Kubernetes `Secret` resource. You receive a policy violation if the data key does not exist on the target cluster. View the following configuration policy that enforces a `Secret` resource on the target cluster. The value for the `PASSWORD` data key is a template that references the secret on the target cluster:
