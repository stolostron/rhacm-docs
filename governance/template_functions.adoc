...
| Lists
| `append`, `concat`, `has`, `mustAppend`, `mustHas`, `mustPrepend`, `mustSlice`, `prepend`, `slice`, 

| String functions
| `cat`, `contains`, `hasPrefix`, `hasSuffix`, `lower`, `quote`, `replace`, `split`, `splitn`, `substr`, `trim`, `trimAll`, `trunc`, `upper`
...
// `join`
// `list` ---> assumed that list would be in the list library but it wasn't listed
//Can you all help confirm the library for these functions? | MJ | 10/05/23
