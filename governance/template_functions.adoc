...
View the following configuration policy that enforces a `Secret` resource on the target cluster. The value for the `PASSWORD` data key is a template that references the secret on the target cluster:
...
spec:
  ...
  - complianceType: musthave
    objectDefinition:
      ...
      data:
        USER_NAME: YWRtaW4=
        PASSWORD: '{{ fromSecret "default" "localsecret" "PASSWORD" }}' <1>
      kind: Secret <2>
      metadata:
        name: demosecret
        namespace: test
...
<2> You receive a policy violation if the Kubernetes `Secret` resource does not exist on the target cluster. If the data key does not exist on the target cluster, the value becomes an empty string.
