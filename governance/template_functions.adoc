...
The `skipObject` function is only available in managed cluster templates in a `ConfigurationPolicy`. Calling `{{ skipObject }}` at any time inside a Go template signals to skip that particular object for the policy. When you use the `skipObject` with the `objectSelector`, you can further filter objects selected by name. View the following example that selects objects that have the label `foo: bar` but skips objects that have a name with the suffix `-prod`:
