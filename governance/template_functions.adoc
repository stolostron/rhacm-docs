...
The `fromYaml` (or `fromYAML`) function parses a YAML string into a Golang object. This is particularly useful if a configuration has been provided inside the key of a `ConfigMap`.
...
The `toYaml` (or `toYAML`) function parses a Golang object into a YAML string. In order for a multiline result to be properly indented, the output must be passed to either the `autoindent` or `indent` function.
