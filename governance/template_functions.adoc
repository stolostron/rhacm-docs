...
*Note:* Use the `default` template function, so the correct type is provided to later template functions. See _Sprig open source_.
...
* xref:template_support_intro.adoc#template-processing[Template processing]
* xref:adv_template_process.adoc#adv-template-processing[Advanced template processing in policies]
* xref:policy_cli_tool.adoc#policy-cli-tool[Policy command line interface]
