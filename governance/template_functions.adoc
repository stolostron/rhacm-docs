...
The `skipObject` function is only available in managed cluster templates in a `ConfigurationPolicy`. Calling `{{ skipObject }}` at any time inside of a Go template will signal to skip that particular object for the policy. This is particularly useful when paired with an `objectSelector` in order to further filter objects selected by name. View the following example that selects objects that have the label `foo: bar` but skips objects that have a name with the suffix `-prod`:
