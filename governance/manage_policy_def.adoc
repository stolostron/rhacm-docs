...
For replicated policies to not show up at all in ArgoCD, you can set `spec.copyPolicyMetadata` to `false` on the {product-title-short} policy definition to prevent labels and annotations added by ArgoCD for tracking to the {product-title-short} policy definition of being copied to the replicated policy.
