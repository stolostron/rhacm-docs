...
| Optional
| Describes if you need to delete or recreate an object when an update is required. When you set the object to `IfRequired`, the policy recreates the object when you update an immutable field. When you set the parameter to `Always`, the policy recreates the object if a mismatch is detected. When you set the `remediationAction` to `inform`, the parameter value, `RecreateOption`, has no effect on your object. The `IfRequired` value has no effect on clusters without dry-run update support. The default value is `None`.
