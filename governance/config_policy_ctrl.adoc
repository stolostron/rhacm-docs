...
//what is "dry run update support"
| `spec.object-templates[].recreateOption`
| Optional
| Describes whether to delete and recreate an object when an update is required. When set to `IfRequired` the policy recreates the object when you update an immutable field. When the parameter is set to `Always` the policy recreates the object if a mismatch is detected. The parameter value `RecreateOption` has no effect when the `remediationAction` is set to `inform`. The `IfRequired` value has no effect on clusters without dry run update support. The default value is `None`.
