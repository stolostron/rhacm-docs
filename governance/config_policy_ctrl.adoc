...
| Optional
| Used to add multiple YAML strings for policy templates. *Note:* While `spec.object-templates` and `spec.object-templates-raw` are optional, at least one of the two parameter fields must be set.
...
- Learn about cleaning up resources and other topics in the xref:../governance/#managing-security-policies[Managing security policies] documentation.
