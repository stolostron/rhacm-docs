...
| You can use Kubernetes labels on the policy resources to logically group related policies and view the in the _Discovered policies_ table. You can also use Argo CD `Application` objects for grouping and to get a high-level status.

| Policy event history
| You can view the last 10 events per cluster per policy stored on the hub cluster.
| You can view the last 10 events in the status of the policy on the managed cluster.
