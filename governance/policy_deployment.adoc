...
[#policy-deployment]
= Choose a policy deployment strategy
...
| Yes
| Yes, you can view policies that are deployed by external tools from the _Discover policies_ table. Policies that are deployed with external tools are not displayed from the Governance _Overview_ dashboard. You must enable the search service on your managed cluster.

| Policy grouping
| Yes, you can have a combination of statuses and deployments through `Policy` and `PolicySet` resources.
| You can use Kubernetes labels on your policy resources to logically group related policies and view the _Discovered policies_ table. You can also use Argo CD `Application` objects for grouping and to get a high-level status.

| Policy event history
| You can view the last 10 events per cluster per policy stored on your hub cluster.
| You can view the last 10 events in the status of the policy on your managed cluster.
...
| Yes
| Yes, `cluster_policy_governance_info`.
...
* xref:../governance/pol_deploy_ext_tools.adoc#policy-deploy-ext-tools[External tool deployment]
