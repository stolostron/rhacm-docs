...
A common use of {product-title-short} policies is to install an Operator on one or more managed {ocp} clusters. Continue reading to learn how to install Operators with OpenShift GitOps installation modes:

* <<policy-install-ocp-gitops,Installing OpenShift GitOps by using a policy>>
* <<policy-gen-install-compliance-operator,Installing the Compliance Operator by using a policy>>
* <<policy-gen-install-on-openshift-gitops,Installing the policy generator on OpenShift GitOps (ArgoCD)>>
- <<manage-policy-definitions,Managing policy definitions with OpenShift GitOps (Argo CD)>>
* <<related-resource-grc-gitops,Related resources>>

[#policy-install-ocp-gitops]
== Installing OpenShift GitOps using a policy

Apply a policy that installs OpenShift GitOps by using the policy generator. The OpenShift GitOps operator offers the _all namespaces_ installation mode. Create a `Subscription` manifest file called `openshift-gitops-subscription.yaml`, similar to the following example:
...
A policy generator configuration file called `policy-generator-config.yaml` is required. Use the following policy to install OpenShift GitOps on all {ocp-short} managed clusters:
...
{ocp-short} supports all generated policies.

[#policy-gen-install-compliance-operator]
== Installing the Compliance Operator by using a policy
...
A policy generator configuration file called `policy-generator-config.yaml` is also required. View the following `PolicyGenerator` policy example that installs the Compliance Operator on all OpenShift managed clusters:
...
== Installing the policy generator on OpenShift GitOps (ArgoCD)
...
=== Managing policy definitions with OpenShift GitOps (Argo CD)
...
When a {product-title-short} policy definition is deployed with OpenShift GitOps, a copy of the policy is created in each managed cluster namespace. These copies are called replicated policies. For OpenShift GitOps to not constantly delete this replicated policy or show that the ArgoCD `Application` is out of sync, the `argocd.argoproj.io/compare-options: IgnoreExtraneous` annotation must be set on each {product-title-short} policy definition deployed using OpenShift GitOps.
