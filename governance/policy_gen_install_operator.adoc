...
A common use of {product-title-short} policies is to install an Operator on one or more managed {ocp} clusters. Continue reading to learn how to generate policies using the policy generator, and to install OpenShift GitOps Operator:

* <<policy-install-ocp-gitops,Generating a policy that installs the OpenShift GitOps>>
...
== Generating a policy that installs the OpenShift GitOps

Generate a policy that installs OpenShift GitOps by using the policy generator. The OpenShift GitOps operator offers the _all namespaces_ installation mode. Create a `Subscription` manifest file called `openshift-gitops-subscription.yaml`, similar to the following example:
...
A `PolicyGenerator` configuration file is required. Use the following policy to install OpenShift GitOps on all {ocp-short} managed clusters:
...
== Generating a policy that installs the Policy Generator on OpenShift GitOps (ArgoCD)
