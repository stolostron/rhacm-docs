...
| `spec.operatorGroup`
| Optional
a| By default, if the `operatorGroup` field is not specified, the controller generates an `AllNamespaces` type `OperatorGroup` resource in the same namespace as the subscription, if supported. The operator policy controller produces this resource. View the following parameter descriptions: 
...
- `minorChannelUpgradeAvailable`: Detects if a minor version upgrade is available for the operator on different channels. The default value is `Compliant`.
...
| `spec.versions`
| Optional
| Declare which versions of the operator are compliant. The `versions` parameter is a list of templatable strings that specifies which installed `ClusterServiceVersion` names are compliant when in `inform` mode, and which `InstallPlans` are approved when in `enforce` mode. To give different versions in one entry, separate the versions with commas. If the field is empty, the strings are ignored and all `ClusterServiceVersion` names are approved. The default value is empty.
|===

.Additional resources

* xref:../governance/template_support_intro.adoc#template-processing[Template processing in policies]
* xref:../governance/install_operator.adoc#install-operator-with-policy[Create an operator policy to install an operator]
* xref:../governance/manage_operator.adoc#managing-operator-policies-disconnected[Deploy operators using policies]
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/operators/understanding-operators#olm-subscription_olm-understanding-olm[Subscription]
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/operators/understanding-operators#operator-lifecycle-manager-olm[Operator Lifecycle Manager (OLM)]
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html-single/operators/index#olm-adding-operators-to-a-cluster[Adding Operators to a cluster]
