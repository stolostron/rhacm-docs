...
| `spec.subscription`
| Required
a| Define the configurations to create an operator subscription. Add information in the following fields to create an operator subscription. Default options are selected for the following parameters if there is no entry:
...
- `config`: Specify details for the operator subscription. The following fields are similar to the fields in a `Subscription` object: `channel`, `source`, `sourceNamespace`, and `config`. Use the `name` field rather than the `package` field. You can also add the `namespace` of the operator in the parameter section.

The `name` field is the only required field. The other fields are inferred from the catalog information that is available on the managed cluster.
+
