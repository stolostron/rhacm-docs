...
*Note:* Use the `default` template function, so the correct type is provided to later template functions. See _Sprig open source_ for supported functions.
...
For label selector examples, see _Kubernetes labels and selectors_. View the following example of the configuration policy that enforces a Kubernetes resource on the target managed cluster:
