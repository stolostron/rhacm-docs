...
- Install {acm}.
- Install the Policy Generator Kustomize plugin. See the _Policy Generator_ documentation for more information.
...
| `policy-quay-status`
| Installed onto the {acm-short} hub cluster.

| {acm-short}
| `policy-ocm-observability`
| Sets up the {acm-short} observability service.


.2+| Red Hat OpenShift Data Platform
| `policy-odf`
| Available storage for the hub cluster components that is used by {acm-short} observability and Quay.
...
* xref:../governance/opp_policyset_install.adoc#install-opp-policy-set[Red Hat OpenShift Platform Plus installation]
