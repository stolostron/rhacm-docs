...
The latest version of the policy generator is included in the installation. The policy generator version used is v1.4.1. 
...
spec:
  ...
  startingCSV: openshift-gitops-operator.v<version>
----

To pin to a specific version of the operator, you can set the `spec.startingCSV` value to `openshift-gitops-operator.v<version>`. Replace `<version>` with your preferred version.
