...
- Use the `object-templates-raw` manifest to automatically generate a `ConfigurationPolicy` resource with the content you add. For example, create a manifest file with the following syntax:
...
- Next create a `PolicyGenerator` configuration file similar to the following YAML. For `path`, enter the path to your `manifest.yaml` file:
