...
| Optional
| Specifies the frequency for a policy to be evaluated when it is in a particular compliance state. Use the parameters `compliant` and `noncompliant`. The default value is `"watch"` to leverage Kubernetes API watches instead of polling the Kubernetes API server.
