...
| Optional
| Specify a placement rule by defining a cluster selector using either `key:value`, or providing a `matchExpressions` and or `matchLabels` keys with appropriate values. See `_placementRulePath_` to specify an existing file.
