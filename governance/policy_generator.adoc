...
| Optional
| A list of Kustomize patches to apply to the manifest at the path. If there are multiple manifests and if it is applicable, the patch requires the following fields: `apiVersion`, `kind`, `metadata.name`, and `metadata.namespace`. You must set these fields so that Kustomize can identify the manifest that the patch applies to. If there is a single manifest, you can patch the `metadata.name` and `metadata.namespace` parameters. For more details, see the link:https://kubectl.docs.kubernetes.io/guides/example/inline_patch/#patchesstrategicmerge[Kustomize documentation on strategic merge patches].
