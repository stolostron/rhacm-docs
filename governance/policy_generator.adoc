...
| Optional
| Specify a placement by defining a cluster selector using either `key:value`, or providing `matchExpressions` and/or `matchLabels` keys with appropriate values. See `placementPath` to specify an existing file.
...
| Optional
| Specify a placement rule by defining a cluster selector using either `key:value`, or providing a `matchExpressions` and/or `matchLabels` keys with appropriate values. See `placementRulePath` to specify an existing file.
