...
The `configmap.yaml` file represents a Kubernetes manifest YAML file to be included in the policy. Alternatively, you can set the path to a Kustomize directory, or a directory with multiple Kubernetes manifest YAML files. View the following example:
...
WIth the `policytools` command line interface (CLI), you can use policies locally to help with creating and debugging. 
