...
| Optional
| To reuse an existing placement, specify the path here relative to the `kustomization.yaml` file. If provided, this placement rule is used by all policies by default. See `_clusterSelectors_` to generate a new `Placement`.
