...
[#policy-generator]
= Policy Generator configuration and reference
...
----
+
*Note:* The `policy-generator-config.yaml` file that is referenced in the previous example is a YAML file with the instructions of the policies to generate.
...
----
+
*Note:* The `configmap.yaml` file represents a Kubernetes manifest YAML file to be included in the policy. Alternatively, you can set the path to a Kustomize directory, or a directory with multiple Kubernetes manifest YAML files. View the following `ConfigMap` example:
...
- The generated `Policy` resource, along with the generated `Placement` resource, and `PlacementBinding` resource, might resemble the following examples. *Note:* Specifications for resources are described in the _Policy Generator configuration reference table_. 
...
| Optional
| Defines the label selector for objects that are defined in the `objectDefinition`. If there is an object name that is defined in the `objectDefinition`, the `objectSelector` is ignored.
...
* link:../gitops/gitops_policy_generator.adoc#gitops-policy-generator[Generating a policy to install GitOps Operator]
* xref:../governance/policy_set_ctrl.adoc#policy-set-controller[Policy set controller]
* link:../applications/subscription_sample.adoc#applying-kustomize[Applying Kustomize]
* link:../gitops/gitops_overview.adoc#gitops-overview[GitOps overview]
* xref:../governance/grc_intro.adoc#governance[Understand the governance framework]
* link:https://kubectl.docs.kubernetes.io/references/kustomize/kustomization/[kustomization.yaml file]
* link:https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/[Kubernetes labels and selectors]
* link:https://open-policy-agent.github.io/gatekeeper/website/docs/[Gatekeeper]
* link:https://kustomize.io/[Kustomize documentation]
* link:https://kubectl.docs.kubernetes.io/guides/example/inline_patch/#patchesstrategicmerge[Kustomize documentation on strategic merge patches]
* link:https://kubectl.docs.kubernetes.io/references/kustomize/kustomization/openapi/[Kustomize documentation on openapi]
