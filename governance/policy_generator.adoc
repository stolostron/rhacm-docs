  ...
  - name: config-data
    manifests:
      - path: object-templates-raw: | 
              <your_content> <1>
----
<1> Your content is used in the generated `ConfigurationPolicy` resource and added as an entry for the `policy-templates` of a policy. The `object-templates-raw` generated `ConfigurationPolicy` has the same name as a manifest generated from a `ConfigurationPolicy` that has no specified name.
...
| Optional
| Specifies the evaluation frequency for a compliant policy.  If you want to poll the intervals, set this parameter to `10s`.

| `policyDefaults.evaluationInterval.noncompliant`
| Optional
| Specifies the evaluation frequency for a non-compliant policy. If you want to poll the intervals, set this parameter to `10s`.
