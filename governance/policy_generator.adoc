...
| Optional
| Specifies the frequency for a policy to be evaluated when it is in a particular compliance state. Use the parameters `compliant` and `noncompliant`.
...
| Optional
| Specifies the evaluation frequency for a compliant policy. The default value is `"watch"` to leverage Kubernetes API watches instead of polling the Kubernetes API server. If you want to poll the intervals, set this parameter to `10s`.

| `policyDefaults.evaluationInterval.noncompliant`
| Optional
| Specifies the evaluation frequency for a non-compliant policy. The default value is `"watch"` to leverage Kubernetes API watches instead of polling the Kubernetes API server. If you want to poll the intervals, set this parameter to `10s`.
...
- Non-root policy type manifests such as `IamPolicy`, `CertificatePolicy`, and `ConfigurationPolicy` that have a `Policy` suffix. The previous manifests are not modified except for patches and are directly added as a `policy-templates` entry of the `Policy` resource .
...
//line 222 ConfigurationPolicy API doesn't exist: See the ConfigurationPolicy API documentation for more details.
