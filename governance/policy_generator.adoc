...
| Optional
| A list of Kustomize patches to apply to the manifest at the path. If there are multiple manifests, the patch requires the following fields if they are applicable: `apiVersion`, `kind`, `metadata.name`, and `metadata.namespace`. You must set these fields so Kustomize can identify the manifest that the patch applies to. If there is a single manifest, you can patch the `metadata.name` and `metadata.namespace`.
