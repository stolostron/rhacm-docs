...
//Multiline is not clear to me. The YAML is using multiple lines in general. Or do we mean multiple function use is supported for object template processing . Object templates support processing multiple functions
//Are we able to shorten this example?
//Why might the user want to use multiple functions or the multiline templating in the object-templates-raw section? 
Multiline templating is supported. View the following partial policy example that is configured to create a `MachineSet` resource:
