...
To iterate over many objects filtered by a label with `ConfigurationPolicy`, use the `objectSelector` and specify the `kind` in the `objectDefinition`. The `objectSelector` is similar to the `namespaceSelector`, except that it does not have name filtering capabilities. To filter by name, use the `skipObject` Go template function. If you reference `{{ skipObject }}` at any time, it signals to skip that particular object from those selected by the `objectSelector` for the policy.
