...
To iterate over multiple objects using a label selector with `ConfigurationPolicy`, use the `objectSelector` and specify the kind in the `objectDefinition`. The `objectSelector` is similar to the `namespaceSelector`, except that it does not have name filtering built in. To filter by name, use the `skipObject` function. If you reference `{{ skipObject }}` at any time inside the Go template, it signals to skip that particular object for the policy.
