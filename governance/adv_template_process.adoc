...
By default, {acm-short} processes all Go templates. You might create a policy that contains a template in `policy-templates` that is not intended to be processed by {acm-short}. To bypass template processing for a particular template, change `{{ template content }}` to the following sample:
