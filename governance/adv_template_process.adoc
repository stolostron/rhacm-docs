...
To iterate over many objects that use a label selector with `ConfigurationPolicy`, use the `objectSelector` and specify the `kind` in the `objectDefinition`. The `objectSelector` is similar to the `namespaceSelector`, except that it does not automatically have name filtering capabilities. To filter by name, use the `skipObject` function. If you reference `{{ skipObject }}` at any time inside the Go template, it signals the `objectSelector` to skip that particular object for the policy.

For an example, view the following YAML sample that selects objects that have the label `foo: bar` but skips objects that have a name with the suffix `-prod`:
