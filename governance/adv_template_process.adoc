...
By default, {acm-short} processes all Go templates. You might create a policy that contains a template in `policy-templates` that is not intended to be processed by {acm-short}.  To bypass template processing for a particular template, change `{{ template content }}` to `{{ pass:[`{{ template content }}`] }}`. Then, the template returns a raw string to be processed by a subsequent templating engine.
...
* Also refer to the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/backup_and_restore/control-plane-backup-and-restore#backing-up-etcd-data_backup-etcd[Backing up etcd data].
