...
Alternatively, you can specify the `spec.hubTemplateOptions.serviceAccountName` field in the `Policy` object to a service account in the same namespace as the the `Policy` resource. When you specify the field, the service account is used for all hub cluster template lookups.
