...
| Processing occurs on the managed cluster. ConfigurationPolicies are processed periodically, which automatically updates the resolved object definition with data in the referenced resources. OperatorPolicies automatically update whenever a referenced resource changes.
