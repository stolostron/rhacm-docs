...
- A policy that creates the subscription (`container-security-operator`) to reference the name and channel. This configuration policy should have `spec.remediationAction` set to `enforce` in order to create the resources. The subscription pulls the profile, as a container, that it supports:
[source,yaml]
----
...
- An `inform` configuration policy to audit the `ClusterServiceVersion` to ensure that the container security operator installation succeeded:
[source,yaml]
----
apiVersion: policy.open-cluster-management.io/v1
kind: ConfigurationPolicy
...
- An `inform` configuration policy to audit whether any `ImageManifestVuln` objects were created by the image vulnerability scans.
[source,yaml]
----
apiVersion: policy.open-cluster-management.io/v1
kind: ConfigurationPolicy
