...
* <<toliteral-function,toLiteral function>>
...
The `toLiteral` function removes any quotation marks around the template string after it is processed. You can use this function to convert a JSON string from a ConfigMap field so it can be used as the initial JSON in the manifest. Run the following command to remove quotation marks from the `key` parameter value:
