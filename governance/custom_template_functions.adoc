...
The `toLiteral` function removes any quotation marks around the template string after it is processed. You can update JSON strings from your hub cluster template. Run the following command to remove quotation marks from the `key` parameter value:
