...
.. Download the https://github.com/open-cluster-management/policy-collection/blob/main/community/CM-Configuration-Management/policy-persistent-data-management.yaml[policy-persistent-data-management] policy template from the GitHub repository. In its current format, this policy starts an installation of VolSync on the hub cluster. After it is installed on the hub cluster, it automatically installs the required VolSync components on all of the managed clusters of the hub that were created by {ocp-short}.
+
...
----
+
Replace `namespace` with the namespace of your hub cluster nad managed clusters.

. If your default `StorageClass` is not a CSI configuration, change your default CSI configuration to your VolSync CSI instance. 
+
...
... Copy the name of the secret that is provided as the value of `.status.rsync.sshKeys`. You will have to enter it when you configure the source.  
