...
... After the `ReplicationDestination` custom resource is created, copy the value of `.status.rsync.address`. This value is required when you configure the source. The following example shows the custom resource information that is added to the `ReplicationDestination` after it is created:
+
[source,yaml]
----
...
...
... Copy the name of the secret that is provided as the value of `.status.rsync.sshKeys`. This value is required when you configure the source.  
